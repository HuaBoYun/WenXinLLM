...
public class DatabaseConfig {
    ...
    public SqlSessionFactory sqlSessionFactory(@Qualifier(value = "dataSource") DataSource dataSource) throws Exception {
        ...
        interceptor.addInnerInterceptor(new PaginationInnerInterceptor(DbType.DM));// 需要根据实际使用的数据库进行修改
...
//		bean.setMapperLocations(mappers);
        bean.setTypeAliasesPackage("com.huabo.contract.entity");
        bean.setGlobalConfig(globalConfig());

        // 注册 ClobTypeHandler
        bean.setTypeHandlers(new TypeHandler[]{new ClobTypeHandler()});
