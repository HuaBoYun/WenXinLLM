...
import com.huabo.know.mapper.TblAttachmentMapper;
import com.huabo.know.mapper.TblzsgxlibraryMapper;
import com.huabo.know.service.TblzsgxlibraryService;
...
public class TblzsgxlibraryServiceImpl implements TblzsgxlibraryService {
	...
	public JsonBean getList(String token, String type,Tblzsgxlibrary library, Integer pageNumber, Integer pageSize) throws Exception {
    	...
    	pageInfo.setTlist(tblzsgxlibraryMapper.selectType(pageInfo, library, type, loginStaff.getCurrentOrg().getOrgid()));
    	pageInfo.setTotalRecord(tblzsgxlibraryMapper.selectTypeCount(type, library, loginStaff.getCurrentOrg().getOrgid()));
	...
	public JsonBean saveOrUpdate(String token, Tblzsgxlibrary libary,String attids) throws Exception {
		...
		if(libary!=null && libary.getLibraryid()!=null) {
			tblzsgxlibraryMapper.updateByIdSelective(libary);
		}else {
			libary.setLibraryid(RandomUtil.uuLongId());
			libary.setCretetime(new Date());
			libary.setCreatestaffid(loginStaff.getStaffid().intValue());
			libary.setCreateorganid(loginStaff.getCurrentOrg().getOrgid().intValue());
			tblzsgxlibraryMapper.insertSelective(libary);
	...
	@Override
	public JsonBean findById(String token, Long id) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
	...
	@Override 
	public JsonBean findByattId(String token, Long id) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
	...
	@Override
	public JsonBean deleteattid(String token, Long attid) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
	...
	@Override
	public JsonBean addcxcount(String token, Long libraryid) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
		...
		if(library!=null) {
			...
			tblzsgxlibraryMapper.updateByIdSelective(library);
		}
		resultMap.put("data",library);
		return ResponseFormat.retParam(1,200,resultMap);
	}
	...
	@Override
	public JsonBean addxzcount(String token, Long libraryid) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
