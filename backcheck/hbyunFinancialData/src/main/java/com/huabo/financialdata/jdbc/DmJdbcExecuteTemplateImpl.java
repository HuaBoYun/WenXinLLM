 ...
 */
@Component
public class DmJdbcExecuteTemplateImpl extends JdbcExecuteTemplate {
    private static final Logger log = LoggerFactory.getLogger(DmJdbcExecuteTemplateImpl.class);
    ...
    public String createTableSql(String tableName, List<JdbcColumnVO> columns) {
        if (StringUtils.isBlank(tableName)) {
            log.error("dm createTable 表名为空");
            return null;
        }
        if (CollectionUtils.isEmpty(columns)) {
            log.error("dm createTable 表字段为空");
            return null;
        }
        ...
        log.info("dm 创建数据库表结构，sql：{}", sqlBuilder);
