	...
	public MyJsonBean<TblCeaConferenceMgtOracle> getTblCeaConferenceMgtList(TblCeaConferenceMgtQueryParam param) {
		...
		if (CollectionUtil.isNotEmpty(pageInfo.getList())) {
			...
			pageInfo.getList().forEach(item -> {
				//去除前后2个逗号
				if (StringUtils.isNotBlank(item.getAttendees())) {
					String substring = item.getAttendees().substring(1, item.getAttendees().length() - 1);
					item.setAttendees(substring);
					item.setAttendeesName(tblStaffOracleService.getCreatorUserInfos(item.getAttendees()));
				}
