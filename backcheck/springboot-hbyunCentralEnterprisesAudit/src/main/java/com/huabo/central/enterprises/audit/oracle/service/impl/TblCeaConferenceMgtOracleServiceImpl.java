...
import com.huabo.central.enterprises.audit.oracle.mapper.TblCeaConferenceMgtOracleMapper;
import com.huabo.central.enterprises.audit.oracle.service.TblCeaConferenceMgtOracleService;
...
public class TblCeaConferenceMgtOracleServiceImpl implements TblCeaConferenceMgtOracleService {
	...
	public PageInfo<TblCeaConferenceMgtOracle> getList(TblCeaConferenceMgtQueryParam param) {
		...
		if (Objects.nonNull(param.getCreator())) {
			...
			if (StringUtils.isNotBlank(param.getDeptIds())) {
				criteria.andCondition(
						" (creator = " + param.getCreator() + " or creator in (SELECT DISTINCT STAFFID FROM TBL_USER_ORGRELATION WHERE DEPTID IN ("
								+ param.getDeptIds() + ")) or " + "recipient = " + param.getCreator() + " or " + "conferenceCompere = " + param
		...
		return PageMethod.startPage(param.getPageNumber(), param.getPageSize())
				.doSelectPageInfo(() -> tblCeaConferenceMgtOracleMapper.selectByExample(example));
	}
