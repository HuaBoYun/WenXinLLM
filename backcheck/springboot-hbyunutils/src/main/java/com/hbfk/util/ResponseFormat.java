...
public class ResponseFormat {
    ...
    static {
        ...
        messageMap.put(20008, "两次密码输入不一致");
        messageMap.put(20009, "不能包含中文");
        messageMap.put(20010, "用户未绑定角色");
        messageMap.put(20011, "用户登录失败");
...
//        messageMap.put(80003, "服务器异常");
        ...
        messageMap.put(90011, "该年度已生成审计情况统计表");

        messageMap.put(10001, "该用户权限不足!");
        messageMap.put(10002, "请检查当前下发人员已分配完全!");
        
    }
    public static JsonBean retParam(Integer code,Integer status,Object data) {
    	JsonBean json = new JsonBean(code, messageMap.get(status), data);
        return json;
    }
    
    
    public static JsonBean retParam(Integer code,String msg,Object data) {
    	JsonBean json = new JsonBean(code, msg, data);
        return json;
    }


    public static JsonBean retParam(Integer code,Object data) {
    	JsonBean json = new JsonBean(code, data);
        return json;
    }
}
