...
public class GlVoucherController {
	...
	@Resource
	private IGlVoucherService iGlVoucherService;
	
	@GetMapping(value = "/getFinanceDataList",produces = "application/json; charset=utf-8")
	@Operation(summary = "数据采集-凭证库列表")
	public JsonBean getFinanceDataList(HttpServletRequest request, HttpServletResponse response,GlVoucherVo vo) throws Exception {
		TblStaffUtil staff = userProvider.get();
		if(staff == null) {
			return ResponseFormat.retParam(0, 20006, null);
		}
		return this.iGlVoucherService.getFinanceDataList(staff,vo);
	}
