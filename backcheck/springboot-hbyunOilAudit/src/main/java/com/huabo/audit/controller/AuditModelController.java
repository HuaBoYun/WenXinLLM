...
public class AuditModelController {
	...
	public JsonBean getTblAuditModelExcel(@RequestHeader("token") String token, @PathVariable BigDecimal id) {
		JsonBean jsonBean = null;
		try {
			TblStaffUtil loginStaff = userProvider.get();
			if (loginStaff == null) {
	...
	public JsonBean fileUpload(MultipartFile[] file, @RequestHeader("token") String token, @PathVariable BigDecimal id) throws Exception {
		...
		for (MultipartFile multipartFile : file) {
			...
			model.setExcelId(Integer.valueOf(tblAttachmentEntity.getAttid().toString()));
