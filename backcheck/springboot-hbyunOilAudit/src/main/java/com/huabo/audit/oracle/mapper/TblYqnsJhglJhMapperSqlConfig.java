 ...
 */
public class TblYqnsJhglJhMapperSqlConfig {

    public String selectCountByPageInfo(PageInfo<TblYqnsJhglJh> pageInfo, TblYqnsJhglJh vo) {
        StringBuffer sb = new StringBuffer("SELECT COUNT(0) "
                + " FROM TBL_YQNS_JHGL_JH TBL1 "
                + " WHERE 1=1");
    ...
    public String selectListByPageInfo(PageInfo<TblYqnsJhglJh> pageInfo, TblYqnsJhglJh vo) {
        ...
        sb.append(" ORDER BY TBL1.JHID DESC) T1 WHERE ROWNUM <= " + (pageInfo.getCurrentRecord() + pageInfo.getPageSize()) + " ) T2 WHERE T2.RN > " + pageInfo.getCurrentRecord());
        return sb.toString();
    }

    private void sqlQuery(TblYqnsJhglJh vo, StringBuffer sb) {
        if (vo.getOrgid() != null) {
