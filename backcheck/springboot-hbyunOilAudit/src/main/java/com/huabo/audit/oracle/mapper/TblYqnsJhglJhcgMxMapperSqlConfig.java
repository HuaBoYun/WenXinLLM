...
/**
 * @author wangys
 * @description 针对表【TBL_YQNS_JHGL_JHCG_MX(计划草稿明细)】的数据库操作Mapper
 * @Entity TblYqnsJhglJhcg
 */
public class TblYqnsJhglJhcgMxMapperSqlConfig {

    public String selectCountByPageInfo(PageInfo<TblYqnsJhglJhcgMx> pageInfo, TblYqnsJhglJhcgMx vo) {
        StringBuffer sb = new StringBuffer("SELECT COUNT(0) "
                + " FROM TBL_YQNS_JHGL_JHCG_MX TBL1 "
                + " WHERE 1=1");
    ...
    public String selectListByPageInfo(PageInfo<TblYqnsJhglJhcgMx> pageInfo, TblYqnsJhglJhcgMx vo) {
        ...
        sb.append(" ORDER BY TBL1.jhcgmxid DESC) T1 WHERE ROWNUM <= " + (pageInfo.getCurrentRecord() + pageInfo.getPageSize()) + " ) T2 WHERE T2.RN > " + pageInfo.getCurrentRecord());
        return sb.toString();
    }

    private void sqlQuery(TblYqnsJhglJhcgMx vo, StringBuffer sb) {
        if (vo.getOrgid() != null) {
        ...
        if (vo.getJhcgid() != null) {
            sb.append(" AND TBL1.Jhcgid =").append(vo.getJhcgid());
        }
