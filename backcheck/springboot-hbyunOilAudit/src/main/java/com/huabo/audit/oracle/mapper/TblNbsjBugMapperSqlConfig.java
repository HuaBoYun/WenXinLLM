...
public class TblNbsjBugMapperSqlConfig {
	public String selectPlanCodeByOrgid(TblNbsjBugEntity plan) {
		StringBuffer sb = new StringBuffer("SELECT COUNT(*) FROM TBL_NBSJ_BUG WHERE 1=1 AND BUGNUMBER='"+plan.getBugnumber()+"' ");
	...
	public String selectListByInfo(TblNbsjBugVo tblNbsjBugVo,Integer orgid,Integer companyid) {

		return new SQL(){{
			SELECT("TNA.*,cc.*,ORG.ORGNAME");
			FROM("TBL_NBSJ_BUG TNA ");
			LEFT_OUTER_JOIN("TBL_NBSJ_BUG_CRITERION cir on TNA.BUGID=CIR.BUGID ");
			LEFT_OUTER_JOIN("TBL_NBSJ_BUGCRITERION cc on cc.BUGCRIID=CIR.BUGCRIID");
			LEFT_OUTER_JOIN("TBL_ORGANIZATION ORG on ORG.ORGID=TNA.bugdepartment ");
			if(tblNbsjBugVo.getBugid()!=null) {
				WHERE("TNA.BUGID !="+tblNbsjBugVo.getBugid());
				WHERE("TNA.FATHERBUGID IS NULL");
			}
			if(null != orgid) {
				WHERE("AND BUGDEPARTMENT=#{orgid}");
			}
			if(tblNbsjBugVo.getBugnumber()!=null && tblNbsjBugVo.getBugnumber().length()>0) {
				WHERE("TNA.BUGNUMBER LIKE '%"+tblNbsjBugVo.getBugnumber()+"%'");
			}
...
//			sb.append(" AND TNA.DISCOVERTIME >= TO_DATE('"+tblNbsjBugVo.getStartDate()+"','yyyy-MM-dd')");
				WHERE("TNA.DISCOVERTIME >= #{tblNbsjBugVo.startSqlDate}");
			}
...
//			sb.append(" AND TNA.DISCOVERTIME <= TO_DATE('"+tblNbsjBugVo.getEndDate()+" 23:59:59','yyyy-mm-dd HH24:MI:SS')");
				Date date = cn.hutool.core.date.DateUtil.parse(tblNbsjBugVo.getEndDate()+" 23:59:59","yyyy-MM-dd HH:mm:ss").toSqlDate();
				WHERE("TNA.DISCOVERTIME <= #{tblNbsjBugVo.endSqlDate}");
			}
			ORDER_BY("TNA.BUGID DESC");
		}}.toString();



//		StringBuffer sb = new StringBuffer("SELECT TNA.*,cc.*,ORG.ORGNAME "
//				+ "FROM TBL_NBSJ_BUG TNA "
...
//		if(StrUtil.isNotBlank(tblNbsjBugVo.getStartDate() )){
////			sb.append(" AND TNA.DISCOVERTIME >= TO_DATE('"+tblNbsjBugVo.getStartDate()+"','yyyy-MM-dd')");
//			sb.append("AND TNA.DISCOVERTIME >= #{tblNbsjBugVo.startDate, jdbcType=DATETIME}");
//		}
//
//		if(StrUtil.isNotBlank(tblNbsjBugVo.getEndDate() )){
////			sb.append(" AND TNA.DISCOVERTIME <= TO_DATE('"+tblNbsjBugVo.getEndDate()+" 23:59:59','yyyy-mm-dd HH24:MI:SS')");
//			sb.append("TNA.DISCOVERTIME >= #{endDate + '23:59:59'}");
//		}
//
//		sb.append(" ORDER BY TNA.BUGID DESC");
...
//				+ " AND TNA.BUGBYSYSTEM LIKE '%nbsj%' ");
	...
	public String updateEntity(TblNbsjBugEntity plan) {
		StringBuffer sqlSb = new StringBuffer("UPDATE TBL_NBSJ_BUG SET BUGNUMBER = '"+plan.getBugnumber()+"'");
	...
	public String insertEntity(TblNbsjBugEntity plan){
		StringBuffer colSb = new StringBuffer("INSERT INTO TBL_NBSJ_BUG(BUGID,BUGREFORMSTATUS");
	...
	//==
	public String selectInnerCommonListByInfo(Integer bugid,Integer orgid) {
		StringBuffer sb = new StringBuffer("SELECT TNA.innrulid,TNA.rulecode,TNA.rulename,TNA.publishorg,TNA.publishdate,TNA.rulenumber,ORG.ORGNAME "
				+ "FROM TBL_NBSJ_INNERRULE TNA "
				+ "LEFT JOIN TBL_ORGANIZATION ORG ON ORG.ORGID = TNA.publishorg "
				+ "WHERE INNRULID not in "
				+ "(SELECT DISTINCT INNRULID from TBL_NBSJ_BUG_INNERRULE "
				+ "where bugid="+bugid+") "
				+ "and PUBLISHORG in "
				// TODO 递归改造 查询当前登录用户所在单位及下级单位
				+ "( select ORGID from TBL_ORGANIZATION where (1 = 1  AND orgtype=0 and (STATUS != 1 or STATUS IS NULL) ) "
				+ "or  ORGID = "+orgid+" start with  ORGID="+orgid+" connect by prior ORGID= fatherorgid) ");
				
		sb.append(" ORDER BY TNA.INNRULID DESC");
		return sb.toString();
	}
	...
	public String selectInnerCommonListByLink(PageInfo<TblNbsjInnerrule> pageInfo,Integer bugid,Integer orgid) {
		StringBuffer sb = new StringBuffer("SELECT * FROM "
				...
				+ "FROM TBL_NBSJ_INNERRULE TNA "
				+ "LEFT JOIN TBL_ORGANIZATION ORG ON ORG.ORGID = TNA.publishorg "
				+ " LEFT JOIN TBL_NBSJ_BUG_INNERRULE TBI on TNA.INNRULID=TBI.INNRULID "
				+ " LEFT JOIN TBL_NBSJ_BUG TBUG on TBUG.BUGID=TBI.BUGID "
	...
	//===============
	public String selectOuterCommonListByInfo(Integer bugid,Integer orgid) {
		StringBuffer sb = new StringBuffer("SELECT TNA.outrulid,TNA.rulecode,TNA.rulename,TNA.publishorg,TNA.publishdate,TNA.rulenumber,ORG.ORGNAME "
				+ "FROM TBL_NBSJ_OUTERRULE TNA "
				+ "LEFT JOIN TBL_ORGANIZATION ORG ON ORG.ORGID = TNA.CREATEORGID "
				+ "WHERE OUTRULID not in "
				+ "(SELECT DISTINCT OUTRULID from TBL_NBSJ_BUG_OUTERRULE "
				+ "where bugid="+bugid+") "
				+ "and CREATEORGID in "
		...
		sb.append(" ORDER BY TNA.OUTRULID DESC");
		return sb.toString();
	}
	...
	public String selectOuterCommonListByLink(PageInfo<TblNbsjOuterruleEntity> pageInfo,Integer bugid,Integer orgid) {
		StringBuffer sb = new StringBuffer("SELECT * FROM "
				...
				+ "FROM TBL_NBSJ_OUTERRULE TNA "
				+ "LEFT JOIN TBL_ORGANIZATION ORG ON ORG.ORGID = TNA.CREATEORGID "
				+ " LEFT JOIN TBL_NBSJ_BUG_OUTERRULE TBI on TNA.outrulid=TBI.outrulid "
				+ " LEFT JOIN TBL_NBSJ_BUG TBUG on TBUG.BUGID=TBI.BUGID "
				+ " WHERE TBUG.BUGID = "+bugid);
	...
	public String insertBugOutrulids(String outrulid, Integer bugid){
		StringBuffer colSb = new StringBuffer("INSERT INTO TBL_NBSJ_BUG_OUTERRULE(BUGID,OUTRULID) VALUES("+bugid+","+outrulid+")");
