...
//    	pageInfo.setCondition(tblNbsjWorkReport);
		...
		com.github.pagehelper.PageInfo<TblReportEntity> pageInfo2 = PageMethod.startPage(pageNumber, pageSize)
				.doSelectPageInfo(() -> {
					try {
						this.tblReportMapper.selectListByPageInfo(pageInfo,tblReportVo,orgid.intValue(), finalProjectId);
					} catch (Exception e) {
...
//        }
	...
	public JsonBean dealAuditUserApporvalInfo(String token, String staffid,String taskId,Integer cyId,String transitionName,String examination,String processInstanceId,String processDefinitionId)throws Exception {
		...
		try {
			 TblStaff spstaff = tblStaffMapper.selectById(Integer.parseInt(staffid));
...
//			staff.setRepdesc(null);
	...
	@Override
    public Map<String, Object> submitStaffScoreApproval(Integer staffScoreid, String examination, String token) {
            Map<String, Object> resultMap = new HashMap<String, Object>(0);
            try {
            	...
            	TblNbsjStaffscore entity = tblNbsjStaffscoreMapper.selectNbsjStaffscoreListByID(new BigDecimal(staffScoreid));
            	boolean flag = false;
...
//				log.error("发送审批失败");
	...
	public JsonBean getAuditUserApprovalView(String token, Integer staffid, String taskId) throws Exception {
		...
		if(DateBaseConfig.DATABASETYPE.equals("Oracle")) {
            try {
            	TblNbsjStaffscore entity = tblNbsjStaffscoreMapper.selectNbsjStaffscoreListByID(new BigDecimal(staffid));
