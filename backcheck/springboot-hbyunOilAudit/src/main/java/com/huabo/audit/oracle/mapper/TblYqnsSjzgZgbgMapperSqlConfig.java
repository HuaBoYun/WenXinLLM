 ...
 */
public class TblYqnsSjzgZgbgMapperSqlConfig {

    public String selectCountByPageInfo(PageInfo<TblYqnsSjzgZgbg> pageInfo, TblYqnsSjzgZgbg vo) {
        StringBuffer sb = new StringBuffer("SELECT COUNT(0) "
                + " FROM TBL_YQNS_SJZG_ZGBGINFO TBL1 "
                + " WHERE 1=1");
    ...
    public String selectListByPageInfo(PageInfo<TblYqnsSjzgZgbg> pageInfo, TblYqnsSjzgZgbg vo) {
        ...
        sb.append(" ORDER BY TBL1.ZGBGID DESC) T1 WHERE ROWNUM <= " + (pageInfo.getCurrentRecord() + pageInfo.getPageSize()) + " ) T2 WHERE T2.RN > " + pageInfo.getCurrentRecord());
        return sb.toString();
    }

    private void sqlQuery(TblYqnsSjzgZgbg vo, StringBuffer sb) {
