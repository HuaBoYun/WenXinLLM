...
import com.huabo.audit.oracle.mapper.TblYqnsJsxmJbqkhzMapper;
import com.huabo.audit.service.TblYqnsJsxmJbqkService;
import com.huabo.audit.service.TblYqnsJsxmJbqkhzService;
import com.huabo.audit.util.PageResult;

@Service
public class TblYqnsJsxmJbqkhzServiceImpl extends ServiceImpl<TblYqnsJsxmJbqkhzMapper, TblYqnsJsxmJbqkhz>
        implements TblYqnsJsxmJbqkhzService {
    TblStaffUtil loginStaff;
    
    private TblYqnsJsxmJbqkService tblYqnsJsxmJbqkService;
    ...
    @Override
    @Transactional
    public JsonBean saveOrUpdate(String token, TblYqnsJsxmJbqkhz vo) throws Exception {
...
//        vo.setOrgid(loginStaff.getCurrentOrg().getOrgid().longValue());
        ...
        String jsxmjbqkids = vo.getJsxmjbqkids();
        if(null!=jsxmjbqkids) {
        	String[] jsxmjbqkidz = jsxmjbqkids.split(",");
        	for (String jsxmjbqkid:jsxmjbqkidz) {
        		baseMapper.saveLine(vo.getHzid(), jsxmjbqkid);
			}
        }
     ...
     */
    @Override
    public JsonBean detail(String token, TblYqnsJsxmJbqkhz vo) throws Exception {
        JsonBean retParam = validToken(token);
        if (retParam != null) return retParam;
        TblYqnsJsxmJbqkhz bean = this.getById(vo.getHzid());
        if (bean == null) {
            return ResponseFormat.retParam(0, -1, "记录不存在");
        }

        List<TblYqnsJsxmJbqk> listJsxmJbqk = baseMapper.selectSubListById(vo.getHzid());
        if (listJsxmJbqk != null && listJsxmJbqk.size() > 0) {
            bean.setListJsxmJbqk(listJsxmJbqk);
        }
    ...
    @Override
    @Transactional
    public JsonBean delete(String token, TblYqnsJsxmJbqkhz vo) throws Exception {
        JsonBean retParam = validToken(token);
        if (retParam != null) return retParam;
        ...
        if(null!=subIds && subIds.size()>0) {
        	TblYqnsJsxmJbqk cond = new TblYqnsJsxmJbqk();
            cond.setIds(subIds);
            tblYqnsJsxmJbqkService.delete(token, cond);
        }
     ...
     */
    @Override
    public JsonBean hzlist(String token, Integer pageNumber, Integer pageSize, TblYqnsJsxmJbqkhz vo) throws Exception {
    	TblStaffUtil staff = userProvider.get();
        if (staff == null) {
        	return ResponseFormat.retParam(0, 20006, null);
        }
        IPage<TblYqnsJsxmJbqkhz> query = new Page<>(pageNumber, pageSize);
        //查询条件
        QueryWrapper<TblYqnsJsxmJbqkhz> wrapper = new QueryWrapper<>();
        //查询 需要判空 在查询 模糊查询
        if (StringUtils.isNotBlank(vo.getHzname())) {
            wrapper.lambda().like(TblYqnsJsxmJbqkhz::getHzname, vo.getHzname());
        }
        ...
        //获得数据
        com.github.pagehelper.PageInfo<TblYqnsJsxmJbqkhz> pageInfo = PageMethod.startPage(pageNumber, pageSize)
                .doSelectPageInfo(() -> baseMapper.selectList(wrapper));
        PageResult<TblYqnsJsxmJbqkhz> page = new PageResult<TblYqnsJsxmJbqkhz>().build(pageInfo);
        List<TblYqnsJsxmJbqkhz> tlist = page.getTlist();
