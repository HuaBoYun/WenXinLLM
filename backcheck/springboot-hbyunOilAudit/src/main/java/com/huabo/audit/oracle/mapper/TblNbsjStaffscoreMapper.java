...
public interface TblNbsjStaffscoreMapper extends BaseMapper<TblNbsjStaffscore> {
	...
	List<TblNbsjStaffscore> findList(@Param("orgId")BigDecimal orgId, @Param("re")TblNbsjStaffscore re) throws Exception;
	...
	List<TblNbsjStaffscore> selectNbsjStaffscoreByPageInfo(PageInfo<TblNbsjStaffscore> pageInfo, BigDecimal orgId, TblNbsjStaffscore re) throws Exception;




	 @Select("SELECT 	SS.STAFFSCORE_ID,STA.REALNAME,SS.AUDITORS,SS.AUDITPROJECTNAME,SS.TOTALSCORE,SS.PROJECTID,SS.STATUS FROM TBL_NBSJ_STAFFSCORE ss LEFT JOIN TBL_STAFF sta ON SS.auditors = sta.STAFFID where SS.STAFFSCORE_ID= #{staffScoreid}")
	 @Results({
		 	@Result(column="staffScore_id",property="staffScoreid"),
		 	@Result(column="AUDITPROJECTNAME",property="auditProjectName"),
		 	@Result(column="PROJECTID",property="projectid"),
	...
	Integer selectNbsjStaffscoreByPageCOunt( BigDecimal orgId, TblNbsjStaffscore re) throws Exception;
