...
public class TblNbsjMbMapperSqlConfig {
	...
	public String selectNbsjMbByPageInfo(PageInfo<TblNbsjMb> pageInfo,BigDecimal orgId,TblNbsjMb mb ) throws Exception {
			
			StringBuffer sb = new StringBuffer("SELECT * FROM (SELECT T1.*,ROWNUM RN  FROM (SELECT s.*,STA.REALNAME FROM TBL_NBSJ_MB s " );
			...
			sb.append("  ORDER BY s.AUDITTYPE,MBID) T1 WHERE ROWNUM <= "+(pageInfo.getCurrentRecord()+pageInfo.getPageSize())+" ) T2 WHERE T2.RN > "+pageInfo.getCurrentRecord());
			return sb.toString();
		}
	
	
	
	public String selectNbsjMbByPageInfoCount(BigDecimal orgId,TblNbsjMb mb ) throws Exception {
