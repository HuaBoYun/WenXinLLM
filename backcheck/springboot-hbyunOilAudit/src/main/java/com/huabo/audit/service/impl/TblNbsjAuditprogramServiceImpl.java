    ...
    */
	...
	public JsonBean mergeTblNbsjAuditprogramInfo(TblNbsjAuditprogramEntity program, String token) throws Exception {
		...
		if(program.getProgramId()!= null) {
			...
			tblNbsjAuditprogramMapper.insertEntity(program);
		}
    ...
    /*	Map<String,Object> resultMap = new HashMap<String,Object>(0);
		...
		com.github.pagehelper.PageInfo<TblNbsjAuditprogramEntity> pageInfo = PageMethod.startPage(pageNumber, pageSize)
				.doSelectPageInfo(() -> this.tblNbsjAuditprogramMapper.selectTblNbsjAuditprogramListByPageInfo(null,templeteId,targetId));
		//分页参数 转换成 分页对象 （不同系统分页对象返回可能不一样 可自行转换）
		PageResult<TblNbsjAuditprogramEntity> build = new PageResult<TblNbsjAuditprogramEntity>().build(pageInfo);
