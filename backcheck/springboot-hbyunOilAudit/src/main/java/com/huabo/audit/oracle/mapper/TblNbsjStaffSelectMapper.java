...
public interface TblNbsjStaffSelectMapper {
	...
	@Select("SELECT PROJECTID FROM TBL_NBSJ_STAFFSELECT TNS LEFT JOIN TBL_YQNS_IMPLEMENT_PLAN TIP ON TNS.PROJECTID=TIP.ID "
			+ " WHERE TIP.STATUS<>4 AND STAFFID = #{staffid}")
	...
	void deleteNbsjByProject(Integer projectId) throws Exception;
