 ...
 */
public class TblYqnsSjbgSjbgdgMapperSqlConfig {

    public String selectCountByPageInfo(PageInfo<TblYqnsSjbgSjbgdg> pageInfo, TblYqnsSjbgSjbgdg vo) {
        StringBuffer sb = new StringBuffer("SELECT COUNT(0) "
                + " FROM TBL_YQNS_SJBG_SJBGDG TBL1 "
                + " WHERE 1=1");
    ...
    public String selectListByPageInfo(PageInfo<TblYqnsSjbgSjbgdg> pageInfo, TblYqnsSjbgSjbgdg vo) {
        ...
        sb.append(" ORDER BY TBL1.SJBGDGID DESC) T1 WHERE ROWNUM <= " + (pageInfo.getCurrentRecord() + pageInfo.getPageSize()) + " ) T2 WHERE T2.RN > " + pageInfo.getCurrentRecord());
        return sb.toString();
    }

    private void sqlQuery(TblYqnsSjbgSjbgdg vo, StringBuffer sb) {
