 ...
 */
public class TblYqnsSjxmzdMapperSqlConfig {

    public String selectCountByPageInfo(PageInfo<TblYqnsSjxmzd> pageInfo, TblYqnsSjxmzd vo) {
        StringBuffer sb = new StringBuffer("SELECT COUNT(0) "
                + " FROM TBL_YQNS_SJXMZD TBL1 "
                + " WHERE 1=1");
    ...
    public String selectListByPageInfo(PageInfo<TblYqnsSjxmzd> pageInfo, TblYqnsSjxmzd vo) {
        ...
        sb.append(" ORDER BY TBL1.sjxmzdid DESC) T1 WHERE ROWNUM <= " + (pageInfo.getCurrentRecord() + pageInfo.getPageSize()) + " ) T2 WHERE T2.RN > " + pageInfo.getCurrentRecord());
        return sb.toString();
    }

    private void sqlQuery(TblYqnsSjxmzd vo, StringBuffer sb) {
        if (vo.getOrgid() != null) {
            sb.append(" and orgid = ").append(vo.getOrgid());
        }

        if (StringUtils.isNotEmpty(vo.getZdmc())) {
            sb.append(" AND TBL1.Zdmc LIKE '%" + vo.getZdmc() + "%'");
        }
