...
public String   selectNbsjStaffscoreByPageCount( BigDecimal orgId, TblNbsjStaffscore re) throws Exception {
	...
	public String   selectNbsjStaffscoreByPageInfo(PageInfo<TblNbsjStaffscore> pageInfo, BigDecimal orgId, TblNbsjStaffscore re) throws Exception {
		
		StringBuffer sb = new StringBuffer("SELECT * FROM (SELECT T1.*,ROWNUM RN  FROM (select * from ( SELECT SS.STAFFSCORE_ID,STA.REALNAME,SS.AUDITORS,SS.AUDITPROJECTNAME,SS.TOTALSCORE,SS.PROJECTID,ss.STATUS FROM TBL_NBSJ_STAFFSCORE ss LEFT JOIN TBL_STAFF sta ON SS.auditors = sta.STAFFID where 1=1   " );
