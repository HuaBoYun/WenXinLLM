...
public interface TblTargetTypeMapper extends tk.mybatis.mapper.common.Mapper<TblTargetTypeEntity>{
	...
	List<TblTargetTypeEntity> getChildByUser(BigDecimal projectId,BigDecimal staffId);
	...
	List<TblTargetTypeEntity> getChildByUsers(BigDecimal parentId, BigDecimal projectId,BigDecimal staffId);
	...
	List<TblTargetTypeEntity> selectMyTarget(Integer projectId, BigDecimal targetId, BigDecimal staffid) throws Exception;
    ...
    @Select("SELECT DISTINCT target.* FROM	TBL_NBSJ_TARGETTYPE target LEFT JOIN TBL_NBSJ_AUDITPROGRAM audi ON target.targetId = audi.targetId LEFT JOIN TBL_NBSJ_AUTHORIZATION auth on audi.programid = auth.programid  WHERE  auth.projectid = #{projectId} AND target.PARENTID = #{targetId} AND auth.TEAMSTAFFID = #{staffid}")
	List<TblTargetTypeEntity> selectMyyqTarget(BigDecimal projectId, BigDecimal targetId, BigDecimal staffid) throws Exception;
