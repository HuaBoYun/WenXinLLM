...
import com.huabo.audit.oracle.mapper.TblYqnsLxjytbMapper;
...
@Service
public class TblYqnsLxjytbServiceImpl implements TblYqnsLxjytbService {

	@Resource
	private TblYqnsLxjytbMapper tblYqnsLxjytbMapper;
	
	@Resource
    private UserProvider userProvider;
	
	@Override
	public JsonBean saveOrupdate(String token, TblYqnsLxjytb jd, String glids) throws Exception {
        ...
        if(jd!=null && jd.getTbid()!=null) {
        	tblYqnsLxjytbMapper.updateById(jd);
        	if(glids!=null && glids.length()>0) {
        		tblYqnsLxjytbMapper.deleteglnr(jd.getTbid());
        		String[] glidss = glids.split(",");
        		for (String glid : glidss) {
        			tblYqnsLxjytbMapper.insertglnr(jd.getTbid(),  glid);
				}
        	}
        	
        }else {
        	jd.setTbid(RandomUtil.uuBigDecimalId());
        	tblYqnsLxjytbMapper.insert(jd);
        	if(glids!=null && glids.length()>0) {
        		tblYqnsLxjytbMapper.deleteglnr(jd.getTbid());
        		String[] glidss = glids.split(",");
        		for (String glid : glidss) {
	...
	public JsonBean findByid(String token, BigDecimal jdid) throws Exception {
        ...
        TblYqnsLxjytb tb = tblYqnsLxjytbMapper.selectById(jdid);
        Map<String, Object> resultMap = new HashMap<String, Object>(0);
	...
	public JsonBean findAllList(String token, Integer pageNumber, Integer pageSize, SjdwjdVo vo) throws Exception {
        ...
        QueryWrapper<TblYqnsLxjytb> queryWrapper = new QueryWrapper<>();
        ...
        if(StringUtils.isNotEmpty(vo.getYear())) {
            ...
            if (StringUtils.isNotBlank(vo.getYear())) {
            	queryWrapper.lambda().ge(TblYqnsLxjytb::getCreatedate, startlocalDate);
            	queryWrapper.lambda().lt(TblYqnsLxjytb::getCreatedate, endlocalDate);
            }
        }
        
        //倒序
        queryWrapper.orderByDesc(true, "CREATEDATE");
        com.github.pagehelper.PageInfo<TblYqnsLxjytb> info =  PageMethod.startPage(vo.getPageNum(), vo.getPageSize())
				.doSelectPageInfo(() -> tblYqnsLxjytbMapper.selectList(queryWrapper));
		Map<String,Object> resultMap = new HashMap<String,Object>(0);
		PageResult<TblYqnsLxjytb> build = new PageResult<TblYqnsLxjytb>().build(info);
	...
	public JsonBean deleteone(String token, BigDecimal jdid) throws Exception {
        ...
        tblYqnsLxjytbMapper.deleteglnr(jdid);
        tblYqnsLxjytbMapper.deleteoneById(jdid);
