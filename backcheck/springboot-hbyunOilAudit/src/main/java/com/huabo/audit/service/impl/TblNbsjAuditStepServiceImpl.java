...
public class TblNbsjAuditStepServiceImpl  implements TblNbsjAuditStepService {
	...
	@Override
	public JsonBean findByExper(String token, BigDecimal typeId) throws Exception{
		...
		if(typeId!=null) {
			...
			List<TblNbsjAuditStepEntity> list = stepmapper.findByExperByExperall();
			resultMap.put("data",list);
		}
	...
	public JsonBean zxsql(String token, BigDecimal stepId) throws Exception {
		...
		if(entity!=null && entity.getBookid()!=null) {
			...
			List<String> params = JDBCProperties.getTbable(entity.getSqlstr(), accBook.getAcctid());
			if(!JDBCProperties.isNotExistsTable("NBSJ_MX_"+entity.getStepid(), accBook.getAcctid())) {
				JDBCProperties.executeSql("NBSJ_MX_"+entity.getStepid(), accBook.getAcctid(), params);
			}
			List<Map<String, Object>> list = JDBCProperties.getData(entity.getSqlstr(), accBook.getAcctid());
			JDBCProperties.insertData("NBSJ_MX_"+entity.getStepid(), accBook.getAcctid(), list, loginStaff.getStaffid(), result.getResultid()+"_"+time);
	...
	public JsonBean getDatelistt(String token, BigDecimal resultid,Integer pageNumber,Integer pageSize) throws Exception {
    	...
    	pageInfo.setPageSize(pageSize);
    	pageInfo.setCurrentPage(pageNumber);
    	return  JDBCProperties.GetGather(accBook.getAcctid(), sql, pageInfo);
	}
