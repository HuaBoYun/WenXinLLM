...
public interface TblNbsjBugMapper extends BaseMapper<TblNbsjBugEntity>{
	...
	List<TblAttachment> defectFileList(Integer bugId);
	  ...
	  @Select("SELECT * from TBL_NBSJ_BUG WHERE BUGID= #{bugid} ")
	    ...
	    @SelectProvider(method="selectListByInfo",type=TblNbsjBugMapperSqlConfig.class)
	    @Results({
	    	@Result(column="BUGID",property="bugid"),
	    	@Result(column="BUGNUMBER",property="bugnumber"),
	    	@Result(column="BUGDESCRIPTE",property="bugdescripte"),
		...
		List<TblNbsjBugEntity> selectListByInfo( TblNbsjBugVo tblNbsjBugVo, Integer orgid, Integer companyid);
	...
	//=====内规Begin
	@SelectProvider(method="selectInnerCommonListByInfo",type=TblNbsjBugMapperSqlConfig.class)
    @Results({
	...
	List<TblNbsjInnerrule> selectInnerCommonListByInfo( Integer bugid, Integer orgid);
	...
	//=====外规==========BEGIN==========
	@SelectProvider(method="selectOuterCommonListByInfo",type=TblNbsjBugMapperSqlConfig.class)
    @Results({
	...
	List<TblNbsjOuterruleEntity> selectOuterCommonListByInfo( Integer bugid, Integer orgid);
...
//	void insertBugOutrulids(String outrulid, Integer bugid);
	...
	void insertSheetBug(Integer  sheetid,Integer  bugid);
	@Delete("DELETE FROM TBL_NBSJ_SHEET_BUG WHERE SHEETID =#{sheetid}")
    void deleteSheetBug(Integer sheetid) throws Exception;
	
	
	@Select(" SELECT * FROM TBL_NBSJ_BUG WHERE BUGID IN (SELECT BUGID FROM TBL_NBSJ_SHEET_BUG WHERE SHEETID =#{SHEETID})")
	List<TblNbsjBugEntity> selectNbsjBugList(Integer SHEETID);
