 ...
 */
public class TblYqnsSqdcbgMapperSqlConfig {

    public String selectCountByPageInfo(PageInfo<TblYqnsSqdcbg> pageInfo, TblYqnsSqdcbg vo) {
        StringBuffer sb = new StringBuffer("SELECT COUNT(0) "
                + " FROM TBL_YQNS_SQDCBG TBL1 "
                + " WHERE 1=1");
    ...
    public String selectListByPageInfo(PageInfo<TblYqnsSqdcbg> pageInfo, TblYqnsSqdcbg vo) {
        ...
        sb.append(" ORDER BY TBL1.sqdcbgid DESC) T1 WHERE ROWNUM <= " + (pageInfo.getCurrentRecord() + pageInfo.getPageSize()) + " ) T2 WHERE T2.RN > " + pageInfo.getCurrentRecord());
        return sb.toString();
    }

    private void sqlQuery(TblYqnsSqdcbg vo, StringBuffer sb) {
        if (vo.getOrgid() != null) {
