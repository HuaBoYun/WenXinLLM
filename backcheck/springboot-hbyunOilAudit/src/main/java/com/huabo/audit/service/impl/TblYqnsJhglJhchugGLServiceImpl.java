 ...
 * @description 针对表【TBL_YQNS_JHGL_Jh(计划初稿管理计划) 关联 】的数据库操作Service实现
 */
@Service
public class TblYqnsJhglJhchugGLServiceImpl extends ServiceImpl<TblYqnsJhglJhchugGLMapper, TblYqnsJhglJhchugGL>
        implements TblYqnsJhglJhchugGLService {

	@Resource
	private TblYqnsJhglJhchugGLMapper tblYqnsJhglJhchugGLMapper;
    ...
    @Override
    @Transactional
    public JsonBean saveOrUpdateList(String token, long Jhchugid, List<TblYqnsJhglJhchugGL> voList) throws Exception {
        // 验证token
     ...
     */
    @Override
    public List<TblYqnsJhglJhchugGL> findListByJHCHUGID(String token,String Jhid) {
        // 进行数据获取和查询
        LambdaQueryWrapper<TblYqnsJhglJhchugGL> query = new LambdaQueryWrapper<TblYqnsJhglJhchugGL>()
                .eq(StringUtil.isNotEmpty(Jhid), TblYqnsJhglJhchugGL::getJhchugid, Jhid);
        List<TblYqnsJhglJhchugGL>  list= this.baseMapper.selectList(query);
        return list;
    }
    ...
    @Override
    @Transactional
    public JsonBean saveOrUpdate(String token, TblYqnsJhglJhchugGL entity) throws Exception {
        // 验证token
        TblStaffUtil loginStaff = userProvider.get();
        if (loginStaff == null) {
            return ResponseFormat.retParam(0, 20006, null);
    ...
    public JsonBean deleteGL(String token, String id) throws Exception {
        ...
        if (id != null) {
            ...
            this.tblYqnsJhglJhchugGLMapper.deleteGlRela(id);
        }
        return ResponseFormat.retParam(1, 200, Boolean.TRUE);
    }
	...
	public JsonBean deleteGLByIds(String token, BigDecimal jhcgid, BigDecimal formid) throws Exception {
        ...
        QueryWrapper<TblYqnsJhglJhchugGL> wrapper = new QueryWrapper<TblYqnsJhglJhchugGL>();
        wrapper.eq("JHCHUGID", jhcgid);
	...
	public JsonBean getSjdwlrsjSbList(String token, BigDecimal relaId, BigDecimal id, Integer pageNumber,
        ...
        Page<LeaveAudit3LEntity> page = PageHelper.startPage(pageNumber, pageSize).doSelectPage(() -> this.leaveAudit3LMapper.selectListByjhchugGlId(id,relaId));
	...
	public JsonBean getGcxmjsListByhz(String token, BigDecimal relaId, BigDecimal id, Integer pageNumber,
        ...
        Page<TblYqnsGcxmzjZjb> page = PageHelper.startPage(pageNumber, pageSize).doSelectPage(() -> this.tblYqnsGcxmzjZjbMapper.selectListByjhchugGlRela(id,relaId));
	...
	public JsonBean getJsxmtzListByhz(String token, BigDecimal relaId, BigDecimal id, Integer pageNumber,
        ...
        Page<TblYqnsJsxmTzwcqk> page = PageHelper.startPage(pageNumber, pageSize).doSelectPage(() -> this.tblYqnsJsxmTzwcqkMapper.selectListByjhchugGlId(id,relaId));
	...
	@Override
	public List<TblYqnsJhglJhchugGL> findListByJHCGIDByChugao(String token, String jhchuid) throws Exception {
		QueryWrapper<TblYqnsJhglJhchugGL> query = new QueryWrapper<TblYqnsJhglJhchugGL>();
	     query.eq("JHCHUGID", jhchuid);
	     query.notInSql("ID", "(SELECT RELAID FROM TBL_YQNS_JHGL_JH_GL WHERE RELAID IS NOT NULL)");
	     List<TblYqnsJhglJhchugGL>  list= this.baseMapper.selectList(query);
	     return list;
	}
	...
	@Override
	public JsonBean getJhChuGHuizongList(String token, String jhchugid, String relaid, String glType) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
        if (loginStaff == null) {
            return ResponseFormat.retParam(0, 20006, null);
        }
        List<TblYqnsJhglJhchugGL> list =  this.tblYqnsJhglJhchugGLMapper.selecthzsjListByJhchugId(jhchugid,relaid,glType);
