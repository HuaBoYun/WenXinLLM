package com.huabo.monitor.mapper.Provider;
...
//                "ENDDATE"
        ...
        if (null != staffid) {
			if (Objects.equals(authorityType,0)) {
				sql.AND().WHERE(" (t.createstaffid = #{staffid} or t.leaderid = #{staffid}) ");
			}
        }
        ...
        sql.ORDER_BY("t.assid desc");  //t.assstatus asc,
    ...
    public String findPageBeanPJJG(   @Param("assNumnber") String assNumnber,
        ...
        //提供一些字段
        SQL sql = new SQL().SELECT_DISTINCT(
                "t.*","ta.templename","TAT.FINALSCORE as FINALSCORENEW","TAT.CHECKLEVEL","TAT.CHECKREASON","TAT.FINALLEVEL"

        ).FROM(" TBL_ASSESS t " +
                " inner join TBL_ASSESSTEMPLE ta on t.asstemid=ta.asstemid "+
                " left join TBL_ASSESS_MARK TAM on t.ASSID = TAM.ASSID "
				+ " left join TBL_ASSESS_TARGET TAT on TAT.assid = t.ASSID "
        );


        sql.WHERE(" t.assstatus in (3,4) and tblcomany='"+staff.getCurrentOrg().getOrgid()+"'");
		if (Objects.equals(authorityType,0)) {
			sql.AND().WHERE(" (t.createstaffid = '"+staff.getStaffid()+"' " + " or t.leaderid = '"+ staff.getStaffid() +"') ");
		}

        sql.AND().WHERE(" t.CREATESTAFFID = '"+staff.getStaffid()+"'");
    ...
    评价立项 和 跟踪列表
  */
 public String findPageBeanPJJH(
         @Param("isAudit") boolean isAudit,
         @Param("orgid") BigDecimal orgid,
         @Param("staffid") BigDecimal staffid,
         @Param("realName") String realName,
