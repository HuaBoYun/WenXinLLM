...
public class PjjgServiceImpl implements PjjgService {
    ...
    @Override
    public IPage<TblAssessTargetVo> findMarkByOrgGroupZuPing(BigDecimal assId, BigDecimal staffid,Integer pageNumber,TblStaffUtil staff) {
        IPage<TblAssessTargetVo> iPage = new Page<>(pageNumber, ConstClass.DEFAULT_SIZE);
        String sql="";
        StringBuffer querySql=new StringBuffer();
        try {
        	   querySql.append(" or ( lin.staffid = ").append(staffid).append(" or TA.leaderid=").append(staffid).append(")");
              sql = GeneralSQLConcatConfig.concatSecrectSqlCase2(staff.getCurrentOrg().getUseSecrect(),true, "TA.LINKORGID", "TA.linkdeptid", "TA.createstaffid", "TA.SECRECTLEVELID", "TA.STAFFSCOPEIDS", staff.getStaffid(), staff.getDeptIds(), staff.getSecrectScopeIds(),querySql.toString()); 
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
		}
        return assessTargetMapper.findMarkByOrgGroupZuPing(iPage,assId,staffid,sql);
    }
        ...
        if (!assess.getAssstatus().equals(TblAssess.JISUAN)) {
            ...
            for (TblAssessMarkVo tblAssessMark : assessMarks) {
               // Set<TblAssessStaffVo> assessStaffs = tblAssessMark.getCanpingrens();
            	List<TblAssessStaffVo> assessStaffs = tblAssessMark.getCanpingrens();
                double sum = 0;
            ...
            if (list.size() == 0) {
                ...
                // 计算最终得分   原方法  getSumScoreByAssidAndOrgId
                Double sum1 = this.markVoMapper.getSumScoreByAssidAndOrgIdNew(assId,orgid);
                ...
                if (assessTargets.size() > 0) {
                    ...
                    List<TblAssesslevel> assesslevels = this.tblAssesslevelMapper
                            .getRegionNew(new BigDecimal(sum1).setScale(0, BigDecimal.ROUND_HALF_UP).doubleValue(),staff.getCurrentOrg().getOrgid());
    ...
    public JsonBean yaoSuMingXi(BigDecimal assId, BigDecimal orgid) {
        ...
        final List<Map<String, Object>> tixiquanzhong = this.markVoMapper.getPingFenTiXiQuanZhongNew(assId, orgid);
        tixiquanzhong.stream().map(x->{ x.put("CATW", returnDoublePer(x.get("CATW")));return x;}).collect(Collectors.toList());
        List<Map<String, Object>> pingfenyaosu =  this.markVoMapper.getPingFenYaoSuNew(assId, orgid);  //getPingFenYaoSu
        pingfenyaosu.stream().map(x->{ x.put("CATW", returnDoublePer(x.get("CATW")).replace("%", ""));return x;}).collect(Collectors.toList());
...
//        mv.put("assessMarks",assessMarks);
    ...
    public  Map<String, Object> yaoSuMingXi2(BigDecimal assId, BigDecimal orgid) {
        ...
        TblAssessVo tblAssessVo = this.tblAssessMapper.queryTblAssessVoByAssidAndOrgid(assId, orgid);
        //原方法getPingFenTiXiQuanZhong
        final List<Map<String, Object>> tixiquanzhong = this.markVoMapper.getPingFenTiXiQuanZhongNew(assId, orgid); 
      //原方法getPingFenYaoSu
        List<Map<String, Object>> pingfenyaosu =  this.markVoMapper.getPingFenYaoSuNew(assId, orgid); 
	...
	@Override
	public PageInfo<TblAssessTargetVo> findMarkByOrgGroupZuPingNew(BigDecimal assId, BigDecimal staffid,
			Integer pageNumber) throws Exception {
		// TODO Auto-generated method stub
		return PageMethod.startPage(pageNumber,ConstClass.DEFAULT_SIZE).doSelectPageInfo(()->assessTargetMapper.findMarkByOrgGroupZuPingNew(assId,staffid));
	}
	
	//给数字加百分比 1带百分比 2 不带百分比
	...
	@Override
	public PageInfo<TblAssessTargetVo> getOrgListNew(BigDecimal assId, Integer pageNumber) throws Exception{
		// TODO Auto-generated method stub
		return  PageMethod.startPage(pageNumber,ConstClass.DEFAULT_SIZE).doSelectPageInfo(()->assessTargetMapper.getOrgListNew(assId));
	}
