...
public interface TblBugCriterionMapper extends BaseMapper<TblBugCriterionEntity>{
	...
	List<TblBugCriterionEntity> findList(@Param("queryParam")TblBugCriterionEntityParam queryParam,@Param("sql")String sql);
	...
	@Select("SELECT max(BUGCRIID) FROM TBL_BUGCRITERION where ORGID=#{orgid} GROUP BY BUGCRILEVEL ")
	List<Integer> selectMaxIds(@Param("orgid")String orgid);
}
