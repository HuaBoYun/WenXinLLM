...
import com.huabo.monitor.mysql.entity.TblLoginTypeMySql;
 ...
 * @since 2021-10-20
 */
public interface TblLoginTypeMySqlMapper extends BaseMapper<TblLoginTypeMySql> {

    @Select("SELECT LOGINID FROM TBL_LOGINTYPE where LOGINURL = #{loginType}")
    TblLoginTypeMySql selectLoginTypeInfo(@Param("loginType") String logintype) throws Exception;



    @SelectProvider(type=TblLoginTypeMapperSqlMySqlConfig.class,method="selectListByPageInfo")
    @Results({
    ...
    List<TblLoginTypeMySql> selectListByPageInfo(PageInfo<TblLoginTypeMySql> pageInfo,@Param("orgid") BigDecimal orgid)throws Exception;


    @Select("select count(*) from TBL_ORGANIZATION where ORGTYPE < 100 and STATUS = 0 ")
    TblOrganizationMySql selectcOunt();

    @Select("select ORGNAME,ORGNUMBER,ORGID from TBL_ORGANIZATION where ORGTYPE = 100  ORDER BY orderid ASC")
    List<TblOrganizationMySql> selectIndustry();
    ...
    TblLoginTypeMySql findByLoginId(String loginid);


    @Delete("DELETE FROM TBL_LOGINTYPE WHERE LOGINID= #{loginid}")
    void deleteByLoginId(String loginid);

    @UpdateProvider(type=TblLoginTypeMapperSqlMySqlConfig.class,method="updatetblLoginType")
    void updatetblLoginType(TblLoginTypeMySql tblt);

    @InsertProvider(method="saveTblLoginType",type=TblLoginTypeMapperSqlMySqlConfig.class)
    void saveTblLoginType(TblLoginTypeMySql tnt);
}
