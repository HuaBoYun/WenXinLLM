...
public interface  TblAssessTargetMapper extends BaseMapper<TblAssessTarget> {
   ...
   List<TblAssessTargetVo> findMarkByOrgGroupZuPingNew(@Param("assId") BigDecimal assId,@Param("staffid") BigDecimal staffid);

   List<TblAssessTargetVo> getOrgListNew(@Param("assId") BigDecimal assId);
   ...
   @Select("\n" +
           ...
           "                     TBL_STAFF TS on TS.STAFFID=TA.LEADERID where  1=1 ${sql}")

          <p extends IPage<TblAssessTargetVo>>  p  findMarkByOrgGroupZuPing(p page, @Param("assId") BigDecimal assId,@Param("staffid") BigDecimal staffid,@Param("sql") String sql);
    /**
     *
