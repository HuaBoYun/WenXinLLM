...
public interface TblOrganizationMapper extends BaseMapper<TblOrganization> {
	...
	List<Map<String,Object>> getOrgByassIdNew(@Param("queryParam")TblOrganization queryParam);
...
//	@SelectProvider(method="selectCountPage",type=TblOrganizationMapperSqlConifg.class)
   ...
   // @Select("SELECT * FROM TBL_ORGANIZATION WHERE FATHERORGID = #{parentId} AND ORGTYPE != 0 AND ORGTYPE < 100 ")
	List<TblOrganization> selectCompanyListParentId(@Param("parentId")Integer parentId) throws Exception;
...
//	List<TblOrganization> selectWpzjk(String orgid, String iszy);
    ...
    @Select("select * from Tbl_Organization where orgtype=0 ")
	List<TblOrganization> getOrgidOrgtype();
    
//    @Select("select * from Tbl_Organization where ORGTYPE >= 100")
 	List<TblOrganization> getOrgidAutoNumber();
    
    @Select("select * from Tbl_Organization")
	List<TblOrganization> getOrgAll();

	@Select("SELECT * FROM TBL_ORGANIZATION WHERE ORGID = (SELECT FATHERORGID FROM TBL_ORGANIZATION WHERE ORGID =  #{orgid}  )")
	TblOrganization selectFatherOrgIdInfoByOrgIdIss(BigDecimal orgid);
