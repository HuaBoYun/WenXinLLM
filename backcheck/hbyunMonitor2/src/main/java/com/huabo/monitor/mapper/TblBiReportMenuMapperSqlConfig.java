...
import com.huabo.monitor.entity.TblBiReportMenu;
import com.huabo.monitor.entity.TblCourse;

import java.math.BigDecimal;

public class TblBiReportMenuMapperSqlConfig {

    public String selectType(PageInfo<TblBiReportMenu> pageInfo, String type,BigDecimal orgid) {
        		StringBuffer sbSql = new StringBuffer("select * from (SELECT BUDGET.*,ROWNUM RNUM FROM  (select TBL_BI_REPORT_MENU.*,O.ORGNAME from TBL_BI_REPORT_MENU LEFT JOIN TBL_ORGANIZATION O ON TBL_BI_REPORT_MENU.UNIT = O.ORGID WHERE 1=1 ");
                if ("0".equals(type)) {
        		...
        		sbSql.append(" order by PAGEID) BUDGET WHERE ROWNUM <= "+(pageInfo.getCurrentPage()*pageInfo.getPageSize())+" ) WHERE RNUM > "+pageInfo.getCurrentRecord());
                 return sbSql.toString();
    }

    public String selectOrgid(PageInfo<TblCourse> pageInfo, String type, BigDecimal orgid) {
        StringBuffer sbSql = new StringBuffer("select count(*) from TBL_BI_REPORT_MENU  WHERE 1=1 AND UNIT=" + orgid + "");
        if ("0".equals(type)) {
            sbSql.append(" AND PAGEBODY IS NULL ");
            sbSql.append(" AND (TYPE = 2 OR TYPE = 0)");
        }else if("1".equals(type)) {
    ...
    public String selectMenuList(PageInfo<TblBiReportMenu> pageInfo, Integer type, BigDecimal orgid) {
        StringBuffer sbSql = new StringBuffer("SELECT * FROM (SELECT BUDGET.*,ROWNUM RNUM FROM (select * from TBL_BI_REPORT_MENU WHERE 1=1 AND UNIT=" + orgid + " AND PAGEBODY IS NULL");
    ...
    public String insertTbrm(TblBiReportMenu tbrm) {
        ...
        if(tbrm.getCreatedate() != null) {
            ...
            value.append(",TO_DATE('"+ DateUtil.parseDate(tbrm.getCreatedate(),"yyyy-MM-dd HH:mm:ss") +"', 'YYYY-MM-DD HH24:MI:SS')");
        }
