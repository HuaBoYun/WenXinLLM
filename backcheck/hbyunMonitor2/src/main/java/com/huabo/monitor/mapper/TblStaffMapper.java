...
public interface TblStaffMapper extends BaseMapper<TblStaff> {
	...
	List<TblStaff> findStaffByOrgid(@Param("queryParam") TblStaff queryParam);

	
	@Select("select realname from tbl_staff where staffid in(#{ids})")
	List<TblStaff> getOrgNameByScopeids(@Param("ids")String  ids);
