package com.huabo.monitor.service.impl;

import com.huabo.monitor.entity.TblTestTemplate;
import com.huabo.monitor.entity.TblTestelement;
import com.huabo.monitor.mapper.TblTestelementMapper;
import com.huabo.monitor.service.TblTestElementService;
import com.huabo.monitor.util.FiexibleNameAssignment;
import com.huabo.monitor.util.PageResult;
import com.huabo.monitor.vo.param.fieldOrgStaffId;
import com.huabo.monitor.vo.param.fieldOrgStaffName;

import cn.hutool.db.Page;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.core.toolkit.StringUtils;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.github.pagehelper.page.PageMethod;
import com.hbfk.entity.DealUserToken;
import com.hbfk.entity.TblStaffUtil;
import com.hbfk.util.JsonBean;
import com.hbfk.util.PageInfo;
import com.hbfk.util.ResponseFormat;
import com.hbfk.util.user.UserProvider;

import java.io.IOException;
import java.math.BigDecimal;
import java.util.List;
import java.util.Objects;

import javax.annotation.Resource;

import org.apache.commons.collections.CollectionUtils;
import org.springframework.beans.BeanUtils;
import org.springframework.stereotype.Service;

/**
 * <p>
 *  服务实现类
 * </p>
 *
 * @author yhr
 * @since 2022-09-08
 */
@Service
public class TblTestelementServiceImpl extends ServiceImpl<TblTestelementMapper, TblTestelement> implements TblTestElementService {

	@Resource
	private TblTestelementMapper tblTestelementMapper;
	
	@Resource
    private UserProvider userProvider;
	
	@Override
	public JsonBean save(String token, TblTestelement newEle) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
        if (loginStaff == null) {
            return ResponseFormat.retParam(0, 20006, null);
        }
        
        this.tblTestelementMapper.insertEntity(newEle);
        
        return ResponseFormat.retParam(1, 200, newEle);
	}

	@Override
	public JsonBean modify(String token, TblTestelement newEle) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
        if (loginStaff == null) {
            return ResponseFormat.retParam(0, 20006, null);
        }
        
        this.tblTestelementMapper.updateById(newEle);
        
        return ResponseFormat.retParam(1, 200, newEle);
	}

	@Override
	public JsonBean remvoe(String token, BigDecimal elementid) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
        if (loginStaff == null) {
            return ResponseFormat.retParam(0, 20006, null);
        }
        
        this.tblTestelementMapper.deleteById(elementid);
        
        return ResponseFormat.retParam(1, 200, null);
	}

	@Override
	public JsonBean getPageInfo(String token, Integer pageNumber, Integer pageSize, BigDecimal typeid,
			BigDecimal templid, String elementcode, String businessdesc) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
        if (loginStaff == null) {
            return ResponseFormat.retParam(0, 20006, null);
        }
        TblTestelement ele = new TblTestelement();
        ele.setTypeid(typeid);
        ele.setTemplid(templid);
        ele.setElementcode(elementcode);
        ele.setBusinessdesc(businessdesc);
		
//        PageInfo<TblTestelement> pageInfo = new PageInfo<TblTestelement>();
//        pageInfo.setCurrentPage(pageNumber);
//        pageInfo.setPageSize(pageSize);
//        pageInfo.setCondition(ele);
//        
//        pageInfo.setTlist(this.tblTestelementMapper.selectPageList(pageInfo));
//        pageInfo.setTotalRecord(this.tblTestelementMapper.selectPageCount(pageInfo));
//        
//        
      QueryWrapper<TblTestelement> query=new QueryWrapper<>();
      if(Objects.nonNull(typeid)){
    	  query.eq("typeid", typeid);
      }
      if(Objects.nonNull(templid)){
    	  query.eq("templid", templid);
      }
      if(StringUtils.isNotBlank(elementcode)){
    	  query.like("elementcode", elementcode);
      }
      if(StringUtils.isNotBlank(businessdesc)){
    	  query.like("businessdesc", businessdesc);
      }
		com.github.pagehelper.PageInfo<TblTestelement> pageInfo = PageMethod.startPage(pageNumber, pageSize)
				.doSelectPageInfo(() -> tblTestelementMapper.selectList(query));
		
		FiexibleNameAssignment ment=new FiexibleNameAssignment();
		if(CollectionUtils.isNotEmpty(pageInfo.getList())){
			pageInfo.getList().forEach(entity->{
				try {
					//对灵活字段中的姓名名称及机构名称赋值
					fieldOrgStaffId item=new fieldOrgStaffId();
					BeanUtils.copyProperties(entity,item); 
					fieldOrgStaffName nameEntity=ment.setOpenName(item);
					BeanUtils.copyProperties(nameEntity,entity ); 
					item=null; // 处理并解除引用
					nameEntity=null; // 处理并解除引用
			} catch (Exception e) {
				// TODO: handle exception
				e.printStackTrace();
			}
			} );
			
		}
		PageResult<TblTestelement> build = new PageResult<TblTestelement>().build(pageInfo);
		return ResponseFormat.retParam(1, 200, build);
	}

	@Override
	public JsonBean getInfo(String token, BigDecimal elementid) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
        if (loginStaff == null) {
            return ResponseFormat.retParam(0, 20006, null);
        }
        
		TblTestelement ele = this.tblTestelementMapper.selectEntityById(elementid);
		FiexibleNameAssignment ment = new FiexibleNameAssignment();
		if(ele!=null){
		// 对灵活字段中的姓名名称及机构名称赋值
		fieldOrgStaffId item = new fieldOrgStaffId();
		BeanUtils.copyProperties(ele, item);
		fieldOrgStaffName nameEntity = ment.setOpenName(item);
		BeanUtils.copyProperties(nameEntity, ele);
		}
        return ResponseFormat.retParam(1, 200, ele);
	}

	@Override
	public List<TblTestelement> findByPlanidAll(String planid) throws Exception {
		// TODO Auto-generated method stub
		return tblTestelementMapper.findByPlanidAll(planid);
	}

	@Override
	public List<TblTestelement> findByPlanidJhn(String planid) throws Exception {
		// TODO Auto-generated method stub
		return tblTestelementMapper.findByPlanidJhn(planid);
	}
	
}
