...
public interface YhrPageMapper {
     ...
     @Select("${sql}")
     public List<Map<String, Object>> queryBySqlToList(@Param("sql") String sql);

     @Select("${sql}")
     public List<TestElementResult> getMap(@Param("sql") String sql);
     
     @Select("${sql}")
     List<LinkedHashMap<String, Object>> queryBySqlToListLinkMap(@Param("sql") String sql);
