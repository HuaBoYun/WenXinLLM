...
import com.huabo.monitor.entity.TblTesttemple;
...
public interface TblTestTemplateMapper extends BaseMapper<TblTestTemplate> {
	...
	List<TblTestTemplate> findList(@Param("queryParam") TblTestTemplate queryParam,@Param("sql") String sql);
	
    @Delete("DELETE FROM TBL_TESTTEMPLE WHERE TEMPLENUMBER = #{templeNumber}")
    ...
    @Select("SELECT TT.TESTTEMID,TT.TEMPLENUMBER,TT.TEMPLENAME,TT.MEMO,TT.STAFFID,TT.CREATETIME,TT.TEMPLESTATUS,TT.TEMPLEDESC,TT.SOURCE,TT.TBLCOMANY,TS.REALNAME,TT.SECRECTLEVELID,TT.STAFFSCOPEIDS,TT.STAFFSCOPENAMES  FROM TBL_TESTTEMPLE TT LEFT JOIN TBL_STAFF TS ON TT.STAFFID = TS.STAFFID WHERE TT.TESTTEMID = #{tempId}")
    @Results({
    	@Result(column="TESTTEMID",property="testtemid"),
    	@Result(column="TEMPLENUMBER",property="templeNumber"),
    	@Result(column="TEMPLENAME",property="templename"),
    ...
    @Select("select count(*) from TBL_TESTTEMPLE where TEMPLENUMBER = #{templeNumber} and tblComany=#{tblComany}")
    Integer selectCountForNumber(@Param("templeNumber")String templeNumber,@Param("tblComany")String tblComany) throws Exception;
    
    @Select("select count(*) from TBL_TESTPLAN where testtemid = #{testtemid} ")
    Integer csmb_editcheck(Integer testtemid) throws Exception;
