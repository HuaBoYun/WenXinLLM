...
public class TblTestplanMapperSqlConfig {
	...
	public String insertEntity(TblTestplan tblTestplan) {
		String colsql = "INSERT INTO TBL_TESTPLAN(TESTPLANID";
		String valueSql = " VALUES (HIBERNATE_SEQUENCE.nextval";
				...
				if(tblTestplan.getAssid()!=null){
					colsql+=",ASSID";
					valueSql+=",'"+tblTestplan.getAssid()+"'";
				}
				if(StringUtils.isNotBlank(tblTestplan.getAssessname())){
					colsql+=",ASSESSNAME";
					valueSql+=",'"+tblTestplan.getAssessname()+"'";
				}
				if(tblTestplan.getAssessid()!=null){
					colsql+=",ASSESSID";
					valueSql+=",'"+tblTestplan.getAssessid()+"'";
				}
	...
	public String updateEntity(TblTestplan tblTestplan) {
		String sql = "UPDATE TBL_TESTPLAN SET PLANNUMBER="+"'"+tblTestplan.getPlannumber()+"'";
		if(StringUtils.isNotBlank(tblTestplan.getPlanyear())){
		...
		if(tblTestplan.getAssid()!=null){
			sql+=",ASSID="+tblTestplan.getAssid()+"";
		}
		if(StringUtils.isNotBlank(tblTestplan.getAssessname())){
			sql+=",ASSESSNAME='"+tblTestplan.getAssessname()+"'";
		}
		if(tblTestplan.getAssessid()!=null){
			sql+=",ASSESSID='"+tblTestplan.getAssessid()+"'";
		}
