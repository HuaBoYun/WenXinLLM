...
public class TblTestTemplateMapperSqlConfig {
	...
	public String insertEntity(TblTestTemplate tblTestTemplate) {
		String colsql = "INSERT INTO TBL_COM_EXT_TESTTEMPLE(TESTTEMID, TEMPLENUMBER, TEMPLENAME,CREATEDATETIME,STAFFID, TBLCOMANY ,SOURCE ";
	...
	public String updateEntity(TblTestTemplate tblTestTemplate) {
		String sql = "UPDATE TBL_COM_EXT_TESTTEMPLE SET TEMPLENUMBER = '"+tblTestTemplate.getTempleNumber()+"', TEMPLENAME = '"+tblTestTemplate.getTemplename()+"'";
		if (StringUtils.isNotBlank(tblTestTemplate.getMemo())) {
	...
	public String selectPageInfo(PageInfo<TblTestTemplate> pageInfo) {
		...
		String sql = "SELECT * FROM ( SELECT TMP.*, ROWNUM ROW_ID FROM ( SELECT  TT.TESTTEMID,TT.TEMPLENUMBER,TT.TEMPLENAME,TT.MEMO,TT.STAFFID,TT.CREATEDATETIME,TT.TEMPLESTATUS,TT.TEMPLEDESC,TT.SOURCE,TT.TBLCOMANY,TS.REALNAME,TT.ISSUED FROM TBL_COM_EXT_TESTTEMPLE TT LEFT JOIN TBL_STAFF TS ON TT.STAFFID = TS.STAFFID WHERE TT.TBLCOMANY = "+temp.getTblComany();
	...
	public String selectPageCount(PageInfo<TblTestTemplate> pageInfo) {
		...
		String sql = "SELECT COUNT(0) FROM TBL_COM_EXT_TESTTEMPLE TT LEFT JOIN TBL_STAFF TS ON TT.STAFFID = TS.STAFFID WHERE TT.TBLCOMANY = "+temp.getTblComany();
