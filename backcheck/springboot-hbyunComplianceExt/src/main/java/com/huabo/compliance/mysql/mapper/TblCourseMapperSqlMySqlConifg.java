...
import com.huabo.compliance.mysql.entity.TblCourseMySql;
import org.apache.commons.lang.StringUtils;

import java.math.BigDecimal;

public class TblCourseMapperSqlMySqlConifg {


    public String selectListByPageInfo(PageInfo<TblCourseMySql> pageInfo, BigDecimal orgid, String coursename1, String coursetype1) {
        StringBuffer sqlSb = new StringBuffer("SELECT * from TBL_COURSE where 1=1 AND PARENTID IS NULL AND ORGID=" + orgid);
        ...
        sqlSb.append(" ORDER BY COURSEID DESC LIMIT " + (pageInfo.getCurrentPage() * pageInfo.getPageSize()) + " , " + pageInfo.getCurrentRecord());
        String sql = sqlSb.toString();
        return sql;
    }

    public String selectCountByPageInfo(PageInfo<TblCourseMySql> pageInfo, BigDecimal orgid, String coursename1, String coursetype1) {
        String sqlCount = "select count(*) from TBL_COURSE  WHERE 1=1 AND PARENTID IS NULL AND ORGID=" + orgid;
        if (!StringUtils.isBlank(coursename1)) {
            sqlCount += " and COURSENAME like '%" + coursename1.trim() + "%'";
        }
        if (!StringUtils.isBlank(coursetype1)) {
            sqlCount += " and COURSETYPE like '%" + coursetype1.trim() + "%'";
        }
        sqlCount += " order by COURSEID";
        return sqlCount;
    }

    public String updatetblCourse(TblCourseMySql tblCourse) {
        ...
        if (tblCourse.getCreateDate() != null) {
            sql.append(" ,CREATEDATE = '" + DateUtil.parseDate(tblCourse.getCreateDate(), "yyyy-MM-dd HH:mm:ss") + "'");
        }
        if (tblCourse.getCoursetype() != null && !"".equals(tblCourse.getCoursetype())) {
            sql.append(" , COURSETYPE = '" + tblCourse.getCoursetype() + "'");
        }
        if (tblCourse.getCoursenumber() != null && !"".equals(tblCourse.getCoursenumber())) {
    ...
    public String savetblCourse(TblCourseMySql tblCourse) {
        StringBuffer column = new StringBuffer("INSERT INTO TBL_COURSE (COURSEID");
        StringBuffer value = new StringBuffer(" VALUES ("+tblCourse.getCourseid()+" ");
        ...
        if (tblCourse.getCreateDate() != null) {
            ...
            value.append(",'" + DateUtil.parseDate(tblCourse.getCreateDate(), "yyyy-MM-dd HH:mm:ss") + "'");
        }
