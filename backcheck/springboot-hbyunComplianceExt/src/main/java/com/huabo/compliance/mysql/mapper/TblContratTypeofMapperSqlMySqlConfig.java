...
public class TblContratTypeofMapperSqlMySqlConfig {
    ...
    public String selectPageInfoList(PageInfo<TblContractTypeofMySql> pageInfo) {
        ...
        StringBuffer sqlSb = new StringBuffer("SELECT * FROM (SELECT BUDGET.* FROM (SELECT * from TBL_CONTRACT_TYPEOF WHERE ORGID = " + condition.getOrgId());
        ...
        sqlSb.append(" ORDER BY TYPEID DESC ) BUDGET LIMIT" + (pageInfo.getCurrentPage() * pageInfo.getPageSize()) + " , " + pageInfo.getCurrentRecord());
