...
public interface TblTesttemplTypeMapper extends BaseMapper<TblTesttemplType> {
    ...
    @Select("select * from  TBL_COM_EXT_TESTTEMPL_TYPE t where t.parentid=#{id}")
    List<TblTesttemplType> getchild(BigDecimal id);

    @Select(" select count(*) from TBL_COM_EXT_TESTTEMPL_TYPE t where t.parentid  =#{id}")
    Long getChildCount(BigDecimal id);

    @Select("SELECT * FROM TBL_COM_EXT_TESTTEMPL_TYPE WHERE TESTTEMPLETAID = #{tempId} AND PARENTID IS NULL")
    ...
    @Select("SELECT * FROM TBL_COM_EXT_TESTTEMPL_TYPE WHERE PARENTID = #{typeid}")
    ...
    @Select("SELECT COUNT(0) FROM TBL_COM_EXT_TESTTEMPL_TYPE WHERE PARENTID = #{typeid}")
	Integer selectChidrenCount(BigDecimal typeid) throws Exception;

    @Select("${sql}")
	void executeDelSql(@Param("sql") String sql) throws Exception;

    @Select("SELECT * FROM TBL_COM_EXT_TESTTEMPL_TYPE WHERE TYPEID = #{typeid}")
