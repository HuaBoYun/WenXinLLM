...
import com.huabo.compliance.mysql.entity.TblAccBookMySql;
...
public interface TblAccBookMySqlMapper extends BaseMapper<TblAccBookMySql> {
    ...
    List<TblAcctBookMySql> findAccBookByOrgId(BigDecimal orgId);
    ...
    List<TblAccBookMySql> listBySql(@Param("staffid") BigDecimal staffid, @Param("orgid") BigDecimal orgid);

    @Select("SELECT * FROM TBL_ACCBOOK WHERE BOOKID = #{connectionstrings}")
    TblAccBookMySql findByBookIdOne(String connectionstrings);
}
