package com.regulatory.penetration.util.excel;

import com.regulatory.penetration.util.excel.annotation.ExcelField;
import com.regulatory.penetration.util.excel.annotation.ExcelFields;
import com.regulatory.penetration.util.excel.fieldtype.FieldType;
...
public class ExcelExport implements Closeable {
	...
	public Cell addCell(Row row, int column, Object val) {
		return this.addCell(row, column, val, ExcelField.Align.AUTO, FieldType.class, null);
	}
	...
	public Cell addCell(Row row, int column, Object val, ExcelField.Align align, Class<? extends FieldType> fieldType, String dataFormat) {
		...
		try {
			if (val == null) {
				...
				if (val instanceof String) {
					...
					fieldType = (Class<? extends FieldType>) Class.forName(this.getClass().getName()
			...
			for (Object[] os : annotationList) {
				...
				try {
					if (StringUtils.isNotBlank(ef.attrName())) {
						val = ReflectionUtil.invokeGetter(e, ef.attrName());
					} else {
						if (os[1] instanceof Field) {
							val = ReflectionUtil.invokeGetter(e, ((Field) os[1]).getName());
						} else if (os[1] instanceof Method) {
							//							val = ReflectionUtil.invokeMethod(e, ((Method) os[1]).getName(), new Class[]{},
							//									new Object[]{});
					...
					if (StringUtils.isNotBlank(ef.dictType())) {
						...
						if (dictMap != null && dictMap.containsKey(val)) {
							val = dictMap.get(val);
						}
					}
				} catch (Exception ex) {
	...
	public ExcelExport write(HttpServletResponse response, String fileName) {
		...
		response.addHeader("Content-Disposition", "attachment; filename*=utf-8'zh_cn'" + EscapeUtil.urlEncode(fileName));
		try {
