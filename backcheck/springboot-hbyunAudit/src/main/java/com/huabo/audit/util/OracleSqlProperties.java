...
public class OracleSqlProperties {
	...
	public JsonBean execSql(String sql) throws Exception {
		...
		if (upperSql.startsWith("SELECT") || upperSql.startsWith("SHOW") || upperSql.startsWith("DESC") || upperSql.startsWith("EXPLAIN")) {
			// 查询语句使用 executeQuery
			rsOracle = stmtGRC.executeQuery(sql);
			// 处理结果集...
		} else {
			// DDL 和 DML 语句使用 executeUpdate
			int result = stmtGRC.executeUpdate(sql);
