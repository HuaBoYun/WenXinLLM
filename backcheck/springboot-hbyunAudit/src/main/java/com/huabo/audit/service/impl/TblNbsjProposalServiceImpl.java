...
public class TblNbsjProposalServiceImpl implements TblNbsjProposalService {
	...
	public JsonBean suggestPageList(String token, Integer pageNumber, Integer pageSize,
			TblNbsjProposalVo tblNbsjProposalVo,BigDecimal projectId) throws Exception {
		...
		tblNbsjProposalVo.setProjectId(projectId);
...
//    	pageInfo.setCondition(tblNbsjWorkReport);
    	...
    	pageInfo.setTlist(this.tblNbsjProposalMapper.selectListByPageInfo(pageInfo,tblNbsjProposalVo,projectId));
    	pageInfo.setTotalRecord(this.tblNbsjProposalMapper.selectCountByPageInfo(pageInfo,tblNbsjProposalVo,projectId));
    	pageInfo.getTotalPage();*/
		//链表分页  xml 写法
		com.github.pagehelper.PageInfo<TblNbsjSheetEntity> pageInfo = PageMethod.startPage(pageNumber, pageSize)
				.doSelectPageInfo(() -> tblNbsjProposalMapper.selectListByPageInfoXml(tblNbsjProposalVo));
	...
	public JsonBean suggestAdd(TblNbsjProposalEntity proposal, String token,String attids) throws Exception {
		...
		proposal.setStaffid(loginStaff.getStaffid());
	...
	@Override
	public JsonBean suggestCancel(BigDecimal proid, String token) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
	...
	@Override
	public JsonBean findSuggestDetail(String token, BigDecimal proid) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
	...
	@Override
	public JsonBean suggestDelete(BigDecimal proid, String token) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
	...
	private R deleteRealtionAttInfo(String attId) throws Exception {
        ...
        this.tblNbsjProposalMapper.deleteFileInfoByAttId(att.getAttid());
