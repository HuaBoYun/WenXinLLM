...
public interface TblNbsjAuditplanMapper extends Mapper<TblNbsjAuditplan> {
    ...
    @SelectProvider(method="selectNbsjAuditPlanCountByPageInfo",type=TblNbsjAuditplanMapperSqlConfig.class)
   	Integer selectNbsjAuditPlanCountByPageInfo(PageInfo<TblNbsjAuditplan> pageInfo, String planStartDate,String planEndDate,TblStaffUtil loginStaff, TblNbsjAuditPlanVo plan) throws Exception;

    @Select("SELECT TNA.PROJECTORG,TNA.PRJOECTNAME,TNA.PROJECTTYPE,TNA.COSTS,TNA.APPPROYEARSTART,TNA.APPPROYEAREND,TNA.ACTPROYEARSTART," +
			"TNA.ACTPROYEAREND,TNA.PROJECTMGDEPTID,TNA.PROJECTMGDEPTNAME,TNA.STARTDATE,TNA.ENDDATE,TNA.PROJECTORGADDRESS," +
			"TNA.PROJECTLINKMAN,TNA.PROJECTLINKTEL,TNA.IMPLEMENTAION,TNA.AUDITCODE,TNA.COMMENTS,TNA.AUDITTYPE," +
			"TNA.PLANID,TNA.PLANCODE,TNA.PLANNAME,TNA.PALNYEAR,TNA.PLANTYPE,TNA.PALNCOST,TNA.STARTTIME,TNA.ENDTIME,TNA.REMARKS,TNA.CREATETIME,TNA.STATUS,"
    		+ "TNA.OPINIONSTATUS,TNA.ISAUDITOR,TNA.SECRECTLEVELID,TNA.STAFFSCOPEIDS,TNA.STAFFSCOPENAMES,PRINCIPAL.STAFFID  PRISTAFFID,"
    		+ "PRINCIPAL.REALNAME  PRIREALNAME,LEADER.STAFFID LEASTAFFID,LEADER.REALNAME LEAREALNAME,AUDITORG.ORGID AUDORGID,AUDITORG.ORGNAME AUDORGNAME,"
    ...
    @Results({
    	...
    	@Result(column="PROJECTORG",property="projectorg"),
    	@Result(column="PRJOECTNAME",property="prjoectName"),
    	@Result(column="PROJECTTYPE",property="prjoectType"),
    	@Result(column="COSTS",property="costs"),
    	@Result(column="APPPROYEARSTART",property="appproyearstart"),
    	@Result(column="APPPROYEAREND",property="appproyearend"),
    	@Result(column="ACTPROYEARSTART",property="actproyearstart"),
    	@Result(column="ACTPROYEAREND",property="actproyearend"),
    	@Result(column="PROJECTMGDEPTID",property="projectmgdeptid"),
    	@Result(column="PROJECTMGDEPTNAME",property="projectmgdeptname"),
    	@Result(column="STARTDATE",property="startDate"),
    	@Result(column="ENDDATE",property="endDate"),
    	@Result(column="PROJECTORGADDRESS",property="projectorgaddress"),
    	@Result(column="PROJECTLINKMAN",property="projectlinkman"),
    	@Result(column="PROJECTLINKTEL",property="projectlinktel"),
    	@Result(column="IMPLEMENTAION",property="implementaion"),
    	@Result(column="AUDITCODE",property="auditCode"),
    	@Result(column="AUDITCODE",property="auditCode"),
    	@Result(column="COMMENTS",property="comments"),
    	@Result(column="AUDITTYPE",property="auditType"),
    })
   	TblNbsjAuditplan selectNbsjAuditPlanEntityById(@Param("planid") BigDecimal planid) throws Exception;
	...
	List<TblNbsjAuditplan> selectNbsjAuditPlanListByPageInfo(PageInfo<TblNbsjAuditplan> pageInfo, String planStartDate, String planEndDate, TblStaffUtil loginStaff, TblNbsjAuditPlanVo plan) throws Exception;

    @Delete("DELETE FROM TBL_NBSJ_AUDITPLAN WHERE PLANID = #{planId}")
    void deleteAuditPlanEntityById(@Param("planId")BigDecimal planId) throws Exception;
	...
	List<BigDecimal> findAttIdListByPlanId(@Param("planId")BigDecimal planId) throws Exception;
	...
	List<TblNbsjAuditplan> selectAuditPlanListView(PageInfo<TblNbsjAuditplan> pageInfo,TblStaffUtil loginStaff) throws Exception;

    @SelectProvider(method="selectAuditPlanCountView",type=TblNbsjAuditplanMapperSqlConfig.class)
	Integer selectAuditPlanCountView(PageInfo<TblNbsjAuditplan> pageInfo,TblStaffUtil loginStaff) throws Exception;

    @Select("SELECT * FROM TBL_NBSJ_AUDITPLAN WHERE OPINIONSTATUS =6 and AUDITORGID = #{orgid} ORDER BY PLANID DESC")
    ...
    @Select("SELECT * FROM TBL_NBSJ_AUDITPLAN WHERE planId=#{planId} ")
   	List<TblNbsjAuditplan> selectPlanListPlanId(BigDecimal planId) throws Exception;
    ...
    //=======缺陷管理自动编号
	String selectMaxBugnumber(@Param("bugnumber")String bugnumber) throws Exception;
  ...
  //=======项目管理自动编号
    @Select("SELECT MAX(TO_NUMBER(SUBSTR(PROJECTCODE,INSTR(PROJECTCODE,'-',-1)+5)))  "
			+ " FROM TBL_NBSJ_PROJECT "
			+ " WHERE 1=1 AND PROJECTCODE like #{code} ")
	String selectMaxProjectcode(String code) throws Exception;
    
    //=======计划管理自动编号
    @Select("SELECT MAX(TO_NUMBER(SUBSTR(PLANCODE,INSTR(PLANCODE,'-',-1)+5)))  "
			+ " FROM TBL_NBSJ_AUDITPLAN "
			+ " WHERE 1=1 AND PLANCODE like #{code} ")
	String selectMaxPlancode(String code) throws Exception;


	@Select("SELECT orgid FROM TBL_ORGANIZATION WHERE orgname = #{projectCode} FETCH FIRST 1 ROWS ONLY")
    String selectPrjoectId(String projectCode);

	@Select("SELECT orgid FROM TBL_ORGANIZATION WHERE orgname = #{deptName} AND FATHERORGID = #{prjoectID} FETCH FIRST 1 ROWS ONLY")
	String selectprojectmgdeptID(String prjoectID,String deptName);

	@Select("select typeid from TBL_NBSJ_TYPE where audittype = #{auditType} and orgid = #{orgid} FETCH FIRST 1 ROWS ONLY")
	String selectauditTypeId(String orgid,String auditType);

	@Select("SELECT LEVELID FROM TBL_SECRECT_LEVEL where LEVELTYPE = '2' AND LEVELNAME = #{secrectLevel}")
	String findSecrectLevelId(String secrectLevel);
	...
	@Select("<script>" +
			...
			"</if>"+
			"</script>")
	List<TblOrganization> getOrgidForOrgname(@Param("orgid")String orgid, @Param("orgids")List<TblOrganization> orgids);
