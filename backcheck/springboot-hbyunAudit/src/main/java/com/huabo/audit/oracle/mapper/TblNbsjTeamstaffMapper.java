...
public interface TblNbsjTeamstaffMapper extends BaseMapper<TblNbsjTeamstaffEntity> {
    ...
    void deleteZYByTeamId(BigDecimal teamid) throws Exception;
	
	@Select("SELECT TNA.* FROM TBL_NBSJ_TEAMSTAFF TNA  WHERE TNA.ID = #{teamId}")
	TblNbsjTeamstaffEntity selectById(BigDecimal teamId) throws Exception;
    ...
    Integer getCountByProject(BigDecimal projectid) throws Exception;
	...
	@Select("SELECT * FROM TBL_NBSJ_TEAMSTAFF " +
			...
			"AND STAFFTYPE=0")
	List<TblNbsjTeamstaffEntity> getByProjectid (BigDecimal projectid) throws Exception;

	@Select("SELECT DISTINCT TE.STAFFID from TBL_NBSJ_TEAMSTAFF te LEFT JOIN " +
			...
			" OR STAFFID IN (SELECT PMID from TBL_NBSJ_PROJECT WHERE STATUS=4 AND ORGID = #{orgid} ) ")
    List<BigDecimal> queryTeamstaffAndNbsjproteam(BigDecimal orgid);
}
