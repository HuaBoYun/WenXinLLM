...
public class TblNbsjCertificateServiceImpl implements TblNbsjCertificateService{
	...
	public JsonBean getNbsjCertificateListPage(String token, Integer pageNumber, Integer pageSize,String projectName,String auditMatter,String auditAbstract,BigDecimal projectId) throws Exception {
    	...
    	pageInfo.setTlist(this.tblNbsjCertificateMapper.selectNbsjCertificateListByPageInfo(pageInfo,loginStaff.getCurrentOrg().getOrgid(), projectName, auditMatter, auditAbstract,projectId));
    	pageInfo.setTotalRecord(this.tblNbsjCertificateMapper.selectNbsjCertificateListCountByPageInfo(pageInfo,loginStaff.getCurrentOrg().getOrgid(), projectName, auditMatter, auditAbstract,projectId));
    	pageInfo.getTotalPage();*/
		TblNbsjCertificate certificate = new TblNbsjCertificate();
		certificate.setOrgId(loginStaff.getCurrentOrg().getOrgid());
		certificate.setProjectId(projectId);
		certificate.setPrjoectname(projectName);
		certificate.setAuditMatter(auditMatter);
		certificate.setAuditAbstract(auditAbstract);
		
		String secrectSql = GeneralSQLConcatConfig.concatSecrectSql(loginStaff.getCurrentOrg().getUseSecrect(), true, "TNA.ORGID", "TNA.ORGID",
				"TNA.AUDITUSERID", "TNA.SECRECTLEVELID", "TNA.STAFFSCOPEIDS", loginStaff.getStaffid(), loginStaff.getDeptIds(), loginStaff.getSecrectScopeIds());
		
		//链表分页  xml 写法
		com.github.pagehelper.PageInfo<TblNbsjCertificate> pageInfo = PageMethod.startPage(pageNumber, pageSize)
				.doSelectPageInfo(() -> tblNbsjCertificateMapper.selectNbsjCertificateListByPageInfoXml(certificate,secrectSql));
		PageResult<TblNbsjCertificate> build = new PageResult<TblNbsjCertificate>().build(pageInfo);
	...
	public JsonBean getNbsjCertificateList(String token,TblNbsjCertificate categories, Integer pageNumber, Integer pageSize,String sheetid) throws Exception {
		...
		categories.setOrgId(loginStaff.getCurrentOrg().getOrgid());
    	/*PageInfo<TblNbsjCertificate> pageInfo = new PageInfo<TblNbsjCertificate>();
    	pageInfo.setPageSize(pageSize);
    	pageInfo.setCurrentPage(pageNumber);
    	pageInfo.setCondition(categories);
		TblNbsjCertificateVo*/
    	//此处审计取证单的列表需要排除已经被勾选的
		//链表分页  xml 写法
		com.github.pagehelper.PageInfo<TblNbsjCertificate> pageInfo = PageMethod.startPage(pageNumber, pageSize)
				.doSelectPageInfo(() -> tblNbsjCertificateMapper.selectNbsjCertificateListByPageInfoDgXml(categories,sheetid));
		PageResult<TblNbsjCertificate> build = new PageResult<TblNbsjCertificate>().build(pageInfo);
...
//		}
		...
		if(certificate.getCertificateId() != null) {
			...
			//==附件，先删除 再重新添加
			this.tblAttachmentMapper.deleteAttmentRelationCertificate(certificate.getCertificateId());
			if (attids != null && !"".equals(attids)) {
				String[] ids = attids.split(",");
				for (String id : ids) {
					this.tblAttachmentMapper.insertAttmentRelationCertificate(id, certificate.getCertificateId());
				}
			}
		}else {
			//新增；
			certificate.setAuditUserId(loginStaff.getStaffid());
			certificate.setProjectId(projectId);
			certificate.setCreateDate(new Date());
			//this.tblNbsjCertificateMapper.insertEntity(certificate);
			certificate.setStatus(0);
			certificate.setCertificateId(RandomUtil.uuBigDecimalId());
			certificate.setCreatestaffid(loginStaff.getStaffid());
	...
	private R deleteRealtionAttInfo(String attId) throws Exception {
        ...
        this.tblNbsjCertificateMapper.deleteFileInfoByAttId(att.getAttid());
	...
	@Override
	public JsonBean certificateDelete(BigDecimal certificateId, String token) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
...
//        }
	...
	@Override
	public JsonBean certificateDetail(String token, BigDecimal certificateId) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
	...
	@Override
	public JsonBean getCertificateList(String token, BigDecimal sheetid) throws Exception {
	...
	@Override
	public JsonBean certificateSubmit(String token, BigDecimal certificateId, String examination) throws Exception {
		...
		try {
			...
			cir = this.tblCirculationService.saveTblCirculationnew(TblCirculation.TYPE_QZD, certificate.getPrjoectname(), certificate.getAuditMatter(), TblCirculation.URL_QZD + certificateId, user.getStaffid(), processInstanceId, processDefinitionKey, certificateId.toString());
						...
						if (task.getProcessInstanceId().equals(processInstanceId)) {
							...
							if (result != null && result.equals("true")) {
								...
								if (nextapprover.contains("zsuserid") ) {
									...
									task.setHandle(certificate.getAssistedfgld().toString());
...
//            String definitionId = processInstance.getProcessDefinitionId();
	 ...
	 */
	@Override
	public JsonBean getCertificate(String token, BigDecimal certificateId, String taskId, BigDecimal cyId) throws  Exception{
	 ...
	 */
	@Override
	public JsonBean dealCertificate(String token, BigDecimal cyId, String taskId, String transition, String optDesc, BigDecimal certificateId, String processDefinitionId, String processInstanceId) throws Exception {
		TblStaffUtil user = userProvider.get();
		if (user == null) {
			return ResponseFormat.retParam(0, 20006, null);
		}
	...
	public JsonBean saveStampedDocument(BigDecimal certificateId, String[] attids) throws Exception {
		...
		for (String attid : attids) {
			this.tblNbsjCertificateMapper.insertStampedDocumentFile(certificateId,attid);
		}
		return ResponseFormat.retParam(1,200,null);
	}
	...
	public JsonBean removeStampedDocument(BigDecimal certificateId, String attid) throws Exception {
		...
		this.tblNbsjCertificateMapper.deleteStampedDocumentFile(certificateId,attid);
		return ResponseFormat.retParam(1,200,null);
	}
	...
	public JsonBean getStampedDocumentList(BigDecimal certificateId) throws Exception {
		...
		List<TblAttachment> attList = this.tblAttachmentMapper.selectAttStampedFileListByQzd(certificateId);
