...
//				+ " AND TNA.BUGBYSYSTEM LIKE '%nbsj%' ");
	...
	public String updateEntity(TblNbsjBugEntity plan) {
		...
		//--------中核新增字段
		if(plan.getDefectsname() != null && !"".equals(plan.getDefectsname())) {
			sqlSb.append(" ,DEFECTSNAME = '"+plan.getDefectsname()+"'");
		}
		if(plan.getAmount() != null) {
			sqlSb.append(" ,AMOUNT = '"+plan.getAmount()+"'");
		}
		if(plan.getCauseanalysis() != null && !"".equals(plan.getCauseanalysis())) {
			sqlSb.append(" ,CAUSEANALYSIS = '"+plan.getCauseanalysis()+"'");
		}
		if(plan.getDefectcategory() != null && !"".equals(plan.getDefectcategory())) {
			sqlSb.append(" ,DEFECTCATEGORY = '"+plan.getDefectcategory()+"'");
		}
		
		if(plan.getDefecttype() != null && !"".equals(plan.getDefecttype())) {
			sqlSb.append(" ,DEFECTTYPE = '"+plan.getDefecttype()+"'");
		}
		
		if(plan.getLitigation()!= null && !"".equals(plan.getLitigation())) {
			sqlSb.append(" ,LITIGATION = '"+plan.getLitigation()+"'");
		}
		
		if(plan.getOverseas()!= null && !"".equals(plan.getOverseas())) {
			sqlSb.append(" ,OVERSEAS = '"+plan.getOverseas()+"'");
		}
		
		if(plan.getCreatestaffid()!= null ) {
			sqlSb.append(" ,CREATESTAFFID = '"+plan.getCreatestaffid()+"'");
		}
		
		if(plan.getUnit() != null ) {
			sqlSb.append(" ,UNIT = '"+plan.getUnit()+"'");
		}
		if(plan.getStatus() != null ) {
			sqlSb.append(" ,STATUS = '"+plan.getStatus()+"'");
		}
		
		if(plan.getCreatetime() != null && !"".equals(plan.getCreatetime())) {
			sqlSb.append(" ,CREATETIME = TO_DATE('"+DateUtil.parseDate(plan.getCreatetime(), "yyyy-MM-dd HH:mm:ss")+"','YYYY-MM-DD HH24:mi:ss')");
		}
	...
	//==
	public String insertBugOutrulids(String outrulid, BigDecimal bugid){
