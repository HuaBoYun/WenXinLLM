...
public class TblNbsjQuestionServiceImpl  implements TblNbsjQuestionService {
    @Autowired
    ...
    public com.github.pagehelper.PageInfo<TblNbsjQuestionEntity> auditQuestionList(AuditQuestionVo auditQuestion) throws Exception {
        //检查token
        if(StringUtils.isBlank(auditQuestion.getToken())){
        ...
        //通过token获取组织id(orgId)
        auditQuestion.setOrgId(loginStaff.getCurrentOrg().getOrgid());
        //分页插件
	...
	///////////////////////////////////////==
	@Override
	public JsonBean questionStorePageList(String token, Integer pageNumber, Integer pageSize,TblNbsjQuestionVo tblNbsjQuestionVo,BigDecimal projectId) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
    	...
    	if(null == projectId) {
    		...
    		if(tnp == null) {
    			return ResponseFormat.retParam(1,30003,resultMap);
    		}
    		projectId = tnp.getProjectId();
			tblNbsjQuestionVo.setProjectId(projectId);
    		if(null == projectId) {
    			return ResponseFormat.retParam(1,30003,resultMap);
    		}
    	}
		...
		//密级数据权限
		String sql = GeneralSQLConcatConfig.concatSecrectSql(loginStaff.getCurrentOrg().getUseSecrect(), true, "SHEET.ORGID", "SHEET.ORGID",
				"SHEET.CREATESTAFF", "SHEET.SECRECTLEVELID", "SHEET.STAFFSCOPEIDS", loginStaff.getStaffid(), loginStaff.getDeptIds(), loginStaff.getSecrectScopeIds());
		//链表分页  xml 写法
		com.github.pagehelper.PageInfo<TblNbsjQuestionEntity> pageInfo = PageMethod.startPage(pageNumber, pageSize)
				.doSelectPageInfo(() -> tblNbsjQuestionMapper.selectListByPageInfoXml(tblNbsjQuestionVo,sql));
		PageResult<TblNbsjQuestionEntity> build = new PageResult<TblNbsjQuestionEntity>().build(pageInfo);
	...
	@Override
	public  List<TblNbsjSheetEntity> getQuestionStorePageList(TblStaffUtil loginStaff,TblNbsjQuestionVo tblNbsjQuestionVo,BigDecimal projectid) throws Exception{
    	if(null == projectid) {
    		//==查询当前实施的项目！
    		TblNbsjProject tnp = this.tblNbsjProjectService.getCurrenNbsjProjectByLoginStaff(loginStaff.getStaffid());
    		if(tnp == null) {
    	...
    	//密级数据权限
    			String sql = GeneralSQLConcatConfig.concatSecrectSql(loginStaff.getCurrentOrg().getUseSecrect(), true, "SHEET.ORGID", "SHEET.ORGID",
    					"SHEET.CREATESTAFF", "SHEET.SECRECTLEVELID", "SHEET.STAFFSCOPEIDS", loginStaff.getStaffid(), loginStaff.getDeptIds(), loginStaff.getSecrectScopeIds());
    	 List<TblNbsjSheetEntity> list =this.tblNbsjQuestionMapper.getNbsjQuestList(tblNbsjQuestionVo,projectid,sql);
    	return list;
	}
...
//		tnq.setCreateTime(new Date());
	...
	@Override
	public JsonBean questionStorePageDelete(BigDecimal questionid, String token) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
...
//        }
	...
	@Override
	public JsonBean questionStorePageISfalse(BigDecimal questionid, String token,Integer ifsbg) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
		...
		tblNbsjQuestionMapper.updateIsstatus(questionid,ifsbg);
	...
	@Override
	public JsonBean questionStorePageFQZG(BigDecimal questionid, String token) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
...
//    	pageInfo.setCondition(tblNbsjSheet);
	...
	@Override
	public JsonBean confirmationQuestionStoreLink(String token, BigDecimal factid) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
