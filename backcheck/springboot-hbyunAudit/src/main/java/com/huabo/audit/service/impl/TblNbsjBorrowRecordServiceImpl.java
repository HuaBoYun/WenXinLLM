...
//		}
		...
		br.setStaffid(loginStaff.getStaffid());
	...
	//==借阅日志-借阅次数列表
	@Override
	public JsonBean jyrzCountPageList(String token, Integer pageNumber, Integer pageSize,BigDecimal projectid) throws Exception {
//		TblStaffUtil loginStaff = userProvider.get();
//		  if(loginStaff == null) {
//		   return ResponseFormat.retParam(0,20006,null);
//		  }
...
//	     pageInfo.setCondition(tblNbsjSheet);
	     ...
	     pageInfo.setTlist(this.tblNbsjBorrowRecordMapper.selectListByPageInfo(pageInfo,projectid));
	     pageInfo.setTotalRecord(this.tblNbsjBorrowRecordMapper.selectCountByPageInfo(pageInfo,projectid));
	     pageInfo.getTotalPage();*/
		//链表分页  xml 写法
		com.github.pagehelper.PageInfo<TblNbsjBorrowRecordEntity> pageInfo = PageMethod.startPage(pageNumber, pageSize)
				.doSelectPageInfo(() -> tblNbsjBorrowRecordMapper.selectListByPageInfoXml(projectid));

		//分页参数 转换成 分页对象 （不同系统分页对象返回可能不一样 可自行转换）
		PageResult<TblNbsjBorrowRecordEntity> build = new PageResult<TblNbsjBorrowRecordEntity>().build(pageInfo);
	...
	@Override
	public JsonBean submitRecordApproval(String token, BigDecimal borrowid) throws Exception {
	...
	@Override
	public JsonBean getRecordApprovalInfo(String token, BigDecimal borrowid, String taskId, BigDecimal cyId)
			throws Exception {
	...
	@Override
	public JsonBean dealRecordApporvalInfo(String token, BigDecimal cyId, String taskId, String transition, String optDesc,
	...
	@Override
	public JsonBean tjspBorrowDetail(BigDecimal borrowId,String token) throws Exception {
		TblStaffUtil staff = userProvider.get();
		if(staff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
