...
public interface TblNbsjPlanProjectMapper extends BaseMapper<TblNbsjPlanProject>{
	...
	void deletePlanProjectInfo(@Param("planprojectid")BigDecimal planprojectid) throws Exception;

	@Delete("DELETE FROM TBL_NBSJ_PLANPROJECT WHERE PLANID = #{planId}")
	void deletePlanProjectByPlanId(@Param("planId")BigDecimal planId) throws Exception;

	@Select("SELECT * FROM TBL_NBSJ_PLANPROJECT WHERE PLANID = #{planId} ORDER BY PLANPROJECTID ")
	List<TblNbsjPlanProject> selectPlanProjectListInfoByPlanId(@Param("planId")BigDecimal planId);
	...
	List<TblNbsjProject> selectProjectListInfoByWspJhw(@Param("planId")BigDecimal planId);
	...
	void updatePjPlanByWspJhw(BigDecimal planId,BigDecimal orgid);
