...
public class TblNbsjAuditplanMapperSqlConfig {
	...
	public String selectAuditPlanListView(PageInfo<TblNbsjAuditplan> pageInfo,TblStaffUtil loginStaff) throws Exception {
		TblNbsjAuditplan plan = pageInfo.getCondition();
		StringBuffer sqlSb = new StringBuffer("SELECT PLANID,PLANCODE,PLANNAME,PALNYEAR,SECRECTLEVELID,STAFFSCOPEIDS,STAFFSCOPENAMES,"
				...
				+ "FROM (SELECT PLANID,PLANCODE,PLANNAME,PALNYEAR,SECRECTLEVELID,STAFFSCOPEIDS,STAFFSCOPENAMES,ROWNUM RN FROM (SELECT TNAP.PLANID,TNAP.PLANCODE,TNAP.PLANNAME ,TNAP.SECRECTLEVELID,TNAP.STAFFSCOPEIDS,TNAP.STAFFSCOPENAMES ,PALNYEAR FROM TBL_NBSJ_AUDITPLAN TNAP WHERE TNAP.AUDITORGID = "+plan.getAuditorgid());


		String sql = GeneralSQLConcatConfig.concatSecrectSql(loginStaff.getCurrentOrg().getUseSecrect(),true,"TNAP.AUDITORGID","TNAP.AUDITORGID","TNAP.CREATESTAFFID","TNAP.SECRECTLEVELID","TNAP.STAFFSCOPEIDS",loginStaff.getStaffid(),loginStaff.getDeptIds(),loginStaff.getSecrectScopeIds());
		sqlSb.append(sql);
	...
	public String selectAuditPlanCountView(PageInfo<TblNbsjAuditplan> pageInfo,TblStaffUtil loginStaff) throws Exception {
		TblNbsjAuditplan plan = pageInfo.getCondition();
	...
	public String selectNbsjAuditPlanListByPageInfo(PageInfo<TblNbsjAuditplan> pageInfo, String planStartDate, String planEndDate, /*BigDecimal orgId*/TblStaffUtil loginStaff, TblNbsjAuditPlanVo plan) throws Exception {
		...
		StringBuffer sb = new StringBuffer("SELECT * FROM (SELECT T1.*,ROWNUM RN  FROM "
				...
				+ "TNA.CREATETIME,TNA.STATUS,TNA.OPINIONSTATUS,TNA.SECRECTLEVELID,TNA.STAFFSCOPEIDS,TNA.STAFFSCOPENAMES,TNA.CREATESTAFFID,TSL.LEVELNAME,TNA.ISAUDITOR,PRINCIPAL.STAFFID AS PRISTAFFID,PRINCIPAL.REALNAME AS PRIREALNAME,"
				...
				+ "LEFT JOIN TBL_SECRECT_LEVEL TSL ON TSL.LEVELID = TNA.SECRECTLEVELID "
				+ "WHERE TNA.AUDITORGID = " +orgId);

//		String sql = GeneralSQLConcatConfig.concatSecrectSql(loginStaff.getCurrentOrg().getUseSecrect(),true,"AUDITORG.ORGID","AUDITORG.ORGID","TNA.CREATESTAFFID","TNA.SECRECTLEVELID","TNA.STAFFSCOPEIDS",loginStaff.getStaffid(),loginStaff.getDeptIds(),loginStaff.getSecrectScopeIds());
//		sb = sb.append(sql);
		if(plan.getPrincipalid() != null) {
	...
	public String selectNbsjAuditPlanCountByPageInfo(PageInfo<TblNbsjAuditplan> pageInfo, String planStartDate,String planEndDate, TblStaffUtil loginStaff, TblNbsjAuditPlanVo plan) throws Exception {
		BigDecimal orgid = loginStaff.getLinkOrg().getOrgid();
