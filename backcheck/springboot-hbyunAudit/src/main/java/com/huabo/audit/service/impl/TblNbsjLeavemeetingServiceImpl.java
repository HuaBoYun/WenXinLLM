...
//    	pageInfo.setCondition(tblNbsjLeavemeeting);
	...
	public JsonBean outMeetRecordAdd(TblNbsjLeavemeetingEntity lev, String token,String attids) throws Exception {
		...
		if(lev.getLeaveid() != null) {
			...
			//==附件，先删除 再重新添加
			this.tblAttachmentMapper.deleteAttmentRelationLeavemeeting(lev.getLeaveid());
			if (attids != null && !"".equals(attids)) {
				String[] ids = attids.split(",");
				for (String id : ids) {
					this.tblAttachmentMapper.insertAttmentRelationLeavemeeting(id, lev.getLeaveid());
				}
			}
		}else {
			//新增；
	...
	@Override
	public JsonBean outmetDelete(BigDecimal leaveid, String token) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
...
//        }
	...
	@Override
	public JsonBean findNbsjLeavemeetingDetail(String token, BigDecimal leaveid) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
	...
	@Override
	public JsonBean outMetCalcel(BigDecimal leaveid, String token) throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
		if(loginStaff == null) {
			return ResponseFormat.retParam(0,20006,null);
		}
	...
	private R deleteRealtionAttInfo(String attId) throws Exception {
        ...
        this.tblNbsjLeavemeetingMapper.deleteFileInfoByAttId(att.getAttid());
