...
public class TblNbsjAuditExperienceTypeMapperSqlConfig {
	...
	public String insertEntity(TblNbsjAuditExperienceTypeEntity re){
		StringBuffer colSb = new StringBuffer("INSERT INTO TBL_NBSJ_AUDITEXPERIENCE_TYPE (TYPEID, TYPENAME, CREATETIME,CREATESTAFFID,PARENTID,MPDELTYPE");
		StringBuffer valSb = new StringBuffer("  VALUES (HIBERNATE_SEQUENCE.nextval,'"+re.getTypeName()+"',TO_DATE('"+DateUtil.parseDate(re.getCreateTime(), "yyyy-MM-dd")+"','YYYY-MM-DD HH24:mi:ss'),"+re.getCreateStaffid()+","+re.getParentId()+",'"+re.getMpdeltype()+"'");
	...
	public String getlist(BigDecimal nodeId,String mpdeltype) throws Exception {
		String sql="SELECT * FROM TBL_NBSJ_AUDITEXPERIENCE_TYPE WHERE 1=1 ";
		if(nodeId!=null ) {
			sql+=" and PARENTID= "+nodeId;
		}else {
			sql+=" and  PARENTID is NULL ";
		}
		if(mpdeltype!=null && mpdeltype.length()>0) {
			sql+=" and MPDELTYPE= '"+mpdeltype+"'";
		}
