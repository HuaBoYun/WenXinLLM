...
public class AuditModelServiceImpl implements AuditModelService {
	...
	public JsonBean saveOrUpdateTblAuditModelDataSource(TblAuditModelDataSourceOracle param) throws Exception {
		...
		if (Objects.equals(DateBaseConfig.DATABASE_TYPE, "Oracle")) {
		...
		} else if (Objects.equals(DateBaseConfig.DATABASE_TYPE, "DM8")) {
			//查询用户名是否在数据库已存在
			String getUserSql = "select * from all_users where username='" + model.getDataBaseUsers() + "'";
			JsonBean jsonBean = oracleSqlProperties.execSql(getUserSql);
			List<Map<String, Integer>> data = (List<Map<String, Integer>>) jsonBean.getData();
			...
			//			oracleSqlProperties.execSql(sessionSql);
			//给ABC用户表空间授权 grant connect,resource,dba to crs;
			String sql1 = "grant resource to " + model.getDataBaseUsers();
			oracleSqlProperties.execSql(sql1);
			String sql2 = "grant dba to " + model.getDataBaseUsers();
			oracleSqlProperties.execSql(sql2);
		} else if (Objects.equals(DateBaseConfig.DATABASE_TYPE, "MYSQL")) {
			//查询用户名是否在数据库已存在
			String getUserSql = "select * from all_users where username='" + model.getDataBaseUsers() + "'";
			JsonBean jsonBean = oracleSqlProperties.execSql(getUserSql);
			List<Map<String, Integer>> data = (List<Map<String, Integer>>) jsonBean.getData();
			...
			if (CollectionUtil.isNotEmpty(pageInfo.getList())) {
				...
				pageInfo.getList().forEach(x -> {
					if (x.getExcelId() != null) {
						try {
							TblAttachment tblAttachment = tblAttachmentService.selectEntityById(x.getExcelId());
	 ...
	 */
	@Override
	public JsonBean getTblAuditModelExcel(Long id) throws Exception {
		...
		if (DateBaseConfig.DATABASETYPE.equals("Oracle")) {
			...
			if (auditModelExcel.getExcelId() != null) {
				TblAttachment tblAttachmentEntity = tblAttachmentService.selectEntityById(auditModelExcel.getExcelId());
	 ...
	 */
	@Override
	public JsonBean deleteTblAuditModelExcelExt(Long id) {
	 ...
	 */
	@Override
	public JsonBean getTblAuditModelExcelExt(Long id) {
	 ...
	 */
	@Override
	public JsonBean deleteTblAuditModelExcelTable(Long id) {
	 ...
	 */
	@Override
	public JsonBean getTblAuditModelExcelTable(Long id) {
	...
	private void loopAssembleInsertSql(Map<String, String> columnMap, String table, List<Integer> subscript, List<String> columnNameNe,
		 ...
		 */
		log.info("values 打印:{} ",JsonMapper.INSTANCE.toJson(values));
		StringBuffer insert = new StringBuffer();
		insert.append("insert into " + table + "(ID," + StringUtils.join(columnNameNe, ",") + ")");
		insert.append(" values");
		insert.append(" (" + RandomUtil.uuBigDecimalId() + " ," + StringUtils.join(values, ",") + ")");
...
//			if (Objects.equals(varchar2, columnType)) {
//				str = "CAST('" + str + "' AS VARCHAR2(500))";
//			} else if (Objects.equals(clob, columnType)) {
//				str = "CAST('" + str + "' AS CLOB)";
//			} else if (Objects.equals(varchar24000, columnType)) {
//				str = "CAST('" + str + "' AS VARCHAR2(4000))";
//			}
	...
	private void createTable(List<TblAuditModelExcelTableOracle> list, String table, String passWord, String username) throws Exception {
		...
		stringBuffer.append("CREATE TABLE " + table + "( ID NUMBER(38) NOT NULL, ");
		//组装创建参数
