...
public interface TblNbsjOperateMapper extends tk.mybatis.mapper.common.Mapper<TblNbsjOperateEntity>{
...
//    	@Result(column="SUDITPROCESS",property="suditProcess"),
    ...
    void finalById(BigDecimal operateid) throws Exception;
    
    
    
    //==
    @SelectProvider(method="selectAllCountByPageInfo",type=TblNbsjOperateMapperSqlConfig.class)
   	Integer selectAllCountByPageInfo(PageInfo<TblNbsjOperateEntity> pageInfo,BigDecimal projectid,String businessType,BigDecimal targetId) throws Exception;
...
//    	@Result(column="REALNAME",property="authorization.teamStaff.staff.realname"),
	...
	List<TblNbsjOperateEntity> selectAllListByPageInfo(PageInfo<TblNbsjOperateEntity> pageInfo,BigDecimal projectid,String businessType,BigDecimal targetId) throws Exception;
    
    
    //==
    @SelectProvider(method="selectRWFPCountByPageInfo",type=TblNbsjOperateMapperSqlConfig.class)
   	Integer selectRWFPCountByPageInfo(PageInfo<TblNbsjOperateEntity> pageInfo,BigDecimal tempId,BigDecimal targetId) throws Exception;
...
//    	@Result(column="REALNAME",property="authorization.teamStaff.staff.realname"),
    ...
    @Select("SELECT  DISTINCT team.STAFFID   FROM TBL_NBSJ_AUDITPROGRAM TNA    LEFT JOIN TBL_NBSJ_TEMPLETE ntemp ON ntemp.templeteId = TNA.TEMPID   "
    		+ " LEFT JOIN TBL_NBSJ_AUTHORIZATION auth ON TNA. programid = auth.programid  LEFT JOIN TBL_NBSJ_TEAMSTAFF team on team.id = auth.TEAMSTAFFID   "
    		+ " where  ntemp.templeteId =#{tempId} and  team.staffid is not null")
	List<String> findList(BigDecimal tempId) throws Exception;
    ...
    Integer getCount(BigDecimal projectid) throws Exception;
...
//    	@Result(column="REALNAME",property="renyuan"),
    ...
    List<TblNbsjOperateEntity> findByAuthId(BigDecimal authId) throws Exception;
    
    @Delete("DELETE FROM TBL_NBSJ_OPERATE WHERE operateid = #{operateid}")
    void deleteByOpid(BigDecimal operateid) throws Exception;
    ...
    void InExecuion(BigDecimal operateid) throws Exception;

	List<TblNbsjOperateEntity> findPorgramByUserXml(@Param("tblNbsjOperateVo") TblNbsjOperateVo tblNbsjOperateVo);

	List<TblNbsjOperateEntity> selectListByPageInfoXml(@Param("tblNbsjOperateVo")TblNbsjOperateVo tblNbsjOperateVo);
    @Select("SELECT * FROM TBL_NBSJ_OPERATE TNA LEFT JOIN TBL_NBSJ_AUTHORIZATION auth ON TNA. AUTHID = auth. AUTHID " +
			"LEFT JOIN TBL_NBSJ_AUDITPROGRAM audi ON audi.programid = auth.programid where TNA.operateId=#{operateId}")
	TblNbsjOperateEntity selectAuditprogram(String operateId);

	@Update("update TBL_NBSJ_AUDITPROGRAM set flag = #{flag} where programId = #{programId}")
	void updateFlag(Integer programId,Integer flag);

	List<TblNbsjOperateEntity> selectAllListByPageInfoXml(@Param("projectid")BigDecimal projectid, @Param("businessType")String businessType, @Param("targetId")BigDecimal targetId);
