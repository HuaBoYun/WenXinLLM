...
public interface TblNbsjMbMapper extends BaseMapper<TblNbsjMb>{
	...
	List<TblNbsjMb> selectNbsjMbByPageInfo(PageInfo<TblNbsjMb> pageInfo, TblStaffUtil loginStaff, TblNbsjMb mb);
	 
	@SelectProvider(method="selectNbsjMbByPageInfoCount",type=TblNbsjMbMapperSqlConfig.class)
	Integer selectNbsjMbByPageInfoCount(TblStaffUtil loginStaff, TblNbsjMb mb) throws Exception;
	 ...
	 TblNbsjMb selectNbsjMbByID(@Param("mbid") BigDecimal mbid) throws Exception;
	  ...
	  void insertMbdatapre(String mbid, BigDecimal datepreid) throws Exception;

	  @Insert("DELETE FROM TBL_NBSJ_MB_DATAPRE where  DATAOREID=#{datepreid}")
	  void deletetMbdatapre(BigDecimal datepreid) throws Exception;
	...
	@Select("SELECT s.*,STA.REALNAME FROM TBL_NBSJ_MB s \n" +
			"\tLEFT JOIN TBL_STAFF sta on s.STAFFID=STA.STAFFID\n" +
			"\t\n" +
			"where 1=1 and s.MBID IN ( SELECT DISTINCT MBID from TBL_NBSJ_MB_DATAPRE WHERE DATAOREID =#{dataperid})\n" +
			"\t\t\n" +
			"\t\t\n" +
			" ORDER BY s.AUDITTYPE,MBID")
	 @Results({
	 	@Result(column="MBID",property="mbid"),
	 	@Result(column="MBCODE",property="mbcode"),
	 	@Result(column="MBNAME",property="mbname"),
	...
	List<TblNbsjMb> selectNbsjMbByDateperidPageInfo(BigDecimal dataperid) throws Exception;
