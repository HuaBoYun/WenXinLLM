-- =============================================
-- 电票账户配置表 - 达梦数据库建表脚本 (V2版本)
-- 表名: TC_ETICKET_ACCOUNT_V2
-- 描述: 存储电子票据账户的配置信息,包括账户基本信息、银行信息、额度信息等
-- 作者: 华博云开发团队
-- 创建时间: 2024-12-24
-- 兼容性: 达梦数据库 (DM Database)
-- 说明: 这是V2版本,保留原有TC_ETICKET_ACCOUNT表,创建新表TC_ETICKET_ACCOUNT_V2
-- =============================================

-- 删除已存在的V2表(如果需要)
-- DROP TABLE IF EXISTS TC_ETICKET_ACCOUNT_V2;

-- 创建电票账户配置表 V2
CREATE TABLE TC_ETICKET_ACCOUNT_V2 (
    -- 主键
...
-- 添加表注释
COMMENT ON TABLE TC_ETICKET_ACCOUNT_V2 IS '电票账户配置表 V2版本';
...
-- 添加索引注释
COMMENT ON INDEX IDX_ETICKET_ACCOUNT_V2_NUMBER IS '账户编号唯一索引';
COMMENT ON INDEX IDX_ETICKET_ACCOUNT_V2_NAME IS '账户名称索引';
COMMENT ON INDEX IDX_ETICKET_ACCOUNT_V2_SYSTEM IS '电票系统索引';
COMMENT ON INDEX IDX_ETICKET_ACCOUNT_V2_TYPE IS '账户类型索引';
COMMENT ON INDEX IDX_ETICKET_ACCOUNT_V2_BANK_CODE IS '银行编码索引';
COMMENT ON INDEX IDX_ETICKET_ACCOUNT_V2_STATUS IS '账户状态索引';
COMMENT ON INDEX IDX_ETICKET_ACCOUNT_V2_ORG_ID IS '组织ID索引';
COMMENT ON INDEX IDX_ETICKET_ACCOUNT_V2_CREATE_TIME IS '创建时间索引';
...
-- 插入测试数据
INSERT INTO TC_ETICKET_ACCOUNT_V2 (
...
-- 查询验证数据
-- SELECT * FROM TC_ETICKET_ACCOUNT_V2 ORDER BY CREATE_TIME DESC;
