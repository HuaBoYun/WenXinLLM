...
public class MyResponseFormat<T> {
	...
	public static <T> MyJsonBean<T> retParam(Integer code, Integer status, T data) {
		MyJsonBean<T> json = new MyJsonBean<>(code, messageMap.get(status), data);
		return json;
	}


	public static <T> MyJsonBean<T> retParam(Integer code, String msg, T data) {
		MyJsonBean<T> json = new MyJsonBean<>(code, msg, data);
		return json;
	}

	// 构造函数
	public MyResponseFormat() {}

	public MyResponseFormat(Integer code, String message, T data) {
		this.code = code;
	...
	// 静态方法
	public static <T> MyResponseFormat<T> success(T data) {
		return new MyResponseFormat<>(200, "成功", data);
	}

	public static <T> MyResponseFormat<T> success() {
		return new MyResponseFormat<>(200, "成功", null);
	}

	public static <T> MyResponseFormat<T> error(String message) {
		return new MyResponseFormat<>(500, message, null);
	}

	public static <T> MyResponseFormat<T> error(Integer code, String message) {
		return new MyResponseFormat<>(code, message, null);
	}
