package com.huabo.legal.exam.modules.paper.service.impl;
...
import com.huabo.legal.exam.core.api.ApiError;
...
import com.huabo.legal.exam.modules.paper.dto.ext.PaperQuAnswerExtDTO;
import com.huabo.legal.exam.modules.paper.dto.ext.PaperQuDetailDTO;
import com.huabo.legal.exam.modules.paper.dto.request.PaperAnswerDTO;
import com.huabo.legal.exam.modules.paper.dto.request.PaperListReqDTO;
import com.huabo.legal.exam.modules.paper.dto.response.ExamDetailRespDTO;
import com.huabo.legal.exam.modules.paper.dto.response.ExamResultRespDTO;
...
import com.huabo.legal.exam.modules.paper.entity.PaperQu;
import com.huabo.legal.exam.modules.paper.entity.PaperQuAnswer;
import com.huabo.legal.exam.modules.paper.enums.ExamState;
import com.huabo.legal.exam.modules.paper.enums.PaperState;
import com.huabo.legal.exam.modules.paper.mapper.PaperMapper;
import com.huabo.legal.exam.modules.paper.service.PaperQuAnswerService;
import com.huabo.legal.exam.modules.paper.service.PaperQuService;
import com.huabo.legal.exam.modules.paper.service.PaperService;
import com.huabo.legal.exam.modules.qu.entity.Qu;
import com.huabo.legal.exam.modules.qu.entity.QuAnswer;
import com.huabo.legal.exam.modules.qu.enums.QuType;
import com.huabo.legal.exam.modules.qu.service.QuAnswerService;
import com.huabo.legal.exam.modules.qu.service.QuService;
import com.huabo.legal.exam.modules.sys.user.service.SysUserService;
...
public class PaperServiceImpl extends ServiceImpl<PaperMapper, Paper> implements PaperService {
    ...
    public String createPaper(String userId, String examId) {
        ...
        // 考试题目列表
        List<PaperQu> quList = this.generateByRepo(examId, exam.getExamLevel());
...
//        paper.setDepartId(user.getDepartId());
    ...
    @Override
    public void handExam(String paperId,String userId) {
    ...
    @Override
    public PageResult<PaperListRespDTO> paging(PagingReqDTO<PaperListReqDTO> reqDTO) {
    	PageInfo<PaperListRespDTO> objectPageInfo = PageMethod.startPage(reqDTO.getCurrent(), reqDTO.getSize())
				.doSelectPageInfo(() -> baseMapper.paging(reqDTO.toPage(), reqDTO.getParams()));
		return new PageResult<PaperListRespDTO>().build(objectPageInfo);
    }
	 ...
	 */
	@Override
	public void updateExamLeaveJob() {
		QueryWrapper<Paper> wrapper = new QueryWrapper<>();
		wrapper.lambda().or().eq(Paper::getState, PaperState.ING).or().eq(Paper::getState, PaperState.WAIT_OPT);
		List<Paper> papers = baseMapper.selectList(wrapper);
		if (CollectionUtil.isEmpty(papers)){
			return;
		}
		Date now = new Date();
		papers.forEach(x->{
			Date date = DateUtil.addMinutes(x.getCreateTime(), x.getTotalTime());
			if (now.after(date)){
				baseMapper.updateExamLeaveJob(x.getId());
			}
		});
	}
}
