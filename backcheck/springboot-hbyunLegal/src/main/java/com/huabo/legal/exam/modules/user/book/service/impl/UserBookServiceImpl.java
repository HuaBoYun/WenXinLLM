package com.huabo.legal.exam.modules.user.book.service.impl;
...
import com.huabo.legal.exam.modules.user.book.mapper.UserBookMapper;
...
public class UserBookServiceImpl extends ServiceImpl<UserBookMapper, UserBook> implements UserBookService {
    ...
    @Override
	public JsonBean paging(PagingReqDTO<UserBookDTO> reqDTO, String userId) {
       ...
       // Page query = new Page<>(reqDTO.getCurrent(), reqDTO.getSize());
        ...
        // 查找用户的错题
        wrapper.lambda().eq(UserBook::getUserId,userId);
        ...
        List<UserBook> page = baseMapper.selectList(wrapper);
        
        //转换结果
        PageInfo<UserBookDTO> pageData = JSON.parseObject(JSON.toJSONString(page), new TypeReference<PageInfo<UserBookDTO>>(){});
        
        return ResponseFormat.retParam(200, 200, new PageResult<UserBookDTO>().build(pageData));
     }
