...
public class TblFxgkBugMapperSqlConfig {
	...
	public String selectListByPageInfo(PageInfo<TblNbsjBugEntity> pageInfo,TblFxgkBugVo tblNbsjBugVo,Integer orgid,Integer companyid) {
...
//		}
	...
	public String selectListByPageInfo2(TblFxgkBugVo tblNbsjBugVo,Integer orgid,BigDecimal companyid) {
		String sql = new SQL() {{
			SELECT("TNA.*", "cc.*", "ORG.ORGNAME");
			FROM("TBL_NBSJ_BUG TNA");
			...
			WHERE(" 1=1");
			if (tblNbsjBugVo.getBugid() != null) {
				WHERE("TNA.BUGID !=#{tblNbsjBugVo.bugid} AND TNA.FATHERBUGID IS NULL");
			}
			if (null != orgid) {
				WHERE(" BUGDEPARTMENT=#{orgid}");
			}
			if (tblNbsjBugVo.getBugnumber() != null && tblNbsjBugVo.getBugnumber().length() > 0) {
				WHERE("  TNA.BUGNUMBER LIKE concat('%',CONCAT(#{tblNbsjBugVo.bugnumber}, '%'))");
			}
			if (tblNbsjBugVo.getBugcriid() != null && tblNbsjBugVo.getBugcriid().length() > 0) {
				WHERE("  cir.BUGCRIID LIKE concat('%',CONCAT( #{tblNbsjBugVo.bugcriid}, '%'))");
			}
//
			if (null != tblNbsjBugVo.getStartDate()) {
				WHERE("#{tblNbsjBugVo.startDate} < TNA.DISCOVERTIME");
			}

			if (null != tblNbsjBugVo.getEndDate()) {
				DateTime dateTime = cn.hutool.core.date.DateUtil.offsetDay(tblNbsjBugVo.getEndDate(), 1);
				tblNbsjBugVo.setEndDate(dateTime);
				WHERE("  TNA.DISCOVERTIME < #{tblNbsjBugVo.endDate}");
			}
	...
	public String selectCountByPageInfo(PageInfo<TblNbsjBugEntity> pageInfo,TblFxgkBugVo tblNbsjBugVo,Integer orgid,Integer companyid) {
