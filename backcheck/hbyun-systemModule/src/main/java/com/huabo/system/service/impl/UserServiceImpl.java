...
public class UserServiceImpl implements UserService {
	...
	@Override
	public JsonBean exportUserInfoList(String token, MultipartFile file) throws Exception {
        ...
        String noRootName = "";//记录集团公司为空导入失败的信息
        String noCompanyName = "";//记录所属公司为空导入失败的信息；
        String noUserName = "";//记录账号为空为空导入失败的组织信息
        String noRealName = ""; //记录真实姓名为空为空导入失败的组织信息
        String noDeptName = "";//记录组织编号为空导入失败的组织信息
        String reUserName = "";//记录用户账号名称重复导入失败的组织信息
        ...
        try {
				...
				for (int x = 1; x < sheet.getPhysicalNumberOfRows(); x++) {
					...
					if (row != null) {
                        ...
                        if (cell != null) {
                            //获取单元格的值
                            cell.setCellType(CellType.STRING);//字符串
                            staff.setPassword(MD5Encrypt.md5WithEncoding(cell.getStringCellValue(),"UTF-8"));
                        }
                        cell = row.getCell(2);
                        if (cell != null) {
                            //获取单元格的值
                            cell.setCellType(CellType.STRING);//字符串
                            staff.setRealname(cell.getStringCellValue());
                        }
                        cell = row.getCell(3);
                        if (cell != null) {
                            //获取单元格的值
                            cell.setCellType(CellType.STRING);//字符串
                            staff.setIdCard(cell.getStringCellValue());
                        }
                        ...
                        if (cell != null) {
                            //获取单元格的值
                            cell.setCellType(CellType.STRING);//字符串
                            staff.setStatus("启用".equals(cell.getStringCellValue())?1:0);
                        }
                        ...
                        if (cell != null) {
                            //获取单元格的值
                            cell.setCellType(CellType.STRING);//字符串
                            staff.setCompanyname(cell.getStringCellValue());
                        }
                        ...
                        if (cell != null) {
                            //获取单元格的值
                            cell.setCellType(CellType.STRING);//字符串
                            staff.setBELONGORGNUMBER(cell.getStringCellValue());
                        }
                        ...
                        if (cell != null) {
                            //获取单元格的值
                            cell.setCellType(CellType.STRING);//字符串
                            staff.setOrgname(cell.getStringCellValue());
                        }
                        ...
                        if (cell != null) {
                            //获取单元格的值
                            cell.setCellType(CellType.STRING);//字符串
                            staff.setRoleIdStrs(cell.getStringCellValue());
                        }
                        ...
                        if (cell != null) {
                            //获取单元格的值
                            cell.setCellType(CellType.STRING);//字符串
                            staff.setFixedphone(cell.getStringCellValue());
                        }
                        cell = row.getCell(11);
                        if (cell != null) {
                            //获取单元格的值
                            cell.setCellType(CellType.STRING);//字符串
                            staff.setMiblephone(cell.getStringCellValue());
                        }
                        cell = row.getCell(12);
                        if (cell != null) {
                            //获取单元格的值
                            cell.setCellType(CellType.STRING);//字符串
                            staff.setMemo(cell.getStringCellValue());
                        }
					}
					//校验必填属性不能为空
					if(StringUtils.isBlank(staff.getRealname())) {
						totalCount++;
						noRealName += totalCount+"、";
                    	continue;
					}
					if(StringUtils.isBlank(staff.getUsername())) {
						totalCount++;
						noUserName += staff.getRealname()+"、";
                    	continue;
					}
					if(StringUtils.isBlank(staff.getOrgname())) {
						totalCount++;
						noDeptName += staff.getRealname()+"、";
                    	continue;
					}
					
					//获取集团公司信息
					rootOrg = this.tblOrganizationMapper.selectInfoByOrgNumber(staff.getCompanyname());
					if(rootOrg == null) {
						totalCount++;
						noRootName += staff.getRealname()+"、";
                    	continue;
					}
					
					//获取公司信息
					company = this.tblOrganizationMapper.selectDeptInfoByUniqueNumber(staff.getBELONGORGNUMBER(),rootOrg.getOrgnumber());
					if(company == null) {
						totalCount++;
						noCompanyName += staff.getRealname()+"、";
                    	continue;
					}
					
					dept = this.tblOrganizationMapper.selectDeptInfoByUniqueNumber(staff.getOrgname(),rootOrg.getOrgnumber()+company.getOrgnumber());
					if(dept == null) {
						totalCount++;
						noDeptName += staff.getRealname()+"、";
                    	continue;
					...
					//校验用户名是否重复
					repartCount = this.tblStaffMapper.selectUniqueCountUserName(staff.getUsername(), null);
					if(repartCount > 0 ) {
						totalCount++;
						reUserName += staff.getRealname()+"、";
                    	continue;
					}
					totalCount++;
					importCount++;
					...
					relaList = this.tblUserOrgRelationService.dealInsertRealtionInfo(loginStaff,new String[]{dept.getOrgid().toString()});
					tblStaffMapper.insertUser(staff);
					this.tblUserOrgRelationService.removeRelationByStaffId(staff.getStaffid());
		            this.tblUserOrgRelationService.InsertRealtionByStaffId(staff.getStaffid(),relaList);
			        if(YMUrlStatic.status == 0) {
			        	this.ymBusinessService.dealUniqueStaffInfo(staff.getStaffid(),relaList);
			        }
				}
			}
            ...
            imlog.setCreateStaff(loginStaff.getStaffid());
            imlog.setCreateTime(new Date());
            imlog.setImportCount(importCount);
            imlog.setTotalCount(totalCount);
            String content = "";
            if(StringUtils.isNotBlank(noRealName)) {
            	content += noRealName.substring(0, noRealName.length()-1) +"行，真实姓名为空，";
            }
            if(StringUtils.isNotBlank(noUserName)) {
            	content += noUserName.substring(0, noUserName.length()-1) +"，用户名为空，";
            }
            if(StringUtils.isNotBlank(noRootName)) {
            	content += noRootName.substring(0, noRootName.length()-1) +"，集团公司编号为空，";
            }
            if(StringUtils.isNotBlank(noCompanyName)) {
            	content += noCompanyName.substring(0, noCompanyName.length()-1) +"，主职公司编号为空，";
            }
            if(StringUtils.isNotBlank(noDeptName)) {
            	content += noDeptName.substring(0, noDeptName.length()-1) +"，主职部门编号为空，";
            }
            if(StringUtils.isNotBlank(reUserName)) {
            	content += reUserName.substring(0, reUserName.length()-1) +"用户名重复，";
            }
            ...
            imlog.setImportType(TblSystemImportLog.IMPORTSTAFF);
    ...
    @Override
	public JsonBean exportUserInfoList(TblStaff staff, String token, BigDecimal orgId, HttpServletResponse response) throws Exception {
    	TblStaffUtil loginStaff = userProvider.get();
        if (loginStaff == null) {
            return ResponseFormat.retParam(0, 20006, null);
        }
        ...
        List<Object[]> staffObjList = new ArrayList<Object[]>(0);
		Object[] objs = null;
        //获取当前组织下的所有组织
        String allOrgIds = this.tblOrganizaService.getAllChildrenOrgIdStrs(orgId.toString(),0);
        staff.setOrgIdStrs(allOrgIds);
        
        /*List<String> childrenIdList = this.tblOrganizationResultMapper.selectAllChildrenIdsByTrees(orgId.toString(), 0);
        staff.setOrgIdList(childrenIdList);*/
        
        
        List<TblStaff> staffList = this.tblStaffMapper.selectExprotList(staff);
        List<String> roleNameList = null;
        for (TblStaff ts : staffList) {
        	objs = new Object[9];
        	objs[0] = StringUtils.isNotBlank(ts.getRealname())?ts.getRealname():"";
        	objs[1] = StringUtils.isNotBlank(ts.getOrgname())?ts.getOrgname():"";
        	//查找兼职部门
        	roleNameList = this.tblUserOrgRelationMapper.selectDeptNamesByStaffId(ts.getStaffid());
        	if(roleNameList != null && roleNameList.size() > 0 ) {
        		objs[2] = String.join(",", roleNameList);
        	}else {
        		objs[2] = "";
        	}
        	if(StringUtils.isNotBlank(ts.getRoleIdStrs())) {
        		objs[3] = String.join(",", this.tblRoleMapper.selectRoleNamesByIds(ts.getRoleIdStrs()));
        	}else {
        		objs[3] = "";//角色
        	}
        	objs[4] = StringUtils.isNotBlank(ts.getUsername())?ts.getUsername():"";
        	objs[5] = StringUtils.isNotBlank(ts.getMiblephone())?ts.getMiblephone():"";
        	objs[6] = StringUtils.isNotBlank(ts.getEmail())?ts.getEmail():"";
        	objs[7] = StringUtils.isNotBlank(ts.getAddress())?ts.getAddress():"";
        	objs[8] = StringUtils.isNotBlank(ts.getMemo())?ts.getMemo():"";
        	staffObjList.add(objs);
		}
        String[] cNames = { "姓名", "主责部门","兼职部门","所属角色", "账号","手机号", "邮箱", "地址", "简介"};
        String fileName = "用户信息列表.xlsx";
        String encodedFileName = URLEncoder.encode(fileName, "UTF-8")
                .replaceAll("\\+", "%20"); // 替换+号为%20
        response.setHeader("Content-Disposition", "attachment;filename=" + encodedFileName);
		ServletOutputStream outputStream = response.getOutputStream();
		ImportOrExportExcelUtil.exportExcel(cNames, staffObjList, outputStream, null);
		return ResponseFormat.retParam(1, 200, null);
	}
    ...
    public Map<String, Object> findAllPageBeanPid(String staffId, Find find, Integer pageNumber, Integer pageSize, BigDecimal pid) {
        ...
        try {
            ...
            String orgIdStrs = this.tblOrganizaService.selectAllDeptIdsByOrgId(pid);
            
            Page<TblStaff> page = new Page<TblStaff>(pageNumber,pageSize);
            page.setOptimizeCountSql(false); // 禁用自动优化
            IPage<TblStaff> pageList = tblStaffMapper.selectListByPageInfoOrgid(page, orgIdStrs, find);
            
            List<TblStaff> staffList = pageList.getRecords();
            BigDecimal currentdeptId = null;
	    	String midcomName = null;
	    	for (TblStaff tblStaff : staffList) {
	    		if(tblStaff.getOrgid() == null) {
	    			continue;
	    		}
				if(currentdeptId != null && currentdeptId.compareTo(tblStaff.getOrgid()) == 0) {
						tblStaff.setCompanyname(midcomName);
						continue;
				}
				midcomName = this.tblOrganizaService.getDeptLinkCompanyNameByDeptId(tblStaff.getOrgid());
				currentdeptId = tblStaff.getOrgid();
				tblStaff.setCompanyname(midcomName);
				if(StringUtils.isNotBlank(tblStaff.getRoleIdStrs())) {
					tblStaff.setRoleNames(String.join(",", this.tblRoleMapper.selectRoleNamesByIds(tblStaff.getRoleIdStrs())));
				}
...
//            sql = "INSERT INTO TBL_TESTTEMPLE (TESTTEMID,TEMPLENAME,TEMPLENUMBER,CREATEDATETIME,TEMPLEDESC,TBLCOMANY,STAFFID,SOURCE) VALUES (" + csmbIdone + ",'测试模板名称01','CSMBBH02',SYSDATE,'测试模板说明02','" + organization.getOrgid() + "','" + user.getStaffid() + "','自建')";
//            query = session.createSQLQuery(sql);
//            query.executeUpdate();
//            query = session.createSQLQuery("SELECT HIBERNATE_SEQUENCE.NEXTVAL AS ID FROM DUAL");
//            Object csjdIdrootone = query.uniqueResult();
//            sql = "INSERT INTO TBL_TESTTEMPL_TYPE (TYPEID,TYPENAME,PARENTID,TESTTEMPLETAID,CREATETIME,TYPECODE) VALUES (" + csjdIdrootone + ",'组织架构',NULL," + csmbIdone + ",SYSDATE,'ZZJG')";
//            query = session.createSQLQuery(sql);
//            query.executeUpdate();
//            query = session.createSQLQuery("SELECT HIBERNATE_SEQUENCE.NEXTVAL AS ID FROM DUAL");
//            Object csjdIdroottwo = query.uniqueResult();
//            sql = "INSERT INTO TBL_TESTTEMPL_TYPE (TYPEID,TYPENAME,PARENTID,TESTTEMPLETAID,CREATETIME,TYPECODE) VALUES (" + csjdIdroottwo + ",'财务报告',NULL," + csmbIdone + ",SYSDATE,'CWBG')";
//            query = session.createSQLQuery(sql);
//            query.executeUpdate();
//            query = session.createSQLQuery("SELECT HIBERNATE_SEQUENCE.NEXTVAL AS ID FROM DUAL");
//            Object csjdIdone = query.uniqueResult();
//            sql = "INSERT INTO TBL_TESTTEMPL_TYPE (TYPEID,TYPENAME,PARENTID,TESTTEMPLETAID,CREATETIME,TYPECODE) VALUES (" + csjdIdone + ",'内部组织机构及部门职责'," + csjdIdrootone + "," + csmbIdone + ",SYSDATE,'NBZZJGJBMZZ')";
//            query = session.createSQLQuery(sql);
//            query.executeUpdate();
//            query = session.createSQLQuery("SELECT HIBERNATE_SEQUENCE.NEXTVAL AS ID FROM DUAL");
//            Object csjdIdtwo = query.uniqueResult();
//            sql = "INSERT INTO TBL_TESTTEMPL_TYPE (TYPEID,TYPENAME,PARENTID,TESTTEMPLETAID,CREATETIME,TYPECODE) VALUES (" + csjdIdtwo + ",'董事会工作流程'," + csjdIdrootone + "," + csmbIdone + ",SYSDATE,'DSHGZLC')";
//            query = session.createSQLQuery(sql);
//            query.executeUpdate();
//            query = session.createSQLQuery("SELECT HIBERNATE_SEQUENCE.NEXTVAL AS ID FROM DUAL");
//            Object csjdIdthree = query.uniqueResult();
//            sql = "INSERT INTO TBL_TESTTEMPL_TYPE (TYPEID,TYPENAME,PARENTID,TESTTEMPLETAID,CREATETIME,TYPECODE) VALUES (" + csjdIdthree + ",'日常会计处理和期末关账'," + csjdIdroottwo + "," + csmbIdone + ",SYSDATE,'RCKJCLHQMGZ')";
//            query = session.createSQLQuery(sql);
//            query.executeUpdate();
//            query = session.createSQLQuery("SELECT HIBERNATE_SEQUENCE.NEXTVAL AS ID FROM DUAL");
//            Object csjdIdfour = query.uniqueResult();
//            sql = "INSERT INTO TBL_TESTTEMPL_TYPE (TYPEID,TYPENAME,PARENTID,TESTTEMPLETAID,CREATETIME,TYPECODE) VALUES (" + csjdIdfour + ",'财务报告管理'," + csjdIdroottwo + "," + csmbIdone + ",SYSDATE,'CWBGGL')";
//            query = session.createSQLQuery(sql);
//            query.executeUpdate();
//            sql = "INSERT INTO TBL_TESTELEMENT (ELEMENTID,RISKTYPE,CHECKMETHOD,CONTROLMETHOD,CONTROLTYPE,CONTROLREQ,MATERIAL,TYPEID,CREATETIME,TEMPLID,ELEMENTCODE,BUSINESSDESC,CONTROLTARGET,CONTROLMEASURES) VALUES (HIBERNATE_SEQUENCE.NEXTVAL,'1.组织架构存在缺陷，导致组织的运行效率低下；2.组织架构调整方案不符合企业自身特点及实际情况，可能会影响企业效率；3.新的组织架构运行得不到内部部门的积极支持，可能影响企业经营目标的实现。','1.了解现有组织架构是否应有利于企业战略目标的实现，是否与企业内部主导业务流程相符、是否满足企业内部高校管理的要求、是否广泛征得相关人员如董事、监事、高级管理人员和其他员工的意见；2.《组织架构调整方案》的审批是否符合规定程序和要求;3.新的组织架构文件资料是否按企业统一规范编写、审批、发布','自动','预防性','随时','1.公司章程；2.组织结构调整管理办法。'," + csjdIdone + ",SYSDATE," + csmbIdone + ",'226761','组织架构的评估与调整流程','组织机构能够适应公司发展，运行效率较高，确保企业经营目标的实现。','1.当前组织构架能够满足企业运行需要。2.组织机构调整有会议纪要')";
//            sqlList.add(sql);
...
//    }
    ...
    @Override
    public void findAllPageInfoByacctid(PageInfo<TblStaff> pageInfo, String bookid) throws Exception {
        Map<String, Object> resultMap = new HashMap<String, Object>(0);

        Page<TblStaff> page = new Page<TblStaff>(pageInfo.getCurrentPage(),pageInfo.getPageSize());
        page.setOptimizeCountSql(false); // 禁用自动优化
        IPage<TblStaff> pageList = this.tblStaffMapper.findAllPageInfoByacctid(page, bookid);
        
        List<TblStaff> list = pageList.getRecords();
        
        BigDecimal currentdeptId = null;
    	String midcomName = null;
    	for (TblStaff tblStaff : list) {
    		if(tblStaff.getOrgid() == null) {
    			continue;
    		}
			if(currentdeptId != null && currentdeptId.compareTo(tblStaff.getOrgid()) == 0) {
			...
			midcomName = this.tblOrganizaService.getDeptLinkCompanyNameByDeptId(tblStaff.getOrgid());
			currentdeptId = tblStaff.getOrgid();
			tblStaff.setOrgname(midcomName);
		}
    ...
    public Map<String, Object> findByAll(String pid, PageInfo<TblStaff> pageInfo) {
        ...
        Page<TblStaff> page = new Page<TblStaff>(pageInfo.getCurrentPage(),pageInfo.getPageSize());
        page.setOptimizeCountSql(false); // 禁用自动优化
        IPage<TblStaff> pageList = this.tblStaffMapper.findByAll(pid, page);
    ...
    public Map<String, Object> findByAllORGID(BigDecimal orgid, PageInfo<TblStaff> pageInfo) {
        ...
        Page<TblStaff> page = new Page<TblStaff>(pageInfo.getCurrentPage(),pageInfo.getPageSize());
        page.setOptimizeCountSql(false); // 禁用自动优化
        IPage<TblStaff> pageList = this.tblStaffMapper.findByAllORGID(orgid, page);
        pageInfo.setTlist(pageList.getRecords());
        pageInfo.setTotalRecord((int)pageList.getTotal());
        resultMap.put("code", "1");
        resultMap.put("msg", "数据访问成功");
	...
	@Override
	public Map<String, Object> findAllBmfzrPageBean(String token, Integer pageNumber, Integer pageSize) {
		Map<String, Object> resultMap = new HashMap<String, Object>(0);
        com.hbfk.util.PageInfo<TblStaff> pageInfo = new com.hbfk.util.PageInfo<TblStaff>();
        try {
            ...
            BigDecimal gongsiId = staff.getLinkOrg().getOrgid();
            List<TblRole> listrole = tblStaffMapper.selectRoleByBmfzr(gongsiId);
            if(null!=listrole && listrole.size()>0) {
            	TblRole roleBmfzr = listrole.get(0);
            	roleid = roleBmfzr.getRid();
            }
            
            Page<TblStaff> page = new Page<TblStaff>(pageNumber,pageSize);
            page.setOptimizeCountSql(false); // 禁用自动优化
            IPage<TblStaff> pageList = tblStaffMapper.selectListBmfzrByPageInfo(page, roleid);
            pageInfo.setTlist(pageList.getRecords());
            pageInfo.setTotalRecord((int)pageList.getTotal());
            resultMap.put("code", "1");
            resultMap.put("msg", "访问接口成功");
	...
	@Override
	public Map<String, Object> findAllFgldPageBean(String token, Integer pageNumber, Integer pageSize) {
		Map<String, Object> resultMap = new HashMap<String, Object>(0);
        com.hbfk.util.PageInfo<TblStaff> pageInfo = new com.hbfk.util.PageInfo<TblStaff>();
        try {
            TblStaffUtil staff = userProvider.get();
            ...
            Page<TblStaff> page = new Page<TblStaff>(pageNumber,pageSize);
            page.setOptimizeCountSql(false); // 禁用自动优化
            IPage<TblStaff> pageList = tblStaffMapper.selectListFgldByPageInfo(page,gongsiId);
            pageInfo.setTlist(pageList.getRecords());
            pageInfo.setTotalRecord((int)pageList.getTotal());
            resultMap.put("code", "1");
            resultMap.put("msg", "访问接口成功");
    ...
    public Map<String, Object> findAllPageBean(Find find, Integer pageNumber, Integer pageSize, String token, String staffId) {
        ...
        try {
            ...
            pageInfo.setCurrentPage(pageNumber);
            pageInfo.setPageSize(pageSize);
            //String orgIdStrs = this.tblOrganizationMapper.selectOrgidStrsByOrgId(orgid);
            
            Page<TblStaff> page = new Page<TblStaff>(pageNumber,pageSize);
            page.setOptimizeCountSql(false); // 禁用自动优化
            IPage<TblStaff> pageList = this.tblStaffMapper.selectListByPageInfoFind(page, find);
	...
	@Override
	public JsonBean finduserAllList(TblStaff staff, Integer pageNumber, Integer pageSize, String token, Integer isAll)
			throws Exception {
		TblStaffUtil loginStaff = userProvider.get();
        if (loginStaff == null) {
            return ResponseFormat.retParam(0, 20006, null);
        ...
        List<TblStaff> list =  new ArrayList<TblStaff>(0);
        BigDecimal rootOrgId = null;
		Integer orgtype = null;
		
		if(staff.getOrgid() == null) {
			if(isAll.compareTo(0) == 0) {
				//查询本公司的用户
				rootOrgId = loginStaff.getCurrentOrg().getOrgid();
				orgtype = 0;
			}else {
				//查询全集团的用户
				rootOrgId = this.tblOrganizaService.getRootCompanyByOrgId(loginStaff.getCurrentOrg().getOrgid());
			}
		}else {
			rootOrgId = staff.getOrgid();
		}
		

		String secrectScope = null;
		if(staff.getFormSecrectId() != null) {
			secrectScope = this.tblSecrectLevelMapper.selectStaffScopeById(staff.getFormSecrectId());
			staff.setFormSecrectScope(secrectScope);
		}
		
		if(isAll.compareTo(0) == 0) {
			String allOrgIds = this.tblOrganizaService.getAllChildrenOrgIdStrs(rootOrgId.toString(),orgtype);
			staff.setOrgIdStrs(allOrgIds);
		}else {
			List<String> childrenIdList = this.tblOrganizationResultMapper.selectAllChildrenIdsByTrees(rootOrgId.toString(), 0);
	        staff.setOrgIdList(childrenIdList);
		}
		
		
		PageHelper.startPage(pageNumber, pageSize);
		list = this.tblStaffMapper.selectAllStaffListByPageInfo(staff);
