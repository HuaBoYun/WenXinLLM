...
import com.management.accountant.entity.ncv65.BudgetCustomer;
...
/**
 * NCV65全面预算系统 - BudgetCustomer数据访问接口
 * 
 * @description BudgetCustomer数据访问层，提供BudgetCustomer的数据库操作方法
 * @author AI Assistant
 * @date 2025-01-09
 * @version 1.0.0
 */
@Mapper
public interface BudgetCustomerMapper extends BaseMapper<BudgetCustomer> {
    ...
    BudgetCustomer selectByCode(@Param("code") String code);

    /**
     * 根据名称查询
     */
    @Select("SELECT * FROM ${tableName} WHERE NAME = #{name} AND IS_DELETED = 0")
    List<BudgetCustomer> selectByName(@Param("name") String name);

    /**
     * 根据状态查询
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = #{status} AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetCustomer> selectByStatus(@Param("status") String status);
     ...
     * 分页查询
     */
    IPage<BudgetCustomer> selectBudgetCustomerPage(Page<BudgetCustomer> page, @Param("params") Map<String, Object> params);

    /**
     * 查询激活的记录列表
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = 'ACTIVE' AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetCustomer> selectActiveRecords();
