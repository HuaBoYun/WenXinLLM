...
import com.management.accountant.oracle.entity.TblOrganization;
import com.management.accountant.oracle.entity.TblStaffOracle;
import com.management.accountant.vo.param.UserAllQueryParam;
import com.management.accountant.vo.result.StaffResult;
import com.management.accountant.vo.result.UserAllResult;
import com.management.accountant.vo.result.UserInfo;
...
public interface TblStaffOracleMapper extends Mapper<TblStaffOracle> {
	...
	/**
	 * 查询所有启用状态的员工（避免使用TkMapper的selectAll，防止与MyBatis-Plus冲突）
	 */
	@Select("SELECT STAFFID, REALNAME, STATUS, ORGID FROM TBL_STAFF WHERE STATUS = 1")
	List<TblStaffOracle> selectAllActiveStaff();
