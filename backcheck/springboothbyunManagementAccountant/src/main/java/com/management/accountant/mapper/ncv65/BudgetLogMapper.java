...
import com.management.accountant.entity.ncv65.BudgetLog;
...
/**
 * NCV65全面预算系统 - BudgetLog数据访问接口
 * 
 * @description BudgetLog数据访问层，提供BudgetLog的数据库操作方法
 * @author AI Assistant
 * @date 2025-01-09
 * @version 1.0.0
 */
@Mapper
public interface BudgetLogMapper extends BaseMapper<BudgetLog> {
     ...
     * 分页查询
     */
    IPage<BudgetLog> selectBudgetLogPage(Page<BudgetLog> page, @Param("params") Map<String, Object> params);

    /**
     * 查询激活的记录列表
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = 'ACTIVE' AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetLog> selectActiveRecords();
