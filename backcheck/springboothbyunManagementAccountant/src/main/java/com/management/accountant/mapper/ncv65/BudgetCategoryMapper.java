...
import com.management.accountant.entity.ncv65.BudgetCategory;
...
/**
 * NCV65全面预算系统 - BudgetCategory数据访问接口
 * 
 * @description BudgetCategory数据访问层，提供BudgetCategory的数据库操作方法
 * @author AI Assistant
 * @date 2025-01-09
 * @version 1.0.0
 */
...
public interface BudgetCategoryMapper extends BaseMapper<BudgetCategory> {
    ...
    List<BudgetCategory> selectByName(@Param("name") String name);

    /**
     * 根据状态查询
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = #{status} AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetCategory> selectByStatus(@Param("status") String status);
     ...
     * 分页查询
     */
    IPage<BudgetCategory> selectBudgetCategoryPage(Page<BudgetCategory> page, @Param("params") Map<String, Object> params);

    /**
     * 查询激活的记录列表
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = 'ACTIVE' AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetCategory> selectActiveRecords();
