...
import com.management.accountant.entity.ncv65.BudgetProject;
...
/**
 * NCV65全面预算系统 - BudgetProject数据访问接口
 * 
 * @description BudgetProject数据访问层，提供BudgetProject的数据库操作方法
 * @author AI Assistant
 * @date 2025-01-09
 * @version 1.0.0
 */
...
public interface BudgetProjectMapper extends BaseMapper<BudgetProject> {
    ...
    BudgetProject selectByCode(@Param("code") String code);

    /**
     * 根据名称查询
     */
    @Select("SELECT * FROM ${tableName} WHERE NAME = #{name} AND IS_DELETED = 0")
    List<BudgetProject> selectByName(@Param("name") String name);

    /**
     * 根据状态查询
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = #{status} AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetProject> selectByStatus(@Param("status") String status);
     ...
     * 分页查询
     */
    IPage<BudgetProject> selectBudgetProjectPage(Page<BudgetProject> page, @Param("params") Map<String, Object> params);

    /**
     * 查询激活的记录列表
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = 'ACTIVE' AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetProject> selectActiveRecords();
