...
import com.management.accountant.entity.ncv65.BudgetTag;
...
/**
 * NCV65全面预算系统 - BudgetTag数据访问接口
 * 
 * @description BudgetTag数据访问层，提供BudgetTag的数据库操作方法
 * @author AI Assistant
 * @date 2025-01-09
 * @version 1.0.0
 */
@Mapper
public interface BudgetTagMapper extends BaseMapper<BudgetTag> {
     ...
     * 分页查询
     */
    IPage<BudgetTag> selectBudgetTagPage(Page<BudgetTag> page, @Param("params") Map<String, Object> params);

    /**
     * 查询激活的记录列表
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = 'ACTIVE' AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetTag> selectActiveRecords();
