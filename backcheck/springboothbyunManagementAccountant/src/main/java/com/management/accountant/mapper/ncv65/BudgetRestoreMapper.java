...
import com.management.accountant.entity.ncv65.BudgetRestore;
...
/**
 * NCV65全面预算系统 - BudgetRestore数据访问接口
 * 
 * @description BudgetRestore数据访问层，提供BudgetRestore的数据库操作方法
 * @author AI Assistant
 * @date 2025-01-09
 * @version 1.0.0
 */
@Mapper
public interface BudgetRestoreMapper extends BaseMapper<BudgetRestore> {
    ...
    BudgetRestore selectByCode(@Param("code") String code);

    /**
     * 根据名称查询
     */
    @Select("SELECT * FROM ${tableName} WHERE NAME = #{name} AND IS_DELETED = 0")
    List<BudgetRestore> selectByName(@Param("name") String name);

    /**
     * 根据状态查询
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = #{status} AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetRestore> selectByStatus(@Param("status") String status);
     ...
     * 分页查询
     */
    IPage<BudgetRestore> selectBudgetRestorePage(Page<BudgetRestore> page, @Param("params") Map<String, Object> params);

    /**
     * 查询激活的记录列表
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = 'ACTIVE' AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetRestore> selectActiveRecords();
