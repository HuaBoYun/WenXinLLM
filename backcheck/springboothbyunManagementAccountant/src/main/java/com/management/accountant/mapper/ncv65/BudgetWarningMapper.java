...
import com.management.accountant.entity.ncv65.BudgetWarning;
...
/**
 * NCV65全面预算系统 - BudgetWarning数据访问接口
 * 
 * @description BudgetWarning数据访问层，提供BudgetWarning的数据库操作方法
 * @author AI Assistant
 * @date 2025-01-09
 * @version 1.0.0
 */
...
public interface BudgetWarningMapper extends BaseMapper<BudgetWarning> {
    ...
    BudgetWarning selectByCode(@Param("code") String code);

    /**
     * 根据名称查询
     */
    @Select("SELECT * FROM ${tableName} WHERE NAME = #{name} AND IS_DELETED = 0")
    List<BudgetWarning> selectByName(@Param("name") String name);

    /**
     * 根据状态查询
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = #{status} AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetWarning> selectByStatus(@Param("status") String status);
     ...
     * 分页查询
     */
    IPage<BudgetWarning> selectBudgetWarningPage(Page<BudgetWarning> page, @Param("params") Map<String, Object> params);

    /**
     * 查询激活的记录列表
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = 'ACTIVE' AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetWarning> selectActiveRecords();
