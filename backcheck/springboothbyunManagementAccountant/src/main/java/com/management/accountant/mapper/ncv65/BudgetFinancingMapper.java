...
import com.management.accountant.entity.ncv65.BudgetFinancing;
...
/**
 * NCV65全面预算系统 - BudgetFinancing数据访问接口
 * 
 * @description BudgetFinancing数据访问层，提供BudgetFinancing的数据库操作方法
 * @author AI Assistant
 * @date 2025-01-09
 * @version 1.0.0
 */
@Mapper
public interface BudgetFinancingMapper extends BaseMapper<BudgetFinancing> {
    ...
    BudgetFinancing selectByCode(@Param("code") String code);

    /**
     * 根据名称查询
     */
    @Select("SELECT * FROM ${tableName} WHERE NAME = #{name} AND IS_DELETED = 0")
    List<BudgetFinancing> selectByName(@Param("name") String name);

    /**
     * 根据状态查询
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = #{status} AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetFinancing> selectByStatus(@Param("status") String status);
     ...
     * 分页查询
     */
    IPage<BudgetFinancing> selectBudgetFinancingPage(Page<BudgetFinancing> page, @Param("params") Map<String, Object> params);

    /**
     * 查询激活的记录列表
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = 'ACTIVE' AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetFinancing> selectActiveRecords();
