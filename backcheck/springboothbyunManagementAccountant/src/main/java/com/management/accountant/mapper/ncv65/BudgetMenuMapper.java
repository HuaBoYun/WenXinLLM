...
import com.management.accountant.entity.ncv65.BudgetMenu;
...
/**
 * NCV65全面预算系统 - BudgetMenu数据访问接口
 * 
 * @description BudgetMenu数据访问层，提供BudgetMenu的数据库操作方法
 * @author AI Assistant
 * @date 2025-01-09
 * @version 1.0.0
 */
@Mapper
public interface BudgetMenuMapper extends BaseMapper<BudgetMenu> {
    ...
    BudgetMenu selectByCode(@Param("code") String code);

    /**
     * 根据名称查询
     */
    @Select("SELECT * FROM ${tableName} WHERE NAME = #{name} AND IS_DELETED = 0")
    List<BudgetMenu> selectByName(@Param("name") String name);

    /**
     * 根据状态查询
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = #{status} AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetMenu> selectByStatus(@Param("status") String status);
     ...
     * 分页查询
     */
    IPage<BudgetMenu> selectBudgetMenuPage(Page<BudgetMenu> page, @Param("params") Map<String, Object> params);

    /**
     * 查询激活的记录列表
     */
    @Select("SELECT * FROM ${tableName} WHERE STATUS = 'ACTIVE' AND IS_DELETED = 0 ORDER BY CREATE_TIME DESC")
    List<BudgetMenu> selectActiveRecords();
