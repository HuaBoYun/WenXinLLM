                    ...
                    <el-form-item>
                      ...
                      >
                        <el-option
                          ...
                          :key="`unread-${item.textValue}-${index}`"
                          :label="item.textName"
      ...
      async handleOK(row) {
        try {
          ...
          if (res.code == 1) {
            ...
            // 判断distributionType属于哪个模块
            const module = this.jumpInfo.find((item) =>
              item.types.includes(row.distributionType)
            )
            console.log(module, 'module')
            localStorage.setItem('model', module.moduleType)
            localStorage.setItem('modelname', module.name)
            // setTimeout(() => {
            //   this.$router.push(this.jumpUrl[row.distributionType])
            // }, 1000)

            window.location.href = '/'
            localStorage.setItem('path', this.jumpUrl[row.distributionType])
            // 刷新两个列表数据
            this.fetchTabData(this.UNREAD)
            this.fetchTabData(this.READ)
            // 通知父组件确认接收成功
      ...
      async allSure() {
        ...
        try {
          ...
          if (arr.code == 1) {
            ...
            const module1 = this.jumpInfo.find((item) =>
              item.types.includes(this.select[0].distributionType)
            )
            localStorage.setItem('model', module1.moduleType)
            localStorage.setItem('modelname', module1.name)
            window.location.href = '/'
            localStorage.setItem(
              'path',
              this.jumpUrl[this.select[0].distributionType]
            )
            // 刷新两个列表数据
            this.fetchTabData(this.UNREAD)
            this.fetchTabData(this.READ)
