<script>
  ...
  const isCondition = (data) =>
    data.type === 'condition' ||
    (data.type === 'approver' && (data.isInterflow || data.isBranchFlow))
  ...
  function NodeFactory(ctx, data, h) {
    ...
    let res = [],
      ...
      selfNode = (
        <div class="node-wrap">
          <div
            class={`node-wrap-box ${data.type} ${
              NodeUtils.isInterflowNode(data) ? 'interflow' : ''
            } ${NodeUtils.isBranchFlowNode(data) ? 'branchFlow' : ''} ${
              showErrorTip ? 'error' : ''
            }`}
          >
