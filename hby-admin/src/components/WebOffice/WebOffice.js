...
function WebOffice2015() {
	...
	this.WebOpenBase64 = function () {
		...
		//+"&FILETYPE="+this.FileType+"&RECORDID="+this.RecordID+"&EDITTYPE="+this.EditType
		var send = 'OPTION=LOADFILE&FILENAME=' + this.FileName
		$.ajax({
			type: 'POST',
			url: 'http://192.0.2.200:8082/Test/AJAXServer',
			data: send,
			cache: false,
			processData: false,
			xhr: function () {
	...
	this.WebSaveBase64 = function () {
		...
		xmlhttp.open('POST', 'http://192.0.2.200:8082/Test/AJAXServer', false) // false 是同步
		// xmlhttp.open('POST', 'http://192.0.2.16:8064/fwb/fwbEdit', false) // false 是同步
	...
	this.WebSave = function () {
		this.Status = ''
		var httpclient = this.obj.Http // 设置http对象
		console.log('var httpclient', httpclient);
		console.log('this.obj', this.obj.Http);
		httpclient.Clear()
	...
	/* 调入由Template指定的模板，该功能主要用于模板管理或模板套红 */
	...
	this.WebInsertFile = function () {
		...
		if (httpclient.Open(this.HttpMethod.Post, URL, false)) {
			//Http下载服务器文件
			if (httpclient.Send()) {
	...
	this.CreateFile = function () {
		...
		switch (docType) {
			...
			case this.DocType.PPT:
				...
				break
			default: // 默认创建word文档
				this.obj.CreateNew('Word.Document')
				this.Status = '新建WORD成功'
				break
		}
	}
	...
	this.VBAProtectDocument = function (docType, password) {
		if (docType == this.DocType.WORD) {
			...
			//获取sheet总量
			var count =
				this.obj.ActiveDocument.Application.ActiveWorkbook.Sheets.Count

			for (var i = 1; i < count + 1; i++) {
				//如果有锁定的sheet就不做处理
				if (
					this.obj.ActiveDocument.Application.ActiveWorkbook.Sheets.Item(i)
						.ProtectContents
				) {
					// 判断表单是否是保护的
				...
				//没有锁定的就默认用我们的密码锁定
					...
					this.obj.ActiveDocument.Application.ActiveWorkbook.Sheets.Item(
						i
					).Protect(password)
					this.Status = '文档锁定成功'
				}
	...
	this.LOADFILE = function (httpclient) {
		...
		if (httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)) {
			// true 异步方式 false同步
			if (httpclient.Send()) {
				let aaa = httpclient.GetResponseHeader('MsgError')
				if (aaa == '404') {
					this.CreateFile()
					this.getOfficeVersion() // 打开文档后，判断当前office版本
					httpclient.Clear()
