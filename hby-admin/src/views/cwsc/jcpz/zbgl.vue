<template>
  <div class="system-log-container">
    ...
    <zbglEdit ref="edit" @fetchData="fetchData"></zbglEdit>
...
<script>
  ...
  import zbglEdit from './components/zbglEdit.vue'
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'cwsc-jcpz-zbgl-search',
        tableKey: 'cwsc-jcpz-zbgl-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
        row: {},
    ...
    methods: {
      ...
      async fetchData() {
        ...
        const {
          data: { records, total },
        } = await getZbglList(this.queryForm)
        this.list = records
        this.total = total
      ...
      handleAuth(row) {
        this.row = row
        this.$refs['auth'].showEdit(row)
      },
      async handleAuthSelect(data) {
        const ids = data.map((item) => item.rid).join(',')
        const res = await zbglAuthRole({
          pkAccbookinfo: this.row.pkAccbookinfo,
          roleIds: ids,
        })
        if (res.code == 1) {
      ...
      async handleCancelAuth(row) {
        this.row = row
        this.$refs['cancelAuth'].showEdit(row)
      },
      async handleCancelAuthSelect(data) {
        const ids = data.map((item) => item.rid).join(',')
        const res = await zbglCancelAuthRole({
          pkAccbookinfo: this.row.pkAccbookinfo,
          roleIds: ids,
        })
        if (res.code == 1) {
