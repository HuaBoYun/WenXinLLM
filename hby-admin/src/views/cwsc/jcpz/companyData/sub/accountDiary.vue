    ...
    >
      ...
      <el-table-column align="center" label="摘要" prop="explanation">
        <template #default="{ row }">
          ...
          <el-button type="text" v-else @click="$refs.detail.showEdit(row)">
            {{ row.explanation }}
          </el-button>
        </template>
      </el-table-column>
...
<script>
  ...
  import { getDiaryBookList } from '@/api/cwsc'
  ...
  export default {
    ...
    methods: {
      ...
      selectEnable(row) {
        // 期初余额、本日累计、本月累计、本年累计不能被勾选
        const disabledExplanations = [
          '期初余额',
          '本日累计',
          '本月累计',
          '本年累计',
        ]
        if (disabledExplanations.includes(row.explanation)) {
          return false
        }
        return true
      },
      ...
      async handleExport() {
        ...
        try {
          ...
          link.href = url
          link.download = `日记账_${new Date().getTime()}.xlsx`
