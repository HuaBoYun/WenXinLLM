<template>
  <div class="system-log-container">
    <el-page-header content="日记账" @back="goBack" />
    <div
        ...
        >
          <el-form-item>
            ...
            >
              ...
              <el-option label="凭证字号" value="PZH" />
          ...
          <el-form-item>
            ...
            <el-select
              ...
              v-model="queryForm.status"
              placeholder="条件"
              v-if="queryForm.type == 'MD' || queryForm.type == 'MC'"
            >
              <el-option label="等于" value="等于" />
              <el-option label="不等于" value="不等于" />
    ...
    >
      ...
      >
        ...
        <!-- {{
          bookInfo.bookYear +
          '-' +
          (row.pzDate > 9 ? row.pzDate : '0' + row.pzDate) +
          '-' +
          (row.dayNo > 9 ? row.dayNo : '0' + row.dayNo)
        }} -->
      </el-table-column>
      <el-table-column align="center" label="摘要">
        <template #default="{ row }">
          <el-button type="text" @click="handleEdit(row)" v-if="row.accName">
            {{ row.lineText }}
          </el-button>
  ...
  export default {
    ...
    methods: {
      ...
      async fetchData() {
        ...
        } = await getAccountDiaryList(params)
      ...
      handleSelection(val, row) {
        const id = `${row.pzh}_${row.entryId}_${row.glh}`
      ...
      handleSelectAll(val) {
        const ids = val.map((item) => `${item.pzh}_${item.entryId}_${item.glh}`)
        ids.map((elem) => {
          ...
          this.list
            .filter((item) => {
              return this.select.includes(
                `${item.pzh}_${item.entryId}_${item.glh}`
              )
            })
            .forEach((item) => {
