<template>
  <el-dialog v-if="dialogFormVisible" :close-on-click-modal="false" :append-to-body="true" :title="title"
    :visible.sync="dialogFormVisible" width="1000px" @close="closeTop">
    <el-row :gutter="14">
      <el-form ref="ruleForm" label-width="100px" :model="formData" :rules="rules" size="mini">
        <el-col :span="24">
          <el-button type="success" style="margin: -2% 0 2% 94%" @click="add" :disabled="!footer">
            保存
          </el-button>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item label="缺陷编号" prop="bugnumber">
            <el-input v-model="formData.bugnumber" clearable placeholder="请输入缺陷编号" :style="{ width: '100%' }" disabled />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="缺陷级别" prop="bugcriid">
            <el-select v-model="formData.bugcriid" placeholder="请输入缺陷级别" :disabled="!footer" :style="{ width: '100%' }">
              <el-option v-for="item in bugcriidList" :label="item.bugcrilevel" :value="item.bugcriid"
                :key="item.bugcriid" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="发现日期" prop="discovertime">
            <el-date-picker v-model="formData.discovertime" placeholder="请输入发现日期" type="date" format="yyyy-MM-dd"
              value-format="yyyy-MM-dd" :disabled="!footer" :style="{ width: '100%' }" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="发现人" prop="discoverperson">
            <el-input v-model="formData.discoverperson" clearable placeholder="请输入发现人" :style="{ width: '305px' }"
              disabled />
            <el-button :style="{ marginLeft: '10px' }" type="primary" @click="$refs.executor.show()" :disabled="!footer">
              选择
            </el-button>
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item label="缺陷性质" prop="bugproperty">
            <el-select v-model="formData.bugproperty" placeholder="请输入缺陷性质" :disabled="!footer"
              :style="{ width: '100%' }">
        ...
        <el-col :span="12">
          <el-form-item label="缺陷部门" prop="orgname">
            <el-input v-model="formData.orgname" clearable placeholder="请输入缺陷部门" :style="{ width: '305px' }" disabled />
            <el-button :style="{ marginLeft: '10px' }" type="primary" @click="$refs.department.show()"
              :disabled="!footer">
              选择
            </el-button>
          </el-form-item>
        ...
        <el-col :span="12">
          <el-form-item label="业务单元" prop="businessType">
            <el-input v-model="formData.businessType" clearable placeholder="请输入业务单元" :style="{ width: '100%' }"
              :disabled="!footer" />
          </el-form-item>
        </el-col>
        <el-col :span="24">
          <el-form-item label="业务描述" prop="businessDescription">
            <el-input v-model="formData.businessDescription" clearable type="textarea" :rows="2" placeholder="请输入业务描述"
              :style="{ width: '100%' }" :disabled="!footer" />
          </el-form-item>
        </el-col>

        <el-col :span="24" v-if="formData.needreform == '否'">
          <el-form-item label=" 不整改原因" prop="resonfornoreform">
            <el-input v-model="formData.resonfornoreform" clearable placeholder="请输入不整改原因" :style="{ width: '100%' }"
              :disabled="!footer" />
          </el-form-item>
        </el-col>
        <el-col :span="24">
          <el-form-item label="缺陷描述" prop="bugdescripte">
            <el-input v-model="formData.bugdescripte" clearable type="textarea" :rows="2" placeholder="请输入缺陷描述"
              :style="{ width: '100%' }" :disabled="!footer" />
          </el-form-item>
        </el-col>
        <el-col :span="24">
        ...
        <el-col :span="24">
          ...
          <el-table :data="interiorList">
            ...
            <el-table-column align="center" :formatter="formatDate" label="发布日期" prop="publishdate" />
        ...
        <el-col :span="24">
          <div style="text-align: right; margin-bottom: 5px" v-if="footer">
            <el-upload class="upload-demo" :show-file-list="false" :action="baseApi + api" :headers="headers"
              :on-preview="handlePreview" :before-upload="handleBeforeUpload" :on-success="handleSuccess"
              :file-list="tableData">
          ...
          <el-table :data="tableData">
            ...
            <el-table-column align="center" label="操作" show-overflow-tooltip width="120">
              <template #default="{ row }">
                ...
                <el-button type="text" @click="newHandlePreview(row)" :disabled="false">
                    预览
                  </el-button>
...
import DepartmentOptions from '@/views/audit/question/components/options/department.vue'
import ExecutorOptions from '@/views/audit/question/components/options/executor.vue'
import AssociatedTable from '@/views/audit/question/components/table/AssociatedTable'
import ExternalTable from '@/views/audit/question/components/table/ExternalTable'
import InteriorTable from '@/views/audit/question/components/table/InteriorTable'
...
export default {
  ...
  methods: {
    ...
    handleDepartmentSelected(node) {
      ...
      formData.orgname = node.name
      formData.bugdepartment = node.id
    ...
    handleExecutorSelected(node) {
      let formData = this.formData
      formData.discoverperson = node.realname
      formData.reporter = node.staffid
      this.formData = formData
      // this.$forceUpdate()
    },
    showEdit(title, row, info, fileInfo) {
      this.tableData = [fileInfo] //附件表格
      this.formData.businessType = info
      this.$set(this.formData, 'businessType', info)
      this.dialogFormVisible = true
      if (row) {
        // const { tblStaffByFactstaffid, tblNbsjProject, ...other } = row.bug
        // this.formData = {
        //   realname: tblStaffByFactstaffid.realname,
        //   prjoectName: tblNbsjProject.prjoectName,
        //   ...other,
        // }
        this.formData = row
        this.fetchExternal(row.bugid)
        this.fetchInterior(row.bugid)
        this.fetchAssociate(row.bugid)
