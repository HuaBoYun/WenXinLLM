<template>
  <el-dialog v-if="dialogFormVisible" :append-to-body="true" :visible.sync="dialogFormVisible" width="1200px"
    ...
    <vab-query-form>
      <vab-query-form-top-panel>
        <el-form ref="form" :inline="true" label-width="0" :model="queryForm" @submit.native.prevent>
          <el-form-item>
            <el-input v-model="queryForm.sheetcode" clearable placeholder="底稿编号" />
          </el-form-item>
          <el-form-item>
            <el-input v-model="queryForm.sheetname" clearable placeholder="底稿名称" />
          </el-form-item>
          <el-form-item>
            <el-input v-model="queryForm.realname" clearable placeholder="拟稿人" :style="{ width: '256px' }" />
            <el-button :style="{ marginLeft: '10px' }" type="primary" @click="$refs.executor.show()">
              选择
            </el-button>
          </el-form-item>
          <el-form-item>
    ...
    <el-table v-loading="listLoading" :data="list" @row-click="singleElection"
      ref="multipleTable">
      <el-table-column align="center" width="55" label="">
        <template slot-scope="scope">
          <!-- 可以手动的修改label的值，从而控制选择哪一项 -->
          <el-radio class="radio" v-model="templateSelection" :label="scope.row.sheetId"
            >&nbsp;</el-radio
          >
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="底稿名称" prop="sheetName" show-overflow-tooltip />
      <!-- <el-table-column
        align="center"
        label="审计目标"
        prop="sheetTarget"
      ...
      /> -->
      <el-table-column align="center" label="被审计对象" prop="targetName" show-overflow-tooltip>
        <template>
          {{
      ...
      <el-table-column align="center" label="拟稿人" prop="realname" show-overflow-tooltip />
      <el-table-column align="center" label="审批人" prop="approver" show-overflow-tooltip>
        <template slot-scope="{ row }">
          {{ row.yjfh + ',' + row.ejfh }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="拟稿日期" prop="createTime" show-overflow-tooltip :formatter="formatDate" />
      <el-table-column align="center" label="状态" prop="state" show-overflow-tooltip>
        <template #default="{ row }">
          {{
            row.state == 1
            ? '审批中'
