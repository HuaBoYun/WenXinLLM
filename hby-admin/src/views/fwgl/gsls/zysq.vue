          ...
          <template #default="{ row }">
            ...
            <el-dropdown style="margin-left: 10px">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item
                  @click.native="handleApproval2(row)"
                  :disabled="row.state != 6 || !!row.status"
                >
                  注销审批
                </el-dropdown-item>
                <el-dropdown-item @click.native="handleReport(row)">
                  打印
                </el-dropdown-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'fwgl-gsls-zysq-search',
        tableKey: 'fwgl-gsls-zysq-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
      }
    ...
    methods: {
      ...
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(23, row.practiceApplyId)
      },
      //注销
      handleApproval2(row) {
        //提交审批
        this.$refs['process'].save(33, row.practiceApplyId)
      },
      // 定义表单所有项
      getFiled() {
        let fields = [
      ...
      async handleExport() {
        this.listLoading = true
        const res = await exportZYSQ(this.queryForm)
        downloadFile(res, '执业申请列表.xlsx')
        this.listLoading = false
      },
      /**
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg } = await deleteZYSQList({ id: row.practiceApplyId })
       ...
       */
      handleReport(row) {
        this.$refs['report'].printClick(row)
      },
    },
  }
</script>
