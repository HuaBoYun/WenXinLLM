  ...
  export default {
    ...
    methods: {
      ...
      async doHandler() {
        ...
        const res = await handExam(params)
        if (res) {
          this.$message({
      ...
      async handSave(item, callback) {
        ...
        const res = await fillAnswer(params)
        if (res) {
          // 必须选择一个值
          if (answers.length > 0) {
            // 加入已答列表
      ...
      // 试卷详情
      async fetchQuData(item) {
        // 获得详情
        this.cardItem = item

        // 查找下个详情
        const params = { paperId: this.paperId, quId: item.quId }
        const res = await quDetail(params)
        if (res && res.data) {
          ...
          this.quData.answerList.forEach((item) => {
            if (
              (this.quData.quType === 1 || this.quData.quType === 3) &&
              item.checked
            ) {
      ...
      async fetchData(id) {
        const params = { id: id }
        const res = await paperDetail(params)
        if (res && res.data) {
          // 试卷内容
