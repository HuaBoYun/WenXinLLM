...
<script>
  import { deleteNDJHList, exportNDJH, getNDJHList } from '@/api/fwgl/jhkh'
  ...
  export default {
    ...
    components: { ndjhView, filterTable, filterSearch, ProcessList },
    data() {
      return {
        list: [],
        listLoading: true,
        ...
        localKey: 'fwgl-jhkh-ndjh-search',
        tableKey: 'fwgl-jhkh-ndjh-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
      }
    ...
    methods: {
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(22, row.annualPlanId)
      },
      // 定义表单所有项
      getFiled() {
        let fields = [{ name: '计划名称', key: 'annualPlanName' }]
        return fields
      },
      /**
      ...
      async fetchData() {
        ...
        } = await getNDJHList(this.queryForm)
        this.list = tlist
      ...
      async handleDetail(row) {
        ...
        await this.$refs['ndjhView'].showEdit('detail', row)
      },
      /**
