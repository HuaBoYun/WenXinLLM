...
<script>
  ...
  export default {
    ...
    components: { zdshView, filterTable, filterSearch, ProcessList },
    data() {
      return {
        list: [],
        listLoading: true,
        ...
        localKey: 'fwgl-flsh-zdsh-search',
        tableKey: 'fwgl-flsh-zdsh-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
      }
    ...
    methods: {
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(18, row.institutionAuditId)
      },
      // 定义表单所有项
      getFiled() {
        let fields = [{ name: '名称', key: 'institutionName' }]
        return fields
      },
      /**
      ...
      async handleExport() {
        ...
        const exportParams = {
          // 可选参数：根据查询条件传递
          auditType: this.queryForm.auditType || 1,
       ...
       */
      handleAdd() {
        this.$refs['zdshView'].showEdit('add', null)
      },
      /**
      ...
      async handleEdit(row) {
        await this.$refs['zdshView'].showEdit('edit', {
          id: row.institutionAuditId,
        })
      },
      /**
