  ...
  >
    ...
    >
      <el-row>
        ...
        <el-col :span="12" v-if="form.sslx == '诉讼'">
        ...
        <el-col :span="12" v-if="form.sslx == '仲裁'">
...
<script>
  ...
  import ssOptions from './options/ss.vue'
  import {
    legalExecumgrDetail,
    legalExecumgrSave,
    getDetailInfo,
  ...
  export default {
    components: { xzjfOptions, glzcOptions, ssOptions, CandidateUserSelect },
    props: ['info'],
    data() {
      return {
        form: {
    ...
    methods: {
      async show(data, title, row) {
        if (row) {
          ...
          this.form.disputeid = res.data.disputeid
      ...
      handleSsjd(val) {
        this.$set(this.form, 'disputename', val.disputeitem)
