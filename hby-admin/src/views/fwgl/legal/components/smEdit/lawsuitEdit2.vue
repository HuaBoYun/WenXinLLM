  ...
  >
    <el-row :gutter="15">
      <el-form ref="form" label-width="150px" :model="form" :rules="rules">
        <el-col :span="12">
          <el-form-item label="案号" prop="proceedno">
            <el-input
    ...
    <template #footer v-if="!isReadonly">
      ...
      <el-button
        v-if="(form.state == 2 || form.state == 3) && jurisdictionCode == 1"
        @click="ymsubmit"
        type="primary"
      >
        提交
...
<script>
  ...
  export default {
    name: 'TemplateEdit',
    components: { ExecutorOptions, LowerEdit, CandidateUserSelect, Resubmit },
    data() {
      return {
        baseApi: baseURL,
