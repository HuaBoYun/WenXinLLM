<template>
  <div class="system-log-container">
    ...
    <TZGGview ref="edit" @fetchData="fetchData"></TZGGview>
    <el-pagination
      class="pagination"
      background
...
<script>
  import { getTZGGList, TZGGDelete } from '@/api/systemLog'
  import filterSearch from '@/components/filterSearch.vue'
  import filterTable from '@/components/filterTable.vue'
  import TZGGview from './components/tzggView.vue'

  export default {
    name: 'NormalReportList',
    components: { filterTable, filterSearch, TZGGview },
    data() {
      return {
        list: [],
        listLoading: true,
        ...
        localKey: 'fwgl-rcgl-tzgg-search',
        tableKey: 'fwgl-rcgl-tzgg-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
      }
    ...
    methods: {
      ...
      handleEdit(row) {
        ...
        if (row.createstaff != info.staffid) {
          this.$message.error('只能编辑自己创建的项')
          return
        }
      ...
      handleDelete(row) {
        const info = JSON.parse(localStorage.getItem('userInfo'))
        if (row.createstaff != info.staffid) {
          this.$message.error('只能删除自己创建的项')
          return
        }
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const res = await TZGGDelete({ noticeid: row.noticeid })
          if (res.code == 1) {
