<template>
  ...
  >
    <el-row :gutter="14" v-loading="loading">
      ...
      >
        <el-col :span="12">
          <el-form-item label="编号" prop="lingcode">
            <el-input
              v-model="formData.lingcode"
              disabled
              :style="{ width: '100%' }"
            />
          </el-form-item>
        ...
        <el-col :span="12">
          <el-form-item label="名称 " prop="lingdname">
            <el-input
              v-model="formData.lingdname"
              clearable
              placeholder="请输入名称"
              :style="{ width: '100%' }"
            />
          ...
          <el-table :data="tableData">
            ...
            >
              <template #default="{ row }">
                <div>
                  {{ row.fileSize ? (row.fileSize / 1000).toFixed(2) : '-' }}
                </div>
              </template>
            </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      async handleDelete(row) {
        const { fileId } = row
        const res = await deleteXXYDFile({ attid: fileId })
      ...
      async showEdit(row, title) {
        ...
        if (row) {
          const res = await getXXYDDefaultData({ lingid: row.lingid })

          this.formData.createname = res.data.data.createname
          this.formData.createstaff = res.data.data.createstaff
          this.formData.lingcode = res.data.data.lingcode
          this.formData.lingdname = res.data.data.lingdname
          this.formData.lingid = res.data.data.lingid
          this.formData.createtime = res.data.data.createtime

          const file = await getXXYDDefaultFileData({ lingid: row.lingid })
          this.tableData = file.data.data
        }
        ...
        if (title == 'edit') {
          ...
          this.title = '新增'
          //自动编码
          const name = await getXXYDCode()
          this.formData.lingcode = name.data.data
          const date = new Date()
      ...
      async save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            this.formData.attids = this.tableData.map((x) => x.fileId).join(',')
            const params = JSON.parse(JSON.stringify(this.formData))
            const res = await editXXYD(params)
            if (res && res.code == 1) {
              this.close()
              this.$emit('fetchData')
              this.$message({
