<template>
  <div class="system-log-container">
      ...
      <el-card shadow="never">
        <vab-query-form-top-panel>
          ...
          >
            ...
            >
              <el-input
                v-model="queryForm.suggestionNo"
                clearable
                placeholder="序号"
                v-if="item.name === '序号'"
                @input="handleInput1"
              ...
              <el-input
                v-model="queryForm.concerns"
                clearable
                v-if="item.name === '重点关注领域、项目、事项和风险'"
                placeholder="重点关注领域、项目、事项和风险"
              />
              <el-select
    ...
    />
    <xqjybEdit ref="edit" @fetchData="fetchData" />
...
<script>
  ...
  import xqjybEdit from './components/xqjybEdit'
  ...
  export default {
    ...
    data() {
      return {
        ...
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        localKey: 'oilAudit-jhlx-xqjyb-search',
        tableKey: 'oilAudit-jhlx-xqjyb-list',
        searchMore: true,
      }
    },
    created() {
    ...
    methods: {
      handleInput1(event) {
        ...
        if (!regex.test(event)) {
          ...
          this.queryForm.suggestionNo = this.queryForm.suggestionNo
            .replace(/[^0-9.]/g, '')
            .replace(/(\..*)\./g, '$1')
        }
      },
      // handleApproval(row) {
      //   //提交审批
      //   this.$refs['process'].save(100, row.id)
      // },
      ...
      getFiled() {
        return [
          { name: '序号', key: 'suggestionNo' },
          { name: '重点关注领域、项目、事项和风险', key: 'concerns' },
