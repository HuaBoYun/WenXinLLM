<template>
  ...
  <div class="system-log-container">
    ...
    />
    <cwzxpxbEdit ref="edit" @fetchData="fetchData" />
...
<script>
  ...
  import { cwzxpxbList, cwzxpxbDelete } from '@/oapi/audit/plan'
  import cwzxpxbEdit from './components/cwzxpxbEdit'
  ...
  export default {
    ...
    methods: {
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(118, row.id)
      },
      ...
      async fetchData() {
        ...
        const {
          ...
          msg,
        } = await cwzxpxbList(this.queryForm)
        if (msg === '成功') {
          this.list = tlist || []
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg, code } = await cwzxpxbDelete({ ids: row.id })
          if (code == 1) {
