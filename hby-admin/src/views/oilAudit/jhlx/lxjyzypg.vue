  ...
  <div class="system-log-container">
    ...
    <el-card shadow="never" class="secondCard">
      <vab-query-form-right-panel :span="24">
        ...
        <el-button type="success" @click.native="handleAdd()">新建</el-button>
        <!-- <el-upload
          class="upload-demo"
      ...
      >
        ...
        ></el-table-column>
        <el-table-column align="center" label="编号" prop="tbname" width="120">
    ...
    />
    <lxjyzypgBaseEdit ref="lxjyzypgBaseEdit" @fetchData="fetchData" />
...
<script>
  import {
    ...
    importFile,
    deleteItem,
  } from '@/api/oilAudit/jhgl/lxjyzypg'
  import filterSearch from '@/components/filterSearch.vue'
  import filterTable from '@/components/filterTable.vue'
  import { downloadFile } from '@/utils/otherUtils'
  import lxjyzypgBaseEdit from './components/lxjyzypgBaseEdit.vue'
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'oilAudit-jhlx-lxjyzypg-search',
        tableKey: 'oilAudit-jhlx-lxjyzypg-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
        btnLoading: false,
    ...
    methods: {
      ...
      async handleExport() {
        ...
        downloadFile(data, '立项建议专业评估.xlsx')
      },
      /**
       ...
       */
      handleAdd(type) {
        this.$refs['lxjyzypgBaseEdit'].showEdit('', 'add')
      },
      /**
      ...
      async handleDetail(row) {
        ...
        await this.$refs['lxjyzypgBaseEdit'].showEdit(row, 'detail')
      },
      /**
      ...
      async handleEdit(row) {
        // const data = await approachSummaryDisp({ enterid: row.enterid })
        await this.$refs['lxjyzypgBaseEdit'].showEdit(row, 'edit')
      },
      /**
       * @description: 删除列表数据
       * @param {*} row 当前行数据
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg } = await deleteItem({ tbid: row.tbid })
      ...
      handleApproval(row) {
        try {
          ...
          //提交审批
          this.$refs['process'].save(181, row.tbid)
        } catch (error) {
          this.btnLoading = false
        }
      },
      ...
      handleSelection(val, row) {
        const i = this.select.findIndex((x) => x.tbid == row.tbid)
        if (i < 0) {
      ...
      handleSelectAll(val) {
        ...
        if (curSelected && curSelected.length) {
          curSelected.map((row) => {
            if (row && !this.select.some((x) => x.tbid == row.tbid)) {
              this.select.push(row)
            }
          })
        } else {
