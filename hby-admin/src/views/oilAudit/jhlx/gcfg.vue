      ...
      >
        ...
        <el-col :span="24">
          ...
          >
            ...
            <el-button type="success" @click="push1(false)" v-if="anstatus == 0">督导人员</el-button>
          </div>
          <el-table
          ...
          >
            ...
            <el-table-column
              ...
              v-if="anstatus == 0"
            ></el-table-column>
            <el-table-column align="center" label="操作" width="120" v-if="anstatus == 0">
              <template #default="{ row }">
                <el-button type="text" @click="handleDivide(row)">分工</el-button>
              </template>
            </el-table-column>
          </el-table>
          <!-- <el-pagination
...
<script>
  ...
  import gcsjxmapbView1 from './components/gcsjxmapbView3.vue'
  import gcsjxmapbView2 from './components/gcsjxmapbView4.vue'
  export default {
    components: {
      cwsjxmapbEdit,
      SelectDepartment,
    ...
    methods: {
      ...
      async getChildlistPro1(val) {
        ...
        // 审理科人员
        if(this.isSlk) {
          const res = await gcfpslkry({ ids: arr1, fpslkryname: names1, fpslkryid: ids1 })
          if (res.msg === '成功') {
            this.$message({
              type: 'success',
              message: '下发成功!',
            ...
            const {
              data: { data },
            } = await planOutArrangeDetail({ tbid: this.editId })
            this.tableData = data.list.map((v) => {
              v.fpksrynames = v.fpksrynames || ''
              return v
            })
          ...
          const res = await gcfpksry({ ids: arr1, names: names1, ryids: ids1 })
          if (res.msg === '成功') {
            this.$message({
              type: 'success',
              message: '下发成功!',
