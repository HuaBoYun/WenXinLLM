<template>
  <div class="system-log-container">
    ...
    />
    <jhfirstEdit ref="edit" @fetchData="fetchData" />
...
<script>
  import { jhgljhchugList, jhgljhchugDelete } from '@/api/monitor/question'
  // import DraftInfo from "./components/DraftInfo";
  import jhfirstEdit from './components/jhfirstEdit'
    ...
    methods: {
      handleApproval(row) {
        try {
          ...
          this.$refs['process'].save(120, row.jhchugid)
        } catch (error) {
      ...
      async fetchData() {
        ...
        } = await jhgljhchugList(this.queryForm)
        this.list = tlist || []
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg, code } = await jhgljhchugDelete({
            jhchugid: row.jhchugid,
          })
          if (code == 1) {
