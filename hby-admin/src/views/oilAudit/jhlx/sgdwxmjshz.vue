...
<script>
  import { exportSgdwxmjshzList } from '@/oapi/audit/plan'
    ...
    methods: {
      ...
      async handleExport() {
        ...
        try {
          ...
          const {
            data: { listInto },
            code,
          } = await exportSgdwxmjshzList(exportParams)
          ...
          if (code === 1 && listInto && listInto.length > 0) {
            ...
            import('@/utils/excel').then((excel) => {
              // 定义多级表头
              const multiHeader = [
                ['建设单位', '合计', '', '≥900万元', '', '100~900万元', '', '50~100万元', '', '20~50万元', '', '<20万元', ''],
              ]
              const header = ['', '项目数', '金额', '项目数', '金额', '项目数', '金额', '项目数', '金额', '项目数', '金额', '项目数', '金额']
              ...
              // 生成文件名(包含年份信息)
              const fileName =
                (this.queryForm.queryYear || '') + '施工单位项目结算汇总'
      ...
      async fetchData() {
        ...
        const {
          ...
          code,
        } = await exportSgdwxmjshzList(this.queryForm)
        if (code === 1) {
          this.list = listInto || []
