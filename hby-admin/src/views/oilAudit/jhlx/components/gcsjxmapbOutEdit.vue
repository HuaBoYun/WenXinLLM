...
<script>
  ...
  import details1 from './gcsjxmapbView1.vue'
  import details2 from './gcsjxmapbView2.vue'
  ...
  export default {
    ...
    methods: {
      ...
      handleBack() {
        if (this.select.length > 0) {
          ...
          this.$baseConfirm('你确定要退回当前项吗', null, () => {
            enginAuditProjectXmzsbth({ ids: ids.join() }).then((res) => {
              if (res.code == 1) {
      ...
      async handleExport() {
        ...
        const data = await enginAuditProjectExport({
          ...this.queryForm,
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(123, this.editId)
      },
