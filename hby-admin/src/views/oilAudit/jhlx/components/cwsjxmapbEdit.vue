      ...
      >
        ...
        <el-col :span="12">
          <el-form-item label="审计范围" prop="auditRange">
            <el-input
              v-model="formData.auditRange"
              clearable
              placeholder="请输入审计范围（例:202403-202405）"
              :style="{ width: '100%' }"
            />
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item label="助审" prop="assistApprover">
            ...
            <el-button
              style="margin-left: 10px; height: 30px"
              type="primary"
              @click="handleAssistApproverClear"
            >
              清除
            </el-button>
          </el-form-item>
        </el-col>
        <el-col :span="12" style="height: 50px; clear: both">
...
<script>
  import {
    fundAuditProjectDetail,
    fundAuditProjectSaveOrUpdate,
  ...
  export default {
    ...
    methods: {
      ...
      async showEdit(row, title) {
        ...
        if (row) {
          const res = await fundAuditProjectDetail({ id: row.id })
          this.tableData = res.data.fundAuditProjectAttDtoList || []
      ...
      save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            let fundAuditProjectAttEntityList = []
            this.tableData.map((item) => {
              fundAuditProjectAttEntityList.push({
                attachmentid: item.attid,
              })
            })
            let params = { ...this.formData }
            delete params.fundAuditProjectAttEntityList
            if (this.qftype) {
              params.qftype = this.qftype
            }
            console.log(params)
            const data = await fundAuditProjectSaveOrUpdate({
              ...params,
              fundAuditProjectAttEntityList,
            })
            if (data.code == 1) {
      ...
      handleGroupLeaderClear() {
        this.formData.groupLeader = ''
        this.formData.groupLeaderId = ''
      },
      handleFzzNameClear() {
        this.formData.fzzName = ''
        this.formData.fzzStafffId = ''
      },
      handleApproverClear() {
        this.formData.approver = ''
        this.formData.approverId = ''
      },
      handleAssistApproverClear() {
        this.formData.assistApprover = ''
        this.formData.assistApproverId = ''
      },
