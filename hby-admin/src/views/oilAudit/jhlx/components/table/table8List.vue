<template>
  <el-dialog title="其他审计" :visible.sync="dialogVisible" width="50%" append-to-body :close-on-click-modal="false">
    <!-- <vab-query-form>
    ...
    <el-table :data="list" tooltip-effect="dark" style="width: 100%" :max-height="600" @select-all="handleSelectAll"
      @select="handleSelection" ref="multipleTable">
      <el-table-column type="selection" width="55"></el-table-column>
      <el-table-column align="center" label="序号" type="index" />
      <el-table-column align="center" label="审计项目名称" prop="auditItemName">
      ...
      <el-table-column prop="auditOrgNameStrs" label="被审计单位" show-overflow-tooltip></el-table-column>
      <el-table-column label="审计范围" width="120" prop="auditScope"></el-table-column>
      <el-table-column label="实施类型" width="120" prop="implType"></el-table-column>
      <el-table-column label="审计项目类型" width="120" prop="auditItemType"></el-table-column>
      <el-table-column prop="projectInitUnitName" label="立项单位" show-overflow-tooltip></el-table-column>
    </el-table>
    <!-- <el-pagination
      background
...
import { getListForChoose } from '@/api/oilAudit/jhgl/jhcg'
...
export default {
  ...
  methods: {
    ...
    async showEdit(id, chooseType, list) {
      this.id = id
      this.chooseType = chooseType
      this.table8List = list
      this.queryData()
    ...
    async fetchData() {
      ...
      const res = await getListForChoose({ ...this.queryForm, draftPlanId: this.id, chooseType: this.chooseType, auditIdStrs: this.table8List.map(item =>   item.auditId).join(",") })
