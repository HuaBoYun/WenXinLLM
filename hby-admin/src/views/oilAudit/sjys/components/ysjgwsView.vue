<template>
  ...
  >
    <el-row :gutter="20">
      ...
      >
        <el-col :span="12">
          <el-form-item label="编号" prop="wsnumber">
            <el-input
              v-model="formData.wsnumber"
              style="width: 100%"
              placeholder="请输入编号"
            ></el-input>
          </el-form-item>
        ...
        <el-col :span="12">
          <el-form-item label="标题" prop="wstitle">
            <el-input
              v-model="formData.wstitle"
              style="width: 100%"
              placeholder="请输入标题"
            ></el-input>
          </el-form-item>
        ...
        <el-col :span="24">
          <el-form-item label="处理结果" prop="proresult">
            <el-input
              v-model="formData.proresult"
              clearable
              placeholder="处理结果"
              type="textarea"
              :rows="6"
...
<script>
  ...
  export default {
    components: { projectManage, ProcessList },
    data() {
      return {
        loading: false,
        // baseApi: baseURL,
    ...
    methods: {
      ...
      async getData(row) {
        this.editId = row.id
        const { data } = await ysjgwsDetail({ id: row.id })
        Object.assign(this.formData, data.tblYqnsWgzzYsjgws)
      },
      async getFileData(row) {
        const { data } = await ysjgwsFileList({ id: row.id })
      ...
      save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            const { data, code, msg } = await ysjgwsSave({
              ...params,
              attIds,
            })
            if (code == 1) this.$message.success('保存成功')
            this.editId = data.tblYqnsWgzzYsjgws.id
            this.loading = false
          }
        })
      },
      ...
      handleExecutorSelected(val) {
        this.$set(this.formData, 'handstaffname', val[0].realname)
        this.$set(this.formData, 'handstaffid', val[0].staffid)
        this.$forceUpdate()
      },
      /**
       * @description: 删除列表数据
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(199, this.editId)
      },
