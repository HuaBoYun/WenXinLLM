  ...
  <div class="system-log-container">
        ...
        <vab-query-form-left-panel :span="24">
          ...
          >
            <el-form-item v-for="(item, index) in searchItem" :key="index">
              <el-input
                v-model="queryForm.project.projectName"
                clearable
                placeholder="审计项目名称"
    ...
    <el-card shadow="never" class="secondCard">
      ...
      <el-table v-loading="listLoading" :data="list">
        <div v-for="(item, index) in filedNow" :key="index + item">
          <el-table-column
            v-if="item.name === '审计项目名称'"
            align="center"
            label="审计项目名称"
...
<script>
  import Score from './components/jfkpScore.vue'
  import {
    getProjectEvalList,
    projectEvalScoreDelete,
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'oilAudit-zlpy-sjdg-search',
        tableKey: 'oilAudit-zlpy-sjdg-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
      }
    ...
    methods: {
      ...
      async fetchData() {
        ...
        const {
          ...
          code,
        } = await getProjectEvalList(this.queryForm)
        if (code === 1) {
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const res = await projectEvalScoreDelete({ ids: row.id })
          if (res.code === 1) {
