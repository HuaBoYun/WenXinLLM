<template>
  ...
  >
    ...
    <div slot="footer" v-if="!disabled">
      ...
      <el-button @click="save" type="primary" v-loading="loading">确定</el-button>
      <el-button
...
<script>
  ...
  export default {
    components: { rzsjmxView, SelectDepartment, typeView, ProcessList },
    data() {
      return {
        loading: false,
        baseApi: baseURL,
    ...
    methods: {
      async showEdit(row, title) {
        ...
        if (row) {
          ...
          const {
            data: { data },
          } = await rzsjmxBaseDetail({ tbid: row.tbid })
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(182, this.editId)
      },
    },
  }
</script>
