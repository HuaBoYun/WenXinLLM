        ...
        <el-col :span="24">
          <div style="text-align: right; margin-bottom: 5px">
            <el-button
              type="success"
              v-if="!formDisabled"
              @click="handleEditTable1(null, 'create')"
            >
              新增
            </el-button>
          </div>
          <el-table :data="tableData1">
            ...
            <el-table-column align="center" label="文号" prop="document">
              <template #default="{ row, $index }">
                <el-button
                  type="text"
                  :disabled="false"
                  @click="handleEditTable1(row, 'detail')"
                >
                  {{ row.document }}
                </el-button>
                <!-- <span v-else>
            ...
            >
              <template #default="{ row, $index }">
                <el-button
                  type="text"
                  @click="handleEditTable1(row, 'edit')"
                  :disabled="formDisabled"
                >
                  编辑
                </el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleInput2(field, value) {
        if(this.formData.sfys == 1) {
          // 允许数字和小数点
          const regex = /^\d*\.?\d*$/
          // 如果输入不匹配，就撤回到上一个合法的值
          if (!regex.test(value)) {
            // 这里假设你已经有一个变量 value 来绑定输入的值
            this.formData.tblYqnsSjzgYsnr[field] = this.formData.tblYqnsSjzgYsnr[field]
              .replace(/[^0-9.]/g, '')
              .replace(/(\..*)\./g, '$1')
          }
        }
      ...
      setTable1(row, type) {
        if (type == 'edit') {
          let index = this.tableData1.findIndex(item => item.cjrId === row.cjrId);
          if (index !== -1) {
            this.tableData1.splice(index, 1, { ...this.tableData1[index], ...row });
          }
        } else {
          ...
          if (valid) {
            ...
            const data = await editInfo({
              ...
              tblYqnsSjzgYsnr:
                this.formData.sfys == 1
                  ? {
                      ...this.formData.tblYqnsSjzgYsnr,
                      ysAttIds: attIds3,
                      ysAttList: this.tableDataWS,
                    }
                  : null,
              attIds,
              zgbgList,
