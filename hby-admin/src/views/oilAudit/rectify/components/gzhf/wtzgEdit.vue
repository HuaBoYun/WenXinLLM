<template>
  ...
  -->
        ...
        <el-col :span="12" style="height: 39px">
          <el-form-item
            label="当期整改状态"
            prop="dqzgzt"
            :rules="{
          ...
          >
            <el-select
              v-model="formData.dqzgzt"
              placeholder="请选择当期整改状态"
              :style="{ width: '100%' }"
              @change="$forceUpdate()"
            >
              <el-option label="无法整改" value="1" />
...
<script>
  ...
  import { getTotalMoney } from '@/oapi/yqns_sjzg/sjzgbg'
  import {
  ...
  export default {
    ...
    methods: {
      ...
      async showEdit(row, disabled) {
        ...
        const resZ = await getDetailInfo({
          issueId: row.tblIssueEntity ? row.tblIssueEntity.id : row.issuesId,
        })
        ...
        if (resZ.code == 1) {
          ...
          this.formData.assetLoss = resZ.data.baseData.assetLoss

          this.formData.projectId = resZ.data.baseData.projectId
          this.getgetTotalMoney(resZ.data.baseData.projectId)
          this.formData.projectName = resZ.data.baseData.projectName
        }
        if (row.wtzgid) {
          ...
          this.formData.hxspstatus = res.data.hxspstatus
          ...
          this.formData.iscopy = res.data.iscopy
          ...
          console.log('syys1', res.data.sfys)
          ...
          this.tableDataWS = res.data.sjqyAttList || []
        ...
        console.log('syys2', this.formData.sfys)
      },
      async getgetTotalMoney(projectId) {
