            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item>
                <el-button
                  ...
                  :disabled="row.status != 7 && !!row.hfspstatus"
                >
                  提交审批
                </el-button>
              </el-dropdown-item>
...
<script>
  ...
  import { withdraw, complete, assignment } from '@/oapi/yqns_sjzg/gzhf'
  ...
  export default {
    ...
    methods: {
      handleApproval(row) {
        this.$refs['process'].save(194, row.wtzgid)
      },
      ...
      handleFP(row) {
        this.row = row
        this.projectManagetype = 'thedeptstaffname'
      ...
      async getChildlistPro(val) {
        ...
        const res = await assignment({
          wtzgid: row.wtzgid,
          rectPerson: ids.toString(),
          rectPersonName: names.toString(),
        })
        if (res.code == 1) {
      ...
      async handleCH(row) {
        const res = await withdraw({ wtzgid: row.wtzgid })
        if (res.code == 1) {
          this.$message.success('撤回成功')
          this.fetchData()
        } else {
      ...
      async handleWC(row) {
        const res = await complete({ wtzgid: row.wtzgid })
        if (res.code == 1) {
