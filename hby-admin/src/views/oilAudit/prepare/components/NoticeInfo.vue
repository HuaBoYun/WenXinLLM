<template>
  ...
  >
      ...
      >
        ...
        <el-col :span="12">
          <el-form-item label="主审" label-width="140px" prop="mainreviewer">
            ...
            <el-button
              type="primary"
              @click="selectPeople('mainreviewer')"
              :disabled="!isBG"
            >
              选择
            </el-button>
    ...
    <danxuanPerson
      ...
      :multiple="false"
    />
    <sjtzspList ref="project" @selected="projectSelect" xctype="sjtzs" />
    <NoticeBiangengView ref="bg" />
  ...
  export default {
    ...
    methods: {
      ...
      handleExecutorSelected(node) {
        ...
        } else if (this.selectPeopleType == 'fznames') {
          this.$set(this.formData, 'fznames', node[0].realname)
          this.$set(this.formData, 'fzstaffids', node[0].staffid)
        }
      },
      getChildlistPro(val) {
        const ids = val.map((res) => res.staffid)
        const names = val.map((res) => res.realname)
        this.$set(this.formData, 'helpreviewer', names.toString())
        this.$set(this.formData, 'assistapproverid', ids.toString())
      },
      ...
      //   return isLt2M
      // },
      async showEdit(row, disabled, type) {
      ...
      save() {
        this.$refs['elForm'].validate(async (valid) => {
          if (valid) {
            ...
            let { data, code, msg } = await addNotice({
              ...this.formData,
            ...
            if (code == 1) {
              ...
              this.editId = data.Doubtfulpoint.adviceid
            } else {
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(179, this.editId)
      },
