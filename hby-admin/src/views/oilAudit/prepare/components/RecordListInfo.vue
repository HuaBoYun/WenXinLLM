...
<script>
  import { defCatAdd, getCheckDisp } from '@/oapi/audit/preparation'
    ...
    methods: {
      async showEdit(row, flag) {
        ...
        if (!row) {
          ...
          this.title = '详情'
          this.disabled = true
          let res = await getCheckDisp({
            programid: row.programid,
          })
          this.formData = res.data.entermeeting
        } else {
          this.title = '编辑'
          this.disabled = false
          let res = await getCheckDisp({
          ...
          this.programId = res.data.entermeeting.programId
          this.formData = res.data.entermeeting
        }
      },
      save() {
