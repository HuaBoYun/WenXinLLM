      ...
      <el-card shadow="never">
        <vab-query-form-top-panel>
          ...
          >
            ...
            >  
            <el-date-picker
            v-if="item.name == '项目年度起始'" 
              v-model="queryForm.preXmnd"
              type="year"
              format="yyyy" 
              value-format="yyyy"
              placeholder="项目年度起始">
            ...
            <el-date-picker
            v-if="item.name == '项目年度结束'" 
              v-model="queryForm.endXmnd"
              type="year"
                format="yyyy"
              value-format="yyyy"
              placeholder="项目年度结束">
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="listLoading" :data="list">
        ...
        <div v-for="(item, index) in filedNow" :key="index">
          <el-table-column
            v-if="item.name === '项目启动数量'"
            align="center"
            label="项目启动数量"
            prop="itemCount"
          >
          <template #default="{ row }"> 
            <el-button type="text" @click="handleitemCount(row)">{{ row.itemCount }} </el-button>
          </template>
        </el-table-column>
          <el-table-column
            v-if="item.name === '问题底稿'"
            align="center"
            label="问题底稿"
            prop="workSheetQuCount"
          >
          <template #default="{ row }"> 
            <el-button type="text" @click="handleworkSheetQuCount(row)">{{ row.workSheetQuCount }} </el-button>
          </template>
        </el-table-column>
          <el-table-column
            v-if="item.name === '复核底稿'"
            align="center"
            label="复核底稿"
            prop="workSheetFhCount"
          >
          <template #default="{ row }"> 
            <el-button type="text" @click="handleworkSheetFhCount(row)">{{ row.workSheetFhCount }} </el-button>
          </template>
        </el-table-column>
          <el-table-column
            v-if="item.name === '工作方案'"
          ...
          >
          <template #default="{ row }"> 
            <el-button type="text" @click="handleworkPlanCount(row)">{{ row.workPlanCount }} </el-button>
          </template>
        </el-table-column>
          <el-table-column
            v-if="item.name === '实施方案'"
          ...
          >
          <template #default="{ row }"> 
            <el-button type="text" @click="handleimplSuperCount(row)">{{ row.implSuperCount }} </el-button>
          </template>
        </el-table-column>
          <el-table-column
            v-if="item.name === '督导方案'"
          ...
          > 
          <template #default="{ row }"> 
            <el-button type="text" @click="handleaoverCount(row)">{{ row.aoverCount }} </el-button>
          </template>
        </el-table-column>
          <!-- <el-table-column
            v-if="item.name === '人员姓名'"
...
import DDModel from './components/DDModel'

export default {
  name: 'gcxmzjzjb',
  components: { filterSearch, filterTable, implementationAnalysisModel ,jhlistEdit,YearPicker,XMQDModel,DGModel,FAModel ,SSModel ,DDModel},
  ...
  data() {
    return {
      ...
      searchNow: [], //当前所有搜索项
      searchItem: [], //可见搜索项
      localKey: 'oilAudit-gcgl-overviewAuditPerson-search',
      tableKey: 'oilAudit-gcgl-overviewAuditPerson-list',
      searchMore: true,
      dateValue2:{
  ...
  methods: {
    ...
    handleaoverCount(row){
      this.$refs['DDModel'].showEdit(row)
    },
    handleworkSheetQuCount(row){
      this.$refs['DGModel'].showEdit(row,1, 2)
    },
    handleworkSheetFhCount(row){
      this.$refs['DGModel'].showEdit(row,0, 2)
    },
    handleitemCount(row){
    ...
    getFiled() {
      return [
        { name: '姓名', key: 'staffName' },  
        { name: '项目年度起始', key: 'preXmnd' }, 
        { name: '项目年度结束', key: 'endXmnd' }, 
      ]
    },
    resetQueryForm() {
    ...
    async fetchData() {
      if(this.queryForm.preXmnd){
        if(!this.queryForm.endXmnd){
          this.$message.error( '请选择项目年度结束')
          return 
        }
      }
      if(this.queryForm.endXmnd){
        if(!this.queryForm.preXmnd){
          this.$message.error( '请选择项目年度起始')
          return 
        }
