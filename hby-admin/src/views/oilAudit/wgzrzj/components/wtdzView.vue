<template>
  ...
  >
    ...
    <wthcList ref="list" @submit="selectList" />
...
<script>
  ...
  import wthcList from '@/views/oilAudit/wgzrzj/components/wthcList.vue'
  import ProcessList from '@/views/contract/contractManage/components/ProcessList'
  import { customUpload } from '@/utils/Uploader'
  import { handleDown } from '@/utils/fileHandler'
  export default {
    components: { SelectDepartment, UEditor, wthcList, ProcessList },
    data() {
      return {
        loading: false,
        baseApi: baseURL,
    ...
    methods: {
      ...
      async getData(row) {
        this.editId = row.id
        const { data } = await wtdzDetail({ id: row.id })
        Object.assign(this.formData, data.tblYqnsWgzzWtdz)
      },
      async getFileData(row) {
        const { data } = await wtdzFileList({ id: row.id })
      ...
      save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            const { data, code, msg } = await wtdzSave({
              ...params,
              attIds,
            })
            if (code == 1) this.$message.success('保存成功')
            this.editId = data.tblYqnsWgzzWtdz.id
            this.formData.id = data.tblYqnsWgzzWtdz.id
            this.loading = false
            this.$emit('fetchData')
          }
        })
      ...
      selectList(row) {
        this.formData.dznumber = row[0].hcnumber
        this.formData.hcid = row[0].id
      },
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(201, this.editId)
      },
