  ...
  >
    <el-row :gutter="20">
      ...
      >
        ...
        <el-col :span="12">
          <el-form-item label="名称" prop="slname">
            <el-input
              v-model="formData.slname"
              style="width: 100%"
              placeholder="请输入名称"
            ></el-input>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getData(row) {
        this.editId = row.id
        const { data } = await wtslDetail({ id: row.id })
        Object.assign(this.formData, data.tblYqnsWgzzWtsl)
      },
      async getFileData(row) {
        const { data } = await wtslFileList({ id: row.id })
      ...
      save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            const { data, code, msg } = await wtslSave({
              ...params,
              attIds,
            })
            if (code == 1) this.$message.success('保存成功')
            this.editId = data.tblYqnsWgzzWtsl.id
            this.formData.id = data.tblYqnsWgzzWtsl.id
            this.loading = false
            this.$emit('fetchData')
          }
        })
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          //删除对应的id
          let res = await wtslFileDelete({ attId: row.attid })
          if (res.msg === '成功') {
            this.$message({
              type: 'success',
              message: '删除成功!',
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(202, this.editId)
      },
