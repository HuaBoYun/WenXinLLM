<template>
  ...
  >
    ...
    <wtdzList ref="list" @submit="selectList" />
...
<script>
  ...
  import wtdzList from '@/views/oilAudit/wgzrzj/components/wtdzList.vue'
  import ProcessList from '@/views/contract/contractManage/components/ProcessList'
  import { customUpload } from '@/utils/Uploader'
  import { handleDown } from '@/utils/fileHandler'
  export default {
    components: { wtdzList, ProcessList },
    ...
    methods: {
      ...
      async getData(row) {
        this.editId = row.id
        const { data } = await cljgDetail({ id: row.id })
        Object.assign(this.formData, data.tblYqnsWgzzCljg)
      },
      async getFileData(row) {
        const { data } = await cljgFileList({ id: row.id })
      ...
      save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            delete params.createstaffname
            const { data, code, msg } = await cljgSave({
              ...params,
              attIds,
            })
            if (code == 1) this.$message.success('保存成功')
            this.editId = data.tblYqnsWgzzCljg.id
            this.formData.id = data.tblYqnsWgzzCljg.id
             this.$emit('fetchData')
          }
        })
      },
      ...
      selectList(row) {
        this.formData.clnumber = row[0].dznumber
        this.formData.dzid = row[0].id
      },
      selectedCompany(node) {
        this.formData.editorgname = node.name
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          //删除对应的id
          let res = await cljgFileDelete({ attId: row.attid })
          if (res.msg === '成功') {
            this.$message({
              type: 'success',
              message: '删除成功!',
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(203, this.editId)
      },
