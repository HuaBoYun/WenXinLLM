      ...
      <el-table v-loading="listLoading" :data="list">
        <div v-for="(item, index) in filedNow" :key="index">
          ...
          <el-table-column
            ...
            label="科长审核意见"
            prop="sectionChief"
            v-if="item.name === '科长审核意见'"
          />
          <el-table-column
            align="center"
            label="副主任审核意见"
            prop="deputyDirector"
            v-if="item.name === '副主任审核意见'"
          />
          <el-table-column
            v-if="item.name === '状态'"
            align="center"
...
<script>
  ...
  import { slbgList, slbgDelete, reportExport } from '@/oapi/audit/report'
  import ipViews from './components/slbgView.vue'
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'oilAudit-report-slbg-search',
        tableKey: 'oilAudit-report-slbg-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
      }
    ...
    methods: {
      handleApproval(row) {
        this.$refs['process'].save(177, row.slbgid)
      },
      ...
      async fetchData() {
        ...
        } = await slbgList({ ...this.queryForm, projectId: this.projectId })
        this.list = tlist || []
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg, code } = await slbgDelete({
            ids: row.slbgid,
          })
          if (code == 1) {
