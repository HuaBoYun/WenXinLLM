    ...
    <el-row :gutter="14" v-loading="loading">
      ...
      >
        ...
        <el-col :span="24">
          ...
          >
            <el-input
              v-model="formData.deputyDirector"
              type="textarea"
              :rows="6"
            ></el-input>
          </el-form-item>
        ...
        <el-col :span="24">
          <div style="text-align: right; margin-bottom: 5px" v-if="footer">
            <!-- <el-upload
              ...
              :on-preview="handlePreview"
              :on-success="
                (response, file) => handleSuccess(response, file, '2')
              "
          ...
          <el-table :data="tableData1" class="mb30">
            <el-table-column align="center" label="附件名称" prop="attname" />
            <el-table-column
              align="center"
              label="文件大小(KB)"
        ...
        <el-col :span="24">
          <div style="text-align: right; margin-bottom: 5px" v-if="footer">
            <!-- <el-upload
              ...
              :on-preview="handlePreview"
              :on-success="
                (response, file) => handleSuccess(response, file, '3')
              "
            ...
            <el-upload
              ...
              :headers="headers"
              :on-preview="handlePreview"
              :on-success="handleSuccessFile3"
              :file-list="fileList3"
              :before-upload="handleBeforeUploadFile3"
              :multiple="true"
            >
              <div v-if="!disabled" style="margin-right: 10px">
                <el-button type="success">点击上传</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      async showEdit(row, title) {
        ...
        if (row) {
          this.editId = row.slbgid
          const res = await slbgDetail({ slbgid: row.slbgid })
          res.data.attachments.map((v) => {
            if (v.type == 1) {
      ...
      async save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            let attId_1 = this.tableData
              .map((item) => {
                return item.attid
              })
              .join(',')

            let attId_2 = this.tableData1
              .map((item) => {
                return item.attid
              })
              .join(',')
            let attId_3 = this.tableData2
              .map((item) => {
                return item.attid
              })
              .join(',')
            ...
            const data = await slbgSaveOrUpdate({
              ...params,
              attList,
            })
            if (data.code == 1) {
              this.$baseMessage('保存成功', 'success')
              this.editId = data.data.slbgid
              this.formData.slbgid = data.data.slbgid
              this.$emit('fetchData')
            } else {
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(177, this.editId)
      },
      ...
      handleSuccessFile2(file) {
        if (file.code == 200) {
          ...
          this.tableData1 = [...this.tableData1, ...file.data]
          // this.tableData = list
          this.$baseMessage(file.msg, 'success')
        } else {
          this.$baseMessage(file.msg, 'error')
      ...
      customUploadWrapper3(options) {
        ...
        new Promise((resolve, reject) => {
          customUpload({
            ...
            onProgress: this.handleProgress,
            onSuccess: (response) => {
              this.handleSuccessFile3(response)
              resolve(response) // 成功时调用 resolve
            },
            onError: (error) => {
              // this.handleError(error)
      ...
      handleSuccessFile3(file) {
        if (file.code == 200) {
          this.fileList3 = [...this.fileList3, ...file.data]
          this.tableData2 = [...this.tableData2, ...file.data]
          // this.tableData = list
          this.$baseMessage(file.msg, 'success')
        } else {
          this.$baseMessage(file.msg, 'error')
      ...
      handleBeforeUploadFile3(file, fileList) {
        const isLt2M = file.size / 1024 / 1024 < 100 // 检查文件大小是否小于100MB
        if (!isLt2M) {
          this.$message.error('文件大小不能超过 100MB!')
          return false // 返回false停止上传
        ...
        // 如果文件大小合适，则调用自定义上传逻辑
        this.customUploadWrapper3({ file })
        return false // 停止默认上传行为
      },
      //下载公共方法调用
