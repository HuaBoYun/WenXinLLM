      ...
      <vab-query-form-right-panel :span="24">
        ...
        <el-dropdown style="margin-right: 10px">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item @click.native="handleAdd('三类')">
              三类
            </el-dropdown-item>
            <el-dropdown-item @click.native="handleAdd('四类')">
              四类
            </el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
...
<script>
  ...
  import {
    ejdwlrsjList,
    jsxmtzwcqkhzHzList,
    jsxmtzwcqkCheckMoney,
    jsxmtzwcqkhzHzDelete,
    flVerify,
  } from '@/oapi/audit/plan'
  import jsxmtzwcqkEdit from './components/jsxmtzwcqkView'
  ...
  export default {
    ...
    created() {
      ...
      this.initTable() // 表格显示项
      this.searchNow = this.getFiled()
      this.searchItem = this.searchNow.slice(0, 4)
      this.initSearch()
      // this.getjsxmtzwcqkhzHzList()
    },
    methods: {
      getFiled() {
      ...
      async fetchData() {
        ...
        } = await jsxmtzwcqkhzHzList(this.queryForm)
        this.list = tlist || []
      ...
      async handleAdd(fl) {
        ...
        this.$refs['edit'].showEdit(null, 'add', fl)
      },
      ...
      async handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          this.listLoading = true
          const res = await jsxmtzwcqkhzHzDelete({
            hzid: row.hzid,
          })
          this.listLoading = false
          if (res && res.code === 1) {
      ...
      async handleSubmit(row) {
        try {
          this.$baseConfirm('你确定要提交审批当前项吗', null, async () => {
            ...
            this.$refs['process'].save(111, row.hzid)
          })
        } catch (error) {
          this.btnLoading = false
        }
      ...
      handleCompanyTreeSelected(val) {
        console.log('🚀 ~ handleCompanyTreeSelected ~ val:', val)
        // 选公司后处理
        this.queryForm.tbdwName = val.name
        this.queryForm.tbdwId = val.id
        // this.$set(this.formData, 'tbdwName', val.name)
        // this.$set(this.formData, 'tbdwId', val.id)
      },
    },
  }
</script>
