<template>
  <div class="system-log-container">
    ...
    <el-card shadow="never">
      <vab-query-form-right-panel :span="24" v-if="!isInner">
        <el-tooltip
          class="item"
          effect="dark"
          content="表格筛选"
      ...
      <!-- <el-table
        ...
        @select-all="
          () => {
            multiple ? null : $refs.multipleTable.clearSelection()
          }
        "
        @select="handleSelectionChange"
        @sort-change="sortChange"
      >
        <el-table-column v-if="isInner" type="selection" width="55" /> -->
      <el-table
        v-loading="listLoading"
        :data="list"
        ref="multipleTable"
    ...
    />
    <gcxmzjEdit ref="edit" @fetch-data="fetchData" />
  </div>
</template>

<script>
  import gcxmzjEdit from './components/gcxmzjEdit.vue'
  import { formatDate } from '@/utils/index'
  import filterSearch from '@/components/filterSearch'
  import filterTable from '@/components/filterTable'
  import { searchTableMixis } from '@/mixis/index'
  import { getList, deleteItem, exportList } from '@/api/oilAudit/jhgl/gcxmzj'
  import { baseURL } from '@/config'
  import store from '@/store'
  const token = store.getters['user/token']
  export default {
    name: 'gcxmzj',
    components: { filterSearch, filterTable, gcxmzjEdit },
    mixins: [searchTableMixis],
    props: [
      'isInner', // 是否是其他模块选择框里面的
      'multiple', // 是否多选
    ],
    data() {
      return {
        baseApi: baseURL,
        api: '/oiaudit/gcxmzj/importData',
        headers: { token },
        select: [],
        list: [],
        listLoading: false,
    ...
    methods: {
      async handleExport() {
        const ids = this.select.map((res) => res.gcxmzjid)
        const data = await exportList({...this.queryForm, ids: ids.join()})
        let fileName = '工程项目造价表'
      ...
      async handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg, code } = await deleteItem({ ids: row.gcxmzjid })
          if (code == 1) {
