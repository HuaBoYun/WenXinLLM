<template>
  <div>
    ...
    >
      <el-row :gutter="14">
        <el-col :span="24" style="text-align: right">
          <el-button type="text" @click="openSJDX">审计查证事实</el-button>
          <el-button type="text" @click="openFGZD">审计依据</el-button>
          <el-button type="text" @click="openFXWTDX">发现问题定性</el-button>
        </el-col>
        <el-form
          ref="ruleForm"
          label-width="200px"
      ...
      <project-manage
        ...
        :multiple="false"
      ></project-manage>
      <SJGZJLview ref="edit"></SJGZJLview>
    ...
    <el-dialog
      ...
      append-to-body
    >
      <AuditQuestion v-if="auditQuestionVisible" />
    </el-dialog>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      async showModal(row, isFromMyTask = false, disabled, typeRow) {
        ...
        this.formData.thedeptstaffname = currentProject.projectOrderName
        this.formData.thedeptstaffid = currentProject.projectOrderId
        this.formData.projectName = currentProject.projectName
        this.formData.projectid = currentProject.id
        this.formData.auditeeName = currentProject.auditOrgName
        this.formData.auditeeNameId = currentProject.auditOrgId
        // console.log("row",row)
        if (isFromMyTask) {
          //从我的任务页面打开
      ...
      async handleSubmit() {
        this.$refs.ruleForm.validate((valid) => {
          if (valid) {
            ...
            const reportIds = this.workList.filter((item) =>
              this.workList2.every(
                (subItem) => subItem.workReprotId !== item.workReprotId
              )
            )
      ...
      handleDeleteWork(row) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          //删除对应的id
          let res = await delWorkRecordRela({
            workReprotId: row.id,
            myDraftId: this.formData.id,
          })
          // if (res.msg === '成功') {
          this.$message({
            type: 'success',
      ...
      // },
      openSJDX() {
        if (!this.formData.auditeeName) {
          this.$baseMessage('请选择被审计单位', 'error')
          return
        }
        this.$refs['sjdx'].showEdit({
          companyName: this.formData.auditeeName,
          guide: 'sjdxzy',
        })
      },
      openFGZD() {
        this.$refs['fgzd'].showEdit({ guide: 'flzdzy' })
      },
      openFXWTDX() {
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(150, this.editId)
      },
