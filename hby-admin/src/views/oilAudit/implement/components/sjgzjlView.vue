...
<script>
  ...
  export default {
    components: { DepartmentOptions, ProcessList },
    inheritAttrs: false,
    props: ['fetchData'],
    data() {
      return {
        loading: false,
        baseApi: baseURL,
        saveApi: '/oiaudit/audit/workRecords/saveOrUpdate',
        headers: {
    ...
    methods: {
      ...
      async showEdit(row, title) {
        ...
        if (row) {
          this.editId = row.id
          const res = await workRecordsDetail({ id: row.id })
          Object.assign(this.formData, res.data)
          this.formData.createUser = res.data.realname
          this.tableData = res.data.tblNoteAtts
        }
      ...
      async save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            this.loading = true
            let attids = this.tableData.map((x) => x.attid)
            attids = attids.join(',')
            ...
            formData.append('attids', attids)
            const data = await workRecordsSaveOrUpdate(formData)
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(153, this.editId)
      },
