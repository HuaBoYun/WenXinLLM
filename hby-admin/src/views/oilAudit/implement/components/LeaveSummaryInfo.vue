    ...
    <el-row :gutter="14">
      ...
      >
        <el-col :span="12">
          ...
          >
            <el-input
              v-model="formData.leavecoed"
              clearable
              placeholder="请输入离场纪要编号"
              :style="{ width: '100%' }"
              :disabled="!footer"
            />
          </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      showEdit(title, row) {
        ...
        if (row) {
          this.formData = row.leavemeeting
          this.getFileList(row.leavemeeting.leaveid)
        }
        if (title == 'edit') {
          this.title = '编辑'
      ...
      async getFileList(leaveid) {
        const data = await leaveSummaryFileList({ leaveid })
      ...
      add() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            const { creatrtime, ...other } = this.formData
            const data = await leaveSummarySave({
              ...other,
              attids,
            })
            if (data.code == 1) {
