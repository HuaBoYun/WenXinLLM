<template>
  ...
  >
    <el-row :gutter="14">
      ...
      >
        <el-col :span="12">
          <el-form-item label="编号" prop="sxnumber">
            <el-input
...
<script>
  ...
  import newsjddjlViewDetails from './newsjddjlViewDetails.vue'
  ...
  export default {
    ...
    methods: {
      toProjectDetails() {
        ...
        if (this.currentProject.isgc == '1') {
          this.$refs['sc'].showEdit(this.currentProject)
        } else {
          this.$refs['guideDetails'].showEdit(this.currentProject)
        }
      },
      // 获取当前实施项目
      ...
      projectManager(type) {
        if (type == 'supervisionParticipants') {
          this.projectManageStatus = true
        } else {
          this.projectManageStatus = false
        }
        this.proType = type
        this.$refs['manage'].showEdit()
      },
      getChildlistPro(val) {
        if (this.proType == 'supervisionParticipants') {
          this.$set(
            this.formData,
            'supervisionParticipants',
            val.map((item) => item.realname).join(',')
          )
        } else if (this.proType == 'staffName') {
          this.$set(this.formData, 'auditStaffId', val[0].staffid)
          this.$set(this.formData, 'staffName', val[0].realname)
        } else if (this.proType == 'economicName') {
          this.$set(this.formData, 'economicId', val[0].staffid)
      ...
      async showEdit(row, title) {
        ...
        if (row) {
          this.editId = row.id
          const { data } = await overseeRecordsDetail({ id: row.id })
        ...
        if (title == 'edit') {
          ...
          this.formData.projectId = this.currentProject.id
          this.formData.projectName = this.currentProject.projectName
          this.formData.chiefAuditor = this.currentProject.zsname
          this.formData.zsstaffid = this.currentProject.zsstaffid
          this.formData.assessors =
            this.currentProject.teams[0].teamLeader.realname
        }
      },
      close() {
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(154, this.editId)
      },
