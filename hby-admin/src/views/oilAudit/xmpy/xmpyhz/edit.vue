      ...
      >
        ...
        <el-col :span="24">
          <div
            ...
            v-if="disabled && formData.selectionTeam && isCreator"
          >
  ...
  export default {
    ...
    methods: {
      ...
      async showEdit(title, row, type) {
        ...
        if (row) {
          this.editId = row.id
          const res = await getXmpyhzDetail({ id: row.id })
      ...
      handleRelate(val) {
        ...
        const arr1 = JSON.parse(JSON.stringify(this.tableData1))
        this.tableData1 = arr1.concat(info)
      },
      deleteTable1(row) {
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(186, this.editId)
      },
