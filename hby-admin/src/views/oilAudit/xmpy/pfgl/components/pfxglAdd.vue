...
<script>
  import { saveOrUpdate, getDetail } from '@/oapi/fwgl/pfgl/pfxgl'
  import { getList as getFirstList } from '@/oapi/fwgl/pfgl/pfzdgl'
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          examineType: [
            { required: true, message: '请选择评分表类型！', trigger: 'change' },
          ],
          firstId: [
            { required: true, message: '请选择归属重点！', trigger: 'change' },
          ],
          totalScore: [
            { required: true, message: '请输入分值！', trigger: 'blur' },
          ],
          secondContent: [
            { required: true, message: '请输入评分项内容！', trigger: 'blur' },
          ],
        },
      }
    },
    methods: {
      ...
      async refreshFirstList(preserveFirstId = false) {
        // 归属重点列表
        const keepFirstId = this.postForm.firstId
        if (!preserveFirstId) {
          this.postForm.firstId = ''
        }
        if (this.postForm.examineType && this.postForm.examineType !== 0) {
          await getFirstList({ examineType: this.postForm.examineType }).then(
            (res) => {
              if (res && res.data && res.data.tlist) {
                this.firstList = res.data.tlist
                if (preserveFirstId && keepFirstId !== '' && keepFirstId !== null && keepFirstId !== undefined) {
                  this.postForm.firstId = String(keepFirstId)
                }
      ...
      async fetchData(id) {
        ...
        if (res && res.data && res.data[0]) {
          ...
          this.$set(this, 'postForm', {
            examineType: d.examineType,
            firstId: String(d.firstId),
            totalScore: d.totalScore,
            secondContent: d.secondContent,
          })

          this.$set(this, 'sonList', res.data)

          // 详情带出类型后，再按类型刷新归属重点选项，并保留当前 firstId
          await this.refreshFirstList(true)
          this.$nextTick(() => {
            if (this.$refs.postForm) this.$refs.postForm.clearValidate('firstId')
          })
        }
      },
      submitForm() {
        this.$refs.postForm.validate((valid) => {
          ...
          this.sonList.forEach((x, i) => {
            p.push({
              ...x,
              ...this.postForm,
              firstId: Number(this.postForm.firstId),
              sonSort: i,
            })
          })
      ...
      async showModal(data) {
        if (data && data.transactionId) {
          await this.fetchData(data.transactionId)
        } else {
