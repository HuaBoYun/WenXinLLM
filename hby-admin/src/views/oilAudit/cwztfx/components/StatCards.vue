  ...
  <div>
    <div class="stat-cards-row">
      ...
      <div class="stat-card card-gauge narrow">
        ...
        <div class="gauge-label">年初计划执行率</div>
      </div>
      <!-- 4. 仪表盘2 -->
      <div class="stat-card card-gauge narrow">
        <div ref="gauge2" class="chart-box gauge-box"></div>
        <div class="gauge-label">问题整改率</div>
      </div>
      <!-- 5. 仪表盘3 -->
      <div class="stat-card card-gauge narrow">
        <div ref="gauge3" class="chart-box gauge-box"></div>
        <div class="gauge-label">问题金额整改率</div>
      </div>
      <!-- 6. 仪表盘4 -->
      <div class="stat-card card-gauge narrow">
    ...
    <div class="stat-cards-row">
      <div class="stat-card card-bar-chart wide">
        <div class="chart-title">项目完成情况</div>
        <div ref="barChart3" class="chart-box"></div>
      </div>

      <!-- 8. 柱状图2 -->
      <div class="stat-card card-bar-chart wide">
        <div class="chart-title">人员情况</div>
          ...
          chart.on('click', (params) => {
            if (params.componentType === 'series') {
              ...
              if (name === '已完成数量') {
                ...
                // 传入选中表象的索引作为参数
                const index = type.findIndex((item) => item.name === name)
                this.$refs.modal.show(type[index].index)
              }
            }
          })
        })
