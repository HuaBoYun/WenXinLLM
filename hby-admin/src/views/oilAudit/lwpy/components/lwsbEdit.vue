  ...
  export default {
    ...
    methods: {
      ...
      async showEdit(title, row) {
        ...
        if (row) {
          const res = await getLwsbData({ perid: row.perid })
          this.formData = res.data.data
          const resList = await getLwsbzbList({ perid: row.perid })
      ...
      add() {
        ...
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            const data = await saveLwsbData({
              ...params,
              attids: attIds.toString(),
            })
            if (data.code == 1) {
              this.formData.perid = data.data.perid
      ...
      handleDelete(row) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          //删除对应的id
          let res = await deleteLwsbatt({ attid: row.attid })
          if (res.msg === '成功') {
            this.$message({
              type: 'success',
              message: '删除成功!',
