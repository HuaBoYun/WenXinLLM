<template>
  <div class="system-log-container">
      ...
      <el-table v-loading="loading" :data="list" ref="multipleTable">
        ...
        <div v-for="(item, index) in filedNow" :key="item.name + '_' + index">
          <el-table-column
            label="关联审计方法"
            align="center"
            v-if="item.name === '关联审计方法'"
          >
            <template slot-scope="{ row }">
              {{
                methodMaintainOpts.find((el) => el.id == row.methodMaintainId)
                  ?.methodName
              }}
            </template>
          </el-table-column>
    ...
    <pfxglAdd
      ref="pfxglAdd"
      :methodMaintainOpts="methodMaintainOpts"
      @queryData="queryData"
    />
  </div>
</template>
...
<script>
  import {
    getList,
    handleDelete,
    getMethodMaintainOpts,
  } from '@/oapi/baseConfig/gcsjlx'
  import filterSearch from '@/components/filterSearch.vue'
  import filterTable from '@/components/filterTable.vue'
  import pfxglAdd from './components/gcsjlxView.vue'
  export default {
    name: 'pfxgl',
    components: { filterTable, filterSearch, pfxglAdd },
    data() {
      const fields = [{ name: '类型名称', key: 'typeName' }] // 定义表单所有项
      return {
        list: [],
        methodMaintainOpts: [],
        loading: true,
        ...
        localKey: 'oilAudit-pfgl-gcsjlx-search',
        tableKey: 'oilAudit-pfgl-gcsjlx-list',
    ...
    created() {
      ...
      this.initSearch()
      this.getMethodMaintainOpts()
    },
    methods: {
      async getMethodMaintainOpts() {
        const {
          data: { methodMaintainList },
        } = await getMethodMaintainOpts()
        this.methodMaintainOpts = methodMaintainList
      },
      // 动态表格开始
      initTable() {
        this.$nextTick(() => {
      ...
      async fetchData(type) {
        this.loading = true
        if (type && type === 'reset') {
          this.queryForm.typeName = undefined
