<template>
  <div class="system-log-container">
    ...
    <hysqView ref="edit" @fetchData="fetchData"></hysqView>
    <el-pagination
      class="pagination"
      background
...
<script>
  import hysqView from './components/hysqView.vue'
  import filterSearch from '@/components/filterSearch.vue'
  import filterTable from '@/components/filterTable.vue'
  import { getList, deleteInfo } from '@/oapi/ypns_zhgl/hysq.js'
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'oilAudit-zhgl-hysq-search',
        tableKey: 'oilAudit-zhgl-hysq-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
      }
    ...
    methods: {
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(131, row.id)
      },
      // 办理
      ...
      changeApplyPeriod(val) {
        if (val && val.length) {
          this.queryForm.conferenceTimeStartStart = val[0]
          this.queryForm.conferenceTimeEndEnd = val[1]
        }
      },
      // 定义表单所有项
      getFiled() {
