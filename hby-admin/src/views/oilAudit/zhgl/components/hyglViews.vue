<template>
  ...
  >
    ...
    <div slot="footer" v-if="footer">
      <el-button @click="handle" v-if="formData.id" type="primary">
        提交
      </el-button>
...
<script>
  ...
  export default {
    components: { ExecutorOptions, MultipExecutorOptions },
    inheritAttrs: false,
    props: ['fetchData'],
    data() {
      return {
    ...
    methods: {
      ...
      async handle() {
        const res = await editInfor({ flagSubmit: 1, id: this.formData.id })
        if (res && res.code == 200) {
          this.$message({
            message: '保存成功！',
            type: 'success',
