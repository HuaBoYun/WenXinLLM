<template>
  <div class="system-log-container">
    ...
    <wpsqdViews ref="edit" @fetchData="fetchData"></wpsqdViews>
    <el-pagination
      class="pagination"
      background
    ...
    <xjdViews ref="editXJD" @fetchData="fetchData"></xjdViews>
  </div>
</template>

<script>
  import wpsqdViews from './components/wpsqdViews.vue'
  import filterSearch from '@/components/filterSearch.vue'
  import filterTable from '@/components/filterTable.vue'
  import { deleteInfo, getList } from '@/oapi/ypns_zhgl/wpsqd'
  import WfqdDeal from '@/views/msg/components/options/WfqdDeal'
  import { changeStatus } from '@/oapi/ypns_zhgl/ryqjd'
  import ProcessList from '@/views/contract/contractManage/components/ProcessList'
  import { getFlowPkInfo } from '@/api/setting/system.js'
  import xjdViews from '@/views/oilAudit/zhgl/components/wpXJD.vue'
  export default {
    components: {
      wpsqdViews,
      filterTable,
    ...
    methods: {
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(136, row.id)
        changeStatus({ onDutyStatus: 4, userId: row.applyPeople })
      },
      // 办理
      ...
      xiaojia(row) {
        //1是请假单  2是外派任务
        this.$refs['editXJD'].showEdit(row, '2')
      },
    },
  }
</script>
