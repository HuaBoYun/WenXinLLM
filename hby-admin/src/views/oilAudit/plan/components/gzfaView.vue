  ...
  >
    <el-row :gutter="14" v-loading="loading">
      ...
      >
        ...
        <el-col :span="12">
          <el-form-item label="方案名称" prop="xmmc">
            <el-input
...
<script>
  ...
  import { xmglgzfaDetail, xmglgzfaSaveOrUpdate } from '@/oapi/audit/project'
  ...
  export default {
    components: { cwModal, gcModal, ProcessList },
    inheritAttrs: false,
    props: ['fetchData'],
    data() {
      return {
    ...
    methods: {
      handlePlanRelate() {
        if (this.formData.falx == '基建') {
          this.$refs.gcModal.showEdit()
        } else {
          this.$refs.cwModal.showEdit()
        }
        // this.$refs.xmqdModal.showEdit()
      },
      xmqdModalSelect(val) {
        console.log(val, 'val')
        this.$set(this.formData, `xmdqid`, val[0].xmdqid)
        this.$set(this.formData, `xmmc`, val[0].xmname)
      },
      cwModalSelect(val) {
        this.$set(this.formData, `xmdqid`, val[0].id)
        this.$set(this.formData, `xmmc`, val[0].name)
      },
      gcModalSelect(val) {
        this.$set(this.formData, `xmdqid`, val[0].id)
        this.$set(this.formData, `xmmc`, val[0].name)
      ...
      async showEdit(row, title, type) {
        this.formData.falx = type
        this.formData.zykstype = type
        this.dialogFormVisible = true
        if (row) {
          this.editId = row.gzfaid
          const res = await xmglgzfaDetail({ gzfaid: row.gzfaid })
      ...
      async save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            const res = await xmglgzfaSaveOrUpdate({
              ...params,
              attIds,
            })
            if (res.code == 1) {
              this.$baseMessage('保存成功', 'success')
              this.editId = res.data.gzfaid
            } else {
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(162, this.editId)
      },
