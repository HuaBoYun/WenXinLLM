  ...
  >
    <el-row :gutter="14" v-loading="loading">
      ...
      >
        ...
        <el-col :span="12">
          <el-form-item label="审计项目名称" prop="xmmc">
            <el-input
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      cwModalSelect(val, name) {
        this.$set(this.formData, `xmdqid`, val[0].id)
        this.$set(this.formData, `xmmc`, name? name : val[0].name)
        this.$set(this.formData, `jhmc`, val[0].planname)
      },
      async showEdit(row, title) {
        this.dialogFormVisible = true
        if (row) {
          this.editId = row.sqdcbgid
          const res = await xmglsqdcbgDetail({ sqdcbgid: row.sqdcbgid })
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(161, this.editId)
      },
