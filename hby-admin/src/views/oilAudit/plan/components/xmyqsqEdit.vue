  ...
  export default {
    ...
    methods: {
      ...
      async showEdit(row, title) {
        ...
        if (row) {
          this.editId = row.xmdqid
          const res = await getXmyqsqone({ xmdqid: row.xmdqid })
          const arr = await getXmyqsqattList({ xmdqid: row.xmdqid })
        ...
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            if (res.code == 1) {
              ...
              this.editId = res.data.data.xmdqid
            } else {
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(180, this.editId)
      },
