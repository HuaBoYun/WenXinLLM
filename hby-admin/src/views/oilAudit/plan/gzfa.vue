      ...
      <vab-query-form-right-panel :span="24">
        ...
        <el-dropdown style="margin-right: 10px">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item @click.native="handleAdd('基建')">
              基建
            </el-dropdown-item>
            <el-dropdown-item @click.native="handleAdd('专项')">
              专项
            </el-dropdown-item>
            <el-dropdown-item @click.native="handleAdd('经责')">
              经责
            </el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
...
<script>
  ...
  import Views from './components/gzfaView.vue'
    ...
    methods: {
      handleApproval(row) {
        try {
          ...
          this.$refs['process'].save(162, row.gzfaid)
        } catch (error) {
      ...
      handleSend(row) {
        this.select = [row]
        if (row.falx == '管理及专项审计') {
          this.getChildlistPro([{ staffid: row.xmqd.zsstaffid }])
        } else {
      ...
      handleAdd(type) {
        this.$refs['edit'].showEdit(null, 'add', type)
      },
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg, code } = await xmglgzfaDelete({ ids: row.gzfaid })
          if (code == 1) {
      ...
      handleSelection(val, row) {
        const i = this.select.findIndex((x) => x.gzfaid == row.gzfaid)
        if (i < 0) {
      ...
      handleSelectAll(val) {
        ...
        if (curSelected && curSelected.length) {
          curSelected.map((row) => {
            if (row && !this.select.some((x) => x.gzfaid == row.gzfaid)) {
              this.select.push(row)
            }
          })
        } else {
