        ...
        <div v-for="(item, index) in filedNow" :key="index">
          ...
          >
            <template #default="{ row }">
              <!-- {{ row.status == 1 ? '启动' : '未启动' }} -->
              {{
                row.status == 1 ? '启动' : row.status == 2 ? '终止' : '未启动'
              }}
            </template>
          </el-table-column>
          <!-- <el-table-column
...
<script>
  ...
  import Views from './components/xmqdEdit.vue'
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'oilAudit-plan-xmqd-search',
        tableKey: 'oilAudit-plan-xmqd-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
      }
    ...
    methods: {
      handleApproval(row) {
        this.$refs['process'].save(161, row.sqdcbgid)
      },
      ...
      async fetchData() {
        ...
        } = await xmglqdList(this.queryForm)
        this.list = tlist || []
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg, code } = await xmglqdDelete({ xmdqid: row.xmdqid })
          if (code == 1) {
      ...
      async handleqd(row) {
        const { msg, code } = await xmglqdqd({ xmdqid: row.xmdqid })
        if (code == 1) {
          this.$baseMessage(msg, 'success')
          this.fetchData()
        }
      ...
      async handleStop(row) {
        const { msg, code } = await xmglqdStop({ xmdqid: row.xmdqid })
        if (code == 1) {
          this.$baseMessage(msg, 'success')
          this.fetchData()
        }
