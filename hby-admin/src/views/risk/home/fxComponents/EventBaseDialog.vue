...
<script>
  ...
  import EventEdit from '@/views/risk/events/eventBase/components/EventEdit.vue'

  export default {
    name: 'EventBaseDialog',
    components: {
      EventEdit,
    },
    data() {
    ...
    methods: {
      show(params = {}) {
        ...
        if (params.type) {
          ...
          const typeName = params.type == '1' ? '一般事件' : '重大事件'
          this.title = `风险事件列表 - ${typeName}`
        }
        this.fetchData()
      },
      handleClose() {
      ...
      async fetchData() {
        ...
        try {
          const {
            data: { page },
          } = await getRiskListById(this.queryForm)
