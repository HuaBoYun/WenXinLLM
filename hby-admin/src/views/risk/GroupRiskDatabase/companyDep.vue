  ...
  export default {
    ...
    methods: {
      ...
      handleNodeClick(data) {
        console.log(data)
        this.nodeid = data.id || 'default-id' // Set default id if not present
      ...
      async fetchData(node, resolve) {
        ...
        getAllOrgInfoTree({
          ...
          .then((res) => {
            ...
            if (!node) {
              this.$emit('select', {
                id: tree[0].id || 'default-id', // Set default id if not present
                label: tree[0].name,
              })
              this.expandedKeys.push(tree[0].id || 'default-id') // Set default id if not present
      ...
      formatTree(tree) {
        const list = tree.map((i) => {
          return {
            id: i.id || 'default-id', // Set default id if not present
            label: i.name,
            isLeaf: !i.isParent,
