...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          // reorgName: [
          //   { required: true, message: '请输入相关部门', trigger: 'change' },
          // ],
          revisiontype: [
            {
              required: true,
    ...
    async created() {
      ...
      console.log('来自内控fromNK', this.fromNK)
      this.showMJ = couldMJ() && !this.fromNK // && 内控过来不需要密级
      if (this.showMJ) {
        const res = hasMJ('IdentifyCreation')
        // 容错：如果当前页面不是风险创建页面，levelKeys 中可能找不到，直接用 rightId=259
        const rightId = (res && res.length > 0) ? res[0].menuid : 259
        this.menuId = rightId
        const res2 = await getMJ({ rightId: rightId })
    ...
    methods: {
      ...
      async showEdit(row, type, disabled) {
        ...
        if (type == 'add') {
          ...
          await this.getRiskNo()
          this.$set(this.formData, 'unitname', userInfo.linkOrg.orgname)
          this.$set(this.formData, 'linkDeptName', userInfo.linkDetp.orgname)
          this.$set(this.formData, 'staffname', userInfo.realname)
          this.$set(this.formData, 'riskcreatedt', dayjs().format('YYYY-MM-DD'))
          this.$set(this.formData, 'riskcatid', row.value)

          // 设置风险领域
          this.$set(this.formData, 'riskcatname', row.riskcatname || '')

          // 设置二级风险（可能为空）
          this.$set(this.formData, 'riskcatnametwo', row.riskcatnametwo || '')

          // 设置父级ID
          this.$set(this.formData, 'riskcatidone', row.fatherriskcatid || 0)
          ...
          this.riskcatid = row.riskcatid
          await this.getData()
        }
      },
      async getRiskNo() {
        const { data, code } = await getRiskNo()
        console.log('🚀 ~ getRiskNo ~ data:', data)
        this.$set(this.formData, 'risknumber', data)
      },
      async getData() {
      ...
      save() {
        this.$refs['elForm'].validate(async (valid) => {
          ...
          if (valid) {
            ...
            try {
              const { reorgName, belongstoName, ...data } = this.formData
              let attids = ''
              ...
              const requestData = {
                ...
                param: { ...this.controlList[0] },
              }

              console.log('提交的requestData:', requestData)
              console.log('requestData.param:', requestData.param)
      ...
      async riskProcesChange(value) {
        let res = await getRiskBusiness({ processname: value ? value : '' })
      ...
      async riskBusinessChange(value) {
        ...
        // this.$nextTick(() => {
        //   // 清除指定表单项的验证状态
        //   this.$refs.form.clearValidate('bussinessname')
        // })
      ...
      handleSuccess(file) {
        ...
        // 格式2: { code: "500", message: "...", result: null }
        const code = Number(file.code)
        const data = file.data || file.result
        const msg = file.msg || file.message || '操作完成'
      ...
      async handleDowns(row) {
        try {
          // 构造解密下载地址
          const ext = row.attname
            ? row.attname.substring(row.attname.lastIndexOf('.'))
            : ''
          const downloadUrl =
            'https://www.wenxin.example.com/api/file/file/download/decrypt?fileId=' +
            row.attid +
            '&fullfilename=' +
            row.attid +
            ext
          // 通过 fetch + blob 方式下载，避免跨域 <a download> 不生效
          ...
          link.style.display = 'none'
          link.href = blobUrl
          link.setAttribute('download', row.attname || 'file')
      ...
      handlePreviewFile(row) {
        try {
          ...
          const downloadUrl =
            ...
            ext
          // base64编码后直接拼接在线预览地址
          const encodedUrl = window.btoa(downloadUrl)
          const previewUrl =
            'http://192.0.2.17:8012/onlinePreview?url=' + encodedUrl
          window.open(previewUrl, '_blank')
        } catch (error) {
          console.error('预览失败:', error)
          this.$baseMessage('预览失败', 'error')
        }
      },
      ...
      handleRiskModelSelected(val) {
        console.log('🚀 ~ handleRiskModelSelected ~ val:', val)
        // this.formData.associatedRiskModel = val
