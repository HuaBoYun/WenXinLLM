...
<script>
  import { taskModalInfoProcess, saveTask, submitTask } from '@/api/systemLog'
  import { formatDay } from '@/utils/index'
  import TaskEdit3 from './TaskEdit3.vue'
  ...
  export default {
    ...
    updated() {
      this.$nextTick(() => {
        ...
        this.list.map((item, index) => {
          // this.chageTextColor(item.frequency==0?1:item.frequency,'frequency'+item.index)
          // this.chageTextColor(item.severity==0?1:item.severity,'severity'+item.index)
          this.chageTextColor(
    ...
    methods: {
      ...
      async showEdit(row, type, level) {
        ...
        } = await taskModalInfoProcess(this.queryForm)
        records.map((v, index) => {
