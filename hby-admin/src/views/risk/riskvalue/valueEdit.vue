...
<script>
  ...
  export default {
    ...
    components: { CompanyTreeModel1, controlMeasures, ZXPerson },
    data() {
      return {
        loading: false,
        formData: {
      ...
      createData: {
        handler(newVal) {
          ...
          if (Object.keys(newVal).length) {
            let riskData = newVal.risk
            this.formData.unitDeptid = riskData.belongsto
            this.formData.unitDeptidName = riskData.zrbmName
            this.formData.cooperateOrgName = riskData.cooperateOrgName
            this.formData.levelFourRisk = riskData.levelFourRisk
            this.formData.threeRisk = riskData.riskname
            this.formData.twoRisk = riskData.riskcatnametwo
            this.formData.oneRisk = riskData.riskcatname
            this.formData.leadershipName = riskData.leadershipName
            this.formData.detailRisk = riskData.riskdes
            this.formData.analysisRisk = riskData.riskcause
            this.formData.analysisSol = newVal.copings?.yddes
            this.formData.secrectLevelId = riskData.secrectLevelId
            this.formData.secrectLevelName = riskData.secrectLevelName
            this.formData.staffScopeNames = riskData.staffScopeNames
            this.formData.staffScopeIds = riskData.staffScopeIds
          }
        },
        immediate: true,
      },
      ...
      curRow: {
        handler(newVal) {
          if (newVal && Object.keys(newVal).length) {
            // 从第一条数据获取的字段赋值
            if (newVal.analysisRel) {
              this.formData.analysisRel = newVal.analysisRel
            }
            if (newVal.analysisEve) {
              this.formData.analysisEve = newVal.analysisEve
            }
            if (newVal.assessStan) {
              this.formData.assessStan = newVal.assessStan
            }
            if (newVal.assessScoreOne) {
              this.formData.assessScoreOne = newVal.assessScoreOne
            }
            if (newVal.assessInf) {
              this.formData.assessInf = newVal.assessInf
            }
            if (newVal.assessScoreTwo) {
              this.formData.assessScoreTwo = newVal.assessScoreTwo
            }
            if (newVal.assessSco) {
              this.formData.assessSco = newVal.assessSco
            }
            if (newVal.assessGrade) {
              this.formData.assessGrade = newVal.assessGrade
            }
          }
        },
        immediate: true,
    ...
    methods: {
      async getMJData() {
        ...
        if (this.showMJ) {
          // 获取密级,菜单id
          const res = await hasMJ('RiskValuePage')
      ...
      async fetchData(row) {
        ...
        const res = await monthlyEvaluationDetails({ id: this.curRow.id })
        ...
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            let params = {
              ...
              implementId: this.curRow.implementId,
            }
