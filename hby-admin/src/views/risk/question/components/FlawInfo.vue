...
export default {
  ...
  data() {
    return {
      ...
      interiorList: [],
      baseApi: baseURL,
      api: '/riskcontrol/uploadFileAttInfo',
      headers: {
  ...
  methods: {
    ...
    async handleDown(row) {
      const data = await download({ id: row.attid })
    ...
    async handleDelete(row) {
      ...
      this.tableData = list
      const { msg } = await deleteFieldById({
        moduleType: 'QXGL',
        moduleId: this.formData.bugid,
        attId: row.attid,
      })
