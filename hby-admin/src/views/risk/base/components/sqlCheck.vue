        ...
        <div v-for="item in this.renderTitle" :key="item">
          ...
          >
            <template #default="{ row }">
              <div class="traffic-light-cell">
                <span
                  class="traffic-light-dot"
                  :class="{
                    'dot-red': String(row[colorKey] || '').includes('红'),
                    'dot-yellow': String(row[colorKey] || '').includes('黄'),
                    'dot-green': String(row[colorKey] || '').includes('绿'),
                  }"
                ></span>
              </div>
            </template>
  ...
  export default {
    ...
    methods: {
      ...
      async getExecutorList(info, id) {
        ...
        // 检测arr中是否存在红绿灯三个字
        if (arr.some((item) => item.includes('红绿灯'))) {
          this.showColor = true
