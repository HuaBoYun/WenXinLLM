<!-- 收入批量结账 -->
<template>
  <div class="system-log-container">
    <vab-query-form>
      <el-card shadow="never">
        <vab-query-form-left-panel :span="24">
          <el-form
            ref="form"
            :inline="true"
            label-width="0"
            :model="queryForm"
            @submit.native.prevent
          >
            <el-form-item v-for="(item, index) in searchItem" :key="index">
              <el-input
                v-model="queryForm.jsFinance"
                clearable
                placeholder="会计期间方案"
                v-if="item.name === '会计期间方案'"
              />
              <el-input
                v-model="queryForm.supplier"
                clearable
                placeholder="会计期间"
                v-if="item.name === '会计期间'"
              />
              <el-input
                v-model="queryForm.supplier"
                clearable
                placeholder="组织"
                v-if="item.name === '组织'"
              />
            </el-form-item>
            <el-form-item>
              <el-button
                icon="el-icon-search"
                native-type="submit"
                type="primary"
                @click="fetchData"
              >
                查询
              </el-button>
            </el-form-item>
            <el-form-item>
              <el-button
                native-type="submit"
                type="primary"
                @click="resetSearch"
              >
                重置
              </el-button>
            </el-form-item>
            <el-form-item>
              <el-tooltip
                class="item"
                effect="dark"
                content="搜索筛选"
                placement="top"
              >
                <el-popover placement="left" trigger="click">
                  <filter-search
                    v-if="true"
                    :list="searchAll"
                    :name="localKey"
                    @updateSearchShow="initSearch"
                  />
                  <el-button slot="reference" style="height: 32px">
                    <vab-icon icon="filter" :is-custom-svg="true" />
                  </el-button>
                </el-popover>
              </el-tooltip>
            </el-form-item>
            <el-form-item>
              <span
                :class="searchMore ? 'search-more is-opened' : 'search-more'"
                @click="showMore"
              >
                <span>{{ searchMore ? '收起' : '展开' }}</span>
                <i class="el-icon-arrow-down"></i>
              </span>
            </el-form-item>
          </el-form>
        </vab-query-form-left-panel>
      </el-card>
    </vab-query-form>

    <el-card shadow="never" class="secondCard">
      <vab-query-form-right-panel style="width: 100%">
        <el-tooltip
          class="item"
          effect="dark"
          content="表格筛选"
          placement="top"
        >
          <el-popover placement="right" trigger="click">
            <filter-table
              :list="filedAll"
              :name="tableKey"
              @updateTableShow="initTable"
            />
            <!-- <i class="el-icon-delete" slot="reference"></i> -->
            <el-button
              slot="reference"
              icon="el-icon-s-grid"
              class="biaoge"
              style="margin-bottom: 10px; margin-right: 10px"
            ></el-button>
          </el-popover>
        </el-tooltip>
        <!-- <el-button type="success" @click="handleEdit(false, false)">
          新建
        </el-button> -->
        <!-- <el-button type="primary">导出</el-button> -->
      </vab-query-form-right-panel>

      <el-table v-loading="listLoading" :data="list">
        <el-table-column
          align="center"
          label="序号"
          type="index"
          width="100"
        ></el-table-column>
        <div v-for="(item, index) in filedNow" :key="index">
          <el-table-column
            v-if="item.name === '组织编码'"
            align="center"
            label="组织编码"
            prop="orgCode"
            show-overflow-tooltip
          />
          <el-table-column
            v-if="item.name === '组织名称'"
            align="center"
            label="组织名称"
            prop="orgName"
            show-overflow-tooltip
          />
          <el-table-column
            v-if="item.name === '会计期间'"
            align="center"
            label="会计期间"
            prop="accountingPeriod"
            show-overflow-tooltip
          />
          <el-table-column
            v-if="item.name === '结账'"
            align="center"
            label="结账"
            prop="settlementStatusName"
            show-overflow-tooltip
          />
          <el-table-column
            v-if="item.name === '结账人'"
            align="center"
            label="结账人"
            prop="settlementUserName"
            show-overflow-tooltip
          ></el-table-column>
          <el-table-column
            v-if="item.name === '结账时间'"
            align="center"
            label="结账时间"
            prop="settlementTime"
            show-overflow-tooltip
          ></el-table-column>
          <el-table-column
            v-if="item.name === '取消结账人'"
            align="center"
            label="取消结账人"
            prop="cancelUserName"
            show-overflow-tooltip
          ></el-table-column>
          <el-table-column
            v-if="item.name === '取消结账时间'"
            align="center"
            label="取消结账时间"
            prop="cancelTime"
            show-overflow-tooltip
          ></el-table-column>
        </div>
        <el-table-column
          align="center"
          label="操作"
          show-overflow-tooltip
          width="120"
        >
          <template slot-scope="scope">
            <el-button
              type="text"
              @click="handleEdit(scope.row, false)"
              :disabled="!!scope.row.spzt"
            >
              修改
            </el-button>
            <el-dropdown style="margin-left: 10px">
              <el-button type="text">更多</el-button>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item>
                  <el-button
                    @click="handleDelete(scope.row)"
                    type="text"
                    :disabled="!!scope.row.spzt"
                  >
                    删除
                  </el-button>
                </el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
        </el-table-column>
      </el-table>
    </el-card>

    <el-pagination
      background
      :current-page="queryForm.pageNumber"
      :layout="layout"
      :page-size="queryForm.pageSize"
      :total="total"
      @current-change="handleCurrentChange"
      @size-change="handleSizeChange"
    />
    <Edit ref="edit" @fetch-data="fetchData"></Edit>
  </div>
</template>

<script>
  import {
    getRevenueBatchSettlementList,
    doRevenueBatchSettlement,
    cancelRevenueBatchSettlement,
  } from '@/api/financialSharing/revenueManagement'
  import Edit from './components/cbcjEdit'
  import filterSearch from '@/components/filterSearch'
  import filterTable from '@/components/filterTable'
  import { searchTableMixis } from '@/mixis/index'

  export default {
    name: 'srpljz',
    mixins: [searchTableMixis],
    components: {
      Edit,
      filterSearch,
      filterTable,
    },
    data() {
      return {
        list: [],
        planNum: '',
        listLoading: true,
        layout: 'total, sizes, prev, pager, next, jumper',
        total: 0,
        queryForm: {
          startStatus: undefined,
          sxName: undefined,
          pageNumber: 1,
          pageSize: 10,
          xmnd: '',
        },
        currProjectId: '',
        filedAll: [
          { name: '组织编码' },
          { name: '组织名称' },
          { name: '会计期间' },
          { name: '结账' },
          { name: '结账人' },
          { name: '结账时间' },
          { name: '取消结账人' },
          { name: '取消结账时间' },
        ], //所有表格项
        filedNow: [], //当前表格项
        searchAll: this.getFiled(), //所有搜索项
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        localKey: 'cwgl-srgl-srpljz-search',
        tableKey: 'cwgl-srgl-srpljz-list',
        searchMore: false,
        select: [],
      }
    },
    created() {
      this.fetchData()
      this.initTable()
      this.searchNow = this.getFiled()
      this.searchItem = this.searchNow.slice(0, 4)
      this.initSearch()
    },
    mounted() {
      this.$bus.on('updateMsg', (value) => {
        if (value == 0) {
          this.fetchData()
        }
      })
    },
    methods: {
      getFiled() {
        return [
          { name: '会计期间方案', key: 'sxName' },
          { name: '会计期间', key: 'startStatus1' },
          { name: '组织', key: 'startStatus' },
        ]
      },
      selectTeamList(val, flagTitle) {
        console.log(val, flagTitle)
        if (flagTitle) {
          this.queryForm.projectOrderName = val[0].realname
          this.queryForm.projectOrderId = val[0].staffid
        } else {
          let arrStr = ''
          let arr = []
          val.forEach((item) => {
            arr.push(item.realname)
          })
          arrStr = arr.join(',')
          this.tableData[this.sIndex].zyNames = arrStr
          //拿到组员id字符串
          let arrStrZy = ''
          let arrZy = []
          val.forEach((item) => {
            arrZy.push(item.staffid)
          })
          arrStrZy = arrZy.join(',')
          this.zyStaffids = arrStrZy
        }
      },
      showGroupLeader() {
        this.$refs['select'].showEdit('leader')
      },
      resetQueryForm() {
        this.queryForm = {
          projectOrderName: undefined,
          projectOrderId: undefined,
          pageNumber: 1,
          pageSize: 10,
          xmnd: '',
        }
      },
      resetSearch() {
        this.resetQueryForm()
        this.fetchData()
      },
      handleSizeChange(val) {
        this.queryForm.pageSize = val
        this.fetchData()
      },
      handleCurrentChange(val) {
        this.queryForm.pageNumber = val
        this.fetchData()
      },
      queryData() {
        this.queryForm.pageNumber = 1
        this.fetchData()
      },
      async fetchData() {
        this.listLoading = true
        try {
          let { ...other } = this.queryForm
          const res = await getRevenueBatchSettlementList({
            pageNum: other.pageNumber,
            pageSize: other.pageSize,
            periodScheme: other.jsFinance,
            accountingPeriod: other.supplier,
            orgName: other.organization,
          })
          this.listLoading = false
          if (res && res.code === 1 && res.data) {
            const { tlist, totalRecord } = res.data
            this.list = tlist || []
            this.total = totalRecord || 0
          } else {
            this.list = []
            this.total = 0
            if (res && res.msg) {
              this.$message.warning(res.msg)
            }
          }
        } catch (error) {
          this.listLoading = false
          this.list = []
          this.total = 0
          console.error('获取收入批量结账列表失败:', error)
        }
      },
      days(start, end) {
        let s = new Date(start)
        let e = new Date(end)
        let hours = (e - s) / (1000 * 60 * 60 * 24)
        return hours + '天'
      },

      handleEdit(row, disabled, type) {
        this.$refs['edit'].showEdit(row, disabled, this.planNum, type)
      },
      async handleDelete(row) {
        this.$baseConfirm('你确定要取消结账当前项吗', null, async () => {
          try {
            const res = await cancelRevenueBatchSettlement({
              settlementId: row.settlementId,
              userId: this.$store.getters.userId,
              userName: this.$store.getters.name || this.$store.getters.username
            })
            if (res && res.code === 1) {
              this.$baseMessage(res.msg || '取消结账成功', 'success')
              await this.fetchData()
            } else {
              this.$baseMessage(res.msg || '取消结账失败', 'error')
            }
          } catch (error) {
            console.error('取消结账失败:', error)
            this.$baseMessage('取消结账失败', 'error')
          }
        })
      },
      sendModel() {
        this.$refs['sendModel'].showEdit()
      },
      send() {
        this.$refs['send'].showEdit()
      },
      color(row) {
        if (row.id == this.currProjectId) {
          return { color: '#7fcf7c' }
        } else {
          return { color: '' }
        }
      },
    },
  }
</script>

<style scoped lang="scss">
  .system-log-container {
    padding: 0 !important;
    background: #f6f8f9 !important;
  }
  .margin-b0 {
    margin-bottom: 0;
  }
</style>
