      ...
      <el-table v-loading="listLoading" :data="list">
          ...
          >
            <template slot-scope="scope">
              <el-input
                v-if="scope.row.edit"
                size="small"
                v-model="scope.row.projectOrderName"
              />
              <div v-else>{{ scope.row.projectOrderName }}</div>
            </template>
          </el-table-column>
          <el-table-column
            v-if="item.name === '名称'"
        ...
        >
          <template slot-scope="scope">
            <el-popconfirm
              title="确认要删除吗？"
              @confirm="handleConfirm(scope.row, scope.$index)"
            >
  ...
  export default {
    ...
    data() {
      return {
        ...
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        localKey: 'cwgl-srgl-srqrzc-search',
        tableKey: 'cwgl-srgl-srqrzc-list',
        searchMore: false,
        select: [],
      }
    },
    ...
    methods: {
      handleConfirm(row, num) {
        this.list = this.list.filter((f, index) => index !== num)
      },
      editRow(row, num) {
        this.$set(this.list[num], 'edit', true)
      },
      getFiled() {
        return [
          { name: '编码或名称筛选', key: 'sxName' },
