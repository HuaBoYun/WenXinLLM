<template>
  <el-dialog title="控制维度" :visible.sync="dialogFormVisible" width="1000px">
    ...
    >
      ...
      <el-table-column label="可调剂">
        <template slot-scope="scope">
          <el-checkbox v-model="scope.row.system"></el-checkbox>
        </template>
      </el-table-column>
      ...
      save() {
        ...
        for (let i = 0; i < this.tableData.length; i++) {
          if (!this.tableData[i].name) {
            this.isValid = false
            this.$message.error(`第${i + 1}行的控制维度未填写`)
            break
          }
        }
        if (this.isValid) {
