  ...
  <div class="system-log-container">
          ...
          >
            <el-form-item v-for="(item, index) in searchItem" :key="index">
              ...
              <el-date-picker
                ...
                unlink-panels
                range-separator="至"
                start-placeholder="报销单单据日期开始日期"
                end-placeholder="报销单单据日期结束日期"
                v-if="item.name === '报销单单据日期'"
              ></el-date-picker>
              ...
              <el-input
                ...
                clearable
                placeholder="起摊会计期间"
                v-if="item.name === '起摊会计期间'"
              />
              <el-input
                v-model="queryForm.supplier"
                clearable
                placeholder="结束摊销会计期间"
                v-if="item.name === '结束摊销会计期间'"
              />
            </el-form-item>
            <el-form-item>
              <el-button
    ...
    <el-card shadow="never" class="secondCard">
      <vab-query-form-right-panel style="width: 100%">
        ...
        <!-- <el-button type="primary">联查部门费用明细</el-button> -->
...
<script>
  ...
  import Edit from './components/bmfyhzcxEdit'
  ...
  export default {
    ...
    data() {
      return {
        ...
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        localKey: 'cwgx-fygl-dtfytxqkcx-search',
        tableKey: 'cwgx-fygl-dtfytxqkcx-list',
        searchMore: false,
        select: [],
      }
    },
    ...
    methods: {
      getFiled() {
        return [
          ...
          { name: '会计期间', key: 'startStatus' },
          { name: '报销单编号', key: 'startStatus1' },
          { name: '报销单单据日期', key: 'startStatus2' },
          { name: '起摊会计期间', key: 'startStatus2' },
          { name: '结束摊销会计期间', key: 'startStatus2' },
        ]
      },
