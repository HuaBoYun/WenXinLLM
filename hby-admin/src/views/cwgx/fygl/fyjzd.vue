          ...
          >
            <el-form-item v-for="(item, index) in searchItem" :key="index">
              ...
              <el-date-picker
                ...
                unlink-panels
                range-separator="至"
                start-placeholder="录入日期开始日期"
                end-placeholder="录入日期结束日期"
                v-if="item.name === '录入日期'"
              ></el-date-picker>
              <el-input
                v-model="queryForm.supplier"
...
<script>
  ...
  import Edit from './components/fyjzdEdit'
  ...
  export default {
    ...
    data() {
      return {
        ...
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        localKey: 'cwgx-fygl-fyjzd-search',
        tableKey: 'cwgx-fygl-fyjzd-list',
        searchMore: false,
        select: [],
        options_1: [
          {
    ...
    methods: {
      getFiled() {
        return [
          { name: '所属财务组织', key: 'sxName' },
          { name: '单据编号', key: 'startStatus' },
          { name: '交易类型编码', key: 'startStatus1' },
          { name: '来源方式', key: 'startStatus2' },
          { name: '录入日期', key: 'startStatus2' },
          { name: '合计金额', key: 'startStatus2' },
        ]
      },
