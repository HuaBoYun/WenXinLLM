<template>
  <div class="system-log-container">
    ...
    <el-card shadow="never" class="secondCard">
      ...
      <el-table :data="list" v-loading="loading">
        ...
        <el-table-column align="center" label="参照文件" prop="file1">
          <template #default="{ row, $index }">
            ...
            >
              <el-button type="text" @click="handleIndex($index, row.id)">
                选择
              </el-button>
            </el-upload>
          </template>
        ...
        <el-table-column align="center" label="比较文件" prop="file2">
          <template #default="{ row, $index }">
            <el-button
              type="text"
              @click="handleEditFile(row, $index)"
              v-if="row.tableData1.length"
            >
              编辑
            </el-button>
          </template>
      ...
      <onlineEditor v-show="editorShow" ref="onlineEditor"></onlineEditor>
    </el-card>
...
<script>
  ...
  export default {
    ...
    components: { filterTable, filterSearch, compare, onlineEditor },
    data() {
      return {
        baseApi: 'https://office.wenxin.example.com',
        api: 'api/app/wordfilescompare',
        headers: {
        ...
        localKey: 'xtsz-file-fileWordComparison-search',
        tableKey: 'xtsz-file-fileWordComparison-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
        tableData1: [],
    ...
    methods: {
      ...
      async fetchData() {
        this.loading = true
        axios({
          url: `https://office.wenxin.example.com/api/app/wordfilescompare/`,
          method: 'get',
      ...
      fileUpload(val) {
        ...
        // 将上传的文件放到数据对象中
        formData.append('ori_file1', val.file)
        this.loading = true
        axios({
          url: `https://office.wenxin.example.com/api/app/wordfilescompare/${this.activiteId}/`,
          method: 'patch',
          data: formData,
      ...
      handleEditFile(row, index) {
        this.editorShow = true
        this.$refs.onlineEditor.show(row.call_back_url)
        // window.open(row.call_back_url)
        // 打开编辑页面
      },
      handleDelete(row) {
        this.loading = true
        axios({
          url: `https://office.wenxin.example.com/api/app/wordfilescompare/${row.id}/`,
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' },
        })
          .then((res) => {
      ...
      handleOpen(row) {
        ...
        this.loading = true
        axios({
          url: `https://office.wenxin.example.com/api/app/wordfilescompare/${row.id}/exec_compare/`,
          method: 'post',
          headers: { 'Content-Type': 'application/json' },
        })
          .then((res) => {
            if (res.data.task_id) {
              ...
              polling(
                'get',
                `https://office.wenxin.example.com/api/app/wordfilescompare/${row.id}/exec_compare/?task_id=${res.data.task_id}`,
                2000
              )
                .then((j) => {
                  //轮训结束调取比对详情接口
