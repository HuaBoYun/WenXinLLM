        ...
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            if (data.code == 1) {
              ...
              if (data.data.data.libraryid) {
                const info = JSON.parse(localStorage.getItem('userInfo'))
                axios({
                  url: `https://office.wenxin.example.com/api/app/docshare/`,
                  method: 'post',
                  headers: { 'Content-Type': 'application/json' },
                  data: {
                    doc_type: 'word',
                    contract_id: data.data.data.libraryid,
                    user_id: info.staffid,
                    doc_name: data.data.data.wknane,
                  },
                }).then((res) => {
                  if (res.data) {
