    ...
    methods: {
      ...
      async fetchData() {
        ...
        try {
          ...
          console.error('获取委托付款列表失败:', error)
      ...
      async handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          try {
            const response = await deletePayment({ ids: [row.paymentId || row.id] })
            const successCodes = [200, 0, '200', '0', '1', 1, 2]
            if (successCodes.includes(response.code)) {
              this.$baseMessage(response.message || '删除成功', 'success')
              await this.fetchData()
            } else {
              this.$baseMessage(response.message || '删除失败', 'error')
            }
          } catch (error) {
            console.error('删除委托付款失败:', error)
            this.$baseMessage('删除失败，请检查网络连接', 'error')
          }
        })
      },
      sendModel() {
