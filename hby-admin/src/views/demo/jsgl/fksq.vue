          ...
          >
            <el-form-item v-for="(item, index) in searchItem" :key="index">
              ...
              <el-select
                v-model="queryForm.contracttype"
                filterable
                placeholder="申请付款金额范围"
                v-if="item.name === '申请付款金额范围'"
              >
                <el-option
                  v-for="item in typeOptions"
                  :key="item.value"
...
<script>
  ...
  import Edit from './components/fksqEdit'
  ...
  export default {
    ...
    data() {
      return {
        ...
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        localKey: 'globalTreasurer-jsgl-fksq-search',
        tableKey: 'globalTreasurer-jsgl-fksq-list',
        searchMore: false,
        select: [],
        activeName: 'second',
      }
    ...
    methods: {
      ...
      getFiled() {
        return [
          { name: '付款申请组织', key: 'sxName' },
          { name: '时间范围', key: 'startStatus' },
          { name: '交易类型', key: 'startStatus1' },
          { name: '供应商', key: 'startStatus2' },
          { name: '付款财务组织', key: 'startStatus12' },
          { name: '申请付款金额范围', key: 'startStatus212' },
        ]
      },
