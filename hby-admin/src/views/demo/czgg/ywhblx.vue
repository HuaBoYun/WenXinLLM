<template>
  <div class="system-log-container">
    ...
    <!-- 新增/编辑对话框 -->
    <partner-type-edit ref="edit" @refresh="fetchData" />
  </div>
</template>

<script>
import { getPartnerTypeList, deletePartnerType, updatePartnerTypeStatus } from '@/api/globalTreasurer/czgg'
import PartnerTypeEdit from './components/ywhblxEdit'

export default {
  name: 'PartnerTypeManagement',
  components: {
    PartnerTypeEdit
  },
  data() {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        const response = await getPartnerTypeList(this.queryForm)
        if (response.code === 1) {
    ...
    // 删除
    handleDelete(row) {
      this.$confirm('确定要删除该业务伙伴类型吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        try {
          const response = await deletePartnerType(row.partnerTypeId)
          if (response.code === 1) {
    ...
    handleToggleStatus(row) {
      ...
      this.$confirm(`确定要${action}该业务伙伴类型吗？`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        try {
          ...
          const response = await updatePartnerTypeStatus({
            id: row.partnerTypeId,
            isEnabled: status,
    ...
    handleBatchDelete() {
      ...
      this.$confirm('确定要删除选中的业务伙伴类型吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        try {
          const ids = this.multipleSelection.map(item => item.partnerTypeId)
          // 这里需要实现批量删除接口
          this.$message.success('删除成功')
          this.fetchData()
        } catch (error) {
