...
<template>
  <div class="system-log-container">
    ...
    />
    <czfyxmEdit ref="edit" @fetch-data="fetchData"></czfyxmEdit>
  </div>
</template>
...
<script>
  ...
  import czfyxmEdit from './components/czfyxmEdit'
  ...
  export default {
      ...
      return {
        list: [
          ...
          },
          {
            ...
            expenseCode: 'FY002',
            expenseName: '汇率差额',
            expenseType: 'EXCHANGE_DIFF',
            expenseTypeName: '汇兑损益',
            amount: 1200.00,
            currency: 'CNY',
            description: '外汇交易汇率差额',
            isEnabled: 1,
        ...
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        localKey: 'globalTreasurer-czgg-czfyxm-search',
        tableKey: 'globalTreasurer-czgg-czfyxm-list',
        searchMore: false,
        select: [],
        optionsStatus: [
          {
    ...
    methods: {
      ...
      getFiled() {
        return [
          { name: '名称', key: 'czName' },
          { name: '显示停用', key: 'status' },
        ]
      },
      selectTeamList(val, flagTitle) {
        console.log(val, flagTitle)
        if (flagTitle) {
          this.queryForm.projectOrderName = val[0].realnamedate
      ...
      handleEdit(row, disabled, type) {
        this.$refs['edit'].handleOpenCzfyxmDialog(
          row,
          disabled,
          this.planNum,
          type
