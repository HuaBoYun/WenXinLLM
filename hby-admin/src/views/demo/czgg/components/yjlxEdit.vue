...
<script>
import { saveOrUpdateSealType } from '@/api/globalTreasurer/czgg'
...
export default {
  ...
  methods: {
    ...
    showEdit(row = null) {
      ...
      this.isEdit = !!row
      this.dialogTitle = this.isEdit ? '编辑印鉴类型' : '新增印鉴类型'
    ...
    handleSubmit() {
      this.$refs.form.validate(async (valid) => {
        ...
        try {
          ...
          const response = await saveOrUpdateSealType(this.form)
          if (response.code === 1) {
