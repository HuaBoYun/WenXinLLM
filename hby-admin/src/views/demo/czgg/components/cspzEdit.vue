...
<script>
import { saveOrUpdateParameter } from '@/api/globalTreasurer/czgg'
...
export default {
    ...
    return {
      ...
      rules: {
        paramCode: [
          ...
          { pattern: /^[A-Z][A-Z0-9_]*$/, message: '参数编码必须以大写字母开头，只能包含大写字母、数字和下划线', trigger: 'blur' }
        ],
        paramName: [
  ...
  methods: {
    ...
    showEdit(row = null) {
      ...
      this.isEdit = !!row
      this.dialogTitle = this.isEdit ? '编辑参数' : '新增参数'
    ...
    handleSubmit() {
      this.$refs.form.validate(async (valid) => {
        ...
        try {
          ...
          const response = await saveOrUpdateParameter(this.form)
          if (response.code === 1) {
