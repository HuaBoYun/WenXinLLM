...
<script>
  ...
  // import axios from 'axios' // 切换回旧接口时取消注释
  export default {
    name: 'AiInlineEdit',
    components: {},
    computed: {},
    ...
    methods: {
      show(data) {
        ...
        // 替换选区模式
        if (data.savedRange) {
          this.savedRange = data.savedRange
        }
        // 保存选中的文本，发送时一并带给模型
        if (data.selectedText) {
          this.selectedText = data.selectedText
        }
        if (typeof data.onReplace === 'function') {
          this.onReplace = data.onReplace
        }
      },
      insertText() {
        ...
        // 优先使用替换选区模式
        if (this.savedRange && this.onReplace) {
          this.onReplace(content, this.savedRange)
        ...
        // 兜底：原有 textarea 追加模式
        if (this.dom) {
          this.dom.$emit('input', (this.dom.value || '') + content)
        }
        this.modalStatus = false
      },
      ...
      submit() {
        ...
        const prompt = this.selectedText
          ? `【选中内容】\n${this.selectedText}\n\n【用户需求】\n${this.question}\n\n请根据用户需求对选中内容进行处理，直接输出替换后的纯文本，不要加任何解释说明和 Markdown 标记。`
          : this.question

        // ---- 当前使用 chatStream 接口 ----
        this._callAI(prompt)
        // ---- 切换回旧接口时，注释上面一行，取消注释下面的代码 ----
        // axios({
        ...
        //   },
        // }).then((res) => {
        //   this.addMessage({ position: 'left', content: res.data.text, loading: false })
        //   this.historyArr.push({ role: 'user', content: this.question })
        //   this.historyArr.push({ role: 'assistant', content: res.data.text })
      ...
      _callAI(prompt) {
        ...
        this.messages.push({ position: 'left', content: '' })

        if (this._abortController) this._abortController.abort()
        this._abortController = chatStream(
          { message: prompt, sessionId: 'ai-inline-' + Date.now(), enableThinking: false, enableWebSearch: false, stream: true },
          {
            onThinking: () => {},
            onProgress: () => {},
            onToolCall: () => {},
            onMessage: (content) => {
              accumulated += content
              this.$set(this.messages, idx, { position: 'left', content: accumulated })
            },
            onError: (err) => {
              this.isLoading = false
              this._abortController = null
              this.$message.error('AI 请求失败：' + (err || '未知错误'))
            },
            onComplete: () => {
              this.historyArr.push({ role: 'assistant', content: accumulated })
              this.isLoading = false
              this.isLastWord = true
      ...
      reloadQuestion() {
        this.showButton = false
        this.isLoading = true
        this.messages.push({ position: 'right', content: this.lastQuestion })
        // ---- 当前使用 chatStream 接口 ----
        this._callAI(this.lastQuestion)
        // ---- 切换回旧接口时，注释上面一行，取消注释下面的代码 ----
        // axios({
        //   url: `https://hit-mitlab.cn:7860/generate_doc`,
        //   method: 'post',
        ...
        // }).then((res) => {
        //   this.addMessage({ position: 'left', content: res.data.text, loading: false })
        //   this.historyArr.push({ role: 'user', content: this.lastQuestion })
        //   this.historyArr.push({ role: 'assistant', content: res.data.text })
        //   this.isLoading = false
        //   this.question = ''
        // })
  ...
  .content {
    ...
    .wk-chat {
      ...
      .chat-entry-wrapper_logo {
        background-image: url('~@/components/AI/aiLogo.png');
        background-repeat: no-repeat;
        background-size: 100% 100%;
        border-radius: 50%;
        cursor: grab;
