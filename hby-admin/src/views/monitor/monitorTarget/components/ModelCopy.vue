...
<script>
  import { copyIndex } from '@/api/monitor/watch'
  import { getModelManageIndustry } from '@/api/monitor/model'
  ...
  export default {
    ...
    methods: {
      ...
      showEdit(row) {
        ...
        this.selectProjectid = row.indicatorid
        this.getList()
      },
      close() {
      ...
      async save() {
        this.loading = true
        const { code, msg } = await copyIndex({
