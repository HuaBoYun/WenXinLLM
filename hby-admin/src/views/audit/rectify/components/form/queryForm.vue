  ...
  >
    <el-row :gutter="14">
      ...
      >
        ...
        <el-col :span="24">
          <el-table :data="formData.tblReforms">
            <el-table-column
              align="center"
              label="整改措施"
              prop="reformmeasure"
    ...
    <practicableDetail ref="practicableDetail" />
  </el-dialog>
</template>
...
import { getzgjgReformlist, getReformByid } from '@/api/audit/rectify'
import practicableDetail from '@/views/audit/rectify/components/form/practicableDetail'
const { baseURL } = require('@/config')
export default {
  name: 'FlawInfo',
  components: { practicableDetail },
  inheritAttrs: false,
  props: [],
  data() {
    return {
  ...
  methods: {
    async handleDetail(row) {
      ...
      await this.$refs['practicableDetail'].showEdit(data.data)
    },
    ...
    async getFileList(row) {
      const res = await getzgjgReformlist({
        soultionid: row.solutionid,
        problemid: row.problemid,
      })
      if (res.data.list) {
        this.formData.tblReforms = res.data.list
        this.$forceUpdate()
      } else {
        this.formData.tblReforms = []
      }
    },
    close() {
      this.formData = {}
