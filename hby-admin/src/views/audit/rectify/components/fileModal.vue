  ...
  export default {
    ...
    methods: {
      ...
      async handleDelete(row) {
        ...
        const res = await deleteReportMeetFile({ attid: row.attid })
        console.log(res, 'res')
        if (res.code == 1) {
      ...
      async handleSuccess(file) {
        if (file.code == 200) {
          ...
          // 调用saveReportMeetFileR接口保存文件关联
          try {
            for (const fileItem of file.data) {
              await saveReportMeetFileR({
                reportid: this.editId,
                attid: fileItem.attid,
              })
            }
          } catch (error) {
