  ...
  >
    <div>
      ...
      >
        ...
        <el-table-column align="center" label="拟稿日期" prop="createTime" />
      </el-table>
      <!-- <el-pagination
        background
...
<script>
  import { getRectificationIssuesListByReportType } from '@/api/zgzz/index.js'
  ...
  export default {
    ...
    methods: {
      show(data, issuesList) {
        this.queryForm.reporttype = data.reporttype
        this.queryForm.planIdStrs = data.planIdStrs
      ...
      async fetchData() {
        ...
        const { data = [] } = await getRectificationIssuesListByReportType(
          this.queryForm
        )
        ...
        this.list = data.map((x) => {
          ...
          return {
            ...
            planId: x.planId, // 添加planId
            createTime: dayjs(issues.createTime).format('YYYY-MM-DD'),
          }
        })
        ...
        if (this.selected && this.selected.length) {
          this.$nextTick(() => {
            this.selected.forEach((row) => {
              const matchedRow = this.list.find((item) => {
                // 如果有planId就同时匹配，没有就只匹配issuesId
                if (row.planId && item.planId) {
                  return (
                    row.issuesId == item.issuesId && row.planId == item.planId
                  )
                }
                return row.issuesId == item.issuesId
              })
              if (matchedRow) {
                this.$refs.reportTable.toggleRowSelection(matchedRow, true)
              }
            })
          })
        }
