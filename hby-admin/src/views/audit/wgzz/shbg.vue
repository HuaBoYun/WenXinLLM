<template>
  <div class="system-log-container">
    ...
    <el-card shadow="never">
      ...
      <shbgEdit ref="edit" @fetch-data="fetchData" />
    </el-card>
...
<script>
  import { shbgList, shbgRemove, shbgXQList, shbgSave } from '@/api/audit/wgzz'
  import { formatDate } from '@/utils/index'
  import filterSearch from '@/components/filterSearch'
  import filterTable from '@/components/filterTable'
  import { searchTableMixis } from '@/mixis/index'
  import shbgEdit from './components/shbgEdit'
  ...
  export default {
    ...
    methods: {
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(76, row.id)
      },
      ...
      async handleDetail(row) {
        // const data = await shbgXQList({ id: row.id })
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg } = await shbgRemove({ id: row.id })
