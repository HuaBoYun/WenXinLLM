  ...
  export default {
    ...
    methods: {
      ...
      handleSelection(val) {
       ...
       /**
       * @description:  组件确认函数,把选中的数据返回到父组件,并关闭组件(选择右侧列表的数据)
       * @param {*}  
       * @return {*}
       */  
      save() {
        ...
        this.$emit('auditee', this.multipleSelection, 'right')
        this.dialogVisible = false
      },
      /**
       * @description:   组件确认函数,把选择的数据返回到父组件,并关闭组件(选择左侧树的数据)
       * @param {*}  
       * @return {*}
       */   
      saveTree() {
        ...
        this.$emit('auditee', this.selectTree, 'left')
        this.dialogVisible = false
      },
    },
  }
