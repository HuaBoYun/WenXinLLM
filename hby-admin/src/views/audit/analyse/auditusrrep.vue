<template>
  <div class="system-log-container">
      ...
      <el-table v-loading="listLoading" :data="list">
        ...
        <div v-for="(item, index) in filedNow" :key="index">
          ...
          <el-table-column
            align="center"
            label="主要从事的职责"
            v-if="item.name === '主要从事的职责'"
            prop="primaryResponsibilities"
            show-overflow-tooltip
          />
        </div>
    ...
    <AuditusrrepEdit ref="edit"" @fetchData="fetchData" />
  </div>
</template>

<script>
  import {
    auditPersonnelInfoGetPage,
    auditPersonnelInfoExport,
  } from '@/api/audit/analyse'
  import { doDelete } from '@/api/table'
  import ProjectEdit from '@/views/audit/analyse/components/ProjectEdit'
  import AuditusrrepEdit from './components/auditusrrepEdit.vue'
  ...
  export default {
    ...
    components: { filterSearch, filterTable, ProjectEdit, AuditusrrepEdit },
    data() {
      return {
        list: [],
        listLoading: true,
    ...
    methods: {
      ...
      async fetchData() {
        ...
        } = await auditPersonnelInfoGetPage(this.queryForm)
        this.list = list
        this.total = total
       ...
       */
      async handleExport() {
        const data = await auditPersonnelInfoExport({
          ...this.queryForm,
        })
        let fileName = '审计人员情况报表'
