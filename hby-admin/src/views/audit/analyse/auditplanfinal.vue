<template>
  <div class="system-log-container">
      ...
      <el-table v-loading="listLoading" :data="list">
        ...
        <div v-for="(item, index) in filedNow" :key="index">
          ...
          <el-table-column
            align="center"
            label="被审计单位级次"
            v-if="item.name === '被审计单位级次'"
            prop="auditedEntityLevel"
            show-overflow-tooltip
          />
          <el-table-column
            align="center"
          ...
          <el-table-column
            align="center"
            label="审计资金量（万元）"
            v-if="item.name === '审计资金量（万元）'"
            prop="auditFundAmount"
            show-overflow-tooltip
          />
        </div>
    ...
    <AuditplanfinalEdit ref="edit" @fetchData="fetchData" />
  </div>
</template>

<script>
  import {
    auditPlanExecutionStatisticsGetPage,
    auditPlanExecutionStatisticsExport,
  } from '@/api/audit/analyse'
  import { doDelete } from '@/api/table'
  import ProjectEdit from '@/views/audit/analyse/components/ProjectEdit'
  import AuditplanfinalEdit from './components/auditplanfinalEdit.vue'
  ...
  export default {
    ...
    components: { filterSearch, filterTable, ProjectEdit, AuditplanfinalEdit },
    data() {
      return {
        list: [],
        listLoading: true,
    ...
    methods: {
      ...
      async fetchData() {
        ...
        } = await auditPlanExecutionStatisticsGetPage(this.queryForm)
        this.list = list
        this.total = total
       ...
       */
      async handleExport() {
        const data = await auditPlanExecutionStatisticsExport({
          ...this.queryForm,
        })
        let fileName = '审计计划执行及要点完成情况表.xls'
