<template>
  ...
  >
    <el-row :gutter="14">
      ...
      >
        <el-col :span="12">
          <el-form-item label="审计实施单位名称" prop="auditImplementingUnit">
            <el-input
              v-model="formData.auditImplementingUnit"
              placeholder="请输入审计实施单位名称"
              :style="{ width: '100%' }"
            />
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item label="整改责任单位">
            <el-input
              v-model="formData.issueResponsibleUnit"
              :style="{ width: '100%' }"
        ...
        <el-col :span="12">
          <el-form-item label="审计发现问题类型（国资委、集团公司口径）">
            <el-input
              v-model="formData.issueTypeSasacGroup"
              :style="{ width: '100%' }"
              placeholder="请输入审计发现问题类型（国资委、集团公司口径）"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="审计发现问题类型（审计器口径）">
            <el-input
              v-model="formData.issueTypeAuditTool"
              :style="{ width: '100%' }"
              placeholder="请输入审计发现问题类型（审计器口径）"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="金额类问题类型（审计器口径）">
            <el-input
              v-model="formData.monetaryIssueTypeAuditTool"
              :style="{ width: '100%' }"
              placeholder="请输入金额类问题类型（审计器口径）"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="金额类问题涉及具体金额（审计署口径）（万元）">
            <el-input
              v-model="formData.monetaryIssueAmount"
              :style="{ width: '100%' }"
              placeholder="请输入金额类问题涉及具体金额（审计署口径）（万元）"
               @input="inputNum($event, 'monetaryIssueAmount')"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="非金额类问题类型（审计署口径）">
            <el-input
              v-model="formData.nonMonetaryIssueTypeAuditOffice"
              :style="{ width: '100%' }"
              placeholder="请输入非金额类问题类型（审计署口径）"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="提出审计建议简述">
            <el-input
              v-model="formData.auditRecommendations"
              :style="{ width: '100%' }"
              placeholder="请输入提出审计建议简述"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="整改完成标准从定分类">
            <el-input
              v-model="formData.rectificationCompletionStandard"
              :style="{ width: '100%' }"
              placeholder="请输入整改完成标准从定分类"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="12">
          <el-form-item label="收回资金（万元）">
            <el-input
              v-model="formData.recoveredFundsAmount"
              :style="{ width: '100%' }"
              placeholder="请输入收回资金（万元）"
              @input="inputNum($event, 'recoveredFundsAmount')"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="12">
          <el-form-item label="合计（个）">
            <el-input
              v-model="formData.totalNonMonetaryRectificationMeasures"
              :style="{ width: '100%' }"
              placeholder="请输入合计（个）"
              @input="inputNum($event, 'totalNonMonetaryRectificationMeasures')"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="12">
          <el-form-item label="向司法机关移送问题线索（个）">
            <el-input
              v-model="formData.judicialReferrals"
              :style="{ width: '100%' }"
              placeholder="请输入向司法机关移送问题线索（个）"
              @input="inputNum($event, 'judicialReferrals')"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="涉案人员（人次）">
            <el-input
              v-model="formData.personsInvolved"
              :style="{ width: '100%' }"
              placeholder="请输入涉案人员（人次）"
              @input="inputNum($event, 'personsInvolved')"
            />
          </el-form-item>
        </el-col>
        <el-col :span="12">
...
<script>
  import {
    currentYearAuditIssuesGetDetail,
    currentYearAuditIssuesMergeInfo,
    ...
    data() {
      return {
        ...
        formData: {
          ...
          lossRecoveryAmount: '',
          monetaryIssueAmount: '',
          monetaryIssueTypeAuditTool: '',
          newSystemsEstablished: '',
          nonMonetaryIssueTypeAuditOffice: '',
          ...
          totalNonMonetaryRectificationMeasures: '',
     ...
     inputNum(value, key) {
      ...
      async showEdit(row, title) {
        ...
        if (row) {
          const {
            data: { data },
          } = await currentYearAuditIssuesGetDetail({ id: row.id })
          console.log(data)
          this.formData = data
          // this.fetchData()
        }
      ...
      save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            const res = await currentYearAuditIssuesMergeInfo({
              ...this.formData,
            })
            if (res && res.code === 1) {
              this.close()
