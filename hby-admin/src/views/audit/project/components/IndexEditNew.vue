...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      if (this.showMJ) {
        // 获取密级,菜单id
        const res = await hasMJ('ProjectIndex')
    ...
    methods: {
        ...
        this.$refs['elForm'].validate(async (valid) => {
          if (valid) {
            let params = {
              ...
              actproyearstart: this.formData.actproyear
                ? this.formData.actproyear[0]
                : '',
              actproyearend: this.formData.actproyear
                ? this.formData.actproyear[1]
                : '',
              appproyearstart: this.formData.appproyear
                ? this.formData.appproyear[0]
                : '',
              appproyearend: this.formData.appproyear
                ? this.formData.appproyear[1]
                : '',
            }
            if (this.projectId) {
      ...
      getChildlist(val) {
        ...
        // 设置项目单位信息
        this.$set(this.formData, 'projectorgname', val[0].projectorg)
        this.$set(this.formData, 'projectorgid', val[0].projectorgid)
        this.$set(this.formData, 'projectorgaddress', val[0].projectorgaddress)
        this.$set(this.formData, 'projectlinkman', val[0].projectlinkman)
        this.$set(this.formData, 'projectlinktel', val[0].projectlinktel)

        // 设置项目类型和投资
        this.$set(this.formData, 'projecttype', val[0].projectType)
        this.$set(this.formData, 'costs', val[0].costs)

        // 设置时间相关字段
        if (val[0].appproyearstart && val[0].appproyearend) {
          this.$set(this.formData, 'appproyear', [
            val[0].appproyearstart,
            val[0].appproyearend,
          ])
        }
        if (val[0].actproyearstart && val[0].actproyearend) {
          this.$set(this.formData, 'actproyear', [
            val[0].actproyearstart,
            val[0].actproyearend,
          ])
        }

        // 设置主管部门信息
        this.$set(this.formData, 'projectmgdeptname', val[0].projectmgdeptname)
        this.$set(this.formData, 'projectmgdeptid', val[0].projectmgdeptid)

        // 设置审计时间
        this.$set(this.formData, 'planStartDate', val[0].startDate)
        this.$set(this.formData, 'planEndDate', val[0].endDate)

        // 设置审计类型
        this.$set(this.formData, 'auditType', val[0].auditType)
      },
