    ...
    <div class="content">
      ...
      <el-row :gutter="10">
        <el-col :lg="8" :md="16" :sm="24">
          <div id="chats-1" style="width: 100%; height: 262px"></div>
        </el-col>
      </el-row>
      <el-row :gutter="10">
...
<script>
  ...
  import { getSrfxData } from '@/api/stateAssets/cwztfx'
  ...
  export default {
    ...
    methods: {
      fetchData() {
        getSrfxData({ year: this.queryYear, month: this.queryMonth }).then(res => {
          const d = res.data || res
          if (d.overview) {
            const o = d.overview
            this.overviewData = {
              totalRevenue: o.totalRevenue || '--',
              totalRevenueYoy: o.totalRevenueYoy,
              cumulativeTotalRevenue: o.cumulativeTotalRevenue || '--',
              mainRevenue: o.mainRevenue || '--',
              mainRevenueYoy: o.mainRevenueYoy,
              cumulativeMainRevenue: o.cumulativeMainRevenue || '--'
            }
          }
      ...
      refreshCharts(d) {
        ...
        if (d.monthTrend && d.monthTrend.length) {
          const months = d.monthTrend.map(r => r.month)
          refreshChart('chats-1', { xAxis: { data: months }, series: [{ data: d.monthTrend.map(r => r.totalRevenue) }] })
          refreshChart('chats-2', { xAxis: { data: months }, series: [{ data: d.monthTrend.map(r => r.mainRevenue) }] })
        }
        if (d.companyRank && d.companyRank.length) {
          refreshChart('chats-3', { xAxis: { data: d.companyRank.map(r => r.companyName) }, series: [{ data: d.companyRank.map(r => r.revenue) }] })
        }
        if (d.yearTrend && d.yearTrend.length) {
          refreshChart('chats-4', {
            xAxis: [{ data: d.yearTrend.map(r => r.period) }],
            series: [{ data: d.yearTrend.map(r => r.amount) }, { data: d.yearTrend.map(r => r.yoy) }]
          })
        }
        if (d.monthTrend && d.monthTrend.length) {
          refreshChart('chats-5', {
            xAxis: [{ data: d.monthTrend.map(r => r.month) }],
            series: [{ data: d.monthTrend.map(r => r.totalRevenue) }, { data: d.monthTrend.map(r => r.yoy) }, { data: d.monthTrend.map(r => r.mom) }]
          })
          refreshChart('chats-6', {
            xAxis: [{ data: d.monthTrend.map(r => r.month) }],
            series: [{ data: d.monthTrend.map(r => r.mainRevenue) }, { data: d.monthTrend.map(r => r.yoy) }, { data: d.monthTrend.map(r => r.mom) }]
          })
        }
        if (d.revenueStructure && d.revenueStructure.length) {
          refreshChart('chats-7', { series: [{ data: d.revenueStructure.map(r => ({ name: r.name, value: r.amount })) }] })
        }
      },
      initCharts() {
      ...
      var chartDom6 = document.getElementById('chats-6')
