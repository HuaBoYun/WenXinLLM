  ...
  >
    ...
    <el-row v-if="show == 1" :gutter="15">
      <el-form ref="form" label-width="140px" :model="form" :rules="rules">
        ...
        <el-col :span="12">
          <el-form-item label="已付款金额">
            <span>{{ form.accumulatedpayments }}</span>
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="12">
          <el-form-item label="开票日期" prop="paymentlatedate">
        ...
        <el-col :span="12">
          <el-form-item label="最晚付款日期" prop="paymentlatedate">
            <span>{{ form.paymentlatedate }}</span>
          </el-form-item>
        </el-col> -->
        <el-col :span="24">
...
<script>
  ...
  import ExecutorOptions from './options/executor'
  import fpxxOptions from './options/fpxx.vue'
  import htxxOptions from './options/htxx.vue'
  import lxxxOptions from './options/lxxx.vue'
  ...
  export default {
    ...
    methods: {
      ...
      showEdit(row, disabled) {
        ...
        if (!row) {
          this.title = '添加'
          this.form.applyStaffId = JSON.parse(
            localStorage.getItem('userInfo')
          ).staffid
          this.form.realname = JSON.parse(
            localStorage.getItem('userInfo')
          ).realname
          this.form.orgname = JSON.parse(
            localStorage.getItem('userInfo')
          ).linkDetp.orgname
          this.form.applyOrgId = JSON.parse(
            localStorage.getItem('userInfo')
          ).linkDetp.orgid
        } else {
          if (disabled) {
      ...
      handlehtxdf(val) {
        console.log(val, 'sfdf')
        this.form.counterpartno = val.budgetname
      },
    },
  }
</script>
