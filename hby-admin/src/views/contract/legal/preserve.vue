          ...
          >
            <el-form-item v-for="(item, index) in searchItem" :key="index">
              ...
              <el-input
                ...
                clearable
                placeholder="保全资质性质"
                v-if="item.name === '保全资质性质'"
              />
              <el-row v-if="item.name === '保全资产数额'">
                <el-col :span="11">
                  <el-input
      ...
      <el-table v-loading="listLoading" :data="list">
        ...
        <div v-for="(item, index) in filedNow" :key="index">
          ...
          <el-table-column
            ...
            v-if="item.name === '(被)保全资产数额(万元)'"
          />
          <el-table-column
            align="center"
            label="(被)执金额(万元)"
            prop="exceteamount"
            v-if="item.name === '(被)执金额(万元)'"
          />
        </div>
        <el-table-column
          align="center"
...
<script>
  import { getqualification, Removequalification } from '@/api/contract/legal'
  import PreserveEdit from '@/views/contract/legal/components/PreserveEdit'
  import filterSearch from '@/components/filterSearch.vue'
  import filterTable from '@/components/filterTable.vue'
  export default {
    name: 'Preserve',
    components: { PreserveEdit, filterTable, filterSearch },
    data() {
      return {
        list: [],
        listLoading: true,
        ...
        localKey: 'contract-legal-preserve-search',
        tableKey: 'contract-legal-preserve-list',
        searchNow: [],
        searchMore: true,
        searchItem: [],
    ...
    methods: {
      ...
      getFiled() {
        let fields = [
          ...
          { name: '保全资质性质', key: 'preservednature' },
          { name: '保全资产数额', key: 'minPreservedAmount' },
          { name: '执行金额', key: 'minExceteAmount' },
        ]
        return fields
      },
      initSearch() {
