<template>
  <div class="system-log-container">
      ...
      <el-table v-loading="listLoading" :data="list">
        ...
        <div v-for="(item, index) in filedNow" :key="index">
          ...
          <el-table-column
            ...
            v-if="item.name === '待落实项'"
          >
            <template #default="{ row }">{{ row.plannum }}项</template>
          </el-table-column>
          <el-table-column
            align="center"
            label="合同金额(元)"
        ...
        >
          <template #default="{ row }">
            <el-button
              type="text"
              @click="$refs['track'].show(row, 'practicable')"
            >
              落实
            </el-button>
          </template>
    ...
    />
    <TrackList ref="track" @fetch-data="fetchData" />
    <CreateDetail ref="common" @fetch-data="fetchData" />
  </div>
</template>

<script>
  import { getImplementContractList } from '@/api/contract/fulfil'
  ...
  export default {
    name: 'Procticable',
    components: { TrackList, CreateDetail, filterSearch, filterTable },
    data() {
      return {
        list: [],
        listLoading: true,
        ...
        localKey: 'contract-execute-practicable-search',
        tableKey: 'contract-execute-practicable-list',
        searchNow: [],
        searchMore: true,
        searchItem: [],
    ...
    methods: {
      ...
      async fetchData() {
        ...
        } = await getImplementContractList(this.queryForm)
        this.list = tlist
