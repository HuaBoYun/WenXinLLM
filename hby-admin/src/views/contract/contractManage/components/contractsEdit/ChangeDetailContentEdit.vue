<template>
  <div>
    ...
    <div style="text-align: right; margin-top: 10px" v-if="allDisable">
    ...
    methods: {
      ...
      async fetchItem(row) {
        ...
        Object.keys(this.formData).forEach((key) => {
          this.formData[key] = res.data.tcu ? res.data.tcu[key] : ''
        })
      ...
      openOffice() {
        ...
        temp.action = `http://192.0.2.15:80/api/office/editRealContract?fileType=word&contractId=${this.contractid}&uid=${info.staffid}&name=${info.username}`
        temp.method = 'post'
        temp.target = '_blank'
      ...
      handleoffice() {
        const info = JSON.parse(localStorage.getItem('userInfo'))
        window.open(
          `http://192.0.2.15:80/api/office/getContractReview?fileType=word&contractId=${this.contractid}&uid=${info.staffid}&name=${info.username}`
        )
      },
      //打开模板
      openTemplate() {
      ...
      handleTemplateSelected(val) {
        ...
        const info = JSON.parse(localStorage.getItem('userInfo'))
        let temp = document.createElement('form')
        temp.action = `http://192.0.2.15:80/api/office/editRealContract?fileType=word&templateId=${val.contractid}&contractId=${this.contractid}&uid=${info.staffid}&name=${info.username}`
        temp.method = 'post'
        temp.target = '_blank'
        document.body.appendChild(temp)
        temp.submit()
      ...
      handleFileBefore(file) {
        return new Promise((resolve, reject) => {
          const info = JSON.parse(localStorage.getItem('userInfo'))
          // 此处动态配置action URL
          this.uploadUrl = `http://192.0.2.15:80/api/office/editUpdateContract?fileType=word&contractId=${this.contractid}&uid=${info.staffid}&name=${info.username}&upload=true`
          // dom上传地址更新完成后，触发上传
