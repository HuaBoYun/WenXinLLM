  ...
  >
    <el-row :gutter="15" v-loading="showLoading">
      <el-form ref="form" label-width="150px" :model="formData" :rules="rules">
        ...
        <el-col :span="3">
          <el-button
            :disabled="!contractid || !preContractId"
            class="add-btn"
            size="mini"
            type="primary"
            @click="openTemplate()"
          ...
          <el-table :data="pdfList2">
            ...
            >
              <template #default="{ row }">
                ...
                <!-- <el-button
                  type="text"
                  @click="handleDeleteFilePDF2(row, $index)"
                  size="mini"
                >
                  删除
                </el-button> -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async showEdit(row, type, fromMine, goalStatus) {
      ...
      save() {
        this.$refs['form'].validate(async (valid) => {
          if (valid) {
            ...
            const { describe, ...other } = this.formData
            // let goalStatus = 10
            ...
            if (code == 1) {
              ...
              this.newContractId = data.contractid
              //马上唤起office编辑内容
              const info = JSON.parse(localStorage.getItem('userInfo'))
              let temp = document.createElement('form')
              temp.action = `https://office.wenxin.example.com/api/office/editUpdateContract?fileType=word&contractId=${data.contractid}&parentId=${data.preContractId}&uid=${info.staffid}&name=${info.username}`
              temp.method = 'post'
              temp.target = '_blank'
      ...
      openOffice() {
        ...
        const info = JSON.parse(localStorage.getItem('userInfo'))
        let temp = document.createElement('form')
        temp.action = `https://office.wenxin.example.com/api/office/editUpdateContract?fileType=word&contractId=${this.newContractId}&parentId=${this.preContractId}&uid=${info.staffid}&name=${info.username}`
        temp.method = 'post'
        temp.target = '_blank'
        document.body.appendChild(temp)
        temp.submit()
      ...
      handleTemplateSelected(val) {
        ...
        const info = JSON.parse(localStorage.getItem('userInfo'))
        let temp = document.createElement('form')
        temp.action = `https://office.wenxin.example.com/api/office/editUpdateContract?fileType=word&templateId=${val.contractid}&contractId=${this.newContractId}&uid=${info.staffid}&name=${info.username}`
        temp.method = 'post'
        temp.target = '_blank'
        document.body.appendChild(temp)
        temp.submit()
