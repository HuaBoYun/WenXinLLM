  ...
  <div class="app-container">
    ...
    <el-row :gutter="20" class="statistics-row">
      ...
      <el-col :span="6">
        <el-card class="statistics-card">
          <div class="statistics-content">
            <div class="statistics-value">{{ summaryInfo.avgDelayDays || 0 }}</div>
            <div class="statistics-label">平均延期天数</div>
          </div>
      ...
      <el-col :span="6">
        <el-card class="statistics-card">
          <div class="statistics-content">
            <div class="statistics-value">{{ formatAmount(summaryInfo.totalExecutedAmount) }}</div>
            <div class="statistics-label">已执行金额</div>
          </div>
    ...
    <!-- 新建执行对话框 -->
    <el-dialog title="新建执行" :visible.sync="createDialogVisible" width="600px">
      <el-form ref="createForm" :rules="createRules" :model="createTemp" label-width="100px" style="width:500px;margin-left:30px;">
        <el-form-item label="关联计划" prop="planId">
          <el-select v-model="createTemp.planId" placeholder="请选择关联资金计划" style="width:100%" filterable>
            <el-option
              v-for="item in planOptions"
              :key="item.planId"
              :label="item.planName ? (item.planNo + ' - ' + item.planName) : item.planNo"
              :value="item.planId"
            />
          </el-select>
        </el-form-item>
        <el-form-item label="执行编号" prop="executionNo">
          <el-input v-model="createTemp.executionNo" placeholder="请输入执行编号" />
        </el-form-item>
        <el-form-item label="执行类型" prop="executionType">
          <el-select v-model="createTemp.executionType" placeholder="请选择执行类型" style="width:100%">
        ...
        <el-form-item label="计划金额" prop="plannedAmount">
          <el-input-number v-model="createTemp.plannedAmount" :precision="2" style="width:100%" />
        </el-form-item>
        <el-form-item label="计划日期" prop="plannedDate">
          <el-date-picker v-model="createTemp.plannedDate" type="date" placeholder="选择计划日期" style="width:100%" />
        </el-form-item>
        <el-form-item label="执行说明">
          <el-input v-model="createTemp.executionNotes" type="textarea" :rows="3" placeholder="请输入执行说明" />
        </el-form-item>
      </el-form>
      <div slot="footer">
    ...
    <!-- 效率分析弹窗 -->
    <el-dialog title="执行效率分析" :visible.sync="efficiencyDialogVisible" width="500px">
      <el-descriptions :column="2" border>
        <el-descriptions-item label="执行总数">{{ summaryInfo.totalCount }}</el-descriptions-item>
        <el-descriptions-item label="平均执行率">{{ summaryInfo.avgExecutionRate }}%</el-descriptions-item>
        <el-descriptions-item label="平均延期天数">{{ summaryInfo.avgDelayDays }}</el-descriptions-item>
        <el-descriptions-item label="已执行金额">{{ formatAmount(summaryInfo.totalExecutedAmount) }}</el-descriptions-item>
        <el-descriptions-item label="计划总金额">{{ formatAmount(summaryInfo.totalPlannedAmount) }}</el-descriptions-item>
      </el-descriptions>
      <div slot="footer">
        <el-button type="primary" @click="efficiencyDialogVisible = false">关闭</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
import { getFundPlanExecutionPage, createFundPlanExecution, updateFundPlanExecution, deleteFundPlanExecution,
         ...
         pauseFundPlanExecution, cancelFundPlanExecution, getFundPlanPage } from '@/api/globalTreasurer/zjjh'
  ...
  data() {
    return {
      ...
      createRules: {
        ...
        executionNo: [{ required: true, message: '请输入执行编号', trigger: 'blur' }],
  ...
  methods: {
      ...
      getFundPlanExecutionSummary(this.listQuery).then(response => {
        if (response.code === 1) {
          ...
          this.summaryInfo = {
            ...
            avgExecutionRate: d.AVGEXECUTIONRATE ? parseFloat(d.AVGEXECUTIONRATE).toFixed(2) : 0,
            avgDelayDays: d.AVGDELAYDAYS || 0,
            totalExecutedAmount: d.TOTALEXECUTEDAMOUNT || 0,
            totalPlannedAmount: d.TOTALPLANNEDAMOUNT || 0
          }
        }
      })
    },
    ...
    confirmCreate() {
      this.$refs['createForm'].validate(valid => {
        if (valid) {
          createFundPlanExecution(this.createTemp).then(response => {
            if (response.code === 1) {
    ...
    handleBatchDelete() {
      ...
      this.$confirm(`确认删除选中的 ${this.multipleSelection.length} 条记录?`, '提示', {
        ...
        Promise.all(this.multipleSelection.map(row => deleteFundPlanExecution(row.executionId))).then(() => {
      ...
      this.$confirm('确认开始执行该计划?', '提示', {
        ...
        startFundPlanExecution(row.executionId).then(response => {
          if (response.code === 1) {
            ...
            this.$message.error(response.msg || '开始执行失败')
          }
        })
      })
    },
      ...
      this.$confirm('确认完成该执行?', '提示', {
        ...
        completeFundPlanExecution(row.executionId).then(response => {
          if (response.code === 1) {
            ...
            this.$message.error(response.msg || '完成执行失败')
          }
        })
      })
    },
