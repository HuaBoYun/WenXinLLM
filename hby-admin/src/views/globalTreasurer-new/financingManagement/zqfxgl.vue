<template>
  <div class="bond-issuance-manage">
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column label="发行日期" width="120px" align="center">
          <template slot-scope="{row}">
            <span>{{ formatDate(row.issuanceDate) }}</span>
          </template>
        </el-table-column>
        ...
        <el-tabs v-model="activeFormTab">
          <el-tab-pane label="基本信息" name="basic">
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="发行编号">
                  <el-input v-model="temp.issuanceNo" placeholder="系统自动生成" disabled />
                </el-form-item>
              </el-col>
              <el-col :span="12">
    ...
    <el-dialog title="债券发行详情" :visible.sync="dialogDetailVisible" width="800px">
      <div v-if="currentBond" class="bond-detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="发行日期">{{ formatDate(currentBond.issuanceDate) }}</el-descriptions-item>
          <el-descriptions-item label="到期日期">{{ formatDate(currentBond.maturityDate) }}</el-descriptions-item>
          <el-descriptions-item label="创建时间">{{ formatDate(currentBond.createTime) }}</el-descriptions-item>
        </el-descriptions>
    ...
    <!-- 评级管理对话框 -->
    <el-dialog title="评级管理" :visible.sync="dialogRatingVisible" width="600px">
      <el-form ref="ratingForm" :model="ratingForm" label-width="100px">
        <el-form-item label="债券名称">
          <span>{{ currentBond ? currentBond.bondName : '' }}</span>
        </el-form-item>
        <el-form-item label="评级机构" prop="ratingAgency">
          <el-select v-model="ratingForm.ratingAgency" placeholder="请选择评级机构" style="width: 100%;">
            <el-option label="中诚信国际" value="中诚信国际" />
            <el-option label="联合资信" value="联合资信" />
            <el-option label="大公国际" value="大公国际" />
            <el-option label="东方金诚" value="东方金诚" />
            <el-option label="上海新世纪" value="上海新世纪" />
            <el-option label="穆迪" value="穆迪" />
            <el-option label="标普" value="标普" />
            <el-option label="惠誉" value="惠誉" />
          </el-select>
        </el-form-item>
        <el-form-item label="信用评级" prop="creditRating">
          <el-select v-model="ratingForm.creditRating" placeholder="请选择信用评级" style="width: 100%;">
            <el-option label="AAA" value="AAA" />
            <el-option label="AA+" value="AA+" />
            <el-option label="AA" value="AA" />
            <el-option label="AA-" value="AA-" />
        ...
        <el-form-item label="评级日期" prop="ratingDate">
          <el-date-picker v-model="ratingForm.ratingDate" type="date" placeholder="选择评级日期" style="width: 100%;" />
        </el-form-item>
        <el-form-item label="评级展望" prop="ratingOutlook">
          <el-select v-model="ratingForm.ratingOutlook" placeholder="请选择评级展望" style="width: 100%;">
        ...
        <el-form-item label="评级说明">
          <el-input v-model="ratingForm.ratingNotes" type="textarea" :rows="3" placeholder="请输入评级说明" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogRatingVisible = false">取消</el-button>
        <el-button type="primary" @click="submitRating">保存评级</el-button>
      </div>
    </el-dialog>

    <!-- 上市管理对话框 -->
    <el-dialog title="上市管理" :visible.sync="dialogListingVisible" width="600px">
      <el-form ref="listingForm" :model="listingForm" label-width="100px">
        <el-form-item label="债券名称">
          <span>{{ currentBond ? currentBond.bondName : '' }}</span>
        </el-form-item>
        <el-form-item label="上市交易所" prop="listingExchange">
          <el-select v-model="listingForm.listingExchange" placeholder="请选择上市交易所" style="width: 100%;">
            <el-option label="上海证券交易所" value="SSE" />
            <el-option label="深圳证券交易所" value="SZSE" />
            <el-option label="银行间债券市场" value="CIBM" />
            <el-option label="北京证券交易所" value="BSE" />
          </el-select>
        </el-form-item>
        <el-form-item label="上市日期" prop="listingDate">
          <el-date-picker v-model="listingForm.listingDate" type="date" placeholder="选择上市日期" style="width: 100%;" />
        </el-form-item>
        <el-form-item label="证券代码" prop="stockCode">
          <el-input v-model="listingForm.stockCode" placeholder="请输入证券代码" />
        </el-form-item>
        <el-form-item label="上市说明">
          <el-input v-model="listingForm.listingNotes" type="textarea" :rows="3" placeholder="请输入上市说明" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogListingVisible = false">取消</el-button>
        <el-button type="primary" @click="submitListing">保存上市信息</el-button>
      </div>
    </el-dialog>

    <!-- 兑付管理对话框 -->
    <el-dialog title="兑付管理" :visible.sync="dialogRedeemVisible" width="600px">
      <el-form ref="redeemForm" :model="redeemForm" label-width="100px">
        <el-form-item label="债券名称">
          <span>{{ currentBond ? currentBond.bondName : '' }}</span>
        </el-form-item>
        <el-form-item label="兑付类型" prop="redeemType">
          <el-select v-model="redeemForm.redeemType" placeholder="请选择兑付类型" style="width: 100%;">
            <el-option label="到期兑付" value="MATURITY" />
            <el-option label="提前兑付" value="EARLY" />
            <el-option label="部分兑付" value="PARTIAL" />
            <el-option label="回售兑付" value="PUT" />
          </el-select>
        </el-form-item>
        <el-form-item label="兑付金额" prop="redeemAmount">
          <el-input-number
            v-model="redeemForm.redeemAmount"
            :precision="2"
            :step="1000000"
            :min="0"
            style="width: 100%;"
        ...
        <el-form-item label="兑付日期" prop="redeemDate">
          <el-date-picker v-model="redeemForm.redeemDate" type="date" placeholder="选择兑付日期" style="width: 100%;" />
        </el-form-item>
        <el-form-item label="兑付说明">
          <el-input v-model="redeemForm.redeemNotes" type="textarea" :rows="3" placeholder="请输入兑付说明" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogRedeemVisible = false">取消</el-button>
        <el-button type="primary" @click="submitRedeem">确认兑付</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
import {
  getBondFinancingPage,
  createBondFinancing,
  updateBondFinancing,
  issueBond,
  payBondInterest,
  getBondFinancingOverview,
  getBondIssuancePage,
  getBondTypeDistribution,
  getBondTrend,
  exportBondFinancing,
  downloadBondTemplate,
  batchImportBondFinancing,
  submitBondForApproval,
  approveBondFinancing,
  rejectBondFinancing,
  confirmBondIssuance,
  deleteBondFinancing
...
export default {
  ...
  data() {
    return {
      ...
      dialogListingVisible: false,
      dialogRedeemVisible: false,
      dialogStatus: '',
      dialogTitle: '',
      activeFormTab: 'basic',
      temp: {
  ...
  methods: {
    ...
    getList() {
      ...
      const params = {
        ...
        issuanceNo: this.listQuery.issuanceNo || undefined,
        bondName: this.listQuery.bondName || undefined,
        bondType: this.listQuery.bondType || undefined,
        issuanceStatus: this.listQuery.issuanceStatus || undefined
      }

      // 处理日期范围
      if (this.listQuery.issuanceDateRange && this.listQuery.issuanceDateRange.length === 2) {
        params.startDate = this.listQuery.issuanceDateRange[0]
        params.endDate = this.listQuery.issuanceDateRange[1]
      }

      getBondIssuancePage(params).then(response => {
        // 示例云标准响应格式: { code: 1, msg: "", data: { tlist: [], totalRecord: 0 } }
        if (response && (response.code === 1 || response.code === 200)) {
          // 处理分页数据 - 支持多种格式
          const data = response.data || response
          this.bondList = data.tlist || data.list || data.records || data.rows || []
          this.total = data.totalRecord || data.total || 0
        } else {
          this.bondList = []
          this.total = 0
          this.$message.error(response?.msg || response?.message || '查询债券发行数据失败')
        }
        this.listLoading = false
      }).catch(error => {
        console.error('查询债券发行数据异常:', error)
        this.bondList = []
        this.total = 0
        this.listLoading = false
        this.$message.error('查询债券发行数据失败，请稍后重试')
      })
    },
    initCharts() {
    ...
    updateBondTypeChart() {
      ...
      // 默认颜色列表（用于未知类型）
      const defaultColors = ['#409EFF', '#67C23A', '#E6A23C', '#F56C6C', '#909399', '#E040FB', '#00BCD4', '#FF9800', '#795548']

      // 从API获取债券类型分布数据
      getBondTypeDistribution().then(response => {
        let data = []
        ...
        if (response && response.code === 1 && response.data) {
          ...
          if (Array.isArray(apiData)) {
            ...
            apiData.forEach((item, index) => {
              const typeName = item.name || item.bondTypeName || '其他'
              const count = item.value || item.count || 0
              const color = colorMap[typeName] || defaultColors[index % defaultColors.length]
              data.push({
                name: typeName,
                value: count,
        ...
        if (data.length === 0) {
          data = [
            { name: '企业债券', value: 0, itemStyle: { color: '#409EFF' } },
            { name: '可转换债券', value: 0, itemStyle: { color: '#67C23A' } },
            { name: '政府债券', value: 0, itemStyle: { color: '#E6A23C' } },
            { name: '永续债券', value: 0, itemStyle: { color: '#F56C6C' } }
          ]
        }

        this.renderBondTypeChart(data)
      }).catch(error => {
        console.error('获取债券类型分布数据失败:', error)
        // 使用默认空数据渲染图表
        const data = [
          { name: '企业债券', value: 0, itemStyle: { color: '#409EFF' } },
          { name: '可转换债券', value: 0, itemStyle: { color: '#67C23A' } },
    ...
    renderBondTypeChart(data) {
      ...
      // 使用 notMerge: true 完全替换配置，避免切换图表类型时残留之前的配置
      this.bondTypeChart.setOption(option, { notMerge: true })
    },
    updateTrendChart() {
      // 根据选择的时间周期确定月份数
      let months = 12
      if (this.trendPeriod === '6M') {
        months = 6
      } else if (this.trendPeriod === '3M') {
        months = 3
      } else if (this.trendPeriod === '1Y') {
        months = 12
      }

      // 从API获取趋势数据
      getBondTrend(this.trendPeriod).then(response => {
        let monthLabels = this.generateMonthLabels(months)
        let issuanceData = new Array(months).fill(0)
        let amountData = new Array(months).fill(0)

        // 如果API返回数据，更新图表数据
        if (response && response.code === 1 && response.data) {
          const apiData = response.data

          // 后端返回格式: { months: [], issuanceCount: [], issuanceAmount: [] }
          if (apiData.months && Array.isArray(apiData.months)) {
            // 直接使用后端返回的数据
            monthLabels = apiData.months
            issuanceData = apiData.issuanceCount || new Array(apiData.months.length).fill(0)
            // 金额转换为万元
            amountData = (apiData.issuanceAmount || []).map(amount => amount ? amount / 10000 : 0)
          } else if (Array.isArray(apiData)) {
            // 兼容旧格式：数组格式
            apiData.forEach(item => {
              const month = item.month || item.period
              const count = item.count || item.issuanceCount || 0
              const amount = item.amount || item.issuanceAmount || 0

              // 查找对应月份的索引
              const index = monthLabels.findIndex(label => label === month || label.includes(month))
              if (index !== -1) {
                issuanceData[index] = count
                amountData[index] = amount / 10000 // 转换为万元
              }
            })
          }
        }

        this.renderTrendChart(monthLabels, issuanceData, amountData)
      }).catch(error => {
        console.error('获取债券发行趋势数据失败:', error)
        // 使用默认空数据渲染图表
        const monthLabels = this.generateMonthLabels(months)
        const issuanceData = new Array(months).fill(0)
        const amountData = new Array(months).fill(0)
        this.renderTrendChart(monthLabels, issuanceData, amountData)
      })
    },
    renderTrendChart(months, issuanceData, amountData) {
      const option = {
        tooltip: {
          trigger: 'axis',
          axisPointer: {
    ...
    handleRating(row) {
      ...
      this.ratingForm = {
        ...
        ratingOutlook: row.ratingOutlook || '',
    ...
    handleListing(row) {
      ...
      this.listingForm = {
        ...
        stockCode: row.stockCode || row.bondCode || '',
        listingNotes: row.listingNotes || ''
      }
      this.dialogListingVisible = true
    },
    handleRedeem(row) {
      this.currentBond = row
      // 初始化兑付表单，使用已有数据
      this.redeemForm = {
        redeemType: row.redeemType || 'MATURITY',
        redeemAmount: row.redeemAmount || row.issuanceAmount || row.issueAmount || 0,
        redeemDate: row.redeemDate ? new Date(row.redeemDate) : (row.maturityDate ? new Date(row.maturityDate) : new Date()),
        redeemNotes: row.redeemNotes || ''
      }
      this.dialogRedeemVisible = true
    },
    ...
    handleImport() {
      ...
      input.onchange = (e) => {
        ...
        if (!fileName.endsWith('.xls') && !fileName.endsWith('.xlsx')) {
          this.$message.error('只支持Excel文件格式(.xls/.xlsx)')
          return
        }
        ...
        this.$confirm('确认导入文件: ' + file.name + '?', '批量导入', {
          ...
          batchImportBondFinancing(formData).then(response => {
            loading.close()
            if (response && response.code === 1) {
              ...
              if (parseErrors.length > 0) {
                this.$alert(parseErrors.join('<br/>'), '导入警告', {
    ...
    handleDownloadTemplate() {
      ...
      downloadBondTemplate().then(response => {
        loading.close()
        // 创建Blob并下载
    ...
    handleExport() {
      ...
      exportBondFinancing(params).then(response => {
        loading.close()
        // 创建Blob并下载
        const blob = new Blob([response], {
          type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        ...
        link.download = `债券发行数据_${timestamp}.xlsx`
        document.body.appendChild(link)
        link.click()
        document.body.removeChild(link)
        window.URL.revokeObjectURL(url)
    ...
    submitRating() {
      if (!this.ratingForm.ratingAgency || !this.ratingForm.creditRating) {
        this.$message({
          type: 'warning',
          message: '请选择评级机构和信用评级'
        })
        return
      }
      ...
      // 构建更新数据
      const data = {
        issuanceId: this.currentBond.issuanceId,
        ratingAgency: this.ratingForm.ratingAgency,
        creditRating: this.ratingForm.creditRating,
        ratingDate: this.ratingForm.ratingDate ? this.ratingForm.ratingDate.toISOString().slice(0, 10) : null,
        ratingOutlook: this.ratingForm.ratingOutlook,
        ratingNotes: this.ratingForm.ratingNotes
      }
      ...
      updateBondFinancing(data).then(response => {
        loading.close()
        if (response && response.code === 1) {
        ...
        loading.close()
        console.error('保存评级信息失败:', error)
        this.$message.error('保存失败，请稍后重试')
      })
    },
    submitListing() {
      if (!this.listingForm.listingExchange || !this.listingForm.listingDate) {
        this.$message({
          type: 'warning',
          message: '请选择上市交易所和上市日期'
        })
        return
      }
      ...
      const data = {
        issuanceId: this.currentBond.issuanceId,
        listingExchange: this.listingForm.listingExchange,
        listingDate: this.listingForm.listingDate ? this.listingForm.listingDate.toISOString().slice(0, 10) : null,
        stockCode: this.listingForm.stockCode,
        listingNotes: this.listingForm.listingNotes,
        issuanceStatus: 'LISTED'
      }
      ...
      updateBondFinancing(data).then(response => {
        loading.close()
        if (response && response.code === 1) {
          this.dialogListingVisible = false
          this.$message({
            type: 'success',
            message: '上市信息保存成功!'
          })
        ...
        loading.close()
        console.error('保存上市信息失败:', error)
        this.$message.error('保存失败，请稍后重试')
      })
    },
    submitRedeem() {
      if (!this.redeemForm.redeemAmount || !this.redeemForm.redeemDate) {
        this.$message({
          type: 'warning',
          message: '请填写兑付金额和兑付日期'
        })
        return
      }
      ...
      const data = {
        ...
        redeemAmount: this.redeemForm.redeemAmount,
        redeemDate: this.redeemForm.redeemDate ? this.redeemForm.redeemDate.toISOString().slice(0, 10) : null,
        redeemNotes: this.redeemForm.redeemNotes,
      ...
      updateBondFinancing(data).then(response => {
        loading.close()
        if (response && response.code === 1) {
          this.dialogRedeemVisible = false
          this.$message({
            type: 'success',
            message: '兑付操作成功!'
          })
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          // 构建请求数据
          const data = {
            bondName: this.temp.bondName,
            bondCode: this.temp.bondCode,
            bondType: this.temp.bondType,
            issuanceAmount: this.temp.issuanceAmount,
            currencyCode: this.temp.currencyCode || 'CNY',
            faceValue: this.temp.faceValue,
            couponRate: this.temp.couponRate,
            bondTerm: this.temp.bondTerm,
            termUnit: this.temp.termUnit,
            paymentFrequency: this.temp.paymentFrequency,
            creditRating: this.temp.creditRating,
            underwriter: this.temp.underwriter,
            trustee: this.temp.trustee,
            ratingAgency: this.temp.ratingAgency,
            listingExchange: this.temp.listingExchange,
            planId: this.temp.planId
          }

          // 处理日期
          if (this.temp.issuanceDate) {
            data.issuanceDate = typeof this.temp.issuanceDate === 'object'
              ? this.temp.issuanceDate.toISOString().slice(0, 10)
              : this.temp.issuanceDate
          }
          if (this.temp.maturityDate) {
            data.maturityDate = typeof this.temp.maturityDate === 'object'
              ? this.temp.maturityDate.toISOString().slice(0, 10)
              : this.temp.maturityDate
          }

          // 调用后端API
          this.listLoading = true
          createBondFinancing(data).then(response => {
            this.listLoading = false
            if (response && (response.code === 1 || response.code === '1' || response.code === 200)) {
              ...
              this.$message({
                type: 'error',
                message: response.msg || response.message || '创建失败'
              })
            }
          }).catch(error => {
            this.listLoading = false
            console.error('创建债券发行失败:', error)
            this.$message({
              type: 'error',
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          // 构建请求数据
          const data = {
            issuanceId: this.temp.issuanceId,
            bondName: this.temp.bondName,
            bondCode: this.temp.bondCode,
            bondType: this.temp.bondType,
            issuanceAmount: this.temp.issuanceAmount,
          ...
          updateBondFinancing(data).then(response => {
            ...
            if (response && (response.code === 1 || response.code === '1' || response.code === 200)) {
              ...
              this.$message({
                type: 'error',
                message: response.msg || response.message || '更新失败'
              })
            }
          }).catch(error => {
            this.listLoading = false
            console.error('更新债券发行失败:', error)
            this.$message({
              type: 'error',
              message: '更新失败: ' + (error.message || '网络错误')
            })
          })
        }
      })
    ...
    // 获取债券概览数据
    getOverviewData() {
      getBondFinancingOverview().then(response => {
        // 示例云标准响应格式: { code: 1, msg: "", data: {} }
        if (response && (response.code === 1 || response.code === 200)) {
          const overviewData = response.data || {};
          this.totalBonds = overviewData.totalBonds || 0;
          this.outstandingAmount = overviewData.outstandingAmount || 0;
          this.averageCouponRate = overviewData.averageCouponRate || 0;
          this.averageRating = overviewData.averageRating || '';
        }
      }).catch(error => {
        console.error('获取债券概览数据异常:', error);
      });
    },
