    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column label="最后执行" width="120px" align="center">
          <template slot-scope="{row}">
            <span>{{ formatDate(row.lastExecutionTime) }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="240" class-name="small-padding fixed-width">
          <template slot-scope="{row}">
            ...
            <el-button v-if="row.status === 'INACTIVE'" size="mini" type="success" @click="handleEnable(row)">
              启用
            </el-button>
            <el-button v-if="row.status === 'ACTIVE'" size="mini" type="warning" @click="handleDisable(row)">
              停用
            </el-button>
...
<script>
import { getFundConcentrationStrategyPage, createFundConcentrationStrategy, updateFundConcentrationStrategy, enableConcentrationStrategy, disableConcentrationStrategy, testConcentrationStrategy, batchEnableConcentrationStrategy, deleteConcentrationStrategy, batchDeleteConcentrationStrategy, exportConcentrationStrategy } from '@/api/globalTreasurer/zjjz'
  ...
  methods: {
    async getList() {
      ...
      try {
        const params = {
          ...
          strategyName: this.listQuery.strategyName || undefined,
          strategyType: this.listQuery.strategyType || undefined,
          status: this.listQuery.strategyStatus || undefined
        }
        const res = await getFundConcentrationStrategyPage(params)
        if (res.code === 1) {
          this.strategyList = res.data.tlist || []
          this.total = res.data.totalRecord || 0
          // 更新统计数据
          this.totalStrategies = this.total
          this.activeStrategies = this.strategyList.filter(s => s.status === 'ACTIVE').length
          this.inactiveStrategies = this.strategyList.filter(s => s.status === 'INACTIVE').length
        } else {
    ...
    async handleEnable(row) {
      this.$confirm('确认启用该归集策略?', '提示', {
        ...
        try {
          const res = await enableConcentrationStrategy(row.strategyId)
          if (res.code === 1) {
    ...
    async handleDisable(row) {
      this.$confirm('确认停用该归集策略?', '提示', {
        ...
        try {
          const res = await disableConcentrationStrategy(row.strategyId)
          if (res.code === 1) {
    ...
    async handleTest(row) {
      try {
        const res = await testConcentrationStrategy(row.strategyId)
        if (res.code === 1) {
          this.$message.success('策略测试已启动，请查看执行日志')
        } else {
    ...
    async handleBatchEnable() {
      ...
      this.$confirm(`确认批量启用选中的${this.multipleSelection.length}个策略?`, '提示', {
        ...
        try {
          const strategyIds = this.multipleSelection.map(s => s.strategyId)
          const res = await batchEnableConcentrationStrategy(strategyIds)
          if (res.code === 1) {
    ...
    handleExport() {
      const params = {
        strategyName: this.listQuery.strategyName || undefined,
        strategyType: this.listQuery.strategyType || undefined,
        strategyStatus: this.listQuery.strategyStatus || undefined
      }
      exportConcentrationStrategy(params).then(res => {
        const blob = new Blob([res], { type: 'application/vnd.ms-excel' })
        const url = window.URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url
        a.download = '归集策略配置_' + new Date().getTime() + '.xlsx'
        a.click()
    ...
    handleDelete(row) {
      this.$confirm('确认删除该归集策略?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        try {
          const res = await deleteConcentrationStrategy(row.strategyId)
          if (res.code === 1) {
    ...
    handleBatchDelete() {
      ...
      this.$confirm(`确认批量删除选中的 ${this.multipleSelection.length} 个策略?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        try {
          const strategyIds = this.multipleSelection.map(s => s.strategyId)
          const res = await batchDeleteConcentrationStrategy(strategyIds)
          if (res.code === 1) {
    ...
    async createData() {
      this.$refs['dataForm'].validate(async (valid) => {
        if (valid) {
          try {
            const res = await createFundConcentrationStrategy(this.temp)
            if (res.code === 1) {
              this.dialogFormVisible = false
              this.$message.success('归集策略创建成功')
              this.getList()
            } else {
    ...
    async updateData() {
      this.$refs['dataForm'].validate(async (valid) => {
        if (valid) {
          try {
            const res = await updateFundConcentrationStrategy(this.temp)
            if (res.code === 1) {
              this.dialogFormVisible = false
              this.$message.success('归集策略更新成功')
              this.getList()
            } else {
    ...
    getStrategyTypeText(type) {
      ...
      return textMap[type] || textMap[type && type.toUpperCase()] || '未知类型'
    },
    ...
    getStatusTagType(status) {
      ...
      const key = status != null ? String(status).toUpperCase() : ''
      return typeMap[key] || typeMap[String(status)] || 'info'
    },
    getStatusText(status) {
      ...
      const key = status != null ? String(status).toUpperCase() : ''
      return textMap[key] || textMap[String(status)] || '未知状态'
    },
    formatDate(val) {
      if (!val) return '未执行'
      // 兼容时间戳和字符串，只取年月日部分
      ...
      if (isNaN(d.getTime())) {
        // 如果是 "2024-01-01 12:00:00" 这类字符串，直接截取前10位
