...
<script>
import { listRiskType, getRiskType, delRiskType, addRiskType, updateRiskType, toggleRiskTypeStatus, getRiskStatistics, exportRiskType } from "@/api/globalTreasurer/fxgl"
...
export default {
  ...
  methods: {
    ...
    // 行点击事件
    handleRowClick(row) {
      this.$refs.riskTypeTable.toggleRowSelection(row);
    },
    /** 新增按钮操作 */
    handleAdd() {
      this.reset();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.$confirm('是否确认导出风险类型数据？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        exportRiskType(this.queryParams).then(response => {
          const blob = new Blob([response], {
            type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
          })
          const link = document.createElement('a')
          link.href = URL.createObjectURL(blob)
          link.download = `risk_type_${new Date().getTime()}.xlsx`
    ...
    loadStatistics() {
      getRiskStatistics().then(response => {
        ...
        // 兼容两种返回格式：mock格式(totalRiskTypes)和后端格式(total)
        this.statistics = {
          totalRiskTypes: Number(data.totalRiskTypes || data.total) || 0,
          enabledRiskTypes: Number(data.enabledRiskTypes || data.enabled) || 0,
          highRiskTypes: Number(data.highRiskTypes || (data.levelDistribution || {}).HIGH) || 0,
          criticalRiskTypes: Number(data.criticalRiskTypes || data.disabled) || 0,
    ...
    renderCategoryChart() {
      ...
      const distribution = this.statistics.categoryDistribution || {};
    ...
    renderTrendChart() {
      ...
      const trendData = this.statistics.trendData || [];
      if (trendData.length > 0) {
        // 有趋势数据时渲染折线图
        const months = trendData.map(item => item.month);
        const counts = trendData.map(item => Number(item.count) || 0);
        this.trendChartInstance.setOption({
          title: { text: '风险类型数量趋势', left: 'center' },
        ...
        const categories = Object.keys(distribution).map(key => levelMap[key] || key);
        const values = Object.keys(distribution).map(key => Number(distribution[key]) || 0);
        const colors = Object.keys(distribution).map(key => {
          if (key === 'HIGH') return '#F56C6C';
          if (key === 'MEDIUM') return '#E6A23C';
          return '#67C23A';
        });
        this.trendChartInstance.setOption({
