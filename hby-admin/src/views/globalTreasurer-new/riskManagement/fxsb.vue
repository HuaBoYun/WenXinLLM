<template>
  <div class="risk-identification-container">
    ...
    <!-- 控制措施弹窗 -->
    <el-dialog title="控制措施" :visible.sync="controlDialogVisible" width="600px" append-to-body>
      <el-form ref="controlForm" :model="controlForm" label-width="100px">
        <el-form-item label="风险名称">
          <span>{{ controlForm.riskName }}</span>
        </el-form-item>
        <el-form-item label="控制措施" prop="controlMeasures">
          <el-input
            v-model="controlForm.controlMeasures"
            type="textarea"
            :rows="5"
            placeholder="请输入控制措施"
          />
        ...
        <el-form-item label="应对策略" prop="responseStrategy">
          <el-input
            v-model="controlForm.responseStrategy"
            type="textarea"
            :rows="3"
            placeholder="请输入应对策略"
          />
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" :loading="controlLoading" @click="submitControl">确 定</el-button>
    ...
    <el-dialog title="变更历史" :visible.sync="historyDialogVisible" width="800px">
      <el-table :data="historyList" border stripe>
        ...
        <el-table-column label="风险编号" prop="riskCode" width="150" />
...
import { getRiskIdentificationPage, createRiskIdentification, updateRiskIdentification, deleteRiskIdentification, exportRiskIdentification, batchAssessRiskIdentification, closeRiskIdentification, getRiskIdentificationHistory } from '@/api/globalTreasurer/fxgl'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      getRiskIdentificationPage(this.queryParams).then(response => {
        // 适配后端返回格式：data是数组，result包含分页信息
    ...
    handleUpdate(row) {
      if (!row || row instanceof Event) {
        ...
        row = this.riskList.find(item => item.riskId === this.ids[0])
        if (!row) {
    ...
    submitForm() {
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.riskId) {
            updateRiskIdentification(this.form).then(response => {
              this.$modal.msgSuccess('修改成功')
              this.open = false
              this.getList()
            })
          } else {
            createRiskIdentification(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const riskIds = row.riskId ? [row.riskId] : this.ids
      if (!riskIds || riskIds.length === 0) {
        this.$modal.msgWarning('请先选择要删除的记录')
        return
      }
      this.$modal.confirm('是否确认删除选中的' + riskIds.length + '条数据？').then(() => {
        const promises = riskIds.map(id => deleteRiskIdentification(id))
    ...
    /** 导出按钮操作 */
    handleExport() {
      exportRiskIdentification(this.queryParams).then(response => {
        const blob = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
        const url = window.URL.createObjectURL(blob)
        const link = document.createElement('a')
        link.href = url
        link.download = `risk_identification_${new Date().getTime()}.xlsx`
    ...
    handleBatchAssess() {
      if (this.ids.length === 0) {
        this.$modal.msgWarning('请先选择要评估的记录')
        return
      }
      this.$modal.confirm('是否确认对所有选中的' + this.ids.length + '条风险进行评估？').then(() => {
        batchAssessRiskIdentification({ ids: this.ids }).then(res => {
          const data = res.data
          this.$modal.msgSuccess('批量评估完成，成功' + (data.successCount || 0) + '条，失败' + (data.failCount || 0) + '条')
          this.getList()
        })
      }).catch(() => {})
    },
    ...
    /** 行点击事件 */
    handleRowClick(row) {
      this.$refs.riskTable.toggleRowSelection(row)
    },
    /** 更多操作命令 */
    ...
    handleAssess(row) {
      // 先将风险状态更新为"评估中"
      updateRiskIdentification({ riskId: row.riskId, status: 'ASSESSING' }).then(() => {
        this.$modal.msgSuccess('已进入评估流程')
        // 跳转到风险评估页面，传递风险识别ID和风险类型
        this.$router.push({ path: '/globalTreasurerRisk/fxpg', query: { riskIdentificationId: row.riskId, riskType: row.riskType } })
      }).catch(() => {})
    },
    /** 控制措施 */
    handleControl(row) {
    ...
    submitControl() {
      ...
      updateRiskIdentification({
        riskId: this.controlForm.riskId,
        controlMeasures: this.controlForm.controlMeasures,
        responseStrategy: this.controlForm.responseStrategy
      }).then(() => {
        this.$modal.msgSuccess('控制措施保存成功')
    ...
    /** 变更历史 */
    handleHistory(row) {
      getRiskIdentificationHistory(row.riskId).then(res => {
    ...
    handleClose(row) {
      this.$prompt('请输入关闭原因', '关闭风险', {
        ...
        closeRiskIdentification(row.riskId, { reason: value }).then(() => {
