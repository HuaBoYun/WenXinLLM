...
<script>
import { getBillStatisticsOverview, getBillTypeStatistics, getBillTrendAnalysis, exportBillStatistics } from '@/api/globalTreasurer/pzgl'
...
export default {
  ...
  methods: {
    ...
    /**
     * 构建查询参数（将数组转为逗号分隔字符串，dateRange转为startDate/endDate）
     */
    buildQueryParams(extra = {}) {
      ...
      if (this.queryForm.dateRange && this.queryForm.dateRange.length === 2) {
        ...
        params.startDate = fmt(this.queryForm.dateRange[0])
        params.endDate = fmt(this.queryForm.dateRange[1])
      }
      // 数组转逗号分隔字符串
      if (this.queryForm.billType && this.queryForm.billType.length > 0) {
        params.billType = this.queryForm.billType.join(',')
      }
      if (this.queryForm.billStatus && this.queryForm.billStatus.length > 0) {
        params.billStatus = this.queryForm.billStatus.join(',')
      }
      return params
    },
    ...
    loadChartData() {
      ...
      getBillTypeStatistics(params).then(response => {
    ...
    loadTrendData() {
      ...
      getBillTrendAnalysis(params).then(response => {
        if (response.code === 1 && response.data) {
          this.trendData = {
        ...
        this.trendData = { dates: [], countData: [], amountData: [] }
        this.updateTrendChart()
      })
    },
    ...
    updateTrendChart() {
      const { dates, countData, amountData } = this.trendData
    ...
    loadStatisticsData() {
      ...
      // 调用后端API获取统计数据
      getBillStatisticsOverview(this.buildQueryParams()).then(response => {
        if (response.code === 1 && response.data) {
          // 更新统计概览数据
          ...
          // 更新表格数据
          this.statisticsTableData = data.tlist || []
        }
    ...
    handleExportReport() {
      ...
      exportBillStatistics(params).then(response => {
        const blob = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
        const link = document.createElement('a')
        link.href = URL.createObjectURL(blob)
        link.download = '票据统计报表_' + new Date().getTime() + '.xlsx'
        link.click()
