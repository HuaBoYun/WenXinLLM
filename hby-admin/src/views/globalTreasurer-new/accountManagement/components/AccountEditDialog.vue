...
export default {
  ...
  watch: {
    ...
    dialogVisible(val) {
      if (val) {
        // 等 formData prop 也更新完再初始化，避免时序问题
        this.$nextTick(() => {
          this.initForm()
        })
      }
      ...
      checkAccountNumberUnique(value, orgId, excludeId).then(response => {
        ...
        if (response && response.code === 200) {
          if (response.data === true || response.data === 'true') {
            ...
            // data 格式异常，默认通过验证（避免阻塞用户操作）
            console.warn('验证返回数据格式异常:', response.data)
            callback()
          }
        } else {
          // 后端返回业务错误码，默认通过验证（避免阻塞用户操作）
          console.warn('验证接口返回非 200 状态:', response)
          callback()
        }
      }).catch(error => {
        // 网络错误或异常，默认通过验证（避免阻塞用户操作）
        console.error('验证账户号码异常:', error)
        callback()
      })
    },
    handleConfirm() {
