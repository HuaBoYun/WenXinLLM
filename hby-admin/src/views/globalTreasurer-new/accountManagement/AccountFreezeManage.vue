...
<script>
import {
  getAccountFreezePage,
  getAccountFreezeStatistics,
  getAccountFreezeById,
  createAccountFreeze,
  unfreezeAccountRecord,
  deleteAccountFreeze,
...
export default {
  ...
  methods: {
    async getList() {
      ...
      try {
        const response = await getAccountFreezePage(this.listQuery)
        if (response && response.code === 1) {
          const data = response.data || response.date
    ...
    async updateStatistics() {
      try {
        const response = await getAccountFreezeStatistics()
        if (response && response.code === 1) {
          const stats = response.data || response.date
          this.totalRecords = stats.totalRecords || 0
          this.frozenRecords = stats.frozenRecords || 0
          this.unfrozenRecords = stats.unfrozenRecords || 0
          this.unfreezeRate = stats.unfreezeRate || 0
          this.totalFrozenAmount = stats.totalFrozenAmount || 0
        }
      } catch (error) {
    ...
    async loadAvailableAccounts() {
      try {
        ...
        if (response && response.code === 1) {
          const data = response.data || response.date
          const allAccounts = data.tlist || data || []
          // 只显示未冻结的账户（排除已冻结状态）
          this.availableAccounts = allAccounts.filter(account => account.accountStatus !== 'FROZEN')
        }
      } catch (error) {
    ...
    async handleDelete(row, index) {
      this.$confirm('确认删除该冻结记录?', '提示', {
        ...
        try {
          const response = await deleteAccountFreeze(row.recordId)
          if (response && response.code === 1) {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    async handleBatchUnfreeze() {
      ...
      this.$confirm(`确认批量解冻选中的${frozenRecords.length}个冻结记录?`, '提示', {
        ...
        try {
          const recordIds = frozenRecords.map(r => r.recordId)
          const response = await batchUnfreezeAccounts({ recordIds })
          if (response && response.code === 1) {
            this.$message({
              type: 'success',
              message: '批量解冻成功!'
            ...
            recordIds.forEach(id => {
              const item = this.list.find(r => r.recordId === id)
              if (item) {
                item.freezeStatus = 'UNFROZEN'
                item.unfreezeDate = new Date().toISOString().slice(0, 10)
              }
            })
          ...
          this.$message.error('批量解冻失败: ' + (error.message || '网络错误'))
        }
      })
    },
    async handleExport() {
      try {
        ...
        const params = {
          recordId: this.listQuery.recordId || undefined,
          accountNumber: this.listQuery.accountNumber || undefined,
          freezeType: this.listQuery.freezeType || undefined,
          freezeStatus: this.listQuery.freezeStatus || undefined
        }
        const data = await exportAccountFreeze(params)
        const blob = new Blob([data], {
          type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        })
        const fileName = '账户冻结记录.xlsx'
    ...
    async createData() {
      this.$refs['dataForm'].validate(async (valid) => {
        if (valid) {
          try {
            const response = await createAccountFreeze(this.temp)
            if (response && response.code === 1) {
    ...
    async submitUnfreeze() {
      ...
      try {
        const params = {
          unfreezeType: this.unfreezeForm.unfreezeType,
          unfreezeAmount: this.unfreezeForm.unfreezeAmount,
          unfreezeReason: this.unfreezeForm.unfreezeReason
        }
        const response = await unfreezeAccountRecord(this.currentRecord.recordId, params)
        if (response && response.code === 1) {
          ...
          this.$message.error(response.msg || '解冻失败')
        }
      } catch (error) {
        console.error('解冻失败:', error)
        this.$message.error('解冻失败: ' + (error.message || '网络错误'))
      }
    },
    resetTemp() {
      this.temp = {
