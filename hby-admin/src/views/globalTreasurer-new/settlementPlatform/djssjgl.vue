  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="pendingDataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="业务类型" align="center" prop="businessType" width="120">
        <template slot-scope="scope">
          <span>{{ businessTypeMap[scope.row.businessType] || scope.row.businessType }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="币种" align="center" prop="currencyCode" width="80">
        <template slot-scope="scope">
          <span>{{ currencyCodeMap[scope.row.currencyCode] || scope.row.currencyCode }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="结算状态" align="center" prop="settlementStatus" width="100">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.settlementStatus === 'PENDING'" type="info">待处理</el-tag>
          <el-tag v-else-if="scope.row.settlementStatus === 'APPROVED'" type="success">已审批</el-tag>
          <el-tag v-else-if="scope.row.settlementStatus === 'PROCESSING'" type="warning">处理中</el-tag>
          <el-tag v-else-if="scope.row.settlementStatus === 'SETTLED'" type="success">已结算</el-tag>
          <el-tag v-else-if="scope.row.settlementStatus === 'FAILED'" type="danger">失败</el-tag>
          <span v-else>{{ scope.row.settlementStatus }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="审批状态" align="center" prop="approvalStatus" width="100">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.approvalStatus === 'DRAFT'" type="info">草稿</el-tag>
    ...
    <el-dialog title="待结算数据详情" :visible.sync="detailOpen" width="800px" append-to-body>
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="业务类型">{{ businessTypeMap[detailData.businessType] || detailData.businessType }}</el-descriptions-item>
        <el-descriptions-item label="结算金额">{{ detailData.settlementAmount }}</el-descriptions-item>
        <el-descriptions-item label="币种">{{ currencyCodeMap[detailData.currencyCode] || detailData.currencyCode }}</el-descriptions-item>
    ...
    <el-dialog title="审批待结算数据" :visible.sync="approvalOpen" width="600px" append-to-body>
      <el-form ref="approvalForm" :model="approvalForm" :rules="approvalRules" label-width="100px">
        ...
        <el-form-item label="审批备注" prop="approvalOpinion">
          <el-input v-model="approvalForm.approvalOpinion" type="textarea" placeholder="请输入审批备注" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  created() {
    console.log('===== djssjgl.vue created =====');
    console.log('getPendingDataPage函数:', getPendingDataPage);
    this.getList();
  },
  methods: {
    /** 添加日期范围参数 */
    ...
    getList() {
      ...
      getPendingDataPage(params).then(response => {
        ...
        if (response && response.data && response.data.rows) {
          // 结构1: {code: 1, data: {rows: [], total: 10}}
          listData = response.data.rows;
          totalCount = response.data.total;
          console.log('使用结构1解析数据成功');
        } else if (response && response.rows) {
          // 结构2: {code: 1, rows: [], total: 10}
          listData = response.rows;
          totalCount = response.total;
          console.log('使用结构2解析数据成功');
        } else if (Array.isArray(response)) {
          // 结构3: 直接返回数组
          listData = response;
          totalCount = response.length;
          console.log('使用结构3解析数据成功');
        }

        console.log('最终解析的数据列表:', listData);
        console.log('最终解析的总数:', totalCount);

        this.pendingDataList = listData || [];
        this.total = totalCount || 0;
        this.loading = false;
      }).catch(error => {
        console.error('查询待结算数据失败:', error);
    ...
    handleUpdate(row) {
      this.reset();
      const pendingId = (row && row.pendingId) ? row.pendingId : this.ids[0];
      getPendingDataById(pendingId).then(response => {
        this.form = response.data;
        // 将日期字段转为 yyyy-MM-dd 字符串，确保 el-date-picker 能正常回显
        if (this.form.expectedSettlementDate) {
          const d = new Date(this.form.expectedSettlementDate);
          this.form.expectedSettlementDate = d.getFullYear() + '-' +
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const pendingIds = (row && row.pendingId) ? row.pendingId : this.ids.join(',');
    ...
    handleExport() {
      const params = this.addDateRange(this.queryParams, this.dateRange);
      this.$modal.confirm('是否确认导出所有待结算数据?').then(() => {
        exportPendingData(params).then(response => {
          const blob = new Blob([response], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
          const url = window.URL.createObjectURL(blob);
          const link = document.createElement('a');
          link.href = url;
          link.download = `待结算数据_${new Date().getTime()}.xlsx`;
          link.click();
