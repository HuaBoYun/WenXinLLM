  ...
  <div class="option-transaction-container">
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column label="德尔塔(Δ)" prop="delta" width="100" align="right">
          <template slot-scope="scope">
            <span class="greek-text">{{ formatGreek(scope.row.delta) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="伽马(Γ)" prop="gamma" width="100" align="right">
          <template slot-scope="scope">
            <span class="greek-text">{{ formatGreek(scope.row.gamma) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="西塔(Θ)" prop="theta" width="100" align="right">
          <template slot-scope="scope">
            <span class="greek-text">{{ formatGreek(scope.row.theta) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="维加(ν)" prop="vega" width="100" align="right">
          <template slot-scope="scope">
    ...
    <el-dialog title="期权交易详情" :visible.sync="detailOpen" width="1000px" append-to-body>
      <el-descriptions :column="3" border>
        ...
        <el-descriptions-item label="德尔塔(Δ)">{{ formatGreek(detailData.delta) }}</el-descriptions-item>
        <el-descriptions-item label="伽马(Γ)">{{ formatGreek(detailData.gamma) }}</el-descriptions-item>
        <el-descriptions-item label="西塔(Θ)">{{ formatGreek(detailData.theta) }}</el-descriptions-item>
        <el-descriptions-item label="维加(ν)">{{ formatGreek(detailData.vega) }}</el-descriptions-item>
        <el-descriptions-item label="肉欧(ρ)">{{ formatGreek(detailData.rho) }}</el-descriptions-item>
      ...
      <el-form ref="pricingForm" :model="pricingForm" label-width="120px">
        ...
        <el-form-item label="计算结果" v-if="pricingResult">
          <el-descriptions :column="2" border size="small">
            ...
            <el-descriptions-item label="德尔塔(Δ)">{{ formatGreek(pricingResult.delta) }}</el-descriptions-item>
            <el-descriptions-item label="伽马(Γ)">{{ formatGreek(pricingResult.gamma) }}</el-descriptions-item>
            <el-descriptions-item label="西塔(Θ)">{{ formatGreek(pricingResult.theta) }}</el-descriptions-item>
            <el-descriptions-item label="维加(ν)">{{ formatGreek(pricingResult.vega) }}</el-descriptions-item>
            <el-descriptions-item label="肉欧(ρ)">{{ formatGreek(pricingResult.rho) }}</el-descriptions-item>
          </el-descriptions>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    getList() {
      ...
      getOptionTransactionPage(this.queryParams).then(response => {
        ...
        console.error('查询期权交易列表失败:', error)
        this.transactionList = []
        this.total = 0
        this.loading = false
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      // 行内删除用 row.transactionId，顶部批量删除用 this.ids（过滤掉空值）
      const rawIds = row && row.transactionId
        ? [row.transactionId]
        : (this.ids || []).filter(id => id != null && id !== '')
      if (!rawIds.length) {
        this.$modal.msgWarning('请先选择要删除的记录')
        return
      }
      this.$modal.confirm('是否确认删除期权交易编号为"' + rawIds.join(',') + '"的数据项？').then(() => {
        return deleteOptionTransaction(rawIds)
      }).then((res) => {
        if (res && res.code === 1) {
          this.getList()
    ...
    handleExport() {
      ...
      const query = Object.keys(params)
        ...
        .join('&')
      const url = `/vab-mock-server/qqsk/derivatives/option/export${query ? '?' + query : ''}`
      const link = document.createElement('a')
      link.href = url
      link.download = `期权交易_${new Date().getTime()}.xlsx`
    ...
    handleGreeksCalculation(row) {
      calculateGreeks(row.transactionId, {}).then(response => {
        ...
        this.$modal.msgSuccess(`希腊字母计算完成：
          德尔塔(Δ): ${this.formatGreek(greeks.delta)}
          伽马(Γ): ${this.formatGreek(greeks.gamma)}
          西塔(Θ): ${this.formatGreek(greeks.theta)}
          维加(ν): ${this.formatGreek(greeks.vega)}`)
      })
    },
    /** 行权处理 */
    handleExercise(row) {
    ...
    handleCancel(row) {
      console.log('=== 取消期权开始 ===')
      console.log('row 对象:', JSON.stringify(row))
      console.log('row.transactionId:', row.transactionId)
      console.log('row.transactionId 类型:', typeof row.transactionId)
      this.$prompt('请输入取消原因', '取消合约', {
        ...
        console.log('准备取消，transactionId:', transactionId)
        return cancelOption(transactionId, { reason: value.trim() })
      }).then(response => {
        console.log('取消期权响应:', response)
        this.$modal.msgSuccess('取消成功')
        this.getList()
      }).catch(error => {
        console.error('取消期权失败:', error)
