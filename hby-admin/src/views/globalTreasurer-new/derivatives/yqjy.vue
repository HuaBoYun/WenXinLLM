  ...
  <div class="forward-transaction-container">
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column label="币种" prop="currency" width="100" align="center">
          <template slot-scope="scope">
            {{ {'CNY':'人民币','USD':'美元','EUR':'欧元','JPY':'日元','GBP':'英镑','HKD':'港币'}[scope.row.currency] || scope.row.currency }}
          </template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            ...
            <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)" style="margin-left: 10px">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="execute" v-if="scope.row.status === 'PENDING'" icon="el-icon-check">执行</el-dropdown-item>
                <el-dropdown-item command="settle" v-if="scope.row.status === 'ACTIVE'" icon="el-icon-finished">交割</el-dropdown-item>
    ...
    <el-dialog title="远期交易详情" :visible.sync="detailOpen" width="1000px" append-to-body>
      <el-descriptions :column="3" border>
        ...
        <el-descriptions-item label="币种">{{ {'CNY':'人民币','USD':'美元','EUR':'欧元','JPY':'日元','GBP':'英镑','HKD':'港币'}[detailData.currency] || detailData.currency }}</el-descriptions-item>
...
import {
  ...
  exportForwardTransaction,
  batchValuateForwardContract,
  executeForwardContract,
  cancelForwardContract,
  ...
  methods: {
    ...
    getList() {
      ...
      getForwardTransactionPage(this.queryParams).then(response => {
        ...
        this.transactionList = (data && data.records) || []
    ...
    handleSearch() {
      ...
      if (this.searchForm.maturityDateRange && this.searchForm.maturityDateRange.length === 2) {
        ...
        this.queryParams.maturityDateStart = null
        this.queryParams.maturityDateEnd = null
      }
      // 复制其他搜索字段（排除 maturityDateRange）
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除远期交易编号为"' + ids.join(',') + '"的数据项？').then(function() {
        return deleteForwardTransaction(ids)
      }).then(() => {
        this.getList()
    ...
    handleExport() {
      ...
      const query = Object.keys(params)
        ...
        .join('&')
      const url = `/vab-mock-server/qqsk/derivatives/forward/export${query ? '?' + query : ''}`
      const link = document.createElement('a')
      link.href = url
      link.download = `远期交易_${new Date().getTime()}.xlsx`
    ...
    /** 批量估值 */
    handleValuation() {
      const ids = this.ids && this.ids.length ? this.ids : []
      const msg = ids.length ? `是否确认对已选中的 ${ids.length} 条远期合约进行估值？` : '未选择记录，是否对所有生效中/已执行的合约进行批量估值？'
      this.$modal.confirm(msg).then(() => {
        return batchValuateForwardContract(ids)
      }).then(response => {
        if (response && (response.code === 1 || response.code === '1')) {
          this.$modal.msgSuccess(response.msg || '批量估值完成')
          this.getList()
        }
      }).catch(() => {})
    },
      ...
      getForwardRates({ currency: row.currency }).then(response => {
        if (response && response.code === 1 && response.data) {
          ...
          const lines = Object.keys(data)
            ...
            .map(k => `${k}：${data[k]}`)
            .join('\n')
          this.$alert(lines || '暂无汇率数据', '远期汇率', {
            confirmButtonText: '确定',
            customClass: 'rates-alert'
          })
        }
      })
    },
    ...
    handleCancel(row) {
      this.$prompt('请输入取消原因', '取消合约', {
        ...
        console.log('取消远期，transactionId:', transactionId)
        return cancelForwardContract(transactionId, { reason: value.trim() })
      }).then(response => {
        console.log('取消远期响应:', response)
        this.$modal.msgSuccess('取消成功')
        this.getList()
      }).catch(error => {
        console.error('取消远期失败:', error)
