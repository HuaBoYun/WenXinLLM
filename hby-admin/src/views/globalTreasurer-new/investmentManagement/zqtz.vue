  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="bondList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="债券类型" align="center" prop="bondType">
        <template slot-scope="scope">
          <span>{{ getBondTypeLabel(scope.row.bondType) }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="当前市值" align="center" prop="currentMarketValue">
        <template slot-scope="scope">
          <span>{{ parseFloat(scope.row.currentMarketValue || 0).toLocaleString() }}</span>
        </template>
      </el-table-column>
      <el-table-column label="未实现盈亏" align="center" prop="unrealizedPnl">
        <template slot-scope="scope">
          <span>{{ parseFloat(scope.row.unrealizedPnl || 0).toLocaleString() }}</span>
        </template>
      </el-table-column>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="信用评级" prop="creditRating">
              <el-input v-model="form.creditRating" placeholder="请输入信用评级(可选)" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
    ...
    <el-dialog title="债券投资详情" :visible.sync="viewOpen" width="800px" append-to-body>
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="债券类型">{{ getBondTypeLabel(viewForm.bondType) }}</el-descriptions-item>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      bondInvestmentApi.getList(this.queryParams).then(response => {
        if (response && response.code === 1) {
          this.bondList = response.data?.rows || response.data?.tlist || [];
          this.total = parseInt(response.data?.total) || response.data?.totalRecord || 0;
        } else {
          this.bondList = [];
          this.total = 0;
          this.$message.error(response?.msg || '查询债券投资列表失败');
        }
        this.loading = false;
      }).catch(error => {
        console.error('查询债券投资列表失败:', error);
        this.$message.error('查询债券投资列表失败,请稍后重试');
        this.bondList = [];
        this.total = 0;
        this.loading = false;
      });
    ...
    async handleExport() {
      ...
      try {
        const response = await request({
          method: 'get',
          url: '/qqsk/investment/bond/export',
        ...
        link.href = url;
        link.download = `债券投资列表_${new Date().getTime()}.xlsx`;
        link.click();
    ...
    /** 获取债券类型标签 */
    getBondTypeLabel(bondType) {
      const option = this.bondTypeOptions.find(item => item.value === bondType);
      return option ? option.label : bondType;
    }
  }
};
</script>
