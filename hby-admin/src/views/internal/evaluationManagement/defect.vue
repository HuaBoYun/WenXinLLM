<template>
  <div class="system-log-container">
    ...
    />
    <DefectView ref="edit" @fetch-data="fetchData" />
...
<script>
  import { getHomepageList, deleteQx, exportPjqx } from '@/api/internal/project'

  import filterSearch from '@/components/filterSearch.vue'
  import filterTable from '@/components/filterTable.vue'
  import DefectView from '@/views/internal/evaluationManagement/components/defectView'
  ...
  export default {
    ...
    methods: {
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(208, row.id)
      },
      ...
      getFiled() {
        let fields = [
          { name: '缺陷编号', key: 'defectscode' },
          { name: '缺陷名称', key: 'defectsname' },
        ]
        return fields
      },
      initSearch() {
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg, code } = await deleteQx({ id: row.id })
          if (code == 1) {
