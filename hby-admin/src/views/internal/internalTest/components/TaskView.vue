    ...
    <div class="lr-layout">
      ...
      <div class="right">
        <el-row class="box_row flex">
          <el-button type="primary" @click="sumAll()">全部提交</el-button>
...
<script>
  ...
  import { getLeftTree, getRightList, saveall } from '@/api/internal/tack'
  ...
  export default {
    ...
    methods: {
      ...
      async sumAll() {
        const { code, msg } = await saveall({ planid: this.queryForm.planid })
        if (code == 1) {
      ...
      async fetchData(params = {}) {
        ...
        } = await getRightList({ ...this.queryForm, ...params })
        this.list = list
        this.total = total
        this.planid = planid
