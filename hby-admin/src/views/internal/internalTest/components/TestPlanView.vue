    ...
    <el-row :gutter="14">
      ...
      >
        ...
        <el-col :span="12" v-if="disabled">
          <el-form-item label="下发人员" prop="issuedStaffName">
            <el-input
              v-model.trim="form.issuedStaffName"
              placeholder="请输入计划名称"
            />
          </el-form-item>
        </el-col>
...
<script>
  ...
  import { download } from '@/api/internal/project.js'
  ...
  import TemplateEditStep2 from '@/views/workbench/internalTools/components/TemplateEditStep2.vue'
  export default {
    name: 'PlanView',
    components: {
      ExecutorOptions,
    ...
    async created() {
      ...
      if (this.showMJ) {
        // 获取密级,菜单id
        const res = await hasMJ(this.menuKey)
    ...
    methods: {
      ...
      async getInfo(row) {
        const { data, code, msg } = await groupDetail({
          selectProjectid: Number(row.id),
        })
        if (code == 1) {
          const { id, orgid, testtemple, starttime, endtime, ...other } =
            data.test
          this.form = {
            ...other,
            id,
      ...
      async handleTemplateSelected(node) {
        console.log('🚀 ~ handleTemplateSelected ~ node:', node)
      ...
      handleCompanyTreeSelected(node) {
        if (node instanceof Array) {
          const { testedorgs, ...other } = this.form
          let form = {
            testedorgs: node.map((item) => item.label).join(','),
            ...other,
          }
          this.form = form
        } else {
      ...
      handleChooseCompany(node) {
        console.log('🚀 ~ handleChooseCompany ~ node:', node)
        this.$set(
          this.form,
          'testedorgs',
      ...
      //模板明细查看详情
      handleView(row, type) {
        this.$refs['step2'].show(row, this.form.testtemid, type)
      },
    },
  }
</script>
