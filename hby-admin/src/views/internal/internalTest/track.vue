...
<script>
  ...
  import { getList } from '@/api/internal/track'
  ...
  import TrackView from '@/views/internal/internalTest/components/TrackView'
  import PlanView from '@/views/internal/internalTest/components/PlanView'

  export default {
    name: 'Track',
    // components: {Track},
    components: { TrackView, PlanView, filterTable, filterSearch },
    data() {
      return {
        list: [],
        listLoading: true,
    ...
    methods: {
      ...
      handleDeatil(row) {
        ...
        this.$refs['testListInfo'].showEdit(row, true)
      },
      handleEdit(row) {
