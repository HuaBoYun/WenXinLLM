  ...
  <div class="system-log-container">
      ...
      <el-card shadow="never">
        <vab-query-form-left-panel :span="24">
          ...
          >
            ...
            >
              <el-input
                v-model="queryForm.administratorName"
                clearable
                :placeholder="$translateTitle('管理员名称')"
                v-if="item.name === '管理员名称'"
    ...
    <el-card shadow="never" class="secondCard">
      <vab-query-form-right-panel :span="24">
        ...
        <el-button type="success" @click="handleExp">
          {{ $translateTitle('导出') }}
        </el-button>
      ...
      <el-table v-loading="listLoading" :data="list">
        ...
        <div v-for="(item, index) in filedNow" :key="index">
          ...
          <el-table-column
            align="center"
            :label="$translateTitle('管理员名称')"
            prop="administratorName"
            v-if="item.name === '管理员名称'"
          ></el-table-column>
          <el-table-column
            align="center"
            :label="$translateTitle('联系方式')"
...
<script>
  ...
  import Views from './components/hgglyxxglView.vue'
  import { getXXGLList, XXGLDelete, exphgglyList } from '@/api/hggl/hgjhgl'
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'internal-hggl-hgglyxxgl-search',
        tableKey: 'internal-hggl-hgglyxxgl-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
    ...
    methods: {
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(69, row.id)
      },
      ...
      // 定义表单所有项
      getFiled() {
        let fields = [{ name: '管理员名称', key: 'administratorName' }]
        return fields
      },
      initSearch() {
        let self = this
      ...
      async handleExp() {
        ...
        const params = {
          ...
          administratorName: this.queryForm.administratorName,
        }
        const res = await exphgglyList(params)
        this.listLoading = false
        console.log('res', res)
        downloadFile(res, '合规管理员信息管理.xlsx')
        // let filename = '合规管理员信息管理.xls'
      ...
      fetchData(type) {
        ...
        getXXGLList(this.queryForm).then((res) => {
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg } = await XXGLDelete({ id: row.id })
