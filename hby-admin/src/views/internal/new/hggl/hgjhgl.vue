...
<script>
  ...
  import Views from './components/hgjhglView.vue'
  import { getJHGLList, HGJHDelete } from '@/api/hggl/hgjhgl'
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'internal-hggl-hgjhgl-search',
        tableKey: 'internal-hggl-hgjhgl-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
    ...
    methods: {
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(63, row.id)
      },
      ...
      // 定义表单所有项
      getFiled() {
        let fields = [{ name: '计划名称', key: 'planName' }]
        return fields
      },
      initSearch() {
        let self = this
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg } = await HGJHDelete({ id: row.id })
