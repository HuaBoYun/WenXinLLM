          ...
          >
            ...
            >
              <el-input
                ...
                clearable
                :placeholder="$translateTitle('岗位名称')"
                v-if="item.name === '岗位名称'"
...
<script>
  ...
  import Views from './components/zdgwhgzrView.vue'
  import { getHGZRList, HGZRDelete, downloadExpress } from '@/api/hggl/hgjhgl'
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'internal-hggl-zdgwhgzr-search',
        tableKey: 'internal-hggl-zdgwhgzr-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
    ...
    methods: {
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(68, row.id)
      },
      ...
      // 定义表单所有项
      getFiled() {
        let fields = [{ name: '岗位名称', key: 'postName' }]
        return fields
      },
      initSearch() {
        let self = this
      ...
      fetchData(type) {
        ...
        if (type && type === 'reset') {
          ...
          this.queryForm.postName = ''
        }
        getHGZRList(this.queryForm).then((res) => {
      ...
      async handleExp() {
        ...
        const res = await downloadExpress(params)
        this.listLoading = false
        downloadFile(res, '重点岗位合规责任')
        console.log(res)
      },
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg } = await HGZRDelete({ id: row.id })
