      ...
      <el-card shadow="never">
        <vab-query-form-left-panel :span="24">
          ...
          >
            ...
            >
              <el-input
                v-model="queryForm.manualName"
                clearable
                :placeholder="$translateTitle('手册名称')"
                v-if="item.name === '手册名称'"
...
<script>
  ...
  import Views from './components/hgscglView.vue'
  import { getSCGLList, SCGLDelete } from '@/api/hggl/hgjhgl'
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'internal-hggl-hgscgl-search',
        tableKey: 'internal-hggl-hgscgl-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
    ...
    methods: {
      handleApproval(row) {
        //提交审批
        this.$refs['process'].save(64, row.id)
      },
      ...
      // 定义表单所有项
      getFiled() {
        let fields = [{ name: '手册名称', key: 'manualName' }]
        return fields
      },
      initSearch() {
        let self = this
      ...
      fetchData(type) {
        ...
        getSCGLList(this.queryForm).then((res) => {
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg } = await SCGLDelete({ id: row.id })
