<template>
  <div class="system-log-container">
        ...
        <vab-query-form-left-panel :span="24">
          ...
          >
            <el-form-item v-for="(item, index) in searchItem" :key="index">
              <el-select
                v-model="queryForm.rectificationState"
                clearable
                :placeholder="$translateTitle('整改状态')"
      ...
      <el-table v-loading="listLoading" :data="list">
        ...
        <div v-for="(item, index) in filedNow" :key="index">
          <el-table-column
            ...
            prop="inspectImp.questionType"
            v-if="item.name === '问题类型'"
          />
          <el-table-column
            align="center"
            :label="$translateTitle('发现时间')"
            prop="inspectImp.discoverTime"
            v-if="item.name === '发现时间'"
          />
          <el-table-column
            align="center"
            :label="$translateTitle('业务领域')"
            prop="inspectImp.businessArea"
            v-if="item.name === '业务领域'"
          />
          <el-table-column
            align="center"
            :label="$translateTitle('描述')"
            prop="inspectImp.describe"
            v-if="item.name === '描述'"
          ...
          >
            <template #default="{ row }">
              <span>
                {{ row.rectificationState == 0 ? '未完成' : '已完成' }}
              </span>
            </template>
          </el-table-column>
          <el-table-column
    ...
    />
    <jghzTable ref="table" @selected="handleTable" />
    <jghzEdit ref="edit" @fetch-data="fetchData" />
...
<script>
  ...
  import {
    rectificationctrltestPlanList,
    deleteRectification,
    startPlan,
    rectificationSaveOrUpdate,
  ...
  import jghzTable from '@/views/internal/new/hgjc/components/jghzTable'
  import TestListInfo from '@/views/internal/new/hgjc/components/TestListInfo'
  import jghzEdit from '@/views/internal/new/hgjc/components/jghzEdit'
  ...
  export default {
    ...
    data() {
      return {
        ...
        localKey: 'internal-internalTest-jghz-search',
        tableKey: 'internal-internalTest-jghz-list',
        searchNow: [], //当前所有搜索项
        searchItem: [], //可见搜索项
        searchMore: true,
      }
    ...
    methods: {
      handleApproval(row) {
        this.$refs['process'].save(83, row.id)
      },
      ...
      // 定义表单所有项
      getFiled() {
        let fields = [{ name: '整改状态', key: 'rectificationState' }]
        return fields
      },
      initSearch() {
        let self = this
      ...
      async fetchData() {
        ...
        } = await rectificationctrltestPlanList({ ...this.queryForm })
        console.log(tlist)
        this.list = tlist
      ...
      async handleTable(val) {
        const { msg, code, data } = await rectificationSaveOrUpdate({
          impId: val.id,
          rectificationState: 0,
        })
        if (code == 1) {
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { msg, code, data } = await deleteRectification({
            id: row.id,
          })
          if (code == 1) {
