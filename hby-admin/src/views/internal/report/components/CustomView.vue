...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      if (this.showMJ) {
        // 获取密级,菜单id
        const res = await hasMJ('ReportCustom')
    ...
    methods: {
      ...
      async getInfo(row, type) {
        const data = await modifyReport({ id: row.reportid })
        this.formData = data.data.report
        this.tableData = data.data.atts

        // if (
        //   (data.report.status == 2 || data.report.status == 3) &&
        //   type == 'edit'
        // ) {
        ...
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            if (this.formData.reportid) {
              ...
              const data = await isExistUpdate(
                {
                  ...other,
                  type: 'nk_zdy',
