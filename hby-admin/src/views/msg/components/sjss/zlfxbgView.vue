...
<script>
  import {
    qualityAnalysisReportDetail,
    qualityAnalysisReportUpdate,
  ...
  export default {
    ...
    methods: {
      ...
      async showEdit(
        ...
        this.status = status
        if (formId) {
          const detailData = await qualityAnalysisReportDetail({ id: formId })
          Object.assign(this.formData, detailData.data)
        }
      ...
      async save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            const res = await qualityAnalysisReportUpdate(params)
            if (res && res.code === 1) {
              this.$emit('fetchData')
              this.$message({
                message: '提交成功！',
