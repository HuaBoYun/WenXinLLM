<template>
  <div>
    <el-row :gutter="14" v-loading="loading">
      ...
      >
        <el-col :span="12">
          <el-form-item label="编号" prop="xtcode">
            <el-input
        ...
        <el-col :span="12">
          <el-form-item label="本次审计人员" prop="ext3">
            <el-input
              v-model="formData.ext3"
              clearable
              placeholder="请选择本次审计人员"
...
<script>
  ...
  import { xmglsjxmbDetail, xmglsjxmbSaveOrUpdate } from '@/oapi/audit/project'
  ...
  export default {
    components: { projectManage, DepartmentOptions, Resubmit },
    inheritAttrs: false,
    props: ['fetchData'],
    data() {
      return {
    ...
    methods: {
      ...
      handleDepartmentSelected(node) {
        this.$set(this.formData, `sgdw`, node.name)
      },
      getChildlistPro(val) {
        this.$set(this.formData, 'ext3', val[0].realname)
      },
      async showEdit(
        title,
        formId,
        ...
        if (formId) {
          const res = await xmglsjxmbDetail({ sjxmbid: formId })
      ...
      async save() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            const res = await xmglsjxmbSaveOrUpdate({
              ...params,
              attIds,
            })
            if (res.code == 1) {
