  ...
  <div>
    <el-row :gutter="14">
      ...
      >
        ...
        <el-col :span="12">
          <el-form-item label="实施单位" prop="completeBelongGroupName">
            <el-input
        ...
        <el-col :span="12">
          <el-form-item label="副组长" prop="implementationPlanTeamfzzname">
            <el-input
              v-model="formData.implementationPlanTeamfzzname"
              :style="{ width: '75%' }"
              disabled
              placeholder="请选择组长"
            />
        ...
        <el-col :span="24">
          <el-form-item label="项目主要成果及特点" prop="mainFeatures1">
            <el-input
              v-model="formData.mainFeatures1"
              :style="{ width: '100%' }"
              clearable
              placeholder="提示，该内容自动汇总到项目申报汇总，字数限制在400字以内"
              type="textarea"
              :disabled="!disabled"
              maxlength="400"
              show-word-limit
...
<script>
  ...
  import { getXmpysbDetail, addOrUpdateXmpysb } from '@/oapi/audit/xmpy'
  ...
  export default {
    ...
    data() {
      return {
        ...
        formData: {
          ...
          implementationPlanTeamfzzname: '',
          implementationPlanMainReviewer: '',
          mainFeatures: undefined,
          mainFeatures1: undefined,
          content: undefined,
    ...
    methods: {
      ...
      async showEdit(
        ...
        this.status = status
        if (row) {
          const res = await getXmpysbDetail({ id: row })
      ...
      add() {
        this.$refs['ruleForm'].validate(async (valid) => {
          if (valid) {
            ...
            delete params.implementationPlanReviewers
            delete params.implementationPlanMainReviewer
            delete params.implementationPlanTeamLeader
            const data = await addOrUpdateXmpysb({
              ...params,
      ...
      getDepartmentInfo(val) {
        ...
        this.formData.approvalBelongGroupName = val.label
      },
      getDepartmentInfo1(val) {
        this.formData.completeBelongGroup = val.id
        this.formData.completeBelongGroupName = val.label
      },
      showPeople(type) {
      ...
      getProject(val) {
        console.log(val, 'val')
        this.formData.implementationProjectName = val.projectName
        ...
        this.formData.implementationPlanTeamfzzname = val.fzzstaffName
      },
      //提交
      async ymsubmit() {
        this.$refs['ruleForm'].validate(async (valid) => {
