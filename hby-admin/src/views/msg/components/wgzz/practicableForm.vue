  ...
  <div>
    <el-row :gutter="14">
      ...
      >
        ...
        <el-col :span="12">
          <el-form-item label="矿产资源,产能等(万吨)" prop="reimpl.minerals">
            <el-input
    ...
    <div style="text-align: right; margin-top: 10px" v-if="footer || isName">
...
<script>
  ...
  export default {
    name: 'asdf',
    components: { Resubmit, CompanySelectModal, selectTeam },
    props: [],
    data() {
      // 金额校验规则（整数或两位小数）
    ...
    methods: {
      showEdit(
        ...
        if (flowTaskNodeList.length > 0) {
          let obj = flowTaskNodeList[flowTaskNodeList.length - 1]
          if (obj.nodeName == '审计人员') {
            this.isName = true
          }
        }
      ...
      save() {
        this.$refs.elForm.validate(async (valid) => {
          if (valid) {
            ...
            try {
              const res = await saveRectificationImpl(obj)
              if (res.code === 1) {
