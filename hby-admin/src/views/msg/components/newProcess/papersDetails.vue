...
<script>
  ...
  import hzdg from './option/hZDGmodal.vue'
    ...
    methods: {
      ...
      async showEdit(
        ...
        if (row && row.sheet) {
          ...
          // 保存审批用的密级id
          if (row.sheet.secrectLevelId) {
            localStorage.setItem('SPsecrectLevelId', row.sheet.secrectLevelId)
          }
          this.formData = row.sheet
          this.formData.internalType = Number(row.sheet.internalType)
          this.formData.businessType =
            row.sheet.businessType && row.sheet.businessType.toString()
          this.formData.belongType =
            row.sheet.belongType && row.sheet.belongType.toString()
          this.formData.detailType =
            row.sheet.detailType && row.sheet.detailType.toString()
          this.formData.hgDetailType =
            row.sheet.hgDetailType && row.sheet.hgDetailType.toString()
          // this.formData.internalType =
          //   row.sheet.internalType &&
