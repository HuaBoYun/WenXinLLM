<template>
  <div class="wysx-container">
    <h3 v-if="showTitle" class="page-title">未阅事项</h3>
    ...
    <el-table v-loading="listLoading" border :data="list" style="width: 100%">
      ...
      >
        <template #default="{ row }">
          ...
          <el-button type="text" @click="handleOK(row)">确认接收</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      async handleOK(row) {
        try {
          ...
          if (res.code == 1) {
            ...
            const module = this.typeToModuleMapping.find((item) =>
