<template>
  <div>
     ...
     -->
      ...
      <el-col :span="24" style="margin-top: 20px">
        ...
        <el-table :data="tableData4">
          ...
          <el-table-column align="center" label="项目名称" prop="ldname">
            <template #default="{ row }">
              <el-button
                type="text"
          ...
          <el-table-column label="审计范围" prop="sjfwstarttime" width="220">
            <template #default="{ row }">
              {{ row.sjfwstarttime }} - {{ row.sjfwendtime }}
            </template>
          </el-table-column>
          <!-- <el-table-column
            label="委托时间"
    ...
    <rzsjmxView ref="rzsjmxView" @selected="handleTable1"></rzsjmxView>
    ...
    <LRJYJLRSJview ref="LRJYJLRSJview" @selected="handleTable1"></LRJYJLRSJview>
...
<script>
  ...
  import Table1 from '@/views/oilAudit/jhlx/components/lxjyzypgEdit'
  ...
  import rzsjmxView from '@/views/oilAudit/lrjjzr/components/rzsjmxView2.vue'
  ...
  export default {
    ...
    methods: {
      ...
      handleTable1(val) {
        console.log(val)
        if(this.source == 'tableData1') {
          const arr = this.tableData1.filter((res) => res.id != val.id)
          this.tableData1 = [...arr, val]
        } else if (this.source == 'tableData2') {
          const arr = this.tableData2.filter((res) => res.id != val.id)
          this.tableData2 = [...arr, val]
        } else if (this.source == 'tableData3') {
          const arr = this.tableData3.filter((res) => res.id != val.id)
          this.tableData3 = [...arr, val]
        } else if (this.source == 'tableData4') {
          const arr = this.tableData4.filter((res) => res.id != val.id)
          this.tableData4 = [...arr, val]
        }
      },
      table8Fetch(row) {
      ...
      async fetchData(formId) {
        const res = await jhgljhDetail({ jhid: formId })
        if (res && res.code == 1) {
      ...
      handleAdd(source, ref) {
        ...
        if (source == 'tableData8') {
          ...
          if (ref) {
            if(ref == 'table1') {
              this.$refs[ref].showEdit('add', null, projectType)
            } else if(ref == 'table21') {
              this.$refs['LRJYJLRSJview'].showEdit(null)
            } else if(ref == 'table22') {
              this.$refs['rzsjmxView'].showEdit()
            } else {
      ...
      handleEdit(source, data, index, ref) {
        ...
        if (source === 'tableData1' || source === 'tableData2') {
          ...
          this.$refs['rzsjmxView'].showEdit(data, false)
      ...
      handleApproval() {
        //提交审批
        this.$refs['process'].save(121, this.editId)
      },
    },
  }
</script>
