     ...
     -->
      ...
      <el-col :span="24" style="margin-top: 20px">
        ...
        <el-table :data="tableData4">
          ...
          <el-table-column label="委托时间" prop="entrustTime"></el-table-column>
...
<script>
  import Vue from 'vue'
  import { jhgljhcgSaveOrUpdate, jhgljhcgDetail } from '@/api/monitor/question'
  ...
  export default {
    ...
    methods: {
      ...
      async fetchData(row) {
        this.loading = true
        const res = await jhgljhcgDetail({ jhcgid: row.id })
        this.loading = false
        if (res && res.code == 1) {
      ...
      fetchTable(val) {
        console.log(val)
        if (this.operate === 'add') {
          // if(this.source == 'tableData7'||this.source == 'tableData6'||this.source == 'tableData5'){
          //   let list = this[this.source]
          //   let flag = false
          //   list.map(item=>{
          //     if(item.relaOrgName == val[0].relaOrgName){
          //       flag = true
          //     }
          //   })
          //   if(flag){
