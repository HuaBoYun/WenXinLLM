...
<script>
  import { ejjgrzlxjyUpdate, ejjgrzlxjyDetail } from '@/oapi/audit/plan'
  ...
  export default {
    components: { CompanySelectModal, Resubmit },
    inheritAttrs: false,
    props: ['fetchData'],
    data() {
      return {
    ...
    methods: {
      ...
      handleCompanyTreeSelected(val) {
        this.$set(this.formData.org, val.labelKey, val.label)
      ...
      async showEdit(title, formId, flowtaskinfoflowid, ymFromId, isWfqdedit, status) {
        this.dialogFormVisible = true
        if (formId) {
          const res = await ejjgrzlxjyDetail({ id: formId })
          ...
          this.$set(this.formData, 'createUser', res.data.data.createUser.realname)
        }
