<template>
  ...
  <div>
          ...
          >
            ...
            <el-table-column align="center" label="操作" width="120">
              <template #default="scope">
                ...
                <el-button
                  v-if="
                    scope.row.gljhxmlx == '31' || scope.row.gljhxmlx == '32'
                  "
                  type="text"
    ...
    <details1 ref="details1" />
    <details2 ref="details2" />
...
<script>
  ...
  import details1 from '@/views/oilAudit/jhlx/components/gcsjxmapbView1.vue'
  import details2 from '@/views/oilAudit/jhlx/components/gcsjxmapbView2.vue'
    ...
    methods: {
      async showEdit(
        ...
        if (formId) {
          const {
            data: { data },
          } = await planOutArrangeDetail({ tbid: formId })
          ...
          let res = await enginFileList({ tbid: formId })
      ...
      handleView(row) {
        if (row.gljhxmlx == '31') {
          this.$refs['details1'].showEdit({ data: row })
        } else {
          this.$refs['details2'].showEdit({ data: row })
        }
      },
      handleBack() {
        if (this.select.length > 0) {
          ...
          this.$baseConfirm('你确定要退回当前项吗', null, async () => {
            const res = await enginAuditProjectXmzsbth({ ids: ids.join() })
            if (res.code == 1) {
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const res = await planArrangeDelete({ id: row.id })
          if (res.code == 1) {
            this.$baseMessage('成功', 'success', 'vab-hey-message-success')
      ...
      async getChildlistPro1(val) {
        ...
        const res = await xfksrygc({ ids: arr1, names: names1, ryids: ids1 })
        if (res.msg === '成功') {
          this.$message({
            type: 'success',
            message: '下发成功!',
      ...
      async getChildlistPro2(val) {
        ...
        const names1 = val.map((res) => res.realname).toString()
        const arr1 = this.select.map((res) => res.id).toString()
        const res = await xfxmzrygc({ ids: arr1, names: names1, ryids: ids1 })
        if (res.msg === '成功') {
          this.$message({
            type: 'success',
            message: '下发成功!',
      ...
      async getChildlistPro3(val) {
        ...
        const res = await gcxfzsrys({ ids: arr1, names: names1, ryids: ids1 })
        if (res.msg === '成功') {
          this.$message({
            type: 'success',
            message: '新增成功!',
      ...
      handleDeleteFile(row) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          //删除对应的id
          let res = await enginFileDel({ attid: row.attid })
          if (res.msg === '成功') {
            this.$message({
              type: 'success',
              message: '删除成功!',
