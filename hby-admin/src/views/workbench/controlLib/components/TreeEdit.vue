...
<script>
  //
  import {
    mergeTblNbsjTargetAudit,
    selectNbsjTargetInfoAudit,
  ...
  export default {
    ...
    methods: {
      ...
      async getData(targetId) {
        const res = await selectNbsjTargetInfoAudit({ targetId })
      ...
      save() {
        this.$refs['elForm'].validate(async (valid) => {
          if (valid) {
            const { createTime, updateTime, childrenList, ...other } =
              this.formData
            const { msg, code, data } = await mergeTblNbsjTargetAudit({
              ...other,
