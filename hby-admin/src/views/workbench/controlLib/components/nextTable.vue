...
<script>
  import { getDetailZyAudit } from '@/api/audit/preparation'
  import {
    defCatListZyAudit,
    deleteNbsjAuditprogramAudit,
    deleteNbsjTargetAudit,
    findNbsjTargetTreeAudit,
    findNbsjTargetTree2Audit,
  ...
  export default {
    ...
    methods: {
      ...
      async handleDetail(row) {
        const res = await getDetailZyAudit({
      ...
      async getExecutorTree(templeteId) {
        const res = await findNbsjTargetTreeAudit({ templeteId, nodeId: '' })
      ...
      async getTree(id) {
        const res = await findNbsjTargetTree2Audit({
      ...
      async handleDeleteTree() {
        if (!this.queryForm.targetId) {
          ...
          const res = deleteNbsjTargetAudit({
      ...
      async handleDelete(row) {
        const data = deleteNbsjAuditprogramAudit({
          programId: row.programId,
        })
        this.$baseMessage('删除成功', 'success')
        this.getExecutorList()
