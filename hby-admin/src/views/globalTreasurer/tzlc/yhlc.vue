    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['investment:bankWealth:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['investment:bankWealth:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['investment:bankWealth:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['investment:bankWealth:export']"
        >导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['investment:bankWealth:query']"
          >查看</el-button>
          <el-button
            size="mini"
            type="text"
            ...
            v-hasPermi="['investment:bankWealth:edit']"
          >修改</el-button>
          <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)" v-hasPermi="['investment:bankWealth:subscribe']">
...
export default {
  ...
  methods: {
    ...
    /** 查看按钮操作 */
    handleView(row) {
      bankWealthInvestmentApi.getInfo(row.investmentId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除银行理财投资编号为"' + investmentIds + '"的数据项？').then(function() {
        return bankWealthInvestmentApi.delete(investmentIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('investment/bankWealth/export', {
        ...this.queryParams
      }, `bankWealth_${new Date().getTime()}.xlsx`)
    },
    /** 更多操作 */
    ...
    submitSubscribeForm() {
      this.$refs["subscribeForm"].validate(valid => {
        if (valid) {
          bankWealthInvestmentApi.subscribe(this.subscribeForm).then(response => {
    ...
    handleUpdateValuation(row) {
      this.$prompt('请输入新的净值', '更新投资估值', {
        ...
        return bankWealthInvestmentApi.updateValuation(row.investmentId, value);
      }).then(() => {
        this.getList();
