  ...
  <div class="security-param-config">
          ...
          <el-form-item label="参数类型">
            ...
            >
              ...
              <el-option label="业务参数" value="BUSINESS">
                <i class="el-icon-office-building"></i> 业务参数
              </el-option>
              <el-option label="安全参数" value="SECURITY">
                <i class="el-icon-lock"></i> 安全参数
              </el-option>
              <el-option label="接口参数" value="INTERFACE">
                <i class="el-icon-connection"></i> 接口参数
              </el-option>
            </el-select>
          </el-form-item>
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column label="参数编码" prop="paramCode" align="center" width="150" show-overflow-tooltip>
          <template slot-scope="{row}">
            <el-tag size="small" type="info">{{ row.paramCode }}</el-tag>
          </template>
        </el-table-column>

        <el-table-column label="参数名称" prop="paramName" align="center" width="200" show-overflow-tooltip>
          <template slot-scope="{row}">
        ...
        <el-table-column label="参数类型" prop="paramType" align="center" width="120">
          <template slot-scope="{row}">
        ...
        <el-table-column label="参数值" prop="paramValue" min-width="200" show-overflow-tooltip>
          <template slot-scope="{row}">
...
import { getSecurityParamPage, saveOrUpdateSecurityParam, deleteSecurityParam, updateSecurityParamStatus } from '@/api/globalTreasurer-new/basicConfig/securityAndAccount'
...
export default {
  ...
  data() {
    return {
      ...
      allColumns: [
        ...
        { key: 'paramType', label: '参数类型' },
        { key: 'paramValue', label: '参数值' },
        { key: 'description', label: '描述' },
        { key: 'isEnabled', label: '状态' },
        { key: 'createTime', label: '创建时间' }
      ],
      visibleColumns: JSON.parse(localStorage.getItem('securityParam_visibleColumns') || '["paramCode","paramName","paramType","paramValue","isEnabled","createTime"]')
    }
  },
  created() {
    this.fetchData()
  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        const response = await getSecurityParamPage(this.listQuery)
        ...
        if (successCodes.includes(response.code)) {
          ...
          if (response.data && response.data.tlist !== undefined) {
            ...
            // MyBatis Plus分页格式：{ code: 200, data: { records: [], total: 0 } }
    ...
    handleExport() {
      try {
        ...
        link.href = url
        link.download = `安全参数配置_${new Date().getTime()}.json`
    ...
    handleTableSetting() {
      ...
      this.$msgbox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            // 保存到localStorage
            localStorage.setItem('securityParam_visibleColumns', JSON.stringify(this.visibleColumns))
    ...
    async handleStatusChange(row) {
      try {
        const response = await updateSecurityParamStatus(row.id, row.isEnabled)
        const successCodes = [200, 0, '200', '0', '1', 1, 2]
        if (successCodes.includes(response.code)) {
    ...
    createData() {
      this.$refs['dataForm'].validate(async (valid) => {
        if (valid) {
          try {
            const response = await saveOrUpdateSecurityParam(this.temp)
            const successCodes = [200, 0, '200', '0', '1', 1, 2]
            if (successCodes.includes(response.code)) {
    ...
    async handleDelete(row, index) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
        ...
        try {
          const response = await deleteSecurityParam(row.id)
          const successCodes = [200, 0, '200', '0', '1', 1, 2]
          if (successCodes.includes(response.code)) {
            this.$notify({
              title: '成功',
