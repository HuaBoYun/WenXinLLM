...
<script>
import { getChangeApplicationPage, createChangeApplication, updateChangeApplication, approveChangeApplication } from '@/api/globalTreasurer/zhgl'
...
export default {
  ...
  methods: {
    getList() {
      ...
      setTimeout(() => {
        this.list = [
          {
            applicationId: 1,
            applicationNo: 'CHANGE_20240925_001',
            accountId: 1,
            accountNumber: '1234567890123456789',
            accountName: '示例云科技有限公司',
            changeType: 'CONTACT_INFO',
            changeReason: '公司搬迁，联系信息发生变更',
            changeDetails: '联系电话从13800138000变更为13900139000，联系地址从北京市朝阳区变更为北京市海淀区',
            applicantName: '张三',
          ...
          {
            applicationId: 2,
            applicationNo: 'CHANGE_20240924_002',
            accountId: 2,
            accountNumber: '9876543210987654321',
            accountName: '示例云投资管理有限公司',
            changeType: 'AUTHORIZED_PERSON',
            changeReason: '人事变动，授权人员发生变更',
            changeDetails: '授权人员从李四变更为王五，身份证号从110101199001010001变更为110101199002020002',
            applicantName: '赵六',
    ...
    updateStatistics() {
      this.totalApplications = 67
      this.pendingApplications = 8
      this.approvedApplications = 52
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          this.temp.applicationNo = 'CHANGE_' + new Date().toISOString().slice(0, 10).replace(/-/g, '') + '_' + String(Math.floor(Math.random() * 1000)).padStart(3, '0')
    ...
    submitApproval() {
      ...
      if (index !== -1) {
        ...
        if (this.approvalForm.result === 'APPROVED' && this.approvalForm.effectiveDate) {
          this.list[index].effectiveDate = this.approvalForm.effectiveDate.toISOString().slice(0, 10)
        }
      }
