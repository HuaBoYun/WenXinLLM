  ...
  <div class="direct-connect-auth-manage">
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column label="日限额" width="120px" align="center">
          <template slot-scope="{row}">
            <span class="daily-limit">{{ formatCurrency(row.dailyLimit) }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="到期日期" width="120px" align="center">
          <template slot-scope="{row}">
            <span :class="getExpireDateClass(row.expireDate)">{{ row.expireDate || '永久' }}</span>
          </template>
        </el-table-column>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="私钥路径">
              <el-input v-model="temp.privateKeyPath" placeholder="请输入私钥文件路径" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-form-item label="公钥路径">
          <el-input v-model="temp.publicKeyPath" placeholder="请输入公钥文件路径" />
        </el-form-item>
    ...
    <el-dialog title="授权详情" :visible.sync="dialogDetailVisible" width="800px">
      <div v-if="currentAuth" class="auth-detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="账户号码">{{ currentAuth.accountNumber }}</el-descriptions-item>
          ...
          <el-descriptions-item label="日限额">{{ formatCurrency(currentAuth.dailyLimit) }}</el-descriptions-item>
          <el-descriptions-item label="月限额">{{ formatCurrency(currentAuth.monthLimit) }}</el-descriptions-item>
          <el-descriptions-item label="生效日期">{{ currentAuth.effectiveDate }}</el-descriptions-item>
          <el-descriptions-item label="到期日期">{{ currentAuth.expireDate || '永久有效' }}</el-descriptions-item>
          <el-descriptions-item label="证书路径">{{ currentAuth.certificatePath || '-' }}</el-descriptions-item>
          <el-descriptions-item label="私钥路径">{{ currentAuth.privateKeyPath || '-' }}</el-descriptions-item>
          <el-descriptions-item label="公钥路径">{{ currentAuth.publicKeyPath || '-' }}</el-descriptions-item>
...
<script>
import { getDirectConnectAuthPage, createDirectConnectAuth, updateDirectConnectAuth, deleteDirectConnectAuth } from '@/api/globalTreasurer/zhgl'
...
export default {
  ...
  methods: {
        ...
        this.list = [
          {
            ...
            authLimit: 1000000.00,
            dailyLimit: 100000.00,
            monthLimit: 500000.00,
            certificatePath: '/certs/hbyun_tech.crt',
            privateKeyPath: '/keys/hbyun_tech.key',
            publicKeyPath: '/keys/hbyun_tech.pub',
            remark: '全权限授权',
          ...
          },
          {
            ...
            dailyLimit: 0,
            monthLimit: 0,
            certificatePath: '/certs/hbyun_invest.crt',
            privateKeyPath: '/keys/hbyun_invest.key',
            publicKeyPath: '/keys/hbyun_invest.pub',
            remark: '仅查询权限',
          ...
          },
          {
            ...
            authLimit: 500000.00,
            dailyLimit: 50000.00,
            monthLimit: 200000.00,
            certificatePath: '/certs/hbyun_trade.crt',
            privateKeyPath: '/keys/hbyun_trade.key',
            publicKeyPath: '/keys/hbyun_trade.pub',
            remark: '支付权限，因风险暂停',
    ...
    updateStatistics() {
      this.totalAuths = 156
      this.activeAuths = 128
      this.expiringAuths = 12
      this.suspendedAuths = 16
      this.activeRate = Math.round((this.activeAuths / this.totalAuths) * 100)
    },
    ...
    handleBatchRefresh() {
      ...
      this.$confirm(`确认刷新选中的${this.multipleSelection.length}个授权配置?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.temp.authId = parseInt(Math.random() * 100) + 1024
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          const index = this.list.findIndex(v => v.authId === this.temp.authId)
