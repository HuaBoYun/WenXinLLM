...
<script>
import { getFundPlanPage, createFundPlan, updateFundPlan, deleteFundPlan, submitPlan, approvePlan, executePlan, completePlan, cancelPlan } from '@/api/globalTreasurer/zjjh'
  ...
  methods: {
    async getList() {
      ...
      try {
        ...
        // API调用失败时使用模拟数据
        console.warn('资金计划API调用失败，使用模拟数据:', error)
        this.list = this.generateMockFundPlanData()
    ...
    /** 生成模拟资金计划数据 */
    generateMockFundPlanData() {
      const planTypes = ['INCOME', 'EXPENSE', 'INVESTMENT', 'FINANCING']
      const planStatuses = ['DRAFT', 'APPROVED', 'EXECUTING', 'COMPLETED', 'CANCELLED']

      return Array.from({ length: 6 }, (_, index) => ({
        id: index + 1,
        planCode: `FP${String(Date.now() + index).slice(-8)}`,
        planName: `资金计划${index + 1}`,
        planType: planTypes[Math.floor(Math.random() * planTypes.length)],
        planStatus: planStatuses[Math.floor(Math.random() * planStatuses.length)],
        planAmount: Math.floor(Math.random() * 50000000) + 1000000,
        actualAmount: Math.floor(Math.random() * 45000000) + 800000,
        currencyCode: 'CNY',
        startDate: new Date(Date.now() + Math.floor(Math.random() * 30) * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        endDate: new Date(Date.now() + Math.floor(Math.random() * 90 + 30) * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        description: `这是资金计划${index + 1}的详细描述`,
        createBy: '系统管理员',
    ...
    handleSubmit(row) {
      this.$confirm('确认提交该计划进行审批?', '提示', {
        ...
        submitPlan(row.planId, this.$store.getters.userId).then(response => {
          if (response.code === 1) {
    ...
    handleApprove(row) {
      this.$prompt('请输入审批意见', '审批计划', {
        ...
        approvePlan(row.planId, true, this.$store.getters.userId, value).then(response => {
          if (response.code === 1) {
            this.$notify({
              title: '成功',
              message: '审批通过',
        ...
        if (action === 'cancel') {
          this.$prompt('请输入拒绝理由', '拒绝计划', {
            ...
            approvePlan(row.planId, false, this.$store.getters.userId, value).then(response => {
              if (response.code === 1) {
                this.$notify({
                  title: '成功',
                  message: '已拒绝计划',
    ...
    handleExecute(row) {
      this.$confirm('确认开始执行该计划?', '提示', {
        ...
        executePlan(row.planId, this.$store.getters.userId).then(response => {
          if (response.code === 1) {
            this.$notify({ title: '成功', message: '计划已开始执行', type: 'success', duration: 2000 })
            this.getList()
          } else {
    ...
    handleDetail(row) {
      this.$router.push({
        path: '/globalTreasurer/zjjh/detail',
    ...
    handleComplete(row) {
      this.$confirm('确认完成该计划?', '提示', {
        ...
        completePlan(row.planId, this.$store.getters.userId).then(response => {
          if (response.code === 1) {
            this.$notify({ title: '成功', message: '计划已完成', type: 'success', duration: 2000 })
            this.getList()
          } else {
    ...
    handleCancel(row) {
      this.$confirm('确认取消该计划?', '提示', {
        ...
        cancelPlan(row.planId, this.$store.getters.userId).then(response => {
          if (response.code === 1) {
            this.$notify({ title: '成功', message: '计划已取消', type: 'success', duration: 2000 })
            this.getList()
          } else {
