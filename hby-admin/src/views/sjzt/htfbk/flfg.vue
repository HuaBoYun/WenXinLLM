    ...
    <div class="navBox">
      <el-card shadow="never">
        ...
        <div v-else class="sidebarWrap" v-for="(i, index) in navList" :key="index">
          ...
          <div class="treeWrap">
            <el-tree ref="tree" node-key="value" :data="i.data" :props="defaultProps"
      ...
      <el-card shadow="never">
        <el-form ref="form" :inline="true" label-width="0" :model="queryForm" @submit.native.prevent>
          <el-form-item v-for="(item, index) in searchItem" :key="index">
            ...
            <!-- <el-select multiple collapse-tags v-model="queryForm.issueDepartmentCode" placeholder="制定机关" v-if="item.name === '制定机关'" >
            </el-select> -->
            <el-input v-model="queryForm.documentNo" clearable placeholder="发文字号" v-if="item.name === '发文字号'" />
            <!-- <el-select multiple collapse-tags v-model="queryForm.contractTypeCode" placeholder="合同类型" v-if="item.name === '合同类型'" >
            </el-select>
            <el-select multiple collapse-tags v-model="queryForm.effectivenessDicCode" placeholder="效力级别" v-if="item.name === '效力级别'" >
            </el-select> -->
            <el-date-picker
              v-if="item.name === '发布日期'"
            ...
            <el-date-picker
              v-if="item.name === '实施日期'"
              v-model="queryForm.implementDate"
              type="daterange"
              value-format="yyyy-MM-dd"
              range-separator="至 "
              start-placeholder="实施开始日期"
              end-placeholder="实施结束日期">
            </el-date-picker>
            <!-- <el-select v-model="queryForm.timelinessDicCode" clearable 
              placeholder="时效性" v-if="item.name === '时效性'">
              <el-option value="全部">全部</el-option>
              <el-option value="现行有效">现行有效</el-option>
              <el-option value="尚未生效">尚未生效</el-option>
            </el-select> -->
          </el-form-item>
          <el-form-item>
      ...
      <div v-else class="listData">
        ...
        <div class="listDataItem" v-for="(i, index) in dataList" :key="index">
          <div class="listItemLeft">
            ...
            <div class="itemType">
              <span>{{ i.timelinessDicName }} / </span>
              <span>{{ i.effectivenessDicName }}</span>
              <span> / {{ i.issueDate }}发布 / </span>
              <span>{{ i.implementDate }}实施</span>
            </div>
...
<script>
import { getLawRegulationList, getSidebar, lawRegulationDownload, lawRegulationZipDownload } from '@/api/sjzt/ht/index'
...
export default {
  ...
  data() {
    return {
      ...
      localKey: 'sjzt-htfbk-flfg-search',
      tableKey: 'sjzt-htfbk-flfg-list',
  ...
  watch: {
    'screenList': {
      handler: function (newValue, oldValue) {
        ...
        that.queryForm.effectivenessDicCode = ''
        that.queryForm.issueDepartmentCode = ''
        that.queryForm.timelinessDicCode = ''
        that.queryForm.issueYear = ''
        newValue.map(i =>{
          if(i.type == 'contractType'){
            that.queryForm.contractTypeCode = i.value
          }else if(i.type == 'effectivenessDic'){
            that.queryForm.effectivenessDicCode = i.value
          }else if(i.type == 'issueDepartment'){
            that.queryForm.issueDepartmentCode = i.value
          }else if(i.type == 'timelinessDic'){
            that.queryForm.timelinessDicCode = i.value
  ...
  methods: {
    async getNav() {
      let navRes = await getSidebar({'types': 'contractType,effectivenessDic,issueDepartment,timelinessDic,issueYear'})
    ...
    getFiled() {
      let fields = [
        ...
        { name: '分组', key: 'null' },
        // { name: '制定机关', key: 'issueDepartmentCode' },
        { name: '发文字号', key: 'documentNo' },
        // { name: '合同类型', key: 'contractTypeCode' },
        // { name: '效力级别', key: 'effectivenessDicCode' },
        // { name: '时效性', key: 'timelinessDicCode' },
        { name: '发布日期', key: 'issueDate' },
        { name: '实施日期', key: 'implementDate' },
      ]
      return fields
    },
    showMore() {
    ...
    async fetchData() {
      let res = await getLawRegulationList({
        ...this.queryForm,
        ...
        implementDateStart: this.queryForm.implementDate[0] ? `${this.queryForm.implementDate[0]} 00:00:00` : '',
        implementDateEnd: this.queryForm.implementDate[1] ? `${this.queryForm.implementDate[1]} 23:59:59` : ''
      })
    ...
    async handleDownload(row) {
      const res = await lawRegulationDownload({ id: row.id })
    ...
    async handleZipDownload(row) {
      ...
      const res = await lawRegulationZipDownload({ ids: this.selectedItems.join(',') })
      this.downloadFileByBlob(res, '法律法规')
    },
