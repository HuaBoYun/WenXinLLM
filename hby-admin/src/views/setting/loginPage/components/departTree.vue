...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.$nextTick(() => {
        this.defaultList = this.defaultTreeData.map((res) => res.belongGroup)
        this.arr = this.defaultTreeData.map((res) => res.belongGroup)
      })
    },
    watch: {
      data(val) {
        const ids = this.defaultTreeData.map((res) => res.belongGroup)
