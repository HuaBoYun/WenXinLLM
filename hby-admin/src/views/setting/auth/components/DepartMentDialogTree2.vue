...
<script>
  import { getAllCompanyTree, getDepartmentListInfo } from '@/api/setting/org'
  ...
  export default {
    ...
    methods: {
      ...
      async show(list) {
        ...
        this.hasClickedTree = false

        // 获取userInfo中的组织数据
        const userInfo = JSON.parse(localStorage.getItem('userInfo'))
        if (userInfo && userInfo.linkOrg) {
          this.company = {
            orgid: userInfo.linkOrg.orgid,
            orgname: userInfo.linkOrg.orgname,
          }
          this.queryForm.pid = userInfo.linkOrg.orgid
        }
        ...
        if (list && list.length > 0) {
          ...
          console.log(
            'this.userId',
            list.map((item) => item.orgid)
          )
          await this.setSelection(list)
        }
        this.dialogFormVisible = true
        let that = this
        // 延时执行 重置 解决tree第一次无法加载的bug
        var Date2 = window.setTimeout(function () {
          that.resetSearchTree()
        }, 1000)
      },
      setSelection(list) {
        this.$nextTick(() => {
      ...
      async getExecutorList() {
        let params = {
          fatherorgid: this.queryForm.pid,
          pageNumber: this.queryForm.pageNumber,
          pageSize: this.queryForm.pageSize,
          orgnumber: this.queryForm.code,
          orgname: this.queryForm.name,
          status: 0,
        }
        const {
          data: { list, total },
        } = await getDepartmentListInfo(params)
        this.list = list
        this.total = total
      },
