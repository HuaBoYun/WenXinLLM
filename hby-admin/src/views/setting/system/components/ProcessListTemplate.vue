...
<script>
  import {
    getAllFlowList,
    getContractTypeFlowList,
    saveAllNewFlowList,
    submitByYmWork,
    SSOToJNFD,
    removeWorkFlowFormInfo,
    removeWorkFlowContractType,
    copyWorkFlowFormInfo,
    copyContractTypeFlow,
    startContractTypeFlow,
    ...
    methods: {
      ...
      handleDelete(row) {
        this.$baseConfirm('你确定要删除当前项吗', null, async () => {
          const { code, data } = await removeWorkFlowContractType({
      ...
      async fetchData() {
        ...
        const {
          data: {
            pageInfo: { records: list, total: total },
          },
        } = await getContractTypeFlowList(this.queryForm)
        this.list = list
        this.total = total
        this.listLoading = false
      },
      async show(e, type, typeId) {
      ...
      async handleCompanyTreeSelected(node) {
        if (node instanceof Array) {
          ...
          const { code, data } = await copyContractTypeFlow({
      ...
      async handleChangeStatus(row) {
        ...
        const { data, code } = await startContractTypeFlow(param)
        // 流程校验
        if (this.requireValuedata) {
          //  查询当前是否有流程
          getFlowList({
