/**
 * WenxinClaw Gateway WebSocket 客户端
...
/**
 * WenxinClaw WebSocket 客户端类
 */
export class WenxinClawClient {
  /**
   * 构造函数
  ...
  connect() {
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log('[WenxinClaw] 连接到:', url)

        this.ws = new WebSocket(url)

        // 连接打开
        this.ws.onopen = async () => {
          console.log('[WenxinClaw] WebSocket 连接成功')
          ...
          try {
            ...
            console.log('[WenxinClaw] 设备身份已加载:', {
              deviceId: this.deviceIdentity.deviceId
            })
          } catch (error) {
            console.error('[WenxinClaw] 加载设备身份失败:', error)
            reject(error)
            return
          }
        ...
        this.ws.onerror = (error) => {
          console.error('[WenxinClaw] WebSocket 错误:', error)
          this.isConnected = false
        ...
        this.ws.onclose = (event) => {
          console.log('[WenxinClaw] 连接关闭, code:', event.code, 'reason:', event.reason)
          this.isConnected = false
          this.connectNonce = null
          ...
          if (this.reconnectAttempts < this.maxReconnectAttempts) {
            ...
            console.log(`[WenxinClaw] 3秒后尝试重连 (${this.reconnectAttempts}/${this.maxReconnectAttempts})`)
            setTimeout(() => this.connect(), 3000)
          } else {
            console.error('[WenxinClaw] 已达到最大重连次数，停止重连')
            this.emit('maxReconnectAttemptsReached')
          }
        }
      } catch (error) {
        console.error('[WenxinClaw] 连接创建失败:', error)
        reject(error)
      }
    })
  }
  ...
  async loadDeviceIdentity() {
    const STORAGE_KEY = 'wenxinclaw-device-identity-v1'
    ...
    if (storage) {
      try {
        ...
        if (parsed.version === 1 &&
          ...
          console.log('[WenxinClaw] 使用已存储的设备身份')
          return
        }
      } catch (error) {
        console.warn('[WenxinClaw] 解析存储的设备身份失败，重新生成:', error)
      }
    }
  ...
  async generateDeviceIdentity() {
    console.log('[WenxinClaw] 使用 Web Crypto API 生成 Ed25519 密钥对...')
    ...
    localStorage.setItem('wenxinclaw-device-identity-v1', JSON.stringify(stored))

    this.deviceIdentity = new DeviceIdentity(deviceId, publicKeyBase64, privateKeyBase64)
    console.log('[WenxinClaw] 新的设备身份已生成:', { deviceId })
  }
  ...
  async sendConnect() {
    if (!this.deviceIdentity && !this.skipDeviceAuth) {
      console.error('[WenxinClaw] 设备身份未加载')
      return
    }

    if (!this.connectNonce) {
      console.error('[WenxinClaw] 连接 nonce 未设置')
      return
    }
    ...
    if (!this.skipDeviceAuth && this.deviceIdentity) {
      ...
      console.log('[WenxinClaw] 签名 payload:', payload)
    ...
    console.log('[WenxinClaw] 发送连接请求，设备对象:', params.device ? Object.keys(params.device) : 'skipped')
    ...
    this.pendingRequests.set(requestId, { resolve: () => {}, reject: () => {}, timeout: setTimeout(() => {
      ...
      console.warn('[WenxinClaw] 连接请求超时')
    }, 10000) })
  ...
  handleMessage(data) {
    try {
      ...
      console.error('[WenxinClaw] 消息解析失败:', error, data)
    }
  }
  ...
  handleEvent(data) {
    ...
    console.log('[WenxinClaw] 收到事件:', event, payload ? payload : '')
    ...
    switch (event) {
      ...
      case 'connect.challenge':
        console.log('[WenxinClaw] 收到连接挑战')
        const nonce = payload?.nonce
        if (nonce && typeof nonce === 'string') {
          this.connectNonce = nonce
          console.log('[WenxinClaw] 设置连接 nonce:', nonce)
          // 收到挑战后发送连接请求
      ...
      case 'tick':
        ...
        break
      default:
        console.log('[WenxinClaw] 未知事件:', event)
    }
  }
  ...
  handleResponse(data) {
    ...
    if (!pendingRequest) {
      console.warn('[WenxinClaw] 未找到待处理请求:', requestId)
      return
    }
    ...
    if (data.ok) {
      ...
      console.error('[WenxinClaw] 请求失败:', errorMsg)
      ...
      if (errorMsg.includes('pairing required')) {
        ...
        console.warn(`[WenxinClaw] 需要设备配对审批，当前设备ID: ${deviceId.substring(0, 16)}...`)
        console.warn('[WenxinClaw] 请在 XingGuang 终端执行: xingguang devices approve --latest')
        this.emit('pairingRequired', { deviceId })
      }

      // 检查是否是设备身份不匹配错误
      if (errorMsg.includes('device identity mismatch')) {
        console.error('[WenxinClaw] 设备身份不匹配，清除并重新生成')
        localStorage.removeItem('wenxinclaw-device-identity-v1')
        this.deviceIdentity = null
      }
  ...
  sendRaw(message) {
    if (!this.isConnected || !this.ws) {
      console.warn('[WenxinClaw] 未连接，消息加入队列')
    ...
    try {
      ...
      console.error('[WenxinClaw] 发送消息失败:', error)
    }
  }
    ...
    if (this.eventHandlers[event]) {
      this.eventHandlers[event].forEach(handler => {
        try {
          ...
          console.error('[WenxinClaw] 事件处理器错误:', error)
        }
      })
    }
  }
...
/**
 * 创建全局 WenxinClaw 客户端实例
 * @param {Object} config 配置对象
 * @returns {WenxinClawClient}
 */
export function createWenxinClawClient(config) {
  return new WenxinClawClient(config)
}
...
/**
 * 获取全局 WenxinClaw 客户端实例
 * @param {Object} config 配置对象（仅第一次创建时使用）
 * @returns {WenxinClawClient}
 */
export function getWenxinClawClient(config) {
  if (!globalClient) {
    globalClient = new WenxinClawClient(config)
  }
  return globalClient
}
